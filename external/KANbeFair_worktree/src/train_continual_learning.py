...
class Continual_Learning_Evaluator:
    ...
    def eval(self, model, device, dataset_index, logger):
        model.eval()
        for i in range(dataset_index + 1):
            test_loader = self.test_loaders[i]
            correct = 0
            with torch.no_grad():
                for data, target in test_loader:
                    data, target = data.to(device), target.to(device)
            ...
            self.accuracies[(dataset_index, i)] = accuracy
            logger.info(f"continual accuracy {i} - {dataset_index}: {accuracy:.2f}")
...
def main():
    ...
    cle = Continual_Learning_Evaluator(args, test_loaders)

    fvctimer = Timer()
    for dataset_index, (train_loader, test_loader) in enumerate(zip(train_loaders, test_loaders)):
        logger.info(f"Strat the {dataset_index}-th Task")
