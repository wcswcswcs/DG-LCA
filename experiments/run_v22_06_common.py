#!/usr/bin/env python3
"""Shared helpers for v22.06 execution, logs, and review packets."""
...
V2206_ROOT = ROOT / "results/v22_06_training_dynamics_metric_geometry_fu_basis_efficiency_4gpu"
V2206_OFFICIAL = V2206_ROOT / "official_v22_06"
V2206_PLAN_DOC = ROOT / "docs/DG-KAN_v22.06_完整版_FunctionalUpdate线优化版.md"
V2206_EXEC_DOC = ROOT / "docs/DG-KAN_v22.06_TrainingDynamics_MetricGeometryFU_BasisEfficiency_4GPU_执行日志.md"
V2206_RECAP_DOC = ROOT / "docs/DG-KAN_v22.06_TrainingDynamics_MetricGeometryFU_BasisEfficiency_4GPU_实验结果复盘.md"
V2204_OFFICIAL = ROOT / "results/v22_04_terminal_source_preservation_diffeomorphic_fu_basis_efficiency/official_v22_04"
V2205_OFFICIAL = ROOT / "results/v22_05_metric_first_functional_update_basis_efficiency_4gpu/official_v22_05"
...
def ensure_out(out_dir: str | Path | None = None) -> Path:
    out = Path(out_dir) if out_dir else V2206_OFFICIAL
...
def append_exec(out_dir: Path, command: str, *, status: str = "", note: str = "") -> None:
    ...
    journal = out_dir / "v22_06_command_journal.csv"
    ...
    with journal.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow(row)
    append_text(V2206_EXEC_DOC, f"\n## {row['timestamp']}\n\n```bash\n{command}\n```\n\n- status: {status or 'recorded'}\n")
    if note:
        append_text(V2206_EXEC_DOC, f"- note: {note}\n")
...
def build_packet(out_dir: Path) -> tuple[Path, Path]:
    packet = out_dir / "v22_06_code_review_packet"
    if packet.exists():
    ...
    for rel in sections:
        ...
        if Path(rel).suffix:
            ...
            target.write_text("v22.06 code review packet\n", encoding="utf-8")
        else:
            target.mkdir(parents=True, exist_ok=True)
    write_text(packet / "00_README.md", "v22.06 packet: self-contained source tree, S0.13 truth gate artifacts, logs, and reproduction commands.\n")
    ...
    for path in sorted(out_dir.glob("v22_06_*")):
        if path.is_file() and path.suffix.lower() != ".zip":
            section = "10_RESULTS_POINTERS"
            low = path.name.lower()
            if "import" in low or "compile" in low or "clean_unzip" in low:
                section = "03_IMPORT_CLOSURE"
            elif "linec" in low:
                section = "04_LINEC_CORRECTNESS"
            elif "source" in low or "retention" in low or "debt" in low:
                section = "05_SOURCE_CHAIN_RETENTION_DEBT"
            elif "semantic" in low or "alias" in low or "contract" in low:
                section = "06_MECHANISM_SEMANTIC_CONTRACTS"
            elif "metric" in low or "solver" in low or "nds" in low:
                section = "07_FUNCTION_SPACE_METRICS"
            elif "efficiency" in low or "drat" in low or "drbf" in low:
                section = "08_BASIS_KERNELS_AND_PROFILERS"
            elif "gpu" in low or "queue" in low or "idle" in low:
    ...
    for doc in [V2206_PLAN_DOC, V2206_EXEC_DOC, V2206_RECAP_DOC]:
    ...
    write_text(
        ...
        f"{PYTHON} experiments/run_v22_06_s013_truth_gate.py --mode all --source-root . --self-contained-import-check 1\n",
    )
    manifest = []
    ...
    zip_path = out_dir / "v22_06_code_review_packet.zip"
    if zip_path.exists():
        zip_path.unlink()
    ...
    bundle = out_dir / "v22_06_results_bundle.zip"
    if bundle.exists():
        bundle.unlink()
    with zipfile.ZipFile(bundle, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for path in sorted(out_dir.rglob("*")):
            if path.is_file() and path.suffix.lower() != ".zip" and "v22_06_code_review_packet" not in str(path):
