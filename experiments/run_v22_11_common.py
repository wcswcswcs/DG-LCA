#!/usr/bin/env python3
"""Shared helpers for v22.11 arbitrary-loss constructive FU execution."""
...
V2211_ROOT = ROOT / "results/v22_11_arbitrary_loss_constructive_fu_basis_efficiency_4gpu"
V2211_OFFICIAL = V2211_ROOT / "official_v22_11"
V2211_PLAN_DOC = ROOT / "docs/DG-KAN_v22.11_ArbitraryLoss_ConstructiveFU_BasisEfficiency_完整计划.md"
V2211_EXEC_DOC = ROOT / "docs/DG-KAN_v22.11_ArbitraryLoss_ConstructiveFU_BasisEfficiency_执行日志.md"
V2211_RECAP_DOC = ROOT / "docs/DG-KAN_v22.11_ArbitraryLoss_ConstructiveFU_BasisEfficiency_实验结果复盘.md"
...
REQUIRED_SOURCE_FILES = [
    ...
    "experiments/run_v22_11_common.py",
    "experiments/run_v22_11_s018_truth_gate.py",
    "experiments/run_v22_11_basis_efficiency.py",
    "experiments/run_v22_11_source_atom_generation.py",
    "experiments/run_v22_11_variational_source_solve.py",
    "experiments/run_v22_11_metric_commit.py",
    "experiments/run_v22_11_arbitrary_loss_horizon.py",
    "experiments/run_v22_11_kan_mapping.py",
    "experiments/run_v22_11_finalize.py",
    "experiments/run_v22_11_full.py",
]
...
def ensure_out(out_dir: str | Path | None = None) -> Path:
    out = Path(out_dir) if out_dir else V2211_OFFICIAL
...
def init_docs() -> None:
    V2211_EXEC_DOC.parent.mkdir(parents=True, exist_ok=True)
    if not V2211_EXEC_DOC.exists():
        V2211_EXEC_DOC.write_text(
            "# DG-KAN v22.11 ArbitraryLoss ConstructiveFU BasisEfficiency 执行日志\n\n"
            f"生成时间：{now_sg()}\n\n"
            "记录原则：只记录真实命令、输入、输出、状态、blocker 与修复尝试；未执行项不写成完成。\n",
            encoding="utf-8",
        )
    if not V2211_RECAP_DOC.exists():
...
def append_exec(out_dir: Path, command: str, *, status: str = "", note: str = "") -> None:
    ...
    journal = out_dir / "v22_11_command_journal.csv"
    ...
    with journal.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow(row)
    append_text(V2211_EXEC_DOC, f"\n## {row['timestamp']}\n\n```bash\n{command}\n```\n\n- status: {status or 'recorded'}\n")
    if note:
        append_text(V2211_EXEC_DOC, f"- note: {note}\n")
...
def packet_source_paths() -> list[Path]:
    ...
    for path in sorted((ROOT / "dgkan").glob("**/*.py")):
        if path not in seen:
            seen.add(path)
            paths.append(path)
    for path in sorted((ROOT / "experiments").glob("run_v22_11*.py")):
        if path not in seen:
            seen.add(path)
            paths.append(path)
    docs = [V2211_PLAN_DOC, V2211_EXEC_DOC, V2211_RECAP_DOC]
    return [p for p in paths + docs if p.exists() and p.is_file()]


def build_code_review_packet(out_dir: Path) -> Path:
    packet = out_dir / "v22_11_code_review_packet.zip"
    if packet.exists():
        packet.unlink()
    ...
    with zipfile.ZipFile(packet, "w", compression=zipfile.ZIP_DEFLATED) as z:
        ...
        for path in sorted(out_dir.rglob("v22_11_*")):
            if path.is_file() and path.suffix.lower() != ".zip" and "_code_packet_unzip" not in path.parts:
                ...
                z.write(path, arcname)
                included.append({"packet_path": str(arcname), "source_path": str(path.resolve()), "size_bytes": path.stat().st_size, "sha256": sha256_file(path)})
    write_rows(out_dir / "v22_11_code_review_packet_manifest.csv", included)
    return packet
...
def build_results_bundle(out_dir: Path) -> Path:
    bundle = out_dir / "v22_11_results_bundle.zip"
    if bundle.exists():
        bundle.unlink()
