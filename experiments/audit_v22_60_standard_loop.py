#!/usr/bin/env python3
"""Static and runtime audit helpers for v22.60 standard training loops."""
...
OFFICIAL_FILES = [
    ROOT / "dgkan/fu/meta_fu_control_law.py",
    ROOT / "dgkan/optim/meta_fu_optimizer_wrapper.py",
    ROOT / "experiments/run_v22_60_bilevel_meta_fu_control_law.py",
]
...
def _forbidden_patterns() -> dict[str, str]:
    return {
        ...
        "runtime_argmax_candidate_used": r"argmax.*candidate",
        "runtime_topk_candidate_used": r"\.topk\s*\(.*candidate",
        ...
        "black_box_update_generator": r"Delta\s*theta|parameter_delta|update_generator",
    }
...
def scan_files(paths: Iterable[Path] | None = None) -> dict[str, Any]:
    ...
    loop_text = runner.read_text(encoding="utf-8", errors="replace") if runner.exists() else ""
    static_pass = int("logits = model(xb)" in loop_text and "loss_task.backward()" in loop_text and "opt.step()" in loop_text)
    return {
        "rows": rows,
        "summary": {
            **aggregate,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(result if args.json else result["summary"], ensure_ascii=False, indent=2 if args.json else None, sort_keys=True))
