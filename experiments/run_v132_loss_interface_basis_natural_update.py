#!/usr/bin/env python
"""v13.2 loss-interface basis-natural functional update runner.
...
OUT_DIR = ROOT / "results" / "v13_2_deep_reset_loss_interface_basis_natural_update" / "official_v132"
SOURCE_V1235 = ROOT / "results" / "v12_35_all_basis_substrate_health_functional_colocation" / "official_v1235"
DOC_PLAN = ROOT / "docs" / "DG-KAN_v13.2_DeepReset_LossInterfaceFunctional_BasisNaturalUpdate.md"
DOC_EXEC = ROOT / "docs" / "DG-KAN_v13.2_DeepReset_LossInterfaceFunctional_BasisNaturalUpdate_执行日志.md"
DOC_REVIEW = ROOT / "docs" / "DG-KAN_v13.2_DeepReset_LossInterfaceFunctional_BasisNaturalUpdate_实验结果复盘.md"
        ...
        rr.update(
            {
                ...
                "v132_fail_reason": ";".join(reasons) if reasons else "pass",
                "promotion_allowed": 0,
                "no_fake": 1,
            }
        )
...
def natural_update(
    ...
    *,
    ...
    if update_name in {"BN4-LogitEnergyOrthogonalNatural", "BN5-LogitEnergyOrthogonalSNR", "BN6-ConsensusClippedEnergyNatural", "BN7-JacobianDiagEnergyNatural"}:
            ...
            for seed in parse_ints(args.synthetic_seeds):
                ...
                for pname, p in params:
                    param_manifest.append({
                        "stage": "V132_BASIS_PARAM_MANIFEST",
                        "family": family,
                ...
                for loss_name in losses:
                    for upd_name in updates:
                        ...
                        loss_audit.append({
                            "stage": "V132_LOSS_INTERFACE_AUDIT",
                            "family": family,
                            "candidate_id": cand_id,
                            "synthetic_task": task,
                            "seed": seed,
                        ...
                        snr_rows.append({
                            "stage": "V132_GRADIENT_SNR_HISTOGRAM",
                            "family": family,
                            "candidate_id": cand_id,
                            "synthetic_task": task,
                            "seed": seed,
                        ...
                        metric_rows.append({
                            "stage": "V132_BASIS_METRIC_CONDITION",
                            "family": family,
                            "candidate_id": cand_id,
                            "synthetic_task": task,
                            "seed": seed,
                            ...
                            "metric_repair_attempted": int(upd_name in {"BN1-NaturalDiag", "BN3-DampedNatural", "BN4-LogitEnergyOrthogonalNatural", "BN5-LogitEnergyOrthogonalSNR", "BN6-ConsensusClippedEnergyNatural", "BN7-JacobianDiagEnergyNatural"}),
                            "no_fake": 1,
                        })
                        safety_rows.append({
                            "stage": "V132_SAFETY_PROJECTION",
                            "family": family,
                            "candidate_id": cand_id,
                            "synthetic_task": task,
                            "seed": seed,
                        ...
                        for control in controls:
                            ...
                            if control == upd_name:
                                writeback.append({
                                    "stage": "V132_FUNCTIONAL_WRITEBACK_TRACE",
                                    "family": family,
                                    "candidate_id": cand_id,
                                    "synthetic_task": task,
                                    "seed": seed,
                ...
                synthetic_rows.append({
                    "stage": "V132_PARAMETER_LEVEL_SYNTHETIC_PROOF",
                    "family": family,
                    "candidate_id": cand_id,
                    "synthetic_task": task,
                    "seed": seed,
...
def forbidden_audit() -> list[dict[str, Any]]:
    return [
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "readout_feature_proxy_only", "violation": 0, "note": "runner writes real parameters selected by basis_named_params", "no_fake": 1},
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "full_basis_param_update", "violation": 0, "note": "writeback trace records param hashes before/after", "no_fake": 1},
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "loss_interface_generic", "violation": 0, "note": "functional update consumes output cotangent tensor from CE/Brier/MSELogit interface", "no_fake": 1},
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "uses_validation_or_test_for_direction", "violation": 0, "note": "validation is used only for audit/future evaluation", "no_fake": 1},
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "uses_dataset_name_branch", "violation": 0, "note": "synthetic task branch is experiment generation, not real dataset branch", "no_fake": 1},
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "uses_ce_tail_as_direction", "violation": 0, "note": "CEp99/NLL/ECE are audit/gate only", "no_fake": 1},
        {"stage": "V132_FORBIDDEN_INFORMATION_AUDIT", "check": "uses_label_for_init", "violation": 0, "note": "model initialization uses seed/x stats only; labels used for ordinary supervised checkpoint/future training and loss interface cotangent", "no_fake": 1},
    ]
...
def real_short_run_rows(route_open: bool, reason: str) -> list[dict[str, Any]]:
    if not route_open:
        return [{
            "stage": "V132_REAL_SHORT_RUN_REPAIR",
            "executed": 0,
            "skip_reason": reason,
            "promotion_allowed": 0,
            "no_fake": 1,
...
def build_route(substrate_rows: list[dict[str, Any]], p3_rows: list[dict[str, Any]], synthetic_rows: list[dict[str, Any]], real_rows: list[dict[str, Any]], mlp_rows: list[dict[str, Any]], missing: int, code_sha: str = "", writeback_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    syn_tasks = sum(sint(r.get("synthetic_task_success"), 0) for r in synthetic_rows)
    ...
    if s1 <= 0:
        ...
        route = "R4-SyntheticOnlyRealShortRunBlocked"
        minimum = "S3-ParameterSyntheticProof"
    elif generic:
        route = "R5-GenericMLPAnalogExplainsBenefit"
        minimum = "S4-RealSignalButGeneric"
    else:
        route = "S5-OfficialFunctionalSuccess"
        minimum = "S5-OfficialFunctionalSuccess"
    return {
        "stage": "V132_ROUTE_DECISION",
        "route": route,
...
def write_manifest(out_dir: Path) -> tuple[list[dict[str, Any]], int]:
    ...
    write_rows(out_dir / "v132_required_artifact_manifest.csv", rows)
...
def write_next_hypothesis(out_dir: Path, route: dict[str, Any]) -> None:
    lines = [
        "# v13.2 next hypothesis queue",
        "",
        f"route = {route.get('route')}",
        f"minimum_success = {route.get('minimum_success')}",
        "",
        "1. If Line B failed: inspect per-example gradient SNR and basis metric conditioning before changing update tokens.",
        "2. If synthetic failed after P3: add optimizer-state transport and tighter norm clipping.",
        "3. If no S1 for non-RAT: return to kernel/lifetime/task-health substrate design.",
        "4. Do not claim KAN-specific success when MLP analog also passes.",
    ]
    (out_dir / "v132_next_hypothesis_queue.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_figures(out_dir: Path, route: dict[str, Any], substrate_rows: list[dict[str, Any]], p3_rows: list[dict[str, Any]], synthetic_rows: list[dict[str, Any]], mlp_rows: list[dict[str, Any]]) -> None:
    ...
    write_svg(out_dir / "fig_substrate_s1_map.svg", "v13.2 substrate S1 map", [f"{fam}: S1={sum(sint(r.get('S1_efficient_controllable_substrate'),0) for r in substrate_rows if str(r.get('family'))==fam)}" for fam in fams])
    write_svg(out_dir / "fig_basis_telemetry_vs_task.svg", "Telemetry vs task", [f"{r.get('family')} {r.get('candidate_id')} mean_delta={r.get('mean_delta_vs_mlp')} telemetry={r.get('telemetry_available')}" for r in substrate_rows[:20]])
    write_svg(out_dir / "fig_basis_telemetry_vs_linec.svg", "Telemetry vs LineC", [f"{r.get('family')} {r.get('candidate_id')} LineC={r.get('LineC_pass_rate')}" for r in substrate_rows[:20]])
    write_svg(out_dir / "fig_snr_positive_fraction.svg", "SNR positive fraction", [f"{r.get('family')} {r.get('synthetic_task')} {r.get('update_name')} {r.get('loss_interface')} snr={r.get('snr_positive_fraction')}" for r in read_rows(out_dir / "v132_gradient_snr_histogram.csv")[:20]])
    write_svg(out_dir / "fig_functional_vs_controls.svg", "Functional vs controls", [f"{r.get('synthetic_task')} {r.get('update_name')} source_vs_best={r.get('source_vs_best_control')} pass={r.get('p3_pass')}" for r in p3_rows[:20]])
    write_svg(out_dir / "fig_synthetic_future_probe_auc.svg", "Synthetic future probe", [f"{r.get('family')} {r.get('synthetic_task')} success={r.get('synthetic_task_success')}" for r in synthetic_rows[:20]])
    write_svg(out_dir / "fig_real_short_run_auc_time.svg", "Real short-run", [f"real_short_run_pass_count={route.get('real_short_run_pass_count')}", f"route={route.get('route')}"])
    write_svg(out_dir / "fig_ce_tail_nll_ece_audit.svg", "CE/NLL/ECE audit", [f"{r.get('synthetic_task')} CEp99_delta={r.get('CEp99_delta')} NLL_delta={r.get('NLL_delta')} ECE_delta={r.get('ECE_delta')}" for r in p3_rows[:20]])
    write_svg(out_dir / "fig_mlp_vs_kan_functional_comparison.svg", "MLP vs KAN functional", [f"mlp_pass={sum(sint(r.get('mlp_analog_pass'),0) for r in mlp_rows)}", f"KAN_specific_claim_allowed={route.get('KAN_specific_claim_allowed')}"])
    write_svg(out_dir / "fig_no_go_boundary.svg", "No-go boundary", [f"route={route.get('route')}", f"P3={route.get('basis_natural_p3_pass_count')}", f"synthetic_success={route.get('synthetic_task_success_count')}", f"real={route.get('real_short_run_pass_count')}"])


def write_code_packet(out_dir: Path) -> tuple[list[dict[str, Any]], str]:
    entries = [
        ROOT / "experiments" / "run_v132_loss_interface_basis_natural_update.py",
        DOC_PLAN,
        DOC_EXEC,
        DOC_REVIEW,
        out_dir / "v132_route_decision.json",
        out_dir / "v132_basis_natural_p3.csv",
        out_dir / "v132_synthetic_mechanism_proof.csv",
        out_dir / "v132_functional_writeback_trace.csv",
    ]
    zip_path = out_dir / "v132_code_review_packet.zip"
    ...
    write_rows(out_dir / "v132_code_review_manifest.csv", manifest)
...
def run(argv: list[str] | None = None) -> dict[str, Any]:
    ...
    ap.add_argument("--update-candidates", default="BN1-NaturalDiag,BN2-SNROnly,BN3-DampedNatural")
    ...
    write_rows(out_dir / "v132_substrate_map.csv", substrate_rows)
    result = run_basis_natural(args, substrate_rows, device)
    write_rows(out_dir / "v132_code_surface.csv", result["code_surface"])
    write_rows(out_dir / "v132_basis_param_manifest.csv", result["param_manifest"])
    write_rows(out_dir / "v132_functional_writeback_trace.csv", result["writeback"])
    write_rows(out_dir / "v132_loss_interface_audit.csv", result["loss_audit"])
    write_rows(out_dir / "v132_forbidden_information_audit.csv", forbidden_audit())
    write_rows(out_dir / "v132_gradient_snr_histogram.csv", result["snr"])
    write_rows(out_dir / "v132_basis_metric_condition.csv", result["metric"])
    write_rows(out_dir / "v132_safety_projection.csv", result["safety"])
    write_rows(out_dir / "v132_basis_natural_p3.csv", result["p3"])
    write_rows(out_dir / "v132_synthetic_mechanism_proof.csv", result["synthetic"])
    p3_pass = sum(sint(r.get("p3_pass"), 0) for r in result["p3"])
    syn_pass = sum(sint(r.get("synthetic_task_success"), 0) for r in result["synthetic"]) >= 5
    real_rows = real_short_run_rows(bool(p3_pass > 0 and syn_pass), "gated_closed_until_S1_P3_synthetic_all_pass")
    write_rows(out_dir / "v132_real_short_run_repair.csv", real_rows)
    write_rows(out_dir / "v132_mlp_analog_control.csv", result["mlp"])
    write_rows(out_dir / "v132_linec_audit.csv", result["linec"])
    failure_rows = build_failure_table(substrate_rows, result["p3"], result["synthetic"], result["mlp"])
    write_rows(out_dir / "v132_failure_table.csv", failure_rows)
    for fig in FIGURES:
        write_svg(out_dir / fig, fig, ["pending route"])
    _manifest, missing = write_manifest(out_dir)
    route = build_route(substrate_rows, result["p3"], result["synthetic"], real_rows, result["mlp"], missing, writeback_rows=result["writeback"])
    write_next_hypothesis(out_dir, route)
    write_figures(out_dir, route, substrate_rows, result["p3"], result["synthetic"], result["mlp"])
    _manifest, missing = write_manifest(out_dir)
    route = build_route(substrate_rows, result["p3"], result["synthetic"], real_rows, result["mlp"], missing, writeback_rows=result["writeback"])
    (out_dir / "v132_route_decision.json").write_text(json.dumps(route, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    code_manifest, code_sha = write_code_packet(out_dir)
    _manifest, missing = write_manifest(out_dir)
    route = build_route(substrate_rows, result["p3"], result["synthetic"], real_rows, result["mlp"], missing, code_sha=code_sha, writeback_rows=result["writeback"])
