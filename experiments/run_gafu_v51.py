#!/usr/bin/env python3
"""DG-KAN v5.1 runner: AB-RBF strong residual smoothing probes."""
...
from run_gafu_v44 import _estimate_geometry_any
...
class V51Params(V48Params):
    ...
    smooth_alpha_phi: float = 1.0
    smooth_alpha_curv: float = 0.5
    smooth_alpha_sob: float = 0.2
    smooth_beta_z: float = 2.0
    smooth_beta_h: float = 1.0
    smooth_beta_m: float = 1.0
    smooth_beta_loss: float = 3.0
    smooth_acc_drop_max: float = 0.005
    smooth_kl_max: float = 0.01
    smooth_logit_max: float = 0.03
    smooth_hidden_max: float = 0.05
    smooth_holdout_worse_max: float = 0.02
    smooth_eta_grid: Tuple[float, ...] = (0.03, 0.06, 0.10, 0.16, 0.24, 0.35, 0.50)
...
class SiluABRBFDense(CoreABRBFDense):
    """SiLU base path + RBF edge backed by core ABRBFDense."""

    def __init__(self, in_dim: int, out_dim: int, basis_count: int, *, bias: bool = False) -> None:
        super().__init__(in_dim, out_dim, basis_count, bias=bias, base_kind="silu")


class LinearSiluABRBFDense(CoreABRBFDense):
    """Constant + linear + SiLU base path + RBF residual backed by core ABRBFDense."""

    def __init__(self, in_dim: int, out_dim: int, basis_count: int, *, bias: bool = False) -> None:
        super().__init__(in_dim, out_dim, basis_count, bias=bias, base_kind="linear_silu")


class BaseOnlyLinearDense(CoreABRBFDense):
    """Strict edge-only constant + linear layer without RBF residual."""

    def __init__(self, in_dim: int, out_dim: int, basis_count: int, *, bias: bool = False) -> None:
        super().__init__(in_dim, out_dim, basis_count, bias=bias, base_kind="baseonly_linear")


class BaseOnlySiluDense(CoreABRBFDense):
    """Strict edge-only SiLU base layer without RBF residual."""

    def __init__(self, in_dim: int, out_dim: int, basis_count: int, *, bias: bool = False) -> None:
        super().__init__(in_dim, out_dim, basis_count, bias=bias, base_kind="baseonly_silu")
...
def _param_count_audit(model: nn.Module) -> Dict[str, float]:
    ...
    return {
        ...
        "edge_param_count_total": edge_params,
        "rbf_coeff_params": coeff_params - base_params,
        "rbf_param_count": coeff_params - base_params,
        "base_params": base_params,
        "base_param_count": base_params,
        ...
        "coverage_edge_total": 1.0 if edge_params else 0.0,
        "base_coverage": 1.0 if base_params else 0.0,
        "coverage_base": 1.0 if base_params else 0.0,
        "coverage_rbf": 1.0 if coeff_params - base_params > 0 else 0.0,
        "coverage_input": 1.0,
        "coverage_block": 1.0,
        "coverage_output": 1.0,
...
def _edge_param_manifest(model: nn.Module, *, dataset: str, method: str, edge_kind: str) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for module_name, module in model.named_modules():
        if not isinstance(module, RBFDense):
            continue
        role_depth = _role_group(_role_for_name(f"{module_name}.coeff"))
        base = _base_dim(module)
        rbf = int(module.coeff.shape[-1]) - base
        for role_edge, count in [("base", base), ("rbf", rbf)]:
            if count <= 0:
                continue
            rows.append(
                {
                    ...
                    "method": method,
                    "edge_kind": edge_kind,
                    "name": f"{module_name}.coeff.{role_edge}",
                    "shape": "x".join(str(x) for x in module.coeff.shape),
                    "numel": int(module.coeff.shape[0] * module.coeff.shape[1] * count),
                    "role_depth": role_depth,
                    "role_edge": role_edge,
                    "role_block_index": _role_for_name(f"{module_name}.coeff"),
                    "optimizer_group": role_edge,
                    "requires_grad": int(module.coeff.requires_grad),
                    "included_in_edge_update": 1,
                    "included_in_base_update": int(role_edge == "base"),
                    "included_in_rbf_update": int(role_edge == "rbf"),
                }
            )
        for attr in ["log_width", "centers_param"]:
            value = getattr(module, attr, None)
                ...
                rows.append(
                    {
                        ...
                        "role_depth": role_depth,
                        "role_edge": "width" if attr == "log_width" else "center",
                        "role_block_index": _role_for_name(f"{module_name}.coeff"),
                        "optimizer_group": attr,
                        "requires_grad": int(value.requires_grad),
                        "included_in_edge_update": 1,
                        "included_in_base_update": 0,
                        "included_in_rbf_update": 0,
                    }
                )
    return rows
...
def _base_rbf_audit(model: nn.Module) -> Dict[str, float]:
    ...
    total_norm = base_norm + rbf_norm
    base_over_rbf = base_norm / rbf_norm if rbf_norm > 1.0e-9 else float("nan")
    return {
        "base_output_norm": base_norm,
        "rbf_output_norm": rbf_norm,
        "base_over_rbf_norm": base_over_rbf,
        "base_fraction": base_norm / max(1.0e-12, total_norm),
        "rbf_fraction": rbf_norm / max(1.0e-12, total_norm),
...
def _tail_smoothing_direction(layer: RBFDense, proposal: str) -> torch.Tensor:
    base = _base_dim(layer)
    coeff = layer.coeff.detach()
    ...
    k = tail.shape[-1]
    key = proposal.lower()
    if "laplacian" in key or "sobolev" in key or "heuristic" in key:
        grad = 0.02 * tail
        if k > 1:
            grad[..., :-1] = grad[..., :-1] + (tail[..., :-1] - tail[..., 1:])
            grad[..., 1:] = grad[..., 1:] + (tail[..., 1:] - tail[..., :-1])
        out[..., base:] = -grad
    elif "high-shrink" in key and "mid" not in key:
        mask = torch.zeros(k, device=tail.device, dtype=tail.dtype)
        mask[max(1, 2 * k // 3) :] = 1.0
        out[..., base:] = -tail * mask
    elif "mid-high" in key:
        mask = torch.zeros(k, device=tail.device, dtype=tail.dtype)
        mask[max(1, k // 3) :] = 1.0
        out[..., base:] = -tail * mask
    elif "data-aware" in key:
        weights = torch.linspace(0.25, 1.25, k, device=tail.device, dtype=tail.dtype)
        out[..., base:] = -tail * weights
    elif "score-gradient" in key:
        weights = torch.linspace(0.1, 2.0, k, device=tail.device, dtype=tail.dtype)
        out[..., base:] = -tail * weights
    else:
        out[..., base:] = -tail
    norm = out[..., base:].norm().clamp_min(1.0e-12)
    ref = tail.norm().clamp_min(1.0e-12)
    out[..., base:] = out[..., base:] / norm * ref
    return out


def _residual_smoothing_updates(model: nn.Module, proposal: str, *, role: str | None = None) -> Dict[str, torch.Tensor]:
    names = [name for name, _ in coefficient_named_params(model)]
    layers = list(model.kan_layers())  # type: ignore[attr-defined]
    ...
    for name, layer in zip(names, layers):
        if role is not None and _role_group(_role_for_name(name)) != role:
            continue
        updates[name] = _tail_smoothing_direction(layer, proposal)
    return updates


def _update_norm(updates: Dict[str, torch.Tensor]) -> float:
    vals = [u.detach().float().reshape(-1) for u in updates.values()]
    return float(torch.cat(vals).norm().detach().cpu()) if vals else 0.0


def _scaled_updates(updates: Dict[str, torch.Tensor], scale: float) -> Dict[str, torch.Tensor]:
    return {k: v * scale for k, v in updates.items()}


def _holdout_loss(model: nn.Module, x: torch.Tensor, y: torch.Tensor) -> float:
    with torch.no_grad():
        return float(F.cross_entropy(model(x), y).detach().cpu())
...
def _score_trial(
    ...
    *,
    ...
    return (
        params.smooth_alpha_phi * phi_red
        + params.smooth_alpha_curv * curv_red
        + params.smooth_alpha_sob * sob_red
        - params.smooth_beta_z * constraints.get("logit_relative_drift", 0.0)
        - params.smooth_beta_h * constraints.get("hidden_relative_drift", 0.0)
        - params.smooth_beta_m * constraints.get("margin_relative_drift", 0.0)
        - params.smooth_beta_loss * max(0.0, holdout_delta)
    )


def _controller_allows(controller: str, stats: Dict[str, float], params: V49Params, *, score: float, holdout_delta: float) -> Tuple[bool, str]:
    key = controller.lower()
    if "c1" in key or "c2" in key or "c3" in key or "c4" in key or "c5" in key:
        if stats["kl_teacher_student"] >= params.smooth_kl_max:
            return False, "kl"
        if stats["logit_relative_drift"] >= params.smooth_logit_max:
            return False, "logit"
    if "c2" in key or "c3" in key or "c4" in key or "c5" in key:
        if stats["hidden_relative_drift"] >= params.smooth_hidden_max:
            return False, "hidden"
    if "c3" in key or "c4" in key or "c5" in key:
        if stats["margin_relative_drift"] >= 0.10:
            return False, "margin"
    if "c4" in key or "c5" in key:
        if score <= 0.0:
            return False, "score"
        if holdout_delta > params.smooth_holdout_worse_max:
            return False, "holdout"
    return True, ""


def _accepted_residual_smoothing_once(
    model: nn.Module,
    bundle: Any,
    params: V49Params,
    device: torch.device,
    *,
    ...
    teacher = _teacher_snapshot(model, hb)
    before_eval = _eval_v49(model, bundle, params, device)
    before_hold = _holdout_loss(model, hb, yh)
    before_phi = before_eval["phi_rbf_p95"]
    before_curv = before_eval["curvature_rbf_p95"]
    before_sob = before_eval["sobolev_rbf_norm"]
    ...
    raw_norm = _update_norm(raw_updates)
    best: Dict[str, Any] = {
        "accepted": 0,
        "accepted_eta": 0.0,
        "score": -1.0e18,
        "reject_reason": "no_positive_score",
        "actual_phiR_red": 0.0,
        "actual_curvR_red": 0.0,
        "actual_sobolev_red": 0.0,
        "KL": 0.0,
        "logit_relative_drift": 0.0,
        "hidden_relative_drift": 0.0,
        "margin_relative_drift": 0.0,
        "holdout_loss_delta": 0.0,
    }
    eta_grid = list(params.smooth_eta_grid)
    if "c5" in controller.lower():
        eta_grid = sorted(set(eta_grid + [0.70, 0.90]))
    for eta in eta_grid:
        _restore(named, snap)
        _apply_updates(named, _scaled_updates(raw_updates, eta))
        model(hb)
        after_eval = _eval_v49(model, bundle, params, device)
        constraints = _teacher_constraints(model, hb, yh, teacher, params)
        hold_delta = _holdout_loss(model, hb, yh) - before_hold
        phi_red = 1.0 - after_eval["phi_rbf_p95"] / max(1.0e-12, before_phi)
        curv_red = 1.0 - after_eval["curvature_rbf_p95"] / max(1.0e-12, before_curv)
        sob_red = 1.0 - after_eval["sobolev_rbf_norm"] / max(1.0e-12, before_sob)
        score = _score_trial(params, phi_red=phi_red, curv_red=curv_red, sob_red=sob_red, constraints=constraints, holdout_delta=hold_delta)
        ok, reason = _controller_allows(controller, constraints, params, score=score, holdout_delta=hold_delta)
        if ok and score > float(best["score"]):
            best = {
                ...
                "score": score,
                "reject_reason": "",
                "actual_phiR_red": phi_red,
                "actual_curvR_red": curv_red,
                "actual_sobolev_red": sob_red,
                "holdout_loss_delta": hold_delta,
                "acc_after_trial": after_eval["test_acc"],
                **{k: constraints.get(k, float("nan")) for k in ["kl_teacher_student", "logit_relative_drift", "hidden_relative_drift", "margin_relative_drift"]},
            }
        elif not ok and score > float(best["score"]) and not int(best.get("accepted", 0)):
    ...
    best.update(
        {
            ...
            "projected_direction_norm": raw_norm,
            "projected_over_raw": 1.0 if raw_norm > 0 else 0.0,
            "predicted_phiR_red": best.get("actual_phiR_red", 0.0),
            "predicted_curvR_red": best.get("actual_curvR_red", 0.0),
            "predicted_sobolev_red": best.get("actual_sobolev_red", 0.0),
            "acc_before": before_eval["test_acc"],
            "acc_after": after_eval["test_acc"],
            "acc_drop": acc_drop,
            "phiR_before": before_phi,
            "phiR_after": after_eval["phi_rbf_p95"],
            "curvR_before": before_curv,
            "curvR_after": after_eval["curvature_rbf_p95"],
            "sobolev_before": before_sob,
            "sobolev_after": after_eval["sobolev_rbf_norm"],
            "rank_after": after_eval["rank_block"],
            "margin_after": after_eval["margin_p10"],
            "ECE_after": after_eval["ece"],
            "seed_used": seed,
        }
    )
    return best
...
def run_p3_residual_audit(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p3_residual_smoothing_proposal_audit.csv")
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in seeds:
            for teacher in P3_TEACHERS:
                for proposal in P3_PROPOSALS:
                    for controller in P3_CONTROLLERS:
                        ...
                        try:
                            ...
                            stats = _accepted_residual_smoothing_once(
                                model,
                                bundle,
                                params,
                                device,
                            ...
                            p3_pass = (
                                stats["acc_drop"] <= params.smooth_acc_drop_max
                                and stats.get("kl_teacher_student", 99.0) < params.smooth_kl_max
                                and stats.get("logit_relative_drift", 99.0) < params.smooth_logit_max
                                and (stats["actual_phiR_red"] > 0.05 or stats["actual_curvR_red"] > 0.10)
                                and int(stats.get("accepted", 0)) == 1
                            )
                            row = {
                                "stage": "P3",
                                "dataset": dataset,
                                "seed": seed,
                                "teacher": teacher,
                                "edge_kind": _edge_kind_from_method(teacher),
                                "proposal": proposal,
                                "controller": controller,
                                "teacher_acc": meta.get("test_acc", float("nan")),
                                "teacher_phiR": meta.get("phi_rbf_p95", float("nan")),
                                "p3_pass": int(p3_pass),
                                "error": "",
                                **stats,
                            }
                            rows.append(row)
                            print(
                                f"P3 {dataset} {teacher} {proposal}/{controller} "
                                f"accDrop={stats['acc_drop']:.4f} phiRRed={stats['actual_phiR_red']:.3f} "
                                f"score={stats['score']:.3f} pass={int(p3_pass)}"
                            )
                        except Exception as exc:
                            if not args.continue_on_error:
                                raise
                            rows.append({"stage": "P3", "dataset": dataset, "seed": seed, "teacher": teacher, "proposal": proposal, "controller": controller, "error": repr(exc)})
                            print(f"P3 ERROR {dataset} {teacher} {proposal}/{controller}: {exc!r}")
                        write_csv(out_dir / "p3_residual_smoothing_proposal_audit.csv", rows)
    return rows


def _p3_smoothing_survivors(out_dir: Path, *, top_k: int = 3) -> List[Tuple[str, str, str]]:
    rows = [r for r in read_csv(out_dir / "p3_residual_smoothing_proposal_audit.csv") if not r.get("error")]
    grouped: Dict[Tuple[str, str, str], List[Dict[str, Any]]] = {}
    for row in rows:
        key = (str(row.get("teacher", "")), str(row.get("proposal", "")), str(row.get("controller", "")))
        grouped.setdefault(key, []).append(row)
    scored: List[Tuple[float, Tuple[str, str, str]]] = []
    for key, rs in grouped.items():
        passed = {r.get("dataset") for r in rs if int(float(r.get("p3_pass", 0) or 0)) == 1}
        if len(passed) >= 2:
            score = _mean([float(r.get("score", 0.0) or 0.0) for r in rs], 0.0)
            scored.append((score, key))
    scored.sort(reverse=True, key=lambda x: x[0])
    return [key for _, key in scored[:top_k]]


def run_p4_accepted_smoothing(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p4_accepted_residual_smoothing.csv")
    trace: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p4_accepted_residual_smoothing_trace.csv")
    survivors = _p3_smoothing_survivors(out_dir)
    if not survivors:
        rows = [{"status": "not_run", "reason": "P3 produced no two-dataset residual smoothing survivor"}]
        write_csv(out_dir / "p4_accepted_residual_smoothing.csv", rows)
        print("P4 not run: no P3 smoothing survivor")
        return rows
    device = get_device(args.device)
    params = V49Params(test_size=256, val_size=256, audit_batch_size=32, eval_batch_size=256)
    seeds = parse_int_list(args.seeds)[:1]
    done = {(r.get("dataset"), int(float(r.get("seed", -1))), r.get("teacher"), r.get("proposal"), r.get("controller"), int(float(r.get("steps", -1)))) for r in rows if not r.get("error") and r.get("status") != "not_run"}
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in seeds:
            for teacher, proposal, controller in survivors:
                for steps in [5, 20]:
                    key = (dataset, seed, teacher, proposal, controller, steps)
                    if key in done:
                        continue
                    try:
                        bundle, model, meta = _load_or_train_teacher(args, dataset, seed, teacher, params, device)
                        before = _eval_v49(model, bundle, params, device)
                        before_hold = _holdout_loss(model, bundle.x_val[: params.audit_batch_size].to(device), bundle.y_val[: params.audit_batch_size].to(device))
                        accepted = 0
                            ...
                            trace.append(
                                {
                                    ...
                                    "accepted_eta": stats.get("accepted_eta", 0.0),
                                    "score": stats.get("score", float("nan")),
                                    "phiR": stats.get("phiR_after", float("nan")),
                                    "curvR": stats.get("curvR_after", float("nan")),
                                    "sobolev_rbf": stats.get("sobolev_after", float("nan")),
                                    "acc": stats.get("acc_after", float("nan")),
                                    "KL": stats.get("kl_teacher_student", float("nan")),
                                    "logit_drift": stats.get("logit_relative_drift", float("nan")),
                                    "hidden_drift": stats.get("hidden_relative_drift", float("nan")),
                                    "margin": stats.get("margin_after", float("nan")),
                                    "rank": stats.get("rank_after", float("nan")),
                                }
                            )
                        ...
                        phi_red = 1.0 - after["phi_rbf_p95"] / max(1.0e-12, before["phi_rbf_p95"])
                        curv_red = 1.0 - after["curvature_rbf_p95"] / max(1.0e-12, before["curvature_rbf_p95"])
                        p4_pass = (
                            acc_drop <= 0.005
                            and phi_red > 0.08
                            and (curv_red > 0.15 or phi_red > 0.12)
                            and hold_delta <= 0.02 * max(1.0e-12, before_hold)
                        )
                        row = {
                            "stage": "P4",
                            "dataset": dataset,
                            ...
                            "acc_drop": acc_drop,
                            "teacher_phiR": before["phi_rbf_p95"],
                            "final_phiR": after["phi_rbf_p95"],
                            "phiR_red": phi_red,
                            "curvR_red": curv_red,
                            "sobolevR_red": 1.0 - after["sobolev_rbf_norm"] / max(1.0e-12, before["sobolev_rbf_norm"]),
                            "holdout_loss_delta": hold_delta,
                            "accept_rate": accepted / max(1, steps),
                            "rank_final": after["rank_block"],
                            "margin_final": after["margin_p10"],
                            "ECE_final": after["ece"],
                            "p4_pass": int(p4_pass),
                            "error": "",
                        }
                        rows.append(row)
                        print(f"P4 {dataset} {teacher} {proposal}/{controller} steps={steps} accDrop={acc_drop:.4f} phiRRed={phi_red:.3f} pass={int(p4_pass)}")
                    except Exception as exc:
                        if not args.continue_on_error:
                            raise
                        rows.append({"stage": "P4", "dataset": dataset, "seed": seed, "teacher": teacher, "proposal": proposal, "controller": controller, "steps": steps, "error": repr(exc)})
                        print(f"P4 ERROR {dataset} {teacher} {proposal}/{controller}: {exc!r}")
                    write_csv(out_dir / "p4_accepted_residual_smoothing.csv", rows)
                    write_csv(out_dir / "p4_accepted_residual_smoothing_trace.csv", trace)
    return rows


def run_p0(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p0_core_smoke.csv")
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        ...
        for method in P0_EDGE_KINDS:
            ...
            try:
                ...
                model(hb)
                finite_forward = int(torch.isfinite(model(hb)).all().item())
                loss = F.cross_entropy(model(hb), yh)
                loss.backward()
                finite_backward = int(all(p.grad is None or torch.isfinite(p.grad).all().item() for _, p in _edge_named_params(model)))
                ...
                row = {
                    ...
                    **pcount,
                    ...
                    "finite_split_geometry": int(math.isfinite(float(_split_geometry_audit(model).get("phi_total_p95", float("nan"))))),
            ...
            if not row.get("error"):
                manifest.extend(_edge_param_manifest(model, dataset=dataset, method=method, edge_kind=edge_kind))  # type: ignore[name-defined]
            print(f"P0 {dataset} {method} nonKAN={row.get('learnable_nonKAN_params')} rollback={row.get('rollback_error')} err={row.get('error','')}")
            write_csv(out_dir / "p0_core_smoke.csv", rows)
...
def run_p1(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p1_architecture_confirm5.csv")
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in parse_int_list(args.seeds):
            for method in methods:
                ...
                write_csv(out_dir / "p1_architecture_confirm5.csv", rows)
...
def run_p2(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p2_split_geometry_calibration.csv")
    device = get_device(args.device)
    params = V49Params()
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in parse_int_list(args.seeds):
            for method in methods:
                ...
                yh = bundle.y_val[: params.audit_batch_size].to(device)
                model(hb)
                split_geom = _split_geometry_audit(model)
                model.zero_grad(set_to_none=True)
                ...
                write_csv(out_dir / "p2_split_geometry_calibration.csv", rows)
    return rows
...
def run_p3_split(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p7_split_residual_training.csv")
    trace: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p7_split_residual_training_trace.csv")
    device = get_device(args.device)
    params = V49Params()
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in parse_int_list(args.seeds):
            for method in methods:
                ...
                write_csv(out_dir / "p7_split_residual_training.csv", rows)
                write_csv(out_dir / "p7_split_residual_training_trace.csv", trace)
    return rows
...
def _p4_survivors(out_dir: Path) -> List[Tuple[str, str, str, str]]:
    rows = [r for r in read_csv(out_dir / "p4_accepted_residual_smoothing.csv") if not r.get("error") and r.get("status") != "not_run"]
    by_key: Dict[Tuple[str, str, str, str], Dict[str, bool]] = {}
    for row in rows:
        key = (row.get("teacher", ""), row.get("proposal", ""), row.get("controller", ""), str(row.get("steps", "")))
...
def run_p5_cycle(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    if not survivors:
        rows = [{"status": "not_run", "reason": "P4 produced no all-dataset accepted smoothing survivor"}]
        write_csv(out_dir / "p5_learn_smooth_refresh_v3.csv", rows)
        print("P5 not run: no P4 all-dataset survivor")
        return rows
    rows: List[Dict[str, Any]] = [] if args.fresh else [r for r in read_csv(out_dir / "p5_learn_smooth_refresh_v3.csv") if r.get("status") != "not_run"]
    device = get_device(args.device)
    params = V49Params()
    seeds = parse_int_list(args.seeds)
    if len(seeds) > 1:
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in seeds:
            for teacher_method, proposal, controller, smooth_steps in survivors[:3]:
                for refresh in ["none", "AdamW-small-5"]:
                    ...
                    for local_step in range(max(1, min(5, int(float(smooth_steps or 5))))):
                        _accepted_residual_smoothing_once(
                            model,
                            bundle,
                            params,
                            device,
                    ...
                    rows.append(row)
                    print(f"P5 {dataset} {teacher_method} {proposal}/{controller} {refresh} accDrop={acc_drop:.4f} phiRed={phi_red:.3f} pass={int(p4_pass)}")
                    write_csv(out_dir / "p5_learn_smooth_refresh_v3.csv", rows)
    return rows
...
def run_p6_events(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    p4 = [r for r in read_csv(out_dir / "p5_learn_smooth_refresh_v3.csv") if not r.get("error") and r.get("status") != "not_run"]
    by_key: Dict[Tuple[str, str, str, str, str], Dict[str, bool]] = {}
    for row in p4:
        key = (row.get("task_teacher", ""), row.get("proposal", ""), row.get("controller", ""), str(row.get("smooth_steps", "")), row.get("refresh", ""))
    ...
    if not survivors:
        ...
        write_csv(out_dir / "p6_event_driven_tan_v2.csv", rows)
        print("P6 not run: no P5 all-dataset survivor")
        return rows
    rows: List[Dict[str, Any]] = []
    device = get_device(args.device)
    params = V49Params()
    task_teacher, proposal, controller, smooth_steps, refresh = survivors[0]
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        ...
        for cycle in range(1, 4):
            ...
            if triggered:
                for local_step in range(max(1, min(5, int(float(smooth_steps or 5))))):
                    _accepted_residual_smoothing_once(model, bundle, params, device, proposal=proposal, controller=controller, seed=seed + 5060 + cycle * 10 + local_step)
            if refresh == "AdamW-small-5":
...
def run_p7_capacity(args: argparse.Namespace) -> List[Dict[str, Any]]:
    out_dir = ensure_dir(args.out_dir)
    rows: List[Dict[str, Any]] = [] if args.fresh else read_csv(out_dir / "p8_capacity_basis_followup.csv")
    device = get_device(args.device)
    seeds = parse_int_list(args.seeds)
    if len(seeds) > 1:
        seeds = [seeds[0]]
    ...
    for dataset in [dataset_name(d) for d in parse_str_list(args.datasets)]:
        for seed in seeds:
            for hidden, basis, depth, edge_kind in P7_CONFIGS:
                ...
                write_csv(out_dir / "p8_capacity_basis_followup.csv", rows)
    return rows
...
def add_args() -> argparse.ArgumentParser:
    ...
    p.set_defaults(packages="V5_1_P0_SMOKE", datasets="MNIST,Fashion-MNIST,KMNIST", out_dir=Path("results/v5_1"), seeds="0,1,2,3,4")
    return p
...
def main() -> int:
    ...
    for package in parse_str_list(args.packages):
        ...
        if key in {"V5_1_P0", "V5_1_P0_SMOKE", "V5_1_P0_CORE"}:
            all_rows = run_p0(args)
        elif key in {"V5_1_P1", "V5_1_P1_CONFIRM5", "V5_1_P1_ARCHITECTURE"}:
            all_rows = run_p1(args)
        elif key in {"V5_1_P2", "V5_1_P2_GEOMETRY", "V5_1_P2_SPLIT_GEOMETRY"}:
            all_rows = run_p2(args)
        elif key in {"V5_1_P3", "V5_1_P3_PROPOSAL", "V5_1_P3_RESIDUAL_SMOOTHING"}:
            all_rows = run_p3_residual_audit(args)
        elif key in {"V5_1_P4", "V5_1_P4_ACCEPTED_SMOOTHING"}:
            all_rows = run_p4_accepted_smoothing(args)
            _blank_later_files(ensure_dir(args.out_dir), "pending P4/P5 gate decision")
        elif key in {"V5_1_P5", "V5_1_P5_LEARN_SMOOTH_REFRESH"}:
            all_rows = run_p5_cycle(args)
        elif key in {"V5_1_P6", "V5_1_P6_EVENT_TAN"}:
            all_rows = run_p6_events(args)
        elif key in {"V5_1_P7", "V5_1_P7_SPLIT_RESIDUAL_TRAINING"}:
            all_rows = run_p3_split(args)
        elif key in {"V5_1_P8", "V5_1_P8_CAPACITY"}:
            all_rows = run_p7_capacity(args)
        else:
            raise ValueError(f"unknown v5.1 package: {package}")
