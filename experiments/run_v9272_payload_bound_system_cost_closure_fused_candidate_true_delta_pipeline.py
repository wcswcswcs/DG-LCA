#!/usr/bin/env python3
"""DG-KAN v9.2.72 payload-bound system cost closure audit.

This runner starts from the v9.2.71 payload-bound controller and attempts to
close the remaining step-level cost gap with a fused candidate true-delta
pipeline. It keeps full-online payload/update binding, uses true branch-delta
logits, and refuses to promote the controller when the measured full path stays
outside the system envelope.
"""
...
PLAN_PATH = ROOT / "docs" / "DG-KAN_v9.2.72_PayloadBoundSystemCostClosure_FusedCandidateTrueDeltaPipeline_完整实验计划.md"
SCRIPT_PATH = ROOT / "experiments" / "run_v9272_payload_bound_system_cost_closure_fused_candidate_true_delta_pipeline.py"
RESULT_ROOT = ROOT / "results" / "real_rerun_20260506"
SRC_V9271 = RESULT_ROOT / "v9271_full_online_payload_binding_functional_update_payload_closure_first_20260513T063000Z"
CUDA_EXTENSION_ERROR = ""
_CANDIDATE_DELTA_EXT = None
...
def _load_candidate_delta_ext() -> Any:
    global CUDA_EXTENSION_ERROR, _CANDIDATE_DELTA_EXT
    if _CANDIDATE_DELTA_EXT is not None:
        return _CANDIDATE_DELTA_EXT
    if not torch.cuda.is_available():
        CUDA_EXTENSION_ERROR = "cuda_not_available"
        return None
    cpp_src = """
    #include <torch/extension.h>
    std::vector<torch::Tensor> candidate_delta_logits_forward(
        torch::Tensor vals_update,
        torch::Tensor ce,
        torch::Tensor margin,
        torch::Tensor tail,
        torch::Tensor pred,
        torch::Tensor y,
        torch::Tensor row_abs_mean,
        torch::Tensor vals_probe,
        torch::Tensor task_probe_logits,
        torch::Tensor ce_mean,
        torch::Tensor vals_abs_mean,
        torch::Tensor task_norm);
    """
    cuda_src = r"""
    #include <torch/extension.h>
    #include <cuda.h>
    ...
    __global__ void raw_delta_norm_kernel(
        const float* __restrict__ vals_update,
        const float* __restrict__ ce,
        const float* __restrict__ margin,
        const bool* __restrict__ tail,
        const long* __restrict__ pred,
        const long* __restrict__ y,
        const float* __restrict__ row_abs_mean,
        const float* __restrict__ ce_mean_ptr,
        const float* __restrict__ vals_abs_mean_ptr,
      ...
      const int h = (idx / C) % H;
      const int k = idx / (H * C);
      const float ce_mean = ce_mean_ptr[0];
      const float vals_abs_mean = vals_abs_mean_ptr[0];
      float strength = (k == 0 ? 0.040f : (k == 1 ? 0.055f : 0.070f));
      float acc = 0.0f;
      for (int b = 0; b < B; ++b) {
        if (!tail[b]) continue;
        float w = ce[b] / fmaxf(ce_mean, 1.0e-6f);
        if (k == 1) {
          w *= (margin[b] < 0.0f ? 1.5f : 0.5f);
        } else if (k == 2) {
          w *= row_abs_mean[b] / fmaxf(vals_abs_mean, 1.0e-6f);
        }
        float sig = 0.0f;
        if (o == (int)y[b]) sig += w;
        if (o == (int)pred[b]) sig -= 0.5f * w;
        acc += vals_update[b * H + h] * (sig / (float)B);
      }
      const float value = strength * acc;
      delta[idx] = value;
      atomicAdd(&norm2[k], value * value);
    }

    __global__ void scale_delta_kernel(
        float* __restrict__ delta,
        const float* __restrict__ norm2,
        const float* __restrict__ task_norm_ptr,
        const int H,
        const int C) {
      const int idx = blockIdx.x * blockDim.x + threadIdx.x;
      const int total = 3 * H * C;
      ...
      const float task_norm = task_norm_ptr[0];
      const float func_norm = sqrtf(fmaxf(norm2[k], 1.0e-24f));
    ...
    __global__ void logits_kernel(
        const float* __restrict__ vals_probe,
        const float* __restrict__ task_probe_logits,
      ...
      const int p = (idx / C) % P;
      const int k = idx / (P * C);
      float acc = task_probe_logits[p * C + o];
      const int delta_base = k * H * C + o;
      for (int h = 0; h < H; ++h) {
        acc += vals_probe[p * H + h] * delta[delta_base + h * C];
      }
      logits[idx] = acc;
    }
    ...
    std::vector<torch::Tensor> candidate_delta_logits_forward(
        ...
        torch::Tensor vals_abs_mean,
        torch::Tensor task_norm) {
      TORCH_CHECK(vals_update.is_cuda(), "vals_update must be CUDA");
      TORCH_CHECK(vals_probe.is_cuda(), "vals_probe must be CUDA");
      TORCH_CHECK(task_probe_logits.is_cuda(), "task_probe_logits must be CUDA");
      TORCH_CHECK(vals_update.is_contiguous(), "vals_update must be contiguous");
      TORCH_CHECK(vals_probe.is_contiguous(), "vals_probe must be contiguous");
      TORCH_CHECK(task_probe_logits.is_contiguous(), "task_probe_logits must be contiguous");
      const int B = (int)vals_update.size(0);
      const int H = (int)vals_update.size(1);
      const int P = (int)vals_probe.size(0);
      const int C = (int)task_probe_logits.size(1);
      auto delta = torch::empty({3, H, C}, vals_update.options());
      auto norm2 = torch::zeros({3}, vals_update.options());
      auto logits = torch::empty({3, P, C}, vals_update.options());
      const int threads = 256;
      const int delta_total = 3 * H * C;
      raw_delta_norm_kernel<<<(delta_total + threads - 1) / threads, threads>>>(
          vals_update.data_ptr<float>(),
          ce.data_ptr<float>(),
          margin.data_ptr<float>(),
          tail.data_ptr<bool>(),
          pred.data_ptr<long>(),
          y.data_ptr<long>(),
          row_abs_mean.data_ptr<float>(),
          ce_mean.data_ptr<float>(),
          vals_abs_mean.data_ptr<float>(),
          delta.data_ptr<float>(),
          norm2.data_ptr<float>(),
          B, H, C);
      scale_delta_kernel<<<(delta_total + threads - 1) / threads, threads>>>(
          delta.data_ptr<float>(),
          norm2.data_ptr<float>(),
          task_norm.data_ptr<float>(),
          H, C);
      const int logits_total = 3 * P * C;
      logits_kernel<<<(logits_total + threads - 1) / threads, threads>>>(
          vals_probe.data_ptr<float>(),
          task_probe_logits.data_ptr<float>(),
          delta.data_ptr<float>(),
    ...
    """
    try:
        _CANDIDATE_DELTA_EXT = load_inline(
            name="dgkan_v9272_candidate_delta_logits_ext",
            cpp_sources=cpp_src,
            cuda_sources=cuda_src,
            functions=["candidate_delta_logits_forward"],
            with_cuda=True,
            extra_cuda_cflags=["-O3"],
            verbose=False,
        )
    except Exception as exc:  # pragma: no cover - recorded in artifacts.
        CUDA_EXTENSION_ERROR = str(exc).replace("\n", " ")[:500]
        _CANDIDATE_DELTA_EXT = None
    return _CANDIDATE_DELTA_EXT
...
def _p0_boundary() -> Dict[str, Any]:
    route = _read_json(SRC_V9271 / "route_decision.json")
    audit_rows = read_csv_rows(SRC_V9271 / "v9271_provenance_audit.csv")
    audit = audit_rows[0] if audit_rows else {}
    p0_pass = int(
        route.get("route") == "R17-PayloadBoundSystemStillExpensive"
        ...
        and _f(route.get("controller_step_ratio_q90", route.get("true_delta_step_ratio_q90"))) > 1.50
        and _i(audit.get("fake_proxy_nonzero_count")) == 0
        and _i(audit.get("proxy_row_used")) == 0
        and _i(audit.get("cpu_offload_used")) == 0
    )
    return {
        "stage": "P0_V9271_BOUNDARY_REPRODUCTION",
        "status": "source_boundary",
        "source_route_v9271": route.get("route", ""),
        ...
        "candidate_tensor_payload_pass": route.get("candidate_tensor_payload_pass", ""),
        "candidate_branch_logits_payload_pass": route.get("candidate_branch_logits_payload_pass", ""),
        "candidate_true_delta_logits_payload_pass": route.get("candidate_true_delta_logits_payload_pass", ""),
        ...
        "functional_update_payload_pass": route.get("functional_update_payload_pass", ""),
        "controller_step_ratio_q90": route.get("controller_step_ratio_q90", route.get("true_delta_step_ratio_q90", "")),
...
def _build_event_table(ctx: Dict[str, Any], p2: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for row in rows:
        ...
        row["source_hash"] = _hash_text(f"v9272|{idx}|{row.get('dataset')}|{row.get('seed')}|{row.get('step')}|{row.get('event_family')}")[:32]
    return rows, summary
...
def _prepare_exact_w2_delta_context(
    ...
    task_norm_tensor = torch.sqrt(sum((d.detach() * d.detach()).sum() for d in task_delta))
    return {
        ...
        "tail": tail,
        "y_update": yu,
        "row_abs_mean": row_abs_mean,
        "ce_mean_tensor": ce.mean().contiguous(),
        "vals_abs_mean_tensor": row_abs_mean.mean().contiguous(),
        "vals_probe": vals_probe,
        "task_probe_logits": task_probe_logits,
        "task_norm_tensor": task_norm_tensor,
    }


def _prepare_exact_w2_delta_context_combined(
    task_params: Sequence[torch.Tensor],
    mu: torch.Tensor,
    std: torch.Tensor,
    spec: lq.LQSpec,
    ...
    x_all = torch.cat([xu, xp], dim=0).contiguous()
    h_all = x_all @ A
    vals_all, _ders_all = lq.basis_from_lift(h_all, mu, std, spec.basis, 2.0, 2.0)
    logits_all = (vals_all[0] @ W0 + vals_all[1] @ W2).contiguous()
    ...
    logits_update = logits_all[:update_n]
    task_probe_logits = logits_all[update_n:].contiguous()
    logp = logits_update.log_softmax(dim=1)
    ce = -logp[torch.arange(yu.numel(), device=yu.device), yu]
    pred = logits_update.argmax(dim=1)
    true_logits = logits_update[torch.arange(yu.numel(), device=yu.device), yu]
...
def _functional_delta_exact_w2(
    ...
    task_norm = float(ctx.get("task_norm", ctx["task_norm_tensor"].detach().cpu()))
...
def _functional_delta_exact_w2_batch(
    params: Sequence[torch.Tensor],
    y_update: torch.Tensor,
    carrier_ids: Sequence[str],
    ctx: Dict[str, Any],
) -> Dict[str, Tuple[List[torch.Tensor], Dict[str, float], torch.Tensor]]:
    vals = ctx["vals_update"]
    vals2 = vals[1]
    ce = ctx["ce"]
    pred = ctx["pred"]
    tail = ctx["tail"]
    margin = ctx["margin"]
    device = y_update.device
    dtype = params[-1].dtype
    c_out = int(params[-1].shape[1])
    idx = torch.arange(y_update.numel(), device=device)
    ce_mean = ce.mean().clamp_min(1.0e-6)
    vals_abs_mean = vals2.abs().mean().clamp_min(1.0e-6)
    ...
    for carrier_id in carrier_ids:
        if carrier_id == "A1-RiskBoundedTailCarrier":
            ...
            strength, cap = 0.055, 0.050
            weights = (ce / ce_mean) * (margin < 0).float().add(0.5)
        else:
            strength, cap = 0.070, 0.070
            weights = (ce / ce_mean) * (vals2.abs().mean(dim=1) / vals_abs_mean)
        signal = torch.zeros((int(y_update.numel()), c_out), device=device, dtype=dtype)
        signal[idx[tail], y_update[tail]] += weights[tail]
        signal[idx[tail], pred[tail]] -= 0.5 * weights[tail]
        signals.append(signal / max(1, int(y_update.numel())))
        strengths.append(strength)
        caps.append(cap)
    signal_stack = torch.stack(signals, dim=0).contiguous()
    d_w2 = torch.einsum("bh,cbo->cho", vals2, signal_stack)
    strength_t = torch.tensor(strengths, device=device, dtype=dtype).view(-1, 1, 1)
    raw_delta = d_w2 * strength_t
    func_norm = torch.sqrt((raw_delta * raw_delta).sum(dim=(1, 2))).clamp_min(1.0e-12)
    cap_t = torch.tensor(caps, device=device, dtype=dtype)
    max_norm = cap_t * ctx["task_norm_tensor"].to(dtype=dtype).clamp_min(1.0e-12)
    scale = torch.minimum(torch.ones_like(func_norm), max_norm / func_norm).view(-1, 1, 1)
    delta_w2 = (raw_delta * scale).contiguous()
    logits_stack = (
        torch.einsum("ph,cho->cpo", ctx["vals_probe"][1], delta_w2)
        + ctx["task_probe_logits"].unsqueeze(0)
    ).contiguous()
    zero_a = torch.zeros_like(params[0])
    zero_w0 = torch.zeros_like(params[1])
    out: Dict[str, Tuple[List[torch.Tensor], Dict[str, float], torch.Tensor]] = {}
    for j, carrier_id in enumerate(carrier_ids):
        out[str(carrier_id)] = ([zero_a, zero_w0, delta_w2[j]], {}, logits_stack[j])
    return out


def _functional_delta_exact_w2_cuda_ext(
    params: Sequence[torch.Tensor],
    carrier_ids: Sequence[str],
    ctx: Dict[str, Any],
    ext: Any,
) -> Dict[str, Tuple[List[torch.Tensor], Dict[str, float], torch.Tensor]]:
    logits_all, delta_all = ext.candidate_delta_logits_forward(
        ctx["vals_update"][1].contiguous(),
        ctx["ce"].contiguous(),
        ctx["margin"].contiguous(),
        ctx["tail"].contiguous(),
        ctx["pred"].contiguous(),
        ctx["y_update"].contiguous(),
        ctx["row_abs_mean"].contiguous(),
        ctx["vals_probe"][1].contiguous(),
        ctx["task_probe_logits"].contiguous(),
        ctx["ce_mean_tensor"],
        ctx["vals_abs_mean_tensor"],
        ctx["task_norm_tensor"].contiguous(),
    )
    zero_a = torch.zeros_like(params[0])
    zero_w0 = torch.zeros_like(params[1])
    out: Dict[str, Tuple[List[torch.Tensor], Dict[str, float], torch.Tensor]] = {}
    for carrier_id in carrier_ids:
        ...
        if carrier == "A1-RiskBoundedTailCarrier":
            j = 0
        elif carrier == "A2-LateAttachControlGapChannel":
            j = 1
        else:
            j = 2
        out[str(carrier_id)] = ([zero_a, zero_w0, delta_all[j]], {}, logits_all[j])
    return out
...
def _materialize_payloads(
    ...
    candidate_delta_ext = None
    if not bool(getattr(args, "disable_cuda_candidate_delta_ext", False)) and device.type == "cuda":
        candidate_delta_ext = _load_candidate_delta_ext()
    ...
    step_candidate_counts: Dict[Tuple[str, int, int], int] = {}
    step_component_ms: Dict[Tuple[str, int, int], Dict[str, float]] = defaultdict(lambda: defaultdict(float))
    candidate_branch_ms: List[float] = []
    true_delta_ms: List[float] = []
    payload_pack_ms: List[float] = []
    payload_apply_ms: List[float] = []
    state_replay_extra_forward_count = 0
    state_replay_delta_construct_count = 0
    optimized_identity_check_count = 0
    optimized_identity_error_max = 0.0
    cuda_ext_candidate_count = 0
    cuda_ext_step_count = 0
    cuda_vs_torch_check_count = 0
    cuda_vs_torch_logits_error_max = 0.0
    cuda_vs_torch_delta_error_max = 0.0
    fused_common_basis_delta_step_count = 0
    fused_common_basis_delta_candidate_count = 0
    event_idx = 0
    ...
    for dataset in [v9248.v92._canonical_task(x) for x in v9248._parse_list(args.datasets)]:
        for seed in v9248._parse_ints(args.seeds):
            ...
            for step in range(int(args.microprobe_steps)):
                ...
                step_candidate_counts[key] = step_candidate_count
                needs_state_replay_this_step = original_accept_carrier in set(v9248.CARRIERS)
                use_exact_w2_step = step_candidate_count >= int(getattr(args, "exact_w2_min_candidates", 1))
                fast_ctx: Dict[str, Any] | None = None
                common_ms = 0.0
                fused_candidate_cache: Dict[str, Tuple[List[torch.Tensor], Dict[str, float], torch.Tensor, float]] = {}
                if use_exact_w2_step:
                    candidate_carriers = [
                        v9248.CARRIERS[i]
                        for i, global_idx in enumerate(step_event_indices)
                        if global_idx in candidate_indices
                    ]
                    if candidate_carriers:
                        use_fused_common = (
                            candidate_delta_ext is not None
                            and not bool(getattr(args, "disable_fused_common_basis_delta_pipeline", False))
                        )
                        v9248._sync(device)
                        fused_t0 = time.perf_counter()
                        if use_fused_common:
                            fast_ctx = _prepare_exact_w2_delta_context_combined(task_params, mu, std, spec, xu, yu, xp, task_delta)
                            batched = _functional_delta_exact_w2_cuda_ext(task_params, candidate_carriers, fast_ctx, candidate_delta_ext)
                            cuda_ext_candidate_count += len(candidate_carriers)
                            cuda_ext_step_count += 1
                            fused_common_basis_delta_step_count += 1
                            fused_common_basis_delta_candidate_count += len(candidate_carriers)
                        else:
                            ...
                            step_component_ms[key]["common_basis_prep_ms"] += common_ms
                            fused_t0 = time.perf_counter()
                            if candidate_delta_ext is not None:
                                batched = _functional_delta_exact_w2_cuda_ext(task_params, candidate_carriers, fast_ctx, candidate_delta_ext)
                                cuda_ext_candidate_count += len(candidate_carriers)
                                cuda_ext_step_count += 1
                            else:
                                batched = _functional_delta_exact_w2_batch(task_params, yu, candidate_carriers, fast_ctx)
                        for cc, (fd_cc, fmeta_cc, logits_cc) in batched.items():
                            fused_candidate_cache[cc] = (fd_cc, fmeta_cc, logits_cc, 0.0)
                        v9248._sync(device)
                        fused_ms = max(0.0, (time.perf_counter() - fused_t0) * 1000.0)
                        if use_fused_common:
                            step_component_ms[key]["fused_common_basis_delta_pipeline_ms"] += fused_ms
                            step_extra[key] += fused_ms
                            per_candidate_ms = fused_ms / max(1, len(candidate_carriers))
                        else:
                            step_component_ms[key]["fused_candidate_delta_logits_ms"] += fused_ms
                            step_extra[key] += common_ms + fused_ms
                            per_candidate_ms = (common_ms + fused_ms) / max(1, len(candidate_carriers))
                        fused_candidate_cache = {
                            cc: (fd_cc, fmeta_cc, logits_cc, per_candidate_ms)
                            for cc, (fd_cc, fmeta_cc, logits_cc, _old_ms) in fused_candidate_cache.items()
                        }
                        if candidate_delta_ext is not None and cuda_vs_torch_check_count < int(getattr(args, "identity_check_candidates", 24)):
                            torch_ref = _functional_delta_exact_w2_batch(task_params, yu, candidate_carriers, fast_ctx)
                            for cc in candidate_carriers:
                                if cuda_vs_torch_check_count >= int(getattr(args, "identity_check_candidates", 24)):
                                    break
                                fd_ext, _meta_ext, logits_ext, _ms_ext = fused_candidate_cache[cc]
                                fd_ref, _meta_ref, logits_ref = torch_ref[cc]
                                logits_err = float((logits_ext - logits_ref).abs().max().detach().cpu())
                                delta_err = float((fd_ext[2] - fd_ref[2]).abs().max().detach().cpu())
                                cuda_vs_torch_logits_error_max = max(cuda_vs_torch_logits_error_max, logits_err)
                                cuda_vs_torch_delta_error_max = max(cuda_vs_torch_delta_error_max, delta_err)
                                cuda_vs_torch_check_count += 1
                ...
                for carrier_id in v9248.CARRIERS:
                    ...
                    branch_ms = 0.0
                    if is_candidate and carrier_id in fused_candidate_cache:
                        fd, fmeta, cand_logits, branch_ms = fused_candidate_cache[carrier_id]
                    ...
                    elif is_candidate or needs_state_replay:
                        v9248._sync(device)
                        ...
                        elif needs_state_replay:
                            state_replay_delta_construct_count += 1
                        v9248._sync(device)
                        branch_ms = max(0.0, (time.perf_counter() - bt0) * 1000.0)
                        if is_candidate:
                            step_component_ms[key]["unfused_candidate_forward_ms"] += branch_ms
                            step_extra[key] += branch_ms
                        if needs_state_replay:
                            commit_params = cand_params
                    if is_candidate and fd is not None and cand_logits is not None:
                        ...
                        step_component_ms[key]["true_delta_selected_feature_ms"] += delta_ms
                        if carrier_id in fused_candidate_cache:
                            step_extra[key] += delta_ms
                        else:
                        ...
                        candidate_payload_rows.append({
                            "stage": "P2_CANDIDATE_PAYLOAD_LIFECYCLE_OPTIMIZATION",
                        ...
                        branch_rows.append({
                            "stage": "P4_FUSED_BRANCH_FORWARD_TRUE_DELTA_BRIDGE_PIPELINE",
                            "status": "candidate_branch_logits",
                            "branch_forward_id": (
                                "BF5-AsyncCombinedBasisFewKernelCudaPipeline"
                                if candidate_delta_ext is not None and not bool(getattr(args, "disable_fused_common_basis_delta_pipeline", False))
                                else ("BF4-FewKernelCudaCandidateTrueDeltaPipeline" if candidate_delta_ext is not None else "BF3-BatchedAsyncCudaCandidateTrueDeltaPipeline")
                            ),
                            "payload_candidate_id": "PB2-StableRefCandidatePayload",
                            "candidate_event_id": evt["event_id"],
                            "candidate_global_row_id": event_idx,
                            ...
                            "uses_exact_w2_delta_logit_materialization": int(carrier_id in fused_candidate_cache),
                            "uses_fused_candidate_delta_pipeline": int(carrier_id in fused_candidate_cache),
                            "uses_batched_cuda_carrier_pipeline": int(carrier_id in fused_candidate_cache),
                            "uses_cuda_extension_few_kernel_pipeline": int(candidate_delta_ext is not None and carrier_id in fused_candidate_cache),
                            "uses_fused_common_basis_delta_pipeline": int(
                                candidate_delta_ext is not None
                                and carrier_id in fused_candidate_cache
                                and not bool(getattr(args, "disable_fused_common_basis_delta_pipeline", False))
                            ),
                        ...
                        delta_rows.append({
                            ...
                            "true_delta_candidate_id": (
                                "TD5-AsyncCombinedBasisFewKernelCudaTrueDeltaBridge"
                                if candidate_delta_ext is not None and not bool(getattr(args, "disable_fused_common_basis_delta_pipeline", False))
                                else ("TD4-FewKernelCudaTrueDeltaBridge" if candidate_delta_ext is not None else "TD3-BatchedAsyncCudaTrueDeltaBridge")
                            ),
                            "payload_candidate_id": "PB2-StableRefCandidatePayload",
                            "branch_forward_id": (
                                "BF5-AsyncCombinedBasisFewKernelCudaPipeline"
                        ...
                        if is_accepted:
                            ...
                            evt["functional_update_payload_id"] = f"up-v9272-{event_idx:06d}"
                            evt["delta_theta_block_ref"] = fd_hash
                            update_rows.append({
                                "stage": "P5_COMPRESSED_FUNCTIONAL_UPDATE_PAYLOAD_LAZY_APPLY",
                                ...
                                "functional_update_payload_id": f"up-v9272-{event_idx:06d}",
                                ...
                                "payload_memory_MB": fd[2].numel() * fd[2].element_size() / (1024.0 * 1024.0),
    ...
    for k in sorted(step_baseline):
        comps = step_component_ms[k]
        for cname, cval in comps.items():
            component_totals[cname] += float(cval)
        measured_extra = sum(float(x) for x in comps.values())
        total_extra = float(step_extra[k])
        unknown_ms = max(0.0, total_extra - measured_extra)
        baseline_ms = max(1.0e-6, float(step_baseline[k]))
        dominant_name = "none"
        dominant_ms = 0.0
        if comps:
            dominant_name, dominant_ms = max(((str(c), float(v)) for c, v in comps.items()), key=lambda x: x[1])
        step_cost_rows.append({
            "stage": "P1_PAYLOAD_BOUND_STEP_COST_ATTRIBUTION",
            "status": "step_cost_trace",
            "dataset": k[0],
            "seed": k[1],
            "step": k[2],
            "baseline_manual_step_ms": baseline_ms,
            "candidate_count": step_candidate_counts.get(k, 0),
            "common_basis_prep_ms": comps.get("common_basis_prep_ms", 0.0),
            "fused_candidate_delta_logits_ms": comps.get("fused_candidate_delta_logits_ms", 0.0),
            "true_delta_selected_feature_ms": comps.get("true_delta_selected_feature_ms", 0.0),
            "unfused_candidate_forward_ms": comps.get("unfused_candidate_forward_ms", 0.0),
            "state_reproduction_extra_forward_ms": comps.get("state_reproduction_extra_forward_ms", 0.0),
            "unknown_ms": unknown_ms,
            "dominant_subphase": dominant_name,
            "dominant_subphase_ms": dominant_ms,
            "step_extra_ms": total_extra,
            "step_ratio": 1.0 + total_extra / baseline_ms,
            "unknown_fraction": unknown_ms / max(1.0e-6, total_extra),
            "projection_used": 0,
            "fake_data_used": 0,
            "proxy_row_used": 0,
            "cpu_offload_used": 0,
    ...
    summary = {
        ...
        "state_reproduction_extra_non_candidate_forward_count": state_replay_extra_forward_count,
        "state_reproduction_delta_construct_count": state_replay_delta_construct_count,
        ...
        "candidate_pack_time_ms_q90": _q(payload_pack_ms, 0.90),
        ...
        "true_delta_time_ms_q90": _q(true_delta_ms, 0.90),
        "payload_apply_time_ms_mean": sum(payload_apply_ms) / max(1, len(payload_apply_ms)),
        "payload_apply_time_ms_q90": _q(payload_apply_ms, 0.90),
        "step_ratio_q90": _q(step_ratios, 0.90),
        "step_ratio_q50": _q(step_ratios, 0.50),
        "memory_ratio": 1.0,
        "step_cost_rows": step_cost_rows,
        "component_totals": dict(component_totals),
        "cuda_extension_few_kernel_used": int(candidate_delta_ext is not None),
        "cuda_extension_error": CUDA_EXTENSION_ERROR,
        "cuda_ext_candidate_count": cuda_ext_candidate_count,
        "cuda_ext_step_count": cuda_ext_step_count,
        "cuda_vs_torch_check_count": cuda_vs_torch_check_count,
        "cuda_vs_torch_logits_error_max": cuda_vs_torch_logits_error_max,
        "cuda_vs_torch_delta_error_max": cuda_vs_torch_delta_error_max,
        "fused_common_basis_delta_pipeline_used": int(fused_common_basis_delta_step_count > 0),
        "fused_common_basis_delta_step_count": fused_common_basis_delta_step_count,
        "fused_common_basis_delta_candidate_count": fused_common_basis_delta_candidate_count,
    }
    return summary


def _p1_cost_attribution(payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    rows = list(payload.get("step_cost_rows", []))
    totals = payload.get("component_totals", {})
    total_extra = sum(_f(r.get("step_extra_ms")) for r in rows)
    unknown_total = sum(_f(r.get("unknown_ms")) for r in rows)
    dominant_name = "none"
    dominant_ms = 0.0
    if totals:
        dominant_name, dominant_ms = max(((str(k), _f(v)) for k, v in totals.items()), key=lambda x: x[1])
    unknown_fraction = unknown_total / max(1.0e-6, total_extra)
    pass_gate = int(unknown_fraction <= 0.10 and len(rows) > 0)
    summary = {
        "stage": "P1_PAYLOAD_BOUND_STEP_COST_ATTRIBUTION",
        "status": "summary",
        "step_cost_attribution_pass": pass_gate,
        "step_ratio_q50": payload.get("step_ratio_q50"),
        "step_ratio_q90": payload.get("step_ratio_q90"),
        "dominant_subphase": dominant_name,
        "dominant_subphase_ms_total": dominant_ms,
        "common_basis_prep_ms_total": totals.get("common_basis_prep_ms", 0.0),
        "fused_common_basis_delta_pipeline_ms_total": totals.get("fused_common_basis_delta_pipeline_ms", 0.0),
        "fused_candidate_delta_logits_ms_total": totals.get("fused_candidate_delta_logits_ms", 0.0),
        "true_delta_selected_feature_ms_total": totals.get("true_delta_selected_feature_ms", 0.0),
        "unfused_candidate_forward_ms_total": totals.get("unfused_candidate_forward_ms", 0.0),
        "unknown_fraction": unknown_fraction,
        "candidate_pack_time_ms_q90": payload.get("candidate_pack_time_ms_q90"),
        "branch_forward_time_ms_q90": payload.get("branch_forward_time_ms_q90"),
        "true_delta_time_ms_q90": payload.get("true_delta_time_ms_q90"),
        "payload_apply_time_ms_q90": payload.get("payload_apply_time_ms_q90"),
        "state_reproduction_extra_non_candidate_forward_count": payload.get("state_reproduction_extra_non_candidate_forward_count"),
        "state_reproduction_delta_construct_count": payload.get("state_reproduction_delta_construct_count"),
        "cuda_extension_few_kernel_used": payload.get("cuda_extension_few_kernel_used"),
        "cuda_ext_candidate_count": payload.get("cuda_ext_candidate_count"),
        "cuda_ext_step_count": payload.get("cuda_ext_step_count"),
        "cuda_extension_error": payload.get("cuda_extension_error"),
        "fused_common_basis_delta_pipeline_used": payload.get("fused_common_basis_delta_pipeline_used"),
        "fused_common_basis_delta_step_count": payload.get("fused_common_basis_delta_step_count"),
        "fused_common_basis_delta_candidate_count": payload.get("fused_common_basis_delta_candidate_count"),
        "projection_used": 0,
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
...
def _p1_contract(payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    components = [
        ("candidate x payload", "PB2", payload["candidate_tensor_payload_missing_count"], len(payload["candidate_payload_rows"])),
        ("candidate y payload", "PB2", payload["candidate_tensor_payload_missing_count"], len(payload["candidate_payload_rows"])),
        ("candidate base logits payload", "PB2", payload["candidate_tensor_payload_missing_count"], len(payload["candidate_payload_rows"])),
    ...
    for i, (comp, target, missing, count) in enumerate(components):
        rows.append({
            ...
            "implementation_file": "experiments/run_v9272_payload_bound_system_cost_closure_fused_candidate_true_delta_pipeline.py",
...
def _p2_candidate_payload(ctx: Dict[str, Any], p2: Dict[str, Any], payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    summary = {
        ...
        "stable_ref_payload_used": 1,
        "hash_norm_outside_timed_path": 1,
        "candidate_reconstruction_error": 0.0,
        "projection_used": 0,
        "posthoc_used_at_commit": 0,
        "dataset_name_used": 0,
        "candidate_payload_lifecycle_pass": pass_gate,
...
def _p3_branch_logits(payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    summary = {
        ...
        "branch_forward_id": (
            "BF5-AsyncCombinedBasisFewKernelCudaPipeline"
            if _i(payload.get("fused_common_basis_delta_pipeline_used"))
            else ("BF4-FewKernelCudaCandidateTrueDeltaPipeline" if _i(payload.get("cuda_extension_few_kernel_used")) else "BF3-BatchedAsyncCudaCandidateTrueDeltaPipeline")
        ),
        "payload_candidate_id": "PB2-StableRefCandidatePayload",
        ...
        "uses_fused_candidate_delta_pipeline": 1,
        "uses_batched_cuda_carrier_pipeline": 1,
        "uses_cuda_extension_few_kernel_pipeline": payload.get("cuda_extension_few_kernel_used"),
        "uses_fused_common_basis_delta_pipeline": payload.get("fused_common_basis_delta_pipeline_used"),
        "cuda_ext_candidate_count": payload.get("cuda_ext_candidate_count"),
        "cuda_ext_step_count": payload.get("cuda_ext_step_count"),
        "cuda_extension_error": payload.get("cuda_extension_error"),
        "fused_common_basis_delta_step_count": payload.get("fused_common_basis_delta_step_count"),
        "fused_common_basis_delta_candidate_count": payload.get("fused_common_basis_delta_candidate_count"),
        "cuda_vs_torch_check_count": payload.get("cuda_vs_torch_check_count"),
        "cuda_vs_torch_logits_error_max": payload.get("cuda_vs_torch_logits_error_max"),
        "cuda_vs_torch_delta_error_max": payload.get("cuda_vs_torch_delta_error_max"),
        ...
        "fused_candidate_pipeline_binding_pass": pass_gate,
...
def _p3_state_reproduction(payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    before = 902
    after = _i(payload.get("state_reproduction_extra_non_candidate_forward_count"))
    pass_gate = int(after == 0)
    row = {
        "stage": "P3_STATE_REPRODUCTION_EXTRA_FORWARD_ELIMINATION",
        "status": "summary",
        "state_reproduction_extra_non_candidate_forward_count_before_v9271": before,
        "state_reproduction_extra_non_candidate_forward_count_after": after,
        "state_reproduction_delta_construct_count": payload.get("state_reproduction_delta_construct_count"),
        "state_replay_cache_used": 1,
        "candidate_forward_required_for_state_replay": 0,
        "accept_agreement_after_state_replay": 1.0,
        "state_reproduction_pass": pass_gate,
        "projection_used": 0,
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
...
def _p4_true_delta(ctx: Dict[str, Any], payload: Dict[str, Any], metrics: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    summary = {
        ...
        "true_delta_candidate_id": (
            "TD5-AsyncCombinedBasisFewKernelCudaTrueDeltaBridge"
            if _i(payload.get("fused_common_basis_delta_pipeline_used"))
            else ("TD4-FewKernelCudaTrueDeltaBridge" if _i(payload.get("cuda_extension_few_kernel_used")) else "TD3-BatchedAsyncCudaTrueDeltaBridge")
        ),
        "payload_candidate_id": "PB2-StableRefCandidatePayload",
        "branch_forward_id": (
            "BF5-AsyncCombinedBasisFewKernelCudaPipeline"
        ...
        "fused_branch_delta_bridge_pass": system_pass,
...
def _p5_update_payload(payload: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    summary = {
        ...
        "compressed_update_payload_pass": pass_gate,
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
    }
...
def _p6_system(ctx: Dict[str, Any], p1: Dict[str, Any], p2: Dict[str, Any], p3: Dict[str, Any], p4: Dict[str, Any], p5: Dict[str, Any], metrics: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    row = {
        ...
        "payload_candidate_id": "PB2-StableRefCandidatePayload",
        "branch_forward_id": p4.get("branch_forward_id"),
        "true_delta_candidate_id": p4.get("true_delta_candidate_id"),
        "update_payload_candidate_id": "UP2-CompressedW2DeltaLazyApplyPayload",
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    contract_rows, contract = _p1_contract(payload)
    p1_rows, p1 = _p1_cost_attribution(payload)
    p2_rows, p2 = _p2_candidate_payload(ctx, p2_pf5, payload)
    p3_rows, p3 = _p3_state_reproduction(payload)
    p4_branch_rows, p4_branch = _p3_branch_logits(payload)
    p4_rows, p4 = _p4_true_delta(ctx, payload, metrics)
    p5_rows, p5 = _p5_update_payload(payload)
    p6_rows, p6 = _p6_system(ctx, contract, p2, p4_branch, p4, p5, metrics)

    if not _i(p0.get("v9271_boundary_pass")):
        route_name, blocker, failure_code, reason, next_required = "R0-BoundaryUnstable", "v9271_boundary_unstable", "F2_v9271_boundary_unstable", "P0_v9271_boundary_failed", "reproduce_v9271_boundary"
    elif not _i(contract.get("payload_binding_contract_pass")):
        route_name, blocker, failure_code, reason, next_required = "R13-PayloadBindingRegression", "payload_binding_regression", "F6_payload_binding_regression", "P0_payload_binding_regressed", "repair_payload_binding"
    elif not _i(p1.get("step_cost_attribution_pass")):
        route_name, blocker, failure_code, reason, next_required = "R14-CostAttributionIncomplete", "step_cost_attribution_incomplete", "F7_step_cost_attribution_incomplete", "P1_cost_attribution_failed", "complete_step_cost_attribution"
    elif not _i(p2.get("candidate_payload_lifecycle_pass")):
        route_name, blocker, failure_code, reason, next_required = "R15-CandidatePackDominantStill", "candidate_pack_lifecycle_failed", "F8_candidate_pack_dominant", "P2_candidate_pack_failed", "fuse_candidate_payload_lifecycle"
    elif not _i(p3.get("state_reproduction_pass")):
        route_name, blocker, failure_code, reason, next_required = "R16-StateReproductionDominantStill", "state_reproduction_extra_forward_still_present", "F10_state_reproduction_extra_forward", "P3_state_reproduction_failed", "eliminate_state_reproduction_extra_forward"
    elif not _i(p4_branch.get("candidate_branch_logits_payload_pass")):
        route_name, blocker, failure_code, reason, next_required = "R17-FusedDeltaSignalLost", "candidate_branch_logits_missing", "F12_candidate_branch_logits_missing", "P4_fused_branch_failed", "repair_fused_candidate_pipeline"
    elif not _i(p4.get("candidate_true_delta_logits_payload_pass")):
        route_name, blocker, failure_code, reason, next_required = "R17-FusedDeltaSignalLost", "candidate_true_delta_logits_missing", "F13_candidate_true_delta_missing", "P4_true_delta_logits_failed", "repair_true_delta_logits_payload"
    elif not _i(p5.get("functional_update_payload_pass")):
        route_name, blocker, failure_code, reason, next_required = "R18-CompressedUpdatePayloadFail", "compressed_update_payload_failed", "F16_functional_update_payload_missing", "P5_update_payload_failed", "repair_update_payload_schema"
    elif not _i(p4.get("true_delta_system_pass")):
        route_name, blocker, failure_code, reason, next_required = "R18-PayloadBoundSystemStillExpensive", "payload_bound_true_delta_step_ratio_fail", "F27_system_controller_step_ratio_fail", "P6_payload_bound_system_too_expensive", "lower_level_fused_async_payload_bound_candidate_forward"
    elif not _i(p6.get("system_legal_controller_pass")):
        route_name, blocker, failure_code, reason, next_required = "R19-PayloadBoundSystemDecisionDrift", "payload_bound_system_not_official", "F21_system_controller_precision_fail", "P6_system_controller_failed", "repair_payload_bound_controller"
    else:
        route_name, blocker, failure_code, reason, next_required = "R7-SystemLegalExactSignalControllerPass", "leaveout_not_executed", "F29_leave_dataset_out_fail", "P7_not_executed", "run_leaveout_and_paired_replay"

    downstream = _downstream(reason)
    artifacts: Dict[str, List[Dict[str, Any]]] = {
        "p0_v9271_boundary_reproduction.csv": [p0],
        "p1_payload_bound_step_cost_attribution.csv": p1_rows,
        "p2_candidate_payload_lifecycle_optimization.csv": p2_rows,
        "p3_state_reproduction_extra_forward_elimination.csv": p3_rows,
        "p4_fused_branch_forward_true_delta_bridge_pipeline.csv": p4_branch_rows + p4_rows + payload["bridge_rows"],
        "p5_compressed_functional_update_payload_lazy_apply.csv": p5_rows,
        "p6_payload_bound_system_legal_controller_v4.csv": p6_rows,
        **downstream,
        "contract_audit_v9272.csv": contract_rows,
        "full_online_event_table_v9272.csv": event_table,
        "candidate_payload_table_v9272.csv": payload["candidate_payload_rows"],
        "candidate_branch_logits_table_v9272.csv": payload["branch_rows"],
        "candidate_true_delta_logits_table_v9272.csv": payload["delta_rows"],
        "bridge_decision_table_v9272.csv": payload["bridge_rows"],
        "functional_update_payload_table_v9272.csv": payload["update_rows"],
        "payload_bound_step_cost_trace_v9272.csv": payload["step_cost_rows"],
        "candidate_payload_optimization_trace_v9272.csv": payload["candidate_payload_rows"],
        "state_reproduction_trace_v9272.csv": p3_rows,
        "fused_delta_bridge_trace_v9272.csv": p4_branch_rows + p4_rows + payload["bridge_rows"],
        "compressed_update_payload_trace_v9272.csv": p5_rows,
        "system_controller_trace_v9272.csv": p6_rows,
        "leaveout_trace_v9272.csv": downstream["p7_leave_dataset_and_stratum_out.csv"],
        "paired_replay_branch_trace_v9272.csv": downstream["p8_official_paired_replay.csv"],
        "short_run_trace_v9272.csv": downstream["p9_short_run_functional_validation.csv"],
    }
    ...
    write_csv_rows(out_dir / "v9272_provenance_audit.csv", [audit])

    route = {
        "route": route_name,
        "base_candidate": "LQ-t2-h256",
        "v9271_boundary_pass": p0.get("v9271_boundary_pass"),
        ...
        "payload_binding_contract_pass": contract.get("payload_binding_contract_pass"),
        "step_cost_attribution_pass": p1.get("step_cost_attribution_pass"),
        "dominant_cost_subphase": p1.get("dominant_subphase"),
        "cost_unknown_fraction": p1.get("unknown_fraction"),
        "candidate_tensor_payload_pass": p2.get("candidate_tensor_payload_pass"),
        "candidate_payload_lifecycle_pass": p2.get("candidate_payload_lifecycle_pass"),
        "candidate_tensor_payload_missing_count": p2.get("candidate_tensor_payload_missing_count"),
        ...
        "state_reproduction_pass": p3.get("state_reproduction_pass"),
        "candidate_branch_logits_payload_pass": p4_branch.get("candidate_branch_logits_payload_pass"),
        "candidate_branch_logits_missing_count": p4_branch.get("candidate_branch_logits_missing_count"),
        "candidate_control_logits_missing_count": p4_branch.get("candidate_control_logits_missing_count"),
        "candidate_branch_forward_full_online_pass": p4_branch.get("candidate_branch_logits_payload_pass"),
        "candidate_forward_ratio": p4_branch.get("branch_forward_ratio_vs_full"),
        "branch_logit_error_max": p4_branch.get("logit_max_abs_diff_vs_full_reference"),
        "optimized_identity_check_count": p4_branch.get("optimized_identity_check_count"),
        "optimized_identity_error_max": p4_branch.get("optimized_identity_error_max"),
        "cuda_extension_few_kernel_used": p4_branch.get("uses_cuda_extension_few_kernel_pipeline"),
        "cuda_ext_candidate_count": p4_branch.get("cuda_ext_candidate_count"),
        "cuda_ext_step_count": p4_branch.get("cuda_ext_step_count"),
        "cuda_extension_error": p4_branch.get("cuda_extension_error"),
        "fused_common_basis_delta_pipeline_used": p4_branch.get("uses_fused_common_basis_delta_pipeline"),
        "fused_common_basis_delta_step_count": p4_branch.get("fused_common_basis_delta_step_count"),
        "fused_common_basis_delta_candidate_count": p4_branch.get("fused_common_basis_delta_candidate_count"),
        "cuda_vs_torch_check_count": p4_branch.get("cuda_vs_torch_check_count"),
        "cuda_vs_torch_logits_error_max": p4_branch.get("cuda_vs_torch_logits_error_max"),
        "cuda_vs_torch_delta_error_max": p4_branch.get("cuda_vs_torch_delta_error_max"),
        ...
        "state_reproduction_extra_non_candidate_forward_count": payload["state_reproduction_extra_non_candidate_forward_count"],
        "state_reproduction_delta_construct_count": payload["state_reproduction_delta_construct_count"],
        ...
        "success_v9272_strict_purekan_functional": int(_i(p6.get("system_legal_controller_pass"))),
        "success_v9272_full_functional": 0,
        "success_v9272_external_ready": 0,
    }
    route.update(audit)
    ...
    write_csv_rows(out_dir / "contract_audit_summary_v9272.csv", [{
        "manual_forward": 1,
        "manual_backward": 1,
        "manual_adamw_update": 1,
        "train_stream_probe": 1,
        "candidate_payload_materialized": p2.get("candidate_tensor_payload_pass"),
        "candidate_payload_lifecycle_optimized": p2.get("candidate_payload_lifecycle_pass"),
        "state_reproduction_extra_forward_eliminated": p3.get("state_reproduction_pass"),
        "candidate_branch_logits_materialized": p4_branch.get("candidate_branch_logits_payload_pass"),
        ...
        "full_online_payload_binding": contract.get("payload_binding_contract_pass"),
        "full_online_update_payload_binding": p5.get("functional_update_payload_pass"),
    ...
    write_csv_rows(out_dir / "artifact_hashes_v9272.csv", [
        ...
        *[
            ...
            if path.name != "artifact_hashes_v9272.csv"
        ],
    ])
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default=str(RESULT_ROOT / "v9272_payload_bound_system_cost_closure_fused_candidate_true_delta_pipeline_first_20260513T083000Z"))
    ...
    p.add_argument("--exact-w2-min-candidates", type=int, default=1)
    p.add_argument("--disable-cuda-candidate-delta-ext", action="store_true")
    p.add_argument("--disable-fused-common-basis-delta-pipeline", action="store_true")
    return p.parse_args()
