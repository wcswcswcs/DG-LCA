#!/usr/bin/env python3
"""v20 MLP M16 retained-source anatomy runner."""
...
from experiments.run_v20_common import (  # noqa: E402
    ...
    read_rows,
    slug,
    v20_functional_specs,
    write_rows,
)
from dgkan.fu.source_channel import grouped_source_retention  # noqa: E402


def parser(default_scope: str = "mlp_anatomy") -> argparse.ArgumentParser:
    p = argparse.ArgumentParser()
    p.add_argument("--out-dir", default=None)
    p.add_argument("--scope", default=default_scope, choices=["mlp_anatomy", "kan_writer"])
    p.add_argument("--device", default="cuda:0")
    p.add_argument("--data-root", default="data")
    p.add_argument("--carriers", default="MLP" if default_scope == "mlp_anatomy" else "D-CHE,D-FOU")
    ...
    p.add_argument("--basis-repair-variant", default="R0-current" if default_scope == "mlp_anatomy" else "CHE-R2-low-degree-k3-triton")
    p.add_argument("--basis-repair-variant-fou", default="FOU-R2-low-frequency-k2-stream")
...
def selected_specs(args: argparse.Namespace) -> list[dict[str, Any]]:
    specs = v20_functional_specs(str(args.scope))
    wanted = {x.strip() for x in str(args.spec_ids).split(",") if x.strip()}
    if wanted:
        specs = [
            s
            for s in specs
            if str(s.get("v20_id")) in wanted
...
def prefix(scope: str) -> str:
    return "v20_mlp_m16_anatomy" if scope == "mlp_anatomy" else "v20_kan_source_writer"
...
def run_shard(args: argparse.Namespace) -> None:
    ...
    append_exec(out_dir, f"{PYTHON} experiments/run_v20_mlp_retained_source_anatomy.py --scope {args.scope} --shard-index {args.shard_index} --device {args.device}", status="started", note=f"jobs={len(selected)}")
    rows = []
    trace_rows = []
    for job in selected:
        ...
        try:
            ...
            train_seed = 200_000 + int(job["job_index"]) + int(job.get("init_seed_offset", 0))
    ...
    append_exec(out_dir, f"{PYTHON} experiments/run_v20_mlp_retained_source_anatomy.py --scope {args.scope} --shard-index {args.shard_index}", status="completed", note=f"rows={len(rows)} traces={len(trace_rows)}")
...
def merge(args: argparse.Namespace) -> None:
    ...
    for row in rows:
        ...
        for step in ["800", "1600", "3200", "4800"]:
            tr = hrows.get(step, {})
            for key in ["val_loss", "train_loss", "CEp99", "NLL", "ECE", "Brier", "LineC_channel_loss", "LineC_fast_loss", "ActuationR2", "B1_gain", "B2_transfer_gain", "B3_safety_gain", "source_state_gate_accept", "generalization_gate_accept"]:
                if key in tr:
    ...
    summary = grouped_source_retention(enriched, group_keys=("carrier", "basis_repair_variant", "v20_id"))
    by_id = {str(r.get("v20_id")): r for r in enriched}
    for item in summary:
        sample = by_id.get(str(item.get("v20_id")), {})
    ...
    target = f"{pfx}.csv" if args.scope == "mlp_anatomy" else "v20_kan_source_writer_matrix.csv"
    write_rows(out_dir / target, summary)
    if args.scope == "mlp_anatomy":
        write_rows(out_dir / "v20_optimizer_washout_matrix.csv", [r for r in summary if "AdamW" in str(r.get("v20_id")) or "M16" in str(r.get("v20_id")) or "M2" in str(r.get("continuation_id"))])
    else:
        write_rows(out_dir / "v20_poprisk_slow_state_matrix.csv", [r for r in summary if "PopRisk" in str(r.get("v20_id")) or "poprisk" in str(r.get("hypothesis"))])
        write_rows(out_dir / "v20_matrix_block_fu_matrix.csv", [r for r in summary if "MatrixBlock" in str(r.get("v20_id")) or "matrix_block" in str(r.get("hypothesis"))])
    append_exec(out_dir, f"{PYTHON} experiments/run_v20_mlp_retained_source_anatomy.py --scope {args.scope} --merge-only", status="completed", note=f"rows={len(enriched)} grouped={len(summary)}")
