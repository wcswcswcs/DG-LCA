#!/usr/bin/env python3
"""Shared helpers for v22.02 execution artifacts, logs, and packets."""
...
V2201_OFFICIAL = ROOT / "results/v22_01_early_source_retention_terminal_collapse_kernel_officialization_drat_drbf_4gpu/official_v22_01"
V2202_ROOT = ROOT / "results/v22_02_terminal_collapse_early_source_selector_basis_efficiency_4gpu"
V2202_OFFICIAL = V2202_ROOT / "official_v22_02"
V2202_PLAN_DOC = ROOT / "docs/DG-KAN_v22.02_TerminalCollapse_EarlySourceSelector_BasisEfficiency_4GPU_完整计划.md"
V2202_EXEC_DOC = ROOT / "docs/DG-KAN_v22.02_TerminalCollapse_EarlySourceSelector_BasisEfficiency_4GPU_执行日志.md"
V2202_RECAP_DOC = ROOT / "docs/DG-KAN_v22.02_TerminalCollapse_EarlySourceSelector_BasisEfficiency_4GPU_实验结果复盘.md"
...
def ensure_out(out_dir: str | Path | None = None) -> Path:
    out = Path(out_dir) if out_dir else V2202_OFFICIAL
...
def append_exec(out_dir: Path, command: str, *, status: str = "", note: str = "") -> None:
    ...
    journal = out_dir / "v22_02_command_journal.csv"
    ...
    with journal.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow(row)
    append_text(V2202_EXEC_DOC, f"\n## {row['timestamp']}\n\n```bash\n{command}\n```\n\n- status: {status or 'recorded'}\n")
    if note:
        append_text(V2202_EXEC_DOC, f"- note: {note}\n")
...
def build_packet(out_dir: Path) -> tuple[Path, Path]:
    packet = out_dir / "v22_02_code_review_packet"
    if packet.exists():
    ...
    for rel in [
        ...
        if Path(rel).suffix:
            ...
            target.write_text("v22.02 code review packet\n", encoding="utf-8")
        else:
            target.mkdir(parents=True, exist_ok=True)
    write_text(packet / "00_README.md", "v22.02 packet: clean source tree, truth-gate artifacts, logs, and reproduction commands.\n")
    ...
    for path in ["experiments/provenance_guard.py"]:
        copy_if_exists(ROOT / path, source_tree / path)
    if (ROOT / "tests").exists():
        shutil.copytree(ROOT / "tests", source_tree / "tests", ignore=_ignore_for_packet)
    for doc in [V2202_PLAN_DOC, V2202_EXEC_DOC, V2202_RECAP_DOC]:
        copy_if_exists(doc, packet / "12_RESULTS" / "docs" / doc.name)
    for path in sorted(out_dir.glob("v22_02_*")):
    ...
    zip_path = out_dir / "v22_02_code_review_packet.zip"
    if zip_path.exists():
        zip_path.unlink()
    ...
    with zipfile.ZipFile(bundle, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for path in sorted(out_dir.rglob("*")):
            if path.is_file() and path.suffix.lower() != ".zip" and "v22_02_code_review_packet" not in str(path):
