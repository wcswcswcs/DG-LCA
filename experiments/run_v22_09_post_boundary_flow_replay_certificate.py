#!/usr/bin/env python3
"""v22.09 post-boundary Boundary-Value Flow Replay certificate.

This is intentionally outside the C-O13/C-O15 family. It tests whether a
train-only source step behaves like a persistent boundary condition under short
local optimizer replay: source residual after replay should remain useful on
disjoint train folds and beat random/sign/corrupt boundary controls. Future
horizon columns are audit labels only.
"""
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--local-replay-steps", type=int, default=4)
...
def _vector_update(vector: torch.Tensor, reference: torch.Tensor, kind: str) -> UpdateTensor:
    return UpdateTensor(
        ...
        kind=kind,
        sign_rule="add",
        space="boundary_value_flow_replay",
        source="train_only_boundary_value_flow_replay_certificate",
        mechanism="BVFR-BoundaryValueFlowReplay",
        role="all",
        one_step_descent_claim=0,
    )


def _train_replay_steps(model: torch.nn.Module, x: torch.Tensor, y: torch.Tensor, args: argparse.Namespace) -> None:
    opt = torch.optim.AdamW(model.parameters(), lr=float(args.local_flow_lr), weight_decay=float(args.weight_decay))
    for _ in range(max(1, int(args.local_replay_steps))):
...
def _boundary_replay_vector(
    ...
    apply_update(perturbed, _vector_update(vector, reference, "boundary_seed"), lr=float(args.boundary_lr))
    _train_replay_steps(base, x_flow, y_flow, args)
    _train_replay_steps(perturbed, x_flow, y_flow, args)
    return (flat_params(perturbed) - flat_params(base)).detach().float()
...
def _tiny_effects(
    ...
    apply_update(clone, _vector_update(vector, reference, "boundary_replay_effect"), lr=float(lr))
    return {
        "A_loss_gain": base_a - _loss_value(clone, x_a, y_a),
...
def _probe_one(args: argparse.Namespace, row: dict[str, str], job_order: int) -> dict[str, Any]:
    ...
    if int(x_c.shape[0]) == 0:
        x_c, y_c = x_b, y_b
    gen = torch.Generator(device=device).manual_seed(229_901 + train_seed + job_order)
    mechanism = str(row.get("mechanism", ""))
    raw = make_update(model, mechanism, x_a, y_a, seed=train_seed + 901)
    corrupt = make_update(model, mechanism, x_a, (y_a + 1) % max(2, int(args.classes)), seed=train_seed + 902)
    source_vec = _signed_displacement(raw)
    corrupt_vec = _signed_displacement(corrupt)
    random_vec = _random_like(source_vec, gen)
    sign_vec = -source_vec
    replay_source = _boundary_replay_vector(model, source_vec, source_vec, x_b, y_b, args)
    replay_random = _boundary_replay_vector(model, random_vec, source_vec, x_b, y_b, args)
    replay_sign = _boundary_replay_vector(model, sign_vec, source_vec, x_b, y_b, args)
    replay_corrupt = _boundary_replay_vector(model, corrupt_vec, source_vec, x_b, y_b, args)
    source_eff = _tiny_effects(model, replay_source, source_vec, x_a, y_a, x_b, y_b, x_c, y_c, float(args.fu_lr))
    random_eff = _tiny_effects(model, replay_random, source_vec, x_a, y_a, x_b, y_b, x_c, y_c, float(args.fu_lr))
    sign_eff = _tiny_effects(model, replay_sign, source_vec, x_a, y_a, x_b, y_b, x_c, y_c, float(args.fu_lr))
    corrupt_eff = _tiny_effects(model, replay_corrupt, source_vec, x_a, y_a, x_b, y_b, x_c, y_c, float(args.fu_lr))
    source_norm = float(torch.linalg.vector_norm(source_vec).item())
    replay_norm = float(torch.linalg.vector_norm(replay_source).item())
    ...
    control_replay_norm = max(
        float(torch.linalg.vector_norm(replay_random).item()),
        float(torch.linalg.vector_norm(replay_sign).item()),
        float(torch.linalg.vector_norm(replay_corrupt).item()),
    )
    ...
    return {
        ...
        "source_boundary_replay_B_loss_gain": source_eff["B_loss_gain"],
        "source_boundary_replay_C_safety_loss_gain": source_eff["C_safety_loss_gain"],
        "B_boundary_replay_control_gap": source_eff["B_loss_gain"] - control_b,
        "C_safety_control_gap": source_eff["C_safety_loss_gain"] - control_c,
        "source_boundary_replay_B_margin_gain": source_eff["B_margin_gain"],
        "source_boundary_replay_C_margin_gain": source_eff["C_margin_gain"],
        "source_update_norm": source_norm,
        "boundary_replay_norm": replay_norm,
        "boundary_replay_norm_ratio": replay_norm / max(1.0e-12, source_norm),
        "boundary_replay_norm_control_gap": replay_norm / max(1.0e-12, source_norm) - control_replay_norm / max(1.0e-12, source_norm),
        "boundary_replay_source_cosine": cosine(source_vec, replay_source),
        "boundary_replay_random_cosine": cosine(random_vec, replay_random),
        "boundary_replay_sign_cosine": cosine(sign_vec, replay_sign),
        "boundary_replay_corrupt_cosine": cosine(corrupt_vec, replay_corrupt),
        "boundary_replay_alignment_control_gap": cosine(source_vec, replay_source) - max(cosine(random_vec, replay_random), cosine(sign_vec, replay_sign), cosine(corrupt_vec, replay_corrupt)),
...
def _score_probe(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    feature_keys = [
        "source_boundary_replay_B_loss_gain",
        "B_boundary_replay_control_gap",
        "source_boundary_replay_C_safety_loss_gain",
        "C_safety_control_gap",
        "source_boundary_replay_B_margin_gain",
        "boundary_replay_norm_ratio",
        "boundary_replay_norm_control_gap",
        "boundary_replay_source_cosine",
        "boundary_replay_alignment_control_gap",
    ]
    z = {key: _zscore([_f(r.get(key)) for r in rows], groups) for key in feature_keys}
    out = []
    for i, row in enumerate(rows):
        score = (
            0.35 * z["source_boundary_replay_B_loss_gain"][i]
            + 0.30 * z["B_boundary_replay_control_gap"][i]
            + 0.20 * z["source_boundary_replay_C_safety_loss_gain"][i]
            + 0.15 * z["C_safety_control_gap"][i]
            + 0.10 * z["source_boundary_replay_B_margin_gain"][i]
            + 0.20 * z["boundary_replay_norm_ratio"][i]
            + 0.20 * z["boundary_replay_norm_control_gap"][i]
            + 0.25 * z["boundary_replay_source_cosine"][i]
            + 0.25 * z["boundary_replay_alignment_control_gap"][i]
        )
        train_gate = int(
            _f(row.get("source_boundary_replay_B_loss_gain"), -999.0) > 0.0
            and _f(row.get("B_boundary_replay_control_gap"), -999.0) >= -1.0e-3
            and _f(row.get("C_safety_control_gap"), -999.0) >= -1.0e-3
            and _f(row.get("boundary_replay_norm_ratio"), 0.0) > 1.0e-8
            and _f(row.get("boundary_replay_source_cosine"), -999.0) > -0.10
        )
        item = dict(row)
        item["BVFR_score"] = score
        item["BVFR_train_only_certificate_pass"] = train_gate
        item["certificate_family"] = "BVFR_Boundary_Value_Flow_Replay_Certificate"
        out.append(item)
    return sorted(out, key=lambda r: _f(r.get("BVFR_score"), -999.0), reverse=True)


def _summary(rows: list[dict[str, Any]], top_k: int) -> dict[str, Any]:
    scores = [_f(r.get("BVFR_score")) for r in rows]
    ...
    return {
        "certificate_family": "BVFR_Boundary_Value_Flow_Replay_Certificate",
        "rows": len(rows),
        ...
        "BVFR_train_only_certificate_pass_rows": sum(int_flag(r.get("BVFR_train_only_certificate_pass")) for r in rows),
        "BVFR_official_observer_pass": int(not blockers),
        "blocker": ";".join(dict.fromkeys(blockers)),
    }


def _boundary_replay_update(
    model: torch.nn.Module,
    raw: UpdateTensor,
    x_flow: torch.Tensor,
    y_flow: torch.Tensor,
    ...
    *,
    ...
    if random_gen is not None:
        replay = _random_like(source_vec, random_gen)
        replay_cosine = 0.0
        replay_ratio = 0.0
    else:
        replay = _boundary_replay_vector(model, source_vec, source_vec, x_flow, y_flow, args)
        replay_cosine = cosine(source_vec, replay)
        replay_ratio = float(torch.linalg.vector_norm(replay).item()) / max(1.0e-12, float(torch.linalg.vector_norm(source_vec).item()))
    mixed = normalized_like(0.50 * _unit(source_vec) + 0.50 * _unit(replay), source_vec)
    return UpdateTensor(
        tensor=normalized_like(mixed.detach().float(), raw.tensor.detach().float()),
        kind="boundary_value_flow_replay",
        sign_rule="add",
        space="boundary_value_flow_replay",
        source="train_only_boundary_value_flow_replay_certificate",
        mechanism="BVFR-BoundaryValueFlowReplay",
        role="all",
        one_step_descent_claim=0,
        diagnostics={"boundary_replay_source_cosine": replay_cosine, "boundary_replay_norm_ratio": replay_ratio},
    )
...
def _train_fresh(args: argparse.Namespace, source_row: dict[str, Any], run_kind: str) -> dict[str, Any]:
    ...
    gen = torch.Generator(device=device).manual_seed(229_912 + train_seed + sum(ord(c) for c in run_kind))
    ...
    replay_cosine_sum = 0.0
    replay_ratio_sum = 0.0
    replay_loss_gain_sum = 0.0
    for step in range(1, int(args.steps) + 1):
        ...
        if run_kind == "CTRL-AdamW":
            ...
            if step % max(1, int(args.refresh_interval)) == 0:
                ...
                update = _boundary_replay_update(model, raw, x_b, y_b, args, random_gen=gen if run_kind == "CTRL-RandomMatchedBoundaryReplayFU" else None)
                replay_cosine_sum += float(update.diagnostics.get("boundary_replay_source_cosine", 0.0) if update.diagnostics else 0.0)
                replay_ratio_sum += float(update.diagnostics.get("boundary_replay_norm_ratio", 0.0) if update.diagnostics else 0.0)
                injection_count += 1
                ...
                replay_loss_gain_sum += before - _loss_value(model, x_b, y_b)
        if step in horizons:
    ...
    out: dict[str, Any] = {
        ...
        "seed": seed,
        "train_seed": train_seed,
        "BVFR_score": source_row.get("BVFR_score", ""),
        "refresh_interval": int(args.refresh_interval),
        "local_replay_steps": int(args.local_replay_steps),
        "injection_count": injection_count,
        "injection_norm_sum": injection_norm_sum,
        "boundary_replay_source_cosine_mean": replay_cosine_sum / max(1, injection_count),
        "boundary_replay_norm_ratio_mean": replay_ratio_sum / max(1, injection_count),
        "boundary_replay_loss_gain_mean": replay_loss_gain_sum / max(1, injection_count),
        "execution_status": "measured",
    }
    for h in HORIZONS:
...
def _fresh_summary(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    candidates = [r for r in enriched if str(r.get("run_kind")) == "BVFR-AdamWPlusBoundaryReplayFU"]
    summary = []
        ...
        summary.append(
            {
                ...
                "boundary_replay_source_cosine_mean": row.get("boundary_replay_source_cosine_mean", ""),
                "boundary_replay_norm_ratio_mean": row.get("boundary_replay_norm_ratio_mean", ""),
                "boundary_replay_loss_gain_mean": row.get("boundary_replay_loss_gain_mean", ""),
                "injection_count": row.get("injection_count", ""),
                "injection_norm_sum": row.get("injection_norm_sum", ""),
                "debt_not_exploded": debt_ok,
                "BVFR_fresh_C3_boundary_replay_pass": int(source_pass and debt_ok),
                "official_C3_pass": 0,
                "blocker": "" if source_pass and debt_ok else "source_horizon_or_debt_gate_failed",
            }
        )
    return enriched, {"summary": summary, "route": {"fresh_rows": len(summary), "fresh_C3_boundary_replay_pass_rows": sum(int_flag(r.get("BVFR_fresh_C3_boundary_replay_pass")) for r in summary), "official_C3_pass_rows": 0}}
...
def main() -> None:
    ...
    eligible = [r for r in scored if int_flag(r.get("BVFR_train_only_certificate_pass"))]
    selected = (eligible if eligible else scored)[: max(0, int(args.fresh_top_k))]
    fresh_rows = []
    for row in selected:
        for run_kind in [
            "BVFR-AdamWPlusBoundaryReplayFU",
            "CTRL-AdamW",
            "CTRL-SGD",
            "CTRL-NoOp",
            "CTRL-RandomMatchedBoundaryReplayFU",
        ]:
    ...
    blockers = []
    if not int_flag(summary.get("BVFR_official_observer_pass")):
        blockers.append("official_observer_gate_failed")
    if not int(fresh_route["fresh_C3_boundary_replay_pass_rows"]):
        blockers.append("fresh_C3_source_horizon_failed")
    blockers.append("official_C3_gate_not_claimed")
    route = {
        "route": "PostBoundaryBVFRFreshC3Opened" if int(fresh_route["fresh_C3_boundary_replay_pass_rows"]) else "PostBoundaryBVFRBlocked",
        "previous_route": prev_route.get("route", ""),
        "certificate_family": "BVFR_Boundary_Value_Flow_Replay_Certificate",
        "probe_rows": len(scored) + len(blocked),
        "probe_measured_rows": len(scored),
        "BVFR_train_only_certificate_pass_rows": summary.get("BVFR_train_only_certificate_pass_rows", 0),
        "BVFR_official_observer_pass": summary.get("BVFR_official_observer_pass", 0),
        "fresh_C3_boundary_replay_pass_rows": fresh_route["fresh_C3_boundary_replay_pass_rows"],
        "official_C3_pass_rows": 0,
        "promotion_allowed": 0,
        "blocker": ";".join(dict.fromkeys(blockers)),
        "next_codex_action": "BVFR failed fresh/official gate; next attempt should change principle, not thresholds",
    }
    write_rows(out_dir / "v22_09_boundary_flow_replay_certificate.csv", scored + blocked)
    write_rows(out_dir / "v22_09_boundary_flow_replay_summary.csv", [summary])
    write_rows(out_dir / "v22_09_boundary_flow_replay_selected_candidates.csv", selected)
    write_rows(out_dir / "v22_09_boundary_flow_replay_fresh_c3_matrix.csv", fresh_enriched)
    write_rows(out_dir / "v22_09_boundary_flow_replay_fresh_c3_summary.csv", fresh_summary)
    write_json(out_dir / "v22_09_boundary_flow_replay_route.json", route)
    simple_svg(out_dir / "figures/v22_09_boundary_flow_replay_certificate.svg", "v22.09 BVFR certificate", scored, "BVFR_score")
    simple_svg(out_dir / "figures/v22_09_boundary_flow_replay_fresh_h3200.svg", "v22.09 BVFR fresh h3200", fresh_summary, "source_vs_best_control_h3200")
    append_exec(
        out_dir,
        f"{PYTHON} experiments/run_v22_09_post_boundary_flow_replay_certificate.py --source-dir {args.source_dir} --device {args.device} --max-probe-jobs {int(args.max_probe_jobs)} --fresh-top-k {int(args.fresh_top_k)} --steps {int(args.steps)} --local-replay-steps {int(args.local_replay_steps)} --refresh-interval {int(args.refresh_interval)} --out-dir {out_dir}",
        status="completed",
        note=f"probe_rows={len(scored)+len(blocked)} measured={len(scored)} train_only_pass={summary.get('BVFR_train_only_certificate_pass_rows', 0)} fresh_pass={fresh_route['fresh_C3_boundary_replay_pass_rows']} route={route['route']}",
    )
