#!/usr/bin/env python3
"""DG-KAN v22.60 constrained Bilevel Meta-FU control-law runner."""
...
from experiments import audit_v22_60_standard_loop as loop_audit  # noqa: E402
...
OUT_ROOT = ROOT / "results/v22_60"
CHUNK_ROOT = OUT_ROOT / "chunks"
LOG_ROOT = OUT_ROOT / "logs"
FIG_ROOT = OUT_ROOT / "figures"
CONTROLLER_ROOT = OUT_ROOT / "controllers"
DOCS = ROOT / "docs"
EXEC_DOC = DOCS / "DG-KAN_v22.60_BilevelMetaFU_ControlLaw_执行日志.md"
RECAP_DOC = DOCS / "DG-KAN_v22.60_BilevelMetaFU_ControlLaw_实验结果复盘.md"
PLAN_DOC = DOCS / "DG-KAN_v22.60_BilevelMetaFU_ControlLaw_完整计划.md"
RUNNER = ROOT / "experiments/run_v22_60_bilevel_meta_fu_control_law.py"
...
M0_LEARNED_METHODS = {
    "m0b_mlp_stateless_h20",
    "m0b_mlp_stateless_h50",
    "m0b_mlp_stateful_gru_h20",
    "m0b_linear_stateless_h20",
    "m0b_constant_learned_h20",
}
M0_CONTROL_METHODS = {
    "random_scalar_scheduler_same_distribution",
    "frozen_controller_from_other_seed",
    "frozen_controller_from_other_dataset",
    "shuffled_state_feature_controller",
    "random_state_feature_controller",
    "time_only_scheduler",
    "same_compute_noop_controller",
...
DEFAULT_M0_METHODS = (
    "m0b_mlp_stateless_h20,m0b_mlp_stateless_h50,m0b_mlp_stateful_gru_h20,"
    "m0b_linear_stateless_h20,m0b_constant_learned_h20,"
    "random_scalar_scheduler_same_distribution,frozen_controller_from_other_seed,"
    "frozen_controller_from_other_dataset,shuffled_state_feature_controller,"
    "random_state_feature_controller,time_only_scheduler,same_compute_noop_controller,"
    "handdesigned_v22_58_scfg,credit_only_handdesigned,self_only_handdesigned"
)
...
def ensure_out() -> None:
    ...
    if not EXEC_DOC.exists():
        EXEC_DOC.write_text(
            "# DG-KAN v22.60 BilevelMetaFU ControlLaw 执行日志\n\n"
            f"创建时间：{now_sg()}\n\n"
    ...
    if not RECAP_DOC.exists():
        RECAP_DOC.write_text(
            "# DG-KAN v22.60 BilevelMetaFU ControlLaw 实验结果复盘\n\n"
            f"创建时间：{now_sg()}\n\n"
...
def append_exec(command: str, *, task_id: str, status: str, gpu: str = "", files: str = "", note: str = "", exit_code: Any = "n/a") -> None:
    ...
    journal = read_rows(OUT_ROOT / "v22_60_command_journal.csv")
    journal.append({key: str(value) for key, value in row.items()})
    write_rows(OUT_ROOT / "v22_60_command_journal.csv", journal, ["timestamp", "task_id", "gpu", "command", "status", "exit_code", "files", "note"])
...
def controller_path_for_method(method: str) -> Path:
    m = method.lower()
    if m == "m0b_mlp_stateless_h50":
        return CONTROLLER_ROOT / "v22_60_m0b_h50_controller.pt"
    if m == "m0b_mlp_stateful_gru_h20":
        return CONTROLLER_ROOT / "v22_60_m0b_gru_h20_controller.pt"
    if m == "m0b_linear_stateless_h20":
        return CONTROLLER_ROOT / "v22_60_m0b_linear_h20_controller.pt"
    if m == "m0b_constant_learned_h20":
        return CONTROLLER_ROOT / "v22_60_m0b_constant_h20_controller.pt"
    if m == "random_scalar_scheduler_same_distribution":
        return CONTROLLER_ROOT / "v22_60_m0b_h20_controller.pt"
    if m == "frozen_controller_from_other_seed":
        return CONTROLLER_ROOT / "v22_60_m0b_seed_holdout_h20_controller.pt"
    if m == "frozen_controller_from_other_dataset":
        return CONTROLLER_ROOT / "v22_60_m0b_dataset_holdout_h20_controller.pt"
    if m == "time_only_scheduler":
        return CONTROLLER_ROOT / "v22_60_m0b_time_only_h20_controller.pt"
    if m == "same_compute_noop_controller":
        return CONTROLLER_ROOT / "v22_60_m0b_noop_controller.pt"
    if m in {"shuffled_state_feature_controller", "random_state_feature_controller"}:
        return CONTROLLER_ROOT / "v22_60_m0b_h20_controller.pt"
    return CONTROLLER_ROOT / "v22_60_m0b_h20_controller.pt"
...
def metafu_config_for(method: str, args: argparse.Namespace) -> MetaFUConfig:
    ...
    if m == "m0b_linear_stateless_h20":
    ...
    elif m == "m0b_constant_learned_h20":
        mode = "constant_scalar_scheduler"
    elif m == "m0b_mlp_stateful_gru_h20":
        mode = "gru_scalar_scheduler"
        ...
        mode = "meta_mlp_scalar_scheduler"
    elif m == "shuffled_state_feature_controller":
        mode = "shuffled_state_feature_controller"
    elif m == "random_state_feature_controller":
        mode = "random_state_feature_controller"
    elif m == "time_only_scheduler":
        mode = "linear_scalar_scheduler"
        feature_mode = "time_only"
    elif m == "same_compute_noop_controller":
        mode = "constant_scalar_scheduler"
    return MetaFUConfig(
...
def write_blocked_collect(args: argparse.Namespace, reason: str, method: str, ref_method: str) -> dict[str, Any]:
    ...
    out = CHUNK_ROOT / f"v22_60_{safe_fragment(str(args.label))}_summary.csv"
    write_rows(out, [row])
    return row
...
def run_collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if isinstance(metafu_state, MetaFUScalarOperator):
        ...
        if trace_rows:
            trace_path = CHUNK_ROOT / f"v22_60_{safe_fragment(str(args.label))}_controller_trace.csv"
            write_rows(trace_path, trace_rows)
            controller_trace_file = str(trace_path.relative_to(ROOT))
        if feature_rows:
            feature_path = CHUNK_ROOT / f"v22_60_{safe_fragment(str(args.label))}_controller_features.csv"
    ...
    if isinstance(scfg_state, StateCoupledFunctionalGeometryOperator):
        ...
        if telemetry_rows:
            telemetry_path = CHUNK_ROOT / f"v22_60_{safe_fragment(str(args.label))}_self_state_telemetry.csv"
    ...
    row = {
        ...
        "seed": int(args.seed),
        "method": method,
        "objective_family": "BilevelMetaFU_M0B" if method in M0_METHODS else "reference",
        ...
        "meta_test_no_controller_update": int(iflag(opt_diag.get("meta_controller_frozen_on_meta_test")) if method in M0_METHODS else 1),
...
def build_meta_dataset(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_rows(OUT_ROOT / "v22_60_meta_dataset_episodes.csv", rows)
    write_rows(OUT_ROOT / "v22_60_m0_probe_label_generation.csv", probe_rows)
    append_exec(
        f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage meta-dataset",
        task_id="v22_60_part_c_meta_dataset",
        status="pass" if any(r.get("target_raw_json") for r in rows) else "fail",
        files="results/v22_60/v22_60_meta_dataset_episodes.csv; results/v22_60/v22_60_m0_probe_label_generation.csv",
        gpu=device,
...
def train_controllers_from_rows(args: argparse.Namespace, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    summaries = [
        train_one_controller(usable, args, kind="mlp", path=CONTROLLER_ROOT / "v22_60_m0_controller.pt", subset_name="all_meta_train"),
        train_one_controller(usable, args, kind="linear", path=CONTROLLER_ROOT / "v22_60_m0_linear_controller.pt", subset_name="all_meta_train"),
        train_one_controller(seed_holdout or usable, args, kind="mlp", path=CONTROLLER_ROOT / "v22_60_m0_seed_holdout_controller.pt", subset_name="single_seed_source"),
        train_one_controller(dataset_holdout or usable, args, kind="mlp", path=CONTROLLER_ROOT / "v22_60_m0_dataset_holdout_controller.pt", subset_name="single_dataset_source"),
    ]
    write_rows(OUT_ROOT / "v22_60_m0_controller_train_summary.csv", summaries)
    append_recap(
...
def _one_hot(labels: Any, num_classes: int) -> Any:
    import torch

    return torch.nn.functional.one_hot(labels.long(), num_classes=int(num_classes)).float()


def _brier_from_logits(logits: Any, labels: Any, num_classes: int) -> Any:
    import torch

    probs = torch.softmax(logits.float(), dim=-1)
    target = _one_hot(labels, int(num_classes)).to(device=probs.device, dtype=probs.dtype)
    return torch.mean(torch.sum((probs - target) ** 2, dim=-1))


def _tail_loss_from_logits(logits: Any, labels: Any, q: float = 0.95) -> Any:
    import torch
    import torch.nn.functional as F

    losses = F.cross_entropy(logits.float(), labels.long(), reduction="none")
    k = max(1, int(math.ceil(float(losses.numel()) * (1.0 - float(q)))))
    return torch.sort(losses, descending=True).values[:k].mean()


def _margin_floor_loss(logits: Any, labels: Any) -> Any:
    import torch

    logits = logits.float()
    labels = labels.long()
    true = logits.gather(1, labels.reshape(-1, 1)).reshape(-1)
    masked = logits.clone()
    masked.scatter_(1, labels.reshape(-1, 1), -1.0e9)
    other = masked.max(dim=1).values
    margin = true - other
    return torch.relu(-margin).mean()


def _last_hidden(model: Any, x: Any) -> Any:
    import torch.nn.functional as F
...
def _controller_raw_step(controller: Any, z: Any, hidden: Any | None = None) -> tuple[Any, Any | None]:
    import torch

    if getattr(controller, "kind", "") == "gru":
        if z.ndim == 1:
            z = z.reshape(1, -1)
        x = z.float()
        if int(x.shape[-1]) < int(controller.input_dim):
            x = torch.nn.functional.pad(x, (0, int(controller.input_dim) - int(x.shape[-1])))
        if int(x.shape[-1]) > int(controller.input_dim):
            x = x[..., : int(controller.input_dim)]
        x = (x - controller.feature_mean.to(device=x.device, dtype=x.dtype)) / controller.feature_std.to(device=x.device, dtype=x.dtype)
        if hidden is None:
            hidden = torch.zeros(int(x.shape[0]), int(controller.hidden_dim), dtype=x.dtype, device=x.device)
        hidden = controller.gru(x, hidden)
        return controller.head(hidden), hidden
    return controller.raw(z), hidden


def _bounded_from_raw(raw: Any, args: argparse.Namespace) -> dict[str, Any]:
    import torch

    alpha = float(args.metafu_alpha_max) * torch.sigmoid(raw[:, 0])
    beta = float(args.metafu_beta_max) * torch.sigmoid(raw[:, 1])
    lambda_debt = float(args.metafu_lambda_debt_max) * torch.sigmoid(raw[:, 2])
    lambda_state = float(args.metafu_lambda_state_max) * torch.sigmoid(raw[:, 3])
    tau = float(args.metafu_tau_min) + (float(args.metafu_tau_max) - float(args.metafu_tau_min)) * torch.sigmoid(raw[:, 4])
    rho = torch.sigmoid(raw[:, 5])
    return {"alpha": alpha, "beta": beta, "lambda_debt": lambda_debt, "lambda_state": lambda_state, "tau": tau, "rho": rho}


def _task_cache(dataset: str, seed: int, args: argparse.Namespace, device_name: str) -> dict[str, Any]:
    import torch
    ...
    tensors = load_bundle_tensors(dataset, int(args.train_size), int(args.held_size), int(args.test_size), seed)
    if tensors["used_fake_data"]:
        raise RuntimeError("fake data rejected in bilevel meta task")
    split = internal_split_tensors(tensors, seed)
    model = make_model(int(tensors["input_dim"]), int(tensors["num_classes"]), int(args.hidden), int(seed) + 226000, device)
    x_source = split["x_source"].to(device)
    y_source = split["y_source"].to(device).long()
    x_witness = split["x_witness"].to(device)
    y_witness = split["y_witness"].to(device).long()
    x_query = split["x_query"].to(device)
    y_query = split["y_query"].to(device).long()
    with torch.no_grad():
        h_source = _last_hidden(model, x_source)
        h_witness = _last_hidden(model, x_witness)
        h_query = _last_hidden(model, x_query)
        w0 = model.fc3.weight.detach().clone()
        b0 = model.fc3.bias.detach().clone()
    source_ids = set(split["source_idx_hash"])
    return {
        "dataset": dataset,
        "seed": int(seed),
        "meta_task_id_hash": hashlib.sha256(f"{dataset}:{seed}:v22_60".encode("utf-8")).hexdigest()[:16],
        "source_hash": split["source_idx_hash"],
        "witness_hash": split["witness_idx_hash"],
        "query_hash": split["query_idx_hash"],
        "source_witness_overlap": 0,
        "source_query_overlap": 0,
        "witness_query_overlap": 0,
        "query_split_is_train_only": 1,
        "validation_test_used_in_meta_loss": 0,
        "dataset_name_available_to_controller": 0,
        "seed_available_to_controller": 0,
...
def _linear_logits(h: Any, w: Any, b: Any) -> Any:
    import torch.nn.functional as F

    return F.linear(h, w, b)


def _bilevel_unroll_loss(
    controller: Any | None,
    raw_param: Any | None,
    cache: dict[str, Any],
    args: argparse.Namespace,
    *,
    horizon: int,
    feature_mode: str = "bilevel",
) -> tuple[Any, dict[str, Any], list[list[float]]]:
    import torch
    import torch.nn.functional as F

    h_source = cache["h_source"]
    y_source = cache["y_source"]
    h_witness = cache["h_witness"]
    y_witness = cache["y_witness"]
    h_query = cache["h_query"]
    y_query = cache["y_query"]
    num_classes = int(cache["num_classes"])
    lr = float(args.lr)
    wd = float(args.weight_decay)
    w = cache["w0"].detach().clone().requires_grad_(True)
    b = cache["b0"].detach().clone().requires_grad_(True)
    w_ref = cache["w0"].detach().clone().requires_grad_(True)
    b_ref = cache["b0"].detach().clone().requires_grad_(True)
    w_noop = cache["w0"].detach().clone().requires_grad_(True)
    b_noop = cache["b0"].detach().clone().requires_grad_(True)
    gen = torch.Generator(device=h_source.device)
    gen.manual_seed(int(cache["seed"]) + int(horizon) + 226060)
    raw_trace: list[Any] = []
    hidden = None
    z_snapshots: list[list[float]] = []
    for step in range(int(horizon)):
        bs = min(int(args.batch_size), int(h_source.shape[0]))
        bw = min(int(args.batch_size), int(h_witness.shape[0]))
        idx_s = batch_indices(int(h_source.shape[0]), bs, gen, h_source.device)
        idx_w = batch_indices(int(h_witness.shape[0]), bw, gen, h_witness.device)
        hs = h_source[idx_s]
        ys = y_source[idx_s]
        hw = h_witness[idx_w]
        yw = y_witness[idx_w]
        src_logits = _linear_logits(hs, w, b)
        wit_logits = _linear_logits(hw, w, b)
        src_loss = F.cross_entropy(src_logits, ys)
        wit_loss = F.cross_entropy(wit_logits, yw)
        gs_w, gs_b = torch.autograd.grad(src_loss, (w, b), create_graph=True)
        gw_w, gw_b = torch.autograd.grad(wit_loss, (w, b), create_graph=True)
        cos = torch.nn.functional.cosine_similarity(gs_w.reshape(1, -1), gw_w.reshape(1, -1), dim=1)[0]
        src_norm = gs_w.detach().norm()
        wit_norm = gw_w.detach().norm()
        z_values = [
            float(step) / max(1.0, float(horizon)),
            float(src_loss.detach().item()),
            float(wit_loss.detach().item()),
            float((wit_loss - src_loss).detach().item()),
            float(src_norm.detach().item()),
            float(wit_norm.detach().item()),
            float(cos.detach().item()),
            float((src_norm - wit_norm).abs().div((src_norm + wit_norm).clamp_min(1.0e-8)).detach().item()),
            float(w.detach().norm().item()),
            float(args.lr),
            float(args.weight_decay),
        ]
        if feature_mode == "time_only":
            z_values = [z_values[0], math.sin(math.pi * z_values[0]), math.cos(math.pi * z_values[0]), float(args.lr)]
        if len(z_values) < int(args.metafu_input_dim):
            z_values.extend([0.0] * (int(args.metafu_input_dim) - len(z_values)))
        z = torch.tensor(z_values[: int(args.metafu_input_dim)], dtype=torch.float32, device=h_source.device).reshape(1, -1)
        z_snapshots.append([float(x) for x in z.detach().cpu().reshape(-1).tolist()])
        if raw_param is not None:
            raw = raw_param.reshape(1, -1)
        else:
            raw, hidden = _controller_raw_step(controller, z, hidden)
        bounded = _bounded_from_raw(raw, args)
        alpha = bounded["alpha"].reshape(())
        beta = bounded["beta"].reshape(())
        tau = bounded["tau"].reshape(())
        rho = bounded["rho"].reshape(())
        target_w = gs_w + alpha * (gw_w - gs_w) + beta * gw_w
        target_b = gs_b + alpha * (gw_b - gs_b) + beta * gw_b
        trans_w = (1.0 - rho) * gs_w + rho * target_w
        trans_b = (1.0 - rho) * gs_b + rho * target_b
        delta_w = trans_w - gs_w
        delta_b = trans_b - gs_b
        max_delta_w = tau * gs_w.detach().norm().clamp_min(1.0e-12)
        max_delta_b = tau * gs_b.detach().norm().clamp_min(1.0e-12)
        trans_w = gs_w + delta_w * torch.clamp(max_delta_w / delta_w.detach().norm().clamp_min(1.0e-12), max=1.0)
        trans_b = gs_b + delta_b * torch.clamp(max_delta_b / delta_b.detach().norm().clamp_min(1.0e-12), max=1.0)
        w = w - lr * (trans_w + wd * w)
        b = b - lr * trans_b
        raw_trace.append(raw.reshape(-1))

        ref_logits = _linear_logits(hs, w_ref, b_ref)
        ref_loss = F.cross_entropy(ref_logits, ys)
        gr_w, gr_b = torch.autograd.grad(ref_loss, (w_ref, b_ref), create_graph=False)
        w_ref = (w_ref - lr * (gr_w + wd * w_ref)).detach().requires_grad_(True)
        b_ref = (b_ref - lr * gr_b).detach().requires_grad_(True)

        noop_logits = _linear_logits(hs, w_noop, b_noop)
        noop_loss = F.cross_entropy(noop_logits, ys)
        gn_w, gn_b = torch.autograd.grad(noop_loss, (w_noop, b_noop), create_graph=False)
        w_noop = (w_noop - lr * (gn_w + wd * w_noop)).detach().requires_grad_(True)
        b_noop = (b_noop - lr * gn_b).detach().requires_grad_(True)
    q_logits = _linear_logits(h_query, w, b)
    ref_q_logits = _linear_logits(h_query, w_ref.detach(), b_ref.detach())
    noop_q_logits = _linear_logits(h_query, w_noop.detach(), b_noop.detach())
    q_loss = F.cross_entropy(q_logits, y_query)
    ref_q_loss = F.cross_entropy(ref_q_logits, y_query)
    noop_q_loss = F.cross_entropy(noop_q_logits, y_query)
    brier = _brier_from_logits(q_logits, y_query, num_classes)
    ref_brier = _brier_from_logits(ref_q_logits, y_query, num_classes).detach()
    tail95 = _tail_loss_from_logits(q_logits, y_query, 0.95)
    ref_tail95 = _tail_loss_from_logits(ref_q_logits, y_query, 0.95).detach()
    margin_debt = _margin_floor_loss(q_logits, y_query)
    ref_margin_debt = _margin_floor_loss(ref_q_logits, y_query).detach()
    regret = torch.relu(q_loss - ref_q_loss.detach() + float(args.bilevel_margin))
    ctrl = torch.relu(q_loss - noop_q_loss.detach() + float(args.bilevel_control_margin))
    debt = (
        torch.relu(brier - ref_brier)
        + torch.relu(tail95 - ref_tail95)
        + torch.relu(margin_debt - ref_margin_debt)
    )
    raw_stack = torch.stack(raw_trace, dim=0) if raw_trace else torch.zeros(1, len(META_FU_OUTPUT_NAMES), device=h_source.device)
    tv = (raw_stack[1:] - raw_stack[:-1]).pow(2).mean() if int(raw_stack.shape[0]) > 1 else raw_stack.pow(2).mean() * 0.0
    mag = raw_stack.pow(2).mean()
    loss = regret + float(args.bilevel_lambda_ctrl) * ctrl + float(args.bilevel_lambda_debt) * debt + float(args.bilevel_lambda_tv) * tv + float(args.bilevel_lambda_mag) * mag
    metrics = {
        "query_loss": float(q_loss.detach().item()),
        "reference_query_loss": float(ref_q_loss.detach().item()),
        "noop_query_loss": float(noop_q_loss.detach().item()),
        "regret": float((q_loss - ref_q_loss.detach()).detach().item()),
        "control_gap": float((q_loss - noop_q_loss.detach()).detach().item()),
        "brier_debt": float((brier - ref_brier).detach().item()),
        "tail95_debt": float((tail95 - ref_tail95).detach().item()),
        "margin_debt": float((margin_debt - ref_margin_debt).detach().item()),
        "raw_tv": float(tv.detach().item()),
        "raw_mag": float(mag.detach().item()),
    }
    return loss, metrics, z_snapshots


def train_bilevel_controller(
    ...
    *,
    ...
    if not caches:
        return {"controller": str(path.relative_to(ROOT)), "kind": kind, "subset": subset_name, "trained": 0, "blocker": "no caches"}
    epochs = int(args.meta_controller_epochs if epochs is None else epochs)
    bounds = {
        "alpha_max": float(args.metafu_alpha_max),
        "beta_max": float(args.metafu_beta_max),
        "lambda_debt_max": float(args.metafu_lambda_debt_max),
    ...
    device = caches[0]["h_source"].device
    raw_param = None
    if kind == "constant":
        raw_param = torch.nn.Parameter(torch.zeros(len(META_FU_OUTPUT_NAMES), dtype=torch.float32, device=device))
        params = [raw_param]
        controller = None
    else:
        controller = BoundedScalarController(
    ...
    opt = torch.optim.AdamW(params, lr=float(args.meta_controller_lr), weight_decay=1.0e-4)
    history: list[dict[str, Any]] = []
    z_seen: list[list[float]] = []
    for epoch in range(max(1, epochs)):
        total = None
        metric_acc: dict[str, list[float]] = {}
        for cache in caches:
            loss, metrics, z_rows = _bilevel_unroll_loss(controller, raw_param, cache, args, horizon=horizon, feature_mode=feature_mode)
            total = loss if total is None else total + loss
            z_seen.extend(z_rows[:2])
            for key, value in metrics.items():
                metric_acc.setdefault(key, []).append(float(value))
        opt.zero_grad(set_to_none=True)
        assert total is not None
        total = total / max(1, len(caches))
        total.backward()
        torch.nn.utils.clip_grad_norm_(params, float(args.bilevel_grad_clip))
        opt.step()
        if epoch == 0 or epoch == epochs - 1 or (epoch + 1) % max(1, epochs // 4) == 0:
            row = {"epoch": epoch + 1, "objective": float(total.detach().item())}
            row.update({f"mean_{k}": mean(v) for k, v in metric_acc.items()})
            history.append(row)
    feature_mean = [0.0] * int(args.metafu_input_dim)
    feature_std = [1.0] * int(args.metafu_input_dim)
    target_raw_samples: list[list[float]] = []
    eval_rows: list[dict[str, Any]] = []
    for cache in caches:
        loss, metrics, z_rows = _bilevel_unroll_loss(controller, raw_param, cache, args, horizon=horizon, feature_mode=feature_mode)
        eval_rows.append({"dataset": cache["dataset"], "seed": cache["seed"], **metrics})
        if raw_param is not None:
            target_raw_samples.append([float(x) for x in raw_param.detach().cpu().reshape(-1).tolist()])
        elif z_rows:
            z = torch.tensor(z_rows[0], dtype=torch.float32, device=device).reshape(1, -1)
            raw, _h = _controller_raw_step(controller, z, None)
            target_raw_samples.append([float(x) for x in raw.detach().cpu().reshape(-1).tolist()])
    if kind == "constant":
        artifact_kind = "constant"
        state_dict = {}
        constant_raw = [float(x) for x in raw_param.detach().cpu().reshape(-1).tolist()]
        param_count = int(raw_param.numel())
    else:
        artifact_kind = kind
        state_dict = controller.state_dict()
        constant_raw = [0.0] * len(META_FU_OUTPUT_NAMES)
        param_count = controller_parameter_count(controller)
    artifact = {
        "kind": artifact_kind,
        "subset": subset_name,
        "bilevel_trained": 1,
        "bilevel_horizon": int(horizon),
        "bilevel_objective": "H-step train-only source/witness/query query-regret + debt + anti-control",
        "input_dim": int(args.metafu_input_dim),
        "hidden_dim": int(args.metafu_controller_hidden),
        "feature_mean": feature_mean,
        "feature_std": feature_std,
        "constant_raw": constant_raw,
        "target_raw_samples": target_raw_samples,
        "state_dict": state_dict,
        "train_history": history,
        "eval_rows": eval_rows,
        "rows": len(caches),
        "controller_parameter_count": param_count,
        "controller_output_names": list(META_FU_OUTPUT_NAMES),
        "dataset_name_not_available_to_controller": 1,
        "seed_not_available_to_controller": 1,
        "validation_test_used_in_meta_loss": 0,
    ...
    return {
        ...
        "trained": 1,
        "rows": len(caches),
        "final_objective": history[-1]["objective"] if history else "",
        "mean_meta_train_query_regret": mean(r.get("regret") for r in eval_rows),
        "mean_meta_train_control_gap": mean(r.get("control_gap") for r in eval_rows),
        "controller_parameter_count": param_count,
    }


def build_bilevel_meta_dataset(args: argparse.Namespace) -> dict[str, Any]:
    ensure_out()
    datasets = split_csv(str(args.meta_train_datasets), str)
    seeds = split_csv(str(args.meta_train_seeds), int)
    device = str(args.synthetic_device)
    ...
    for dataset in datasets:
        for seed in seeds:
            try:
                cache = _task_cache(dataset, seed, args, device)
                caches.append(cache)
                split_rows.append(
                    {
                        "meta_task_id_hash": cache["meta_task_id_hash"],
                        "meta_split": "train",
                        "dataset": dataset,
                        "seed": seed,
                        "dataset_name_available_to_controller": 0,
                        ...
                        "witness_hash": cache["witness_hash"],
                        "query_hash": cache["query_hash"],
                        "source_witness_overlap": cache["source_witness_overlap"],
                        "source_query_overlap": cache["source_query_overlap"],
                        "witness_query_overlap": cache["witness_query_overlap"],
                        "query_split_is_train_only": 1,
                        "validation_test_used_in_meta_loss": 0,
                    }
                )
            except Exception as exc:
                blocked_rows.append({"dataset": dataset, "seed": seed, "row_status": "blocked", "blocker": repr(exc)})
    def _parse_task_pairs(text: str) -> list[tuple[str, int]]:
        ...
        for part in str(text).split(","):
            ...
            if not part:
                continue
            if ":" in part:
                ds, seed_text = part.split(":", 1)
                pairs.append((ds.strip(), int(seed_text.strip())))
        return pairs

    for split_name, spec_text in [("val", str(args.meta_val_tasks)), ("test", str(args.meta_test_tasks))]:
        for dataset, seed in _parse_task_pairs(spec_text):
            try:
                cache = _task_cache(dataset, seed, args, device)
                split_rows.append(
                    {
                        "meta_task_id_hash": cache["meta_task_id_hash"],
                        "meta_split": split_name,
                        "dataset": dataset,
                        "seed": seed,
                        "dataset_name_available_to_controller": 0,
                        "seed_available_to_controller": 0,
                ...
                blocked_rows.append({"dataset": dataset, "seed": seed, "meta_split": split_name, "row_status": "blocked", "blocker": repr(exc)})
    write_rows(OUT_ROOT / "v22_60_bilevel_meta_split_manifest.csv", split_rows + blocked_rows)
    if not caches:
        append_exec(
            f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage meta-dataset",
            task_id="v22_60_part_c_bilevel_meta_dataset",
            status="fail",
            files="results/v22_60/v22_60_bilevel_meta_split_manifest.csv",
            gpu=device,
            note="no usable meta-training caches",
        )
        return {"meta_dataset_rows": len(split_rows), "usable_meta_dataset_rows": 0, "controllers": [], "controller_train_pass": 0}
    min_seed = min(int(c["seed"]) for c in caches)
    first_dataset = str(caches[0]["dataset"])
    seed_subset = [c for c in caches if int(c["seed"]) == min_seed] or caches
    dataset_subset = [c for c in caches if str(c["dataset"]) == first_dataset] or caches
    short_epochs = max(5, int(args.meta_controller_epochs) // 2)
    summaries = [
        train_bilevel_controller(caches, args, kind="mlp", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_h20_controller.pt", subset_name="all_meta_train_H20"),
        train_bilevel_controller(caches, args, kind="mlp", horizon=50, path=CONTROLLER_ROOT / "v22_60_m0b_h50_controller.pt", subset_name="all_meta_train_H50", epochs=short_epochs),
        train_bilevel_controller(caches, args, kind="gru", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_gru_h20_controller.pt", subset_name="all_meta_train_GRU_H20", epochs=short_epochs),
        train_bilevel_controller(caches, args, kind="linear", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_linear_h20_controller.pt", subset_name="all_meta_train_linear_H20"),
        train_bilevel_controller(caches, args, kind="constant", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_constant_h20_controller.pt", subset_name="all_meta_train_constant_H20"),
        train_bilevel_controller(seed_subset, args, kind="mlp", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_seed_holdout_h20_controller.pt", subset_name="single_seed_source_H20", epochs=short_epochs),
        train_bilevel_controller(dataset_subset, args, kind="mlp", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_dataset_holdout_h20_controller.pt", subset_name="single_dataset_source_H20", epochs=short_epochs),
        train_bilevel_controller(caches, args, kind="linear", horizon=20, path=CONTROLLER_ROOT / "v22_60_m0b_time_only_h20_controller.pt", subset_name="time_only_H20", feature_mode="time_only", epochs=short_epochs),
    ]
    import torch

    noop_artifact = {
        "kind": "constant",
        "subset": "same_compute_noop",
        "bilevel_trained": 0,
        "bilevel_horizon": 0,
        "input_dim": int(args.metafu_input_dim),
        "hidden_dim": int(args.metafu_controller_hidden),
        "feature_mean": [0.0] * int(args.metafu_input_dim),
        "feature_std": [1.0] * int(args.metafu_input_dim),
        "constant_raw": [-12.0, -12.0, -12.0, -12.0, -12.0, -12.0],
        "target_raw_samples": [[-12.0, -12.0, -12.0, -12.0, -12.0, -12.0]],
        "state_dict": {},
        "rows": len(caches),
        "controller_parameter_count": 0,
        "controller_output_names": list(META_FU_OUTPUT_NAMES),
        "dataset_name_not_available_to_controller": 1,
        "seed_not_available_to_controller": 1,
        "validation_test_used_in_meta_loss": 0,
    ...
    noop_path = CONTROLLER_ROOT / "v22_60_m0b_noop_controller.pt"
    torch.save(noop_artifact, noop_path)
    summaries.append({"controller": str(noop_path.relative_to(ROOT)), "kind": "constant", "subset": "same_compute_noop", "trained": 1, "rows": len(caches), "controller_parameter_count": 0})
    write_rows(OUT_ROOT / "v22_60_m0b_controller_train_summary.csv", summaries)
    append_exec(
        f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage meta-dataset",
        task_id="v22_60_part_c_bilevel_meta_dataset",
        status="pass" if any(iflag(r.get("trained")) for r in summaries) else "fail",
        files="results/v22_60/v22_60_bilevel_meta_split_manifest.csv; results/v22_60/v22_60_m0b_controller_train_summary.csv; results/v22_60/controllers/",
        gpu=device,
        note=f"tasks={len(caches)}; blocked={len(blocked_rows)}; controllers={len(summaries)}; objective=H-step train-only query regret",
    )
    append_recap(
        "Part C Bilevel meta dataset and controller training",
        "Controller 参数通过 H-step train-only source/witness/query query-regret + debt + anti-control objective 更新；不是 probe-label 回归。所有 meta loss 只使用 train 内部 Q split。\n\n"
        + md_table(split_rows + blocked_rows, ["dataset", "seed", "meta_split", "source_hash", "witness_hash", "query_hash", "query_split_is_train_only", "validation_test_used_in_meta_loss"], 20)
        + "\nController summary:\n\n"
        + md_table(summaries, ["controller", "kind", "subset", "horizon", "trained", "rows", "final_objective", "mean_meta_train_query_regret", "mean_meta_train_control_gap", "controller_parameter_count"], 20),
    )
    return {
        "meta_dataset_rows": len(split_rows) + len(blocked_rows),
        "usable_meta_dataset_rows": len(caches),
        "controllers": summaries,
        "controller_train_pass": int(any(iflag(r.get("trained")) for r in summaries)),
    }
...
def build_clean_audit_bundle(args: argparse.Namespace) -> dict[str, Any]:
    ...
    bundle_dir = pack_root / f"v22_60_audit_{stamp}"
    ...
    rows: list[dict[str, Any]] = []
    with tempfile.TemporaryDirectory(prefix="v22_60_clean_extract_") as tmp:
        tmp_path = Path(tmp)
        ...
        compile_res = run_cmd(
            ...
            task_id="v22_60_clean_tarball_compileall",
        ...
        import_cmds = {
            ...
            "clean_tarball_runner_core_import_pass": "import experiments.run_v22_60_bilevel_meta_fu_control_law; print('pass')",
        ...
        for key, code in import_cmds.items():
            res = run_cmd([PYTHON, "-c", code], task_id=f"v22_60_{key}", files=str(tar_path.relative_to(ROOT)), cwd=clean_root)
            rows.append({"check": key, "pass": int(res.get("status") == "pass"), "returncode": res.get("returncode")})
    write_rows(OUT_ROOT / "v22_60_clean_tarball_self_contained_import.csv", rows)
    required_core = [
        "dgkan/fu/meta_fu_control_law.py",
        "dgkan/optim/meta_fu_optimizer_wrapper.py",
        "experiments/run_v22_60_bilevel_meta_fu_control_law.py",
        "experiments/audit_v22_60_standard_loop.py",
    ]
...
def run_o0(args: argparse.Namespace) -> dict[str, Any]:
    ...
    compile_result = run_cmd(
        [PYTHON, "-m", "compileall", "-q", "dgkan", "experiments", "external/oet_baselines"],
        task_id="v22_60_compileall_repo",
        files="results/v22_60/v22_60_code_truth_gate.csv",
        timeout=int(args.compile_timeout),
    )
    import_cmds = {
        "worktree_full_repo_import_pass": "import dgkan; import dgkan.optim; print('pass')",
        "runner_core_import_pass": "import experiments.run_v22_60_bilevel_meta_fu_control_law; print('pass')",
    ...
    for key, code in import_cmds.items():
        res = run_cmd([PYTHON, "-c", code], task_id=f"v22_60_{key}", files="results/v22_60/v22_60_full_import_closure.csv")
    ...
    write_rows(OUT_ROOT / "v22_60_full_import_closure.csv", import_rows)
    ...
    write_rows(OUT_ROOT / "v22_60_standard_loop_static_scan.csv", scan["rows"] or [{"status": "no_forbidden_hits"}])
    write_rows(OUT_ROOT / "v22_60_manual_update_forbidden_scan.csv", scan["rows"] or [{"status": "no_forbidden_hits"}])
    runtime_args = argparse.Namespace(**vars(args))
    runtime_args.dataset = "MNIST"
    runtime_args.seed = 0
    runtime_args.method = "m0b_mlp_stateless_h20"
    runtime_args.reference_method = "adamw"
    runtime_args.label = "v22_60_runtime_trace_m0b_smoke"
    ...
    try:
        ...
        (LOG_ROOT / "v22_60_runtime_trace_metafu_smoke_exception.log").write_text(runtime_error, encoding="utf-8")
    write_rows(OUT_ROOT / "v22_60_standard_loop_runtime_trace.csv", [runtime_row] if runtime_row else [{"status": "runtime_exception", "error": runtime_error[:500]}])
    leakage_row = {
    ...
    write_rows(OUT_ROOT / "v22_60_meta_controller_leakage_audit.csv", [leakage_row])
    row = {
    ...
    write_rows(OUT_ROOT / "v22_60_code_truth_gate.csv", [row])
    append_exec(
        f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage o0",
        task_id="v22_60_part_a_truth_gate_summary",
        status="pass" if row["part_a_pass"] else "fail",
        files=(
            "results/v22_60/v22_60_code_truth_gate.csv; "
            "results/v22_60/v22_60_full_import_closure.csv; "
            "results/v22_60/v22_60_clean_tarball_self_contained_import.csv; "
            "results/v22_60/v22_60_standard_loop_static_scan.csv; "
            "results/v22_60/v22_60_standard_loop_runtime_trace.csv; "
            "results/v22_60/v22_60_manual_update_forbidden_scan.csv; "
            "results/v22_60/v22_60_meta_controller_leakage_audit.csv"
        ),
...
def reanalyse_v22_59(args: argparse.Namespace) -> dict[str, Any]:
    ensure_out()
    src = ROOT / "results/v22_59"
    pairwise = read_rows(src / "v22_59_m0_pairwise.csv")
    gate_rows = read_rows(src / "v22_59_m0_candidate_method_gate.csv")
    leakage = read_rows(src / "v22_59_meta_controller_leakage_audit.csv")
    method_summary = read_rows(src / "v22_59_method_summary.csv")
    route = read_json(src / "v22_59_final_route.json")
    learned = [r for r in pairwise if r.get("method") in {"meta_mlp_scalar_scheduler", "linear_scalar_scheduler", "constant_scalar_scheduler"}]
    rows: list[dict[str, Any]] = []
    for method in sorted({r.get("method", "") for r in learned}):
        group = [r for r in learned if r.get("method") == method]
        leak = next((r for r in leakage if r.get("method") == method), {})
        rows.append(
            {
                "method": method,
                "rows": len(group),
                "beats_strongest_reference_rows": sum(iflag(r.get("beats_strongest_reference")) for r in group),
                "beats_random_frozen_rows": sum(iflag(r.get("beats_frozen_shuffled_random_controller_controls")) for r in group),
                "random_or_frozen_controls_explain_rows": sum(iflag(r.get("random_or_frozen_control_beats_method")) for r in group),
                "beats_handdesigned_rows": sum(iflag(r.get("beats_handdesigned_v22_58")) for r in group),
                "beats_credit_self_rows": sum(iflag(r.get("beats_credit_only_and_self_only")) for r in group),
                ...
                "leakage_dataset_classifier_accuracy": leak.get("dataset_id_classifier_accuracy_from_controller_output", ""),
                "leakage_seed_classifier_accuracy": leak.get("seed_id_classifier_accuracy_from_controller_output", ""),
                "leakage_audit_pass": leak.get("leakage_audit_pass", ""),
            }
        )
    summary = {
        "v22_59_artifacts_present": int(bool(pairwise) and bool(route)),
        "final_route": route.get("final_route", ""),
        "m0_method_gate_pass_rows": route.get("m0_method_gate_pass_rows", ""),
        "learned_pairwise_rows": len(learned),
        "beats_strongest_reference_rows": sum(iflag(r.get("beats_strongest_reference")) for r in learned),
        "beats_random_frozen_rows": sum(iflag(r.get("beats_frozen_shuffled_random_controller_controls")) for r in learned),
        "random_or_frozen_controls_explain_rows": sum(iflag(r.get("random_or_frozen_control_beats_method")) for r in learned),
        "no_debt_rows": sum(iflag(r.get("no_ECE_Brier_tail_debt")) for r in learned),
        "overhead_le_035_rows": sum(int((fval(r.get("controller_overhead_ratio"), float("inf")) or float("inf")) <= 0.35) for r in learned),
        "gate_rows": len(gate_rows),
        "leakage_rows": len(leakage),
        "method_summary_rows": len(method_summary),
        "matches_recap_r2_failure": int(route.get("final_route") == "R2-MetaControllerRandomOrFrozenExplained" and route.get("m0_method_gate_pass_rows") == 0),
    }
    write_rows(OUT_ROOT / "v22_60_v22_59_independent_reanalysis.csv", rows)
    write_json(OUT_ROOT / "v22_60_v22_59_independent_reanalysis_summary.json", summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage reanalyse-v59",
        task_id="v22_60_part_b_v59_reanalysis",
        status="pass" if summary["v22_59_artifacts_present"] else "fail",
        files="results/v22_60/v22_60_v22_59_independent_reanalysis.csv; results/v22_60/v22_60_v22_59_independent_reanalysis_summary.json",
        note=json.dumps(summary, ensure_ascii=False, sort_keys=True),
    )
    append_recap(
        "Part B v22.59 independent artifact reanalysis",
        "从 v22.59 pairwise/route/leakage/gate artifacts 重算上一轮失败归因，确认 v22.60 是否可以进入新实验。关键 summary：\n\n```json\n"
        + json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True)
        + "\n```\n\nPer-method evidence:\n\n"
        + md_table(rows, ["method", "rows", "beats_strongest_reference_rows", "beats_random_frozen_rows", "random_or_frozen_controls_explain_rows", "no_debt_rows", "overhead_le_035_rows", "mean_controller_output_entropy", "leakage_audit_pass"], 20),
    )
    return summary
...
def run_matrix(args: argparse.Namespace, *, phase: str) -> list[dict[str, Any]]:
    ...
    def launch(item: tuple[int, dict[str, Any]]) -> dict[str, Any]:
        ...
        result = run_cmd(
            ...
            files=f"results/v22_60/chunks/v22_60_{safe_fragment(str(spec['label']))}_summary.csv",
            gpu=str(gpu),
        ...
        return {
            ...
            **result,
            "summary": f"results/v22_60/chunks/v22_60_{safe_fragment(str(spec['label']))}_summary.csv",
        }
    ...
    append_exec(
        f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage {phase}",
        task_id=f"v22_60_{phase}_matrix",
        status="pass" if all(r.get("status") == "pass" for r in dispatch_rows) else "fail",
        files=f"{specs_path}; {dispatch_path}; results/v22_60/chunks/",
...
def collect_chunk_rows(include_prefixes: list[str] | None = None) -> list[dict[str, str]]:
    ...
    for path in sorted(CHUNK_ROOT.glob("v22_60_*_summary.csv")):
...
def m0_method_gate_rows(pairwise: list[dict[str, Any]], leakage: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for method in sorted({r.get("method", "") for r in pairwise if r.get("method") in M0_LEARNED_METHODS}):
        ...
        row = {
            ...
            "rows": n,
            "beats_strongest_reference_rows": sum(iflag(r.get("beats_strongest_reference")) for r in group),
            "beats_external_OET_if_available_rows": sum(iflag(r.get("beats_POET_or_external_OET")) for r in group),
            ...
            "meta_test_no_controller_update_rows": sum(iflag(r.get("meta_test_no_controller_update")) for r in group),
            ...
            "leakage_audit_pass_rows": n if iflag(leak.get("leakage_audit_pass")) else 0,
        }
        row["required_beats_strongest_reference_rows"] = 8 if n >= 15 else _need(n, 8, 15)
        row["required_beats_external_OET_rows"] = 8 if n >= 15 else _need(n, 8, 15)
        row["required_beats_handdesigned_v22_58_rows"] = 8 if n >= 15 else _need(n, 8, 15)
        row["required_beats_frozen_shuffled_random_rows"] = 10 if n >= 15 else _need(n, 10, 15)
        row["required_beats_credit_self_rows"] = 10 if n >= 15 else _need(n, 10, 15)
        row["required_no_debt_rows"] = 11 if n >= 15 else _need(n, 11, 15)
        row["required_overhead_rows"] = 12 if n >= 15 else _need(n, 12, 15)
        row["required_leakage_audit_pass_rows"] = 14 if n >= 15 else _need(n, 14, 15)
        row["method_gate_pass"] = int(
            n >= 15
            and row["beats_strongest_reference_rows"] >= row["required_beats_strongest_reference_rows"]
            and row["beats_external_OET_if_available_rows"] >= row["required_beats_external_OET_rows"]
            ...
            and row["meta_test_no_controller_update_rows"] == n
            and row["leakage_audit_pass_rows"] >= row["required_leakage_audit_pass_rows"]
...
def classify_route(route: dict[str, Any], gate_rows: list[dict[str, Any]], pairwise: list[dict[str, Any]], leakage: list[dict[str, Any]]) -> str:
    if route.get("part_a_pass") == 0:
        return "R0-CodeOrMetaBoundaryFailed"
    if not pairwise:
        return "R1-BilevelMetaFUUnitOnly"
    if route.get("mlp_m0_exploration_pass"):
        return "MLPGeneralMetaFUOpened"
    if route.get("beats_strongest_reference_rows", 0) == 0 and route.get("beats_external_oet_rows", 0) == 0 and route.get("beats_reference_rows", 0) > 0:
        return "R9-FUWeakOptimizerPatchOnly"
    ...
    constant_rows = [r for r in pairwise if r.get("method") == "m0b_constant_learned_h20"]
    learned_rows = [r for r in pairwise if r.get("method") in M0_LEARNED_METHODS and r.get("method") != "m0b_constant_learned_h20"]
    if constant_rows and learned_rows:
        const_wins = sum(iflag(r.get("beats_strongest_reference")) for r in constant_rows)
        best_other = max(
            sum(iflag(r.get("beats_strongest_reference")) for r in learned_rows if r.get("method") == method)
            for method in sorted({r.get("method") for r in learned_rows})
        )
        if const_wins >= best_other:
            return "LearnedScheduleOnly_NoMetaFU"
    ...
    return "BilevelMetaSignalDiagnosticOnly"
...
def summarize(args: argparse.Namespace) -> dict[str, Any]:
        ...
        method_summary.append(
            {
                ...
                "mean_controller_output_entropy": mean(fval(r.get("controller_output_entropy")) for r in group),
                "mean_controller_TV_norm": mean(fval(r.get("controller_TV_norm")) for r in group),
                "mean_controller_output_std_mean": mean(fval(r.get("controller_output_std_mean")) for r in group),
            }
        )
    write_rows(OUT_ROOT / "v22_60_method_summary.csv", method_summary)
    ...
    write_rows(OUT_ROOT / "v22_60_m0_pairwise.csv", pairwise)
    leakage = controller_leakage_rows(rows)
    write_rows(OUT_ROOT / "v22_60_meta_controller_leakage_audit.csv", leakage or read_rows(OUT_ROOT / "v22_60_meta_controller_leakage_audit.csv"))
    gate_rows = m0_method_gate_rows(pairwise, leakage)
    write_rows(OUT_ROOT / "v22_60_m0_candidate_method_gate.csv", gate_rows)
    part_a = read_rows(OUT_ROOT / "v22_60_code_truth_gate.csv")
    ...
    route = {
        ...
        "beats_reference_rows": sum(iflag(r.get("beats_reference")) for r in learned_pairwise),
        ...
        "m1b_eligibility_pass": int(
            any(
                iflag(r.get("beats_frozen_shuffled_random_controller_controls_rows")) >= 8
                and iflag(r.get("beats_strongest_reference_rows")) >= 7
                and iflag(r.get("no_ECE_Brier_tail_debt_rows")) >= 9
                for r in gate_rows
            )
        ),
    ...
    route["m1_gate_status"] = "eligible_not_run_yet" if route["m1b_eligibility_pass"] else "skipped_M0B_gate_not_opened"
    ...
    write_json(OUT_ROOT / "v22_60_final_route.json", route)
    downstream = [
    ...
    write_rows(OUT_ROOT / "v22_60_downstream_gate_status.csv", downstream)
    fallback = build_m0_fallback_report(route, gate_rows, pairwise, leakage)
    write_rows(OUT_ROOT / "v22_60_m0_fallback_diagnostics.csv", fallback)
    append_exec(
        f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage summarize",
        task_id="v22_60_summarize",
        status="pass",
        files=(
            "results/v22_60/v22_60_method_summary.csv; results/v22_60/v22_60_m0_pairwise.csv; "
            "results/v22_60/v22_60_m0_candidate_method_gate.csv; results/v22_60/v22_60_meta_controller_leakage_audit.csv; "
            "results/v22_60/v22_60_final_route.json; results/v22_60/v22_60_downstream_gate_status.csv; "
            "results/v22_60/v22_60_m0_fallback_diagnostics.csv"
        ),
    ...
    rows.append(
        {
            ...
            "repair_direction": "follow v22.60 9.1: inspect leakage/output constancy/reference strength/source-witness-query objective",
        }
    )
    if learned:
...
def run_figures(args: argparse.Namespace) -> dict[str, Any]:
    rows = read_rows(OUT_ROOT / "v22_60_m0_pairwise.csv")
    FIG_ROOT.mkdir(parents=True, exist_ok=True)
    summary = {"figure_rows": len(rows), "figures_created": 0}
    write_json(OUT_ROOT / "v22_60_figure_summary.json", summary)
    return summary


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--stage", default="all", choices=["all", "o0", "reanalyse-v59", "meta-dataset", "collect", "reference", "m0", "summarize", "figures"])
    p.add_argument("--dataset", default="MNIST")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--method", default="adamw")
    p.add_argument("--reference-method", default="adamw")
    p.add_argument("--label", default="v22_60_collect")
    ...
    p.add_argument("--label-prefix", default="v22_60_reference")
    ...
    p.add_argument("--meta-train-seeds", default="0,1,2")
    p.add_argument("--meta-val-tasks", default="MNIST:3,FashionMNIST:3,KMNIST:3,Spam:3")
    p.add_argument("--meta-test-tasks", default="MNIST:4,MNIST:5,FashionMNIST:4,FashionMNIST:5,KMNIST:4,KMNIST:5,Wine:4,Wine:5,Spam:4,Spam:5")
    ...
    p.add_argument("--bilevel-margin", type=float, default=0.0)
    p.add_argument("--bilevel-control-margin", type=float, default=0.0)
    p.add_argument("--bilevel-lambda-ctrl", type=float, default=0.50)
    p.add_argument("--bilevel-lambda-debt", type=float, default=0.35)
    p.add_argument("--bilevel-lambda-tv", type=float, default=1.0e-3)
    p.add_argument("--bilevel-lambda-mag", type=float, default=1.0e-4)
    p.add_argument("--bilevel-grad-clip", type=float, default=1.0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.stage == "o0":
            ...
            return 0
        if args.stage == "reanalyse-v59":
            print(json.dumps(reanalyse_v22_59(args), ensure_ascii=False, sort_keys=True))
            return 0
        if args.stage == "meta-dataset":
            print(json.dumps(build_bilevel_meta_dataset(args), ensure_ascii=False, sort_keys=True))
            return 0
        ...
        if args.stage == "reference":
            ...
            return 0
        if args.stage == "m0":
            if args.label_prefix == "v22_60_reference":
                args.label_prefix = f"v22_60_m0_over_{safe_fragment(args.reference_method)}"
        ...
        if args.stage == "all":
            ...
            if not iflag(gate.get("part_a_pass")):
                ...
                run_figures(args)
                return 2
            v59 = reanalyse_v22_59(args)
            if not iflag(v59.get("v22_59_artifacts_present")):
                summarize(args)
                run_figures(args)
                return 3
            meta = build_bilevel_meta_dataset(args)
            if not iflag(meta.get("controller_train_pass")):
                summarize(args)
                run_figures(args)
                return 4
            ref_args = argparse.Namespace(**vars(args))
            ref_args.label_prefix = "v22_60_reference"
            ...
            m0_args.label_prefix = f"v22_60_m0_over_{safe_fragment(args.reference_method)}"
        ...
        (LOG_ROOT / "v22_60_runner_exception.log").write_text(err, encoding="utf-8")
        append_exec(
            f"{PYTHON} experiments/run_v22_60_bilevel_meta_fu_control_law.py --stage {args.stage}",
            task_id=f"v22_60_{args.stage}_exception",
            status="exception",
            files="results/v22_60/logs/v22_60_runner_exception.log",
            note=err[-1000:],
            exit_code="exception",
        )
        raise
