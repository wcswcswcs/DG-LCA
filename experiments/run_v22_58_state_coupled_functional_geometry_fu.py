#!/usr/bin/env python3
"""DG-KAN v22.58 state-coupled functional geometry FU runner."""
...
from dgkan.fu.state_coupled_functional_geometry_operator import SCFGConfig, StateCoupledFunctionalGeometryOperator
from dgkan.optim.scfg_optimizer_wrapper import SCFGOptimizerWrapper
from experiments import audit_v22_58_standard_loop as loop_audit
...
OUT_ROOT = ROOT / "results/v22_58"
CHUNK_ROOT = OUT_ROOT / "chunks"
LOG_ROOT = OUT_ROOT / "logs"
FIG_ROOT = OUT_ROOT / "figures"
DOCS = ROOT / "docs"
EXEC_DOC = DOCS / "DG-KAN_v22.58_StateCoupledFunctionalGeometryFU_执行日志.md"
RECAP_DOC = DOCS / "DG-KAN_v22.58_StateCoupledFunctionalGeometryFU_实验结果复盘.md"
PLAN_DOC = DOCS / "DG-KAN_v22.58_StateCoupledFunctionalGeometryFU_完整计划.md"
RUNNER = ROOT / "experiments/run_v22_58_state_coupled_functional_geometry_fu.py"
...
SCFG_METHODS = SCFG_CANDIDATES | SCFG_CONTROLS | SCFG_REPAIR_METHODS
...
DEFAULT_SCFG_METHODS = (
    "scfg-credit-only,scfg-self-only,scfg-static-credit-self,"
    "scfg-state-transition,scfg-state-transition-tail-aware,"
    "scfg-state-transition-oet-compatible,scfg-state-transition-low-overhead,"
    "same_compute_noop,credit_only_operator,self_only_operator,static_credit_self_operator,"
    "same_credit_random_self_metric,"
    "same_self_random_credit,same_generalized_spectrum_random_basis,"
    "same_spectrum_random_out_in_factor,source_only_crossfit_factor,witness_only_operator,shuffled_source_witness_pairing,"
    "signflip_cotangent_factor,random_activation_factor,kfac_style_train_covariance_only,"
    "soap_like_eigenbasis_control,oet_self_geometry_only,"
    "inverse_class_count_gate,hard_loss_gate,loss_rank_gate,margin_shuffle_gate,random_label_gate"
)
...
def ensure_out() -> None:
    ...
    if not EXEC_DOC.exists():
        EXEC_DOC.write_text(
            "# DG-KAN v22.58 StateCoupledFunctionalGeometryFU 执行日志\n\n"
            f"创建时间：{now_sg()}\n\n"
            "记录原则：只记录真实命令、解释器、GPU、输入输出文件、状态、失败与修复尝试。"
            "官方 SCFG 行只允许 task loss backward 与 optimizer-owned gradient transform。\n",
            encoding="utf-8",
        )
    if not RECAP_DOC.exists():
        RECAP_DOC.write_text(
            "# DG-KAN v22.58 StateCoupledFunctionalGeometryFU 实验结果复盘\n\n"
            f"创建时间：{now_sg()}\n\n"
...
def append_exec(command: str, *, task_id: str, status: str, gpu: str = "", files: str = "", note: str = "", exit_code: Any = "n/a") -> None:
    ...
    journal = read_rows(OUT_ROOT / "v22_58_command_journal.csv")
    journal.append({key: str(value) for key, value in row.items()})
    write_rows(OUT_ROOT / "v22_58_command_journal.csv", journal, ["timestamp", "task_id", "gpu", "command", "status", "exit_code", "files", "note"])
...
def select_scfg_params(model: Any, scope: str) -> list[Any]:
    ...
    raise ValueError(f"unknown SCFG parameter scope: {scope}")


def reference_method_for(method: str, args: argparse.Namespace) -> str:
    m = method.lower()
    if m in SCFG_METHODS:
        return str(args.reference_method)
    return m


def scfg_config_for(method: str, args: argparse.Namespace) -> SCFGConfig:
    m = method.lower()
    control = "none"
    variant = "state_transition"
    use_out = True
    use_in = True
    alpha = float(args.scfg_alpha)
    beta = float(args.scfg_beta)
    weight_spectrum_weight = float(args.scfg_weight_spectrum_weight)
    gradient_cov_weight = float(args.scfg_gradient_cov_weight)
    momentum_weight = float(args.scfg_momentum_weight)
    sharpness_weight = 0.0
    state_transition_weight = float(args.scfg_state_transition_weight)
    debt_weight = float(args.scfg_debt_weight)
    tail_weight = float(args.scfg_tail_weight)
    low_overhead = False
    oet_compatible = False
    if m == "scfg-credit-only":
        variant = "candidate_credit_only"
        control = "credit_only_operator"
        state_transition_weight = 0.0
        debt_weight = 0.0
    elif m == "scfg-self-only":
        variant = "candidate_self_only"
        control = "self_only_operator"
        state_transition_weight = 0.0
        debt_weight = 0.0
    elif m == "scfg-static-credit-self":
        variant = "static_credit_self"
        state_transition_weight = 0.0
        debt_weight = 0.0
    elif m == "scfg-state-transition":
        variant = "state_transition"
    elif m == "scfg-state-transition-tail-aware":
        variant = "state_transition_tail_aware"
        debt_weight = max(debt_weight, 0.55)
        tail_weight = max(tail_weight, 0.55)
        alpha = min(alpha, 0.16)
        beta = min(beta, 0.16)
        sharpness_weight = 0.10
    elif m == "scfg-state-transition-oet-compatible":
        variant = "state_transition_oet_compatible"
        oet_compatible = True
        weight_spectrum_weight = max(weight_spectrum_weight, 0.75)
        momentum_weight = max(momentum_weight, 0.25)
    elif m == "scfg-state-transition-low-overhead":
        variant = "state_transition_low_overhead"
        low_overhead = True
        alpha = min(alpha, 0.12)
        beta = min(beta, 0.12)
    elif m == "scfg-debt-tail":
        variant = "state_transition_debt_tail"
        debt_weight = max(debt_weight, 0.80)
        tail_weight = max(tail_weight, 0.90)
        alpha = min(alpha, 0.12)
        beta = min(beta, 0.12)
    elif m in {"scfg-debt-ece", "scfg-state-transition-ece-aware"}:
        variant = "state_transition_debt_ece"
        debt_weight = max(debt_weight, 0.85)
        tail_weight = max(tail_weight, 0.20)
        alpha = min(alpha, 0.12)
        beta = min(beta, 0.12)
    elif m in {"scfg-debt-margin", "scfg-state-transition-margin-aware"}:
        variant = "state_transition_debt_margin"
        debt_weight = max(debt_weight, 0.75)
        tail_weight = max(tail_weight, 0.35)
        alpha = min(alpha, 0.12)
        beta = min(beta, 0.12)
    elif m in {"scfg-debt-combined", "scfg-state-transition-combined-debt"}:
        variant = "state_transition_debt_combined"
        debt_weight = max(debt_weight, 1.00)
        tail_weight = max(tail_weight, 0.80)
        alpha = min(alpha, 0.10)
        beta = min(beta, 0.10)
    elif m in SCFG_CONTROLS:
        control = m
        variant = "control"
    ...
    return SCFGConfig(
        variant=variant,
        control=control,
        rank=max(1, min(int(args.scfg_rank), 2 if low_overhead else int(args.scfg_rank))),
        alpha=alpha,
        beta=beta,
        eta=float(args.scfg_eta),
        eps=float(args.scfg_eps),
        ema_beta=float(args.scfg_ema_beta),
        source_fraction=0.50,
        random_seed=seed,
        c_min=float(args.scfg_c_min),
        use_out=use_out,
        use_in=use_in,
        ...
        state_transition_weight=state_transition_weight,
        debt_weight=debt_weight,
        tail_weight=tail_weight,
        low_overhead=low_overhead,
        oet_compatible=oet_compatible,
...
def write_blocked_collect(args: argparse.Namespace, reason: str, method: str, ref_method: str) -> dict[str, Any]:
    row = {
        ...
        "seed": int(args.seed),
        "method": method,
        "phase": "SCFG" if method in SCFG_METHODS else "reference",
        "reference_method": ref_method if method in SCFG_METHODS else "",
        "final_NLL": "",
        "standard_loop_hard_gate_pass": 0,
        "external_oet_unavailable_local_only": int("unavailable" in reason.lower()),
    }
    out = CHUNK_ROOT / f"v22_58_{safe_fragment(str(args.label))}_summary.csv"
    write_rows(out, [row])
    return row
...
def run_collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if method not in REFERENCE_METHODS and method not in SCFG_METHODS:
    ...
    scope = str(args.scfg_param_scope)
    scfg_params = select_scfg_params(model, scope)
    scfg_state = None
    opt = base_opt
    if method in SCFG_METHODS:
        scfg_state = StateCoupledFunctionalGeometryOperator(scfg_params, scfg_config_for(method, args))
    ...
    hook_handle = None
    if method in SCFG_METHODS and scope == "last_layer" and hasattr(model, "fc3"):
    ...
    try:
        for step in range(int(args.steps)):
            ...
            if method in SCFG_METHODS and scfg_state is not None:
                stat_start = time.perf_counter()
                stats_interval = max(1, int(args.scfg_refresh_interval))
                ...
                if should_observe:
                    ...
                    if scope == "last_layer" and "last_layer_input" in feature_cache:
                        ...
                        cohort_grads, cohort_labels, cohort_losses, cohort_margins = compute_cohort_grads(method, logits, yb, positions, scfg_params, generator)
                    scfg_state.observe(
                        cohort_grads,
    ...
    scfg_diag = opt.diagnostics() if isinstance(opt, SCFGOptimizerWrapper) else {}
    telemetry_file = ""
    if isinstance(scfg_state, StateCoupledFunctionalGeometryOperator):
        telemetry_rows = scfg_state.telemetry_rows()
        if telemetry_rows:
            telemetry_path = CHUNK_ROOT / f"v22_58_{safe_fragment(str(args.label))}_self_state_telemetry.csv"
            write_rows(telemetry_path, telemetry_rows)
            telemetry_file = str(telemetry_path.relative_to(ROOT))
    scfg_observe_cumulative_ms = float(scfg_diag.get("scfg_stats_ms", 0.0) or 0.0)
    scfg_transform_cumulative_ms = float(scfg_diag.get("scfg_transform_ms", 0.0) or 0.0)
    scfg_diag = dict(scfg_diag)
    scfg_diag.pop("scfg_stats_ms", None)
    scfg_diag.pop("scfg_transform_ms", None)
    scfg_stats_step_ms = mean(stats_ms) if stats_ms else 0.0
    scfg_transform_step_ms = scfg_transform_cumulative_ms / max(1, int(args.steps))
    ...
    optimizer_owned = int(scfg_diag.get("optimizer_owned_gradient_transform_pass", 0)) if method in SCFG_METHODS else 1
    audit_flags = runtime_audit.as_dict()
    stats_backend = "last_layer_closed_form" if method in SCFG_METHODS and scope == "last_layer" and hasattr(model, "fc3") else "autograd"
    ...
    cfg = scfg_config_for(method, args) if method in SCFG_METHODS else None
    row = {
        "run_label": str(args.label),
        ...
        "seed": int(args.seed),
                "method": method,
                "objective_family": "SCFG",
                "control_kind": method if method in SCFG_CONTROLS else ("candidate" if method in SCFG_CANDIDATES else ("repair" if method in SCFG_REPAIR_METHODS else "reference")),
        "reference_method": ref_method if method in SCFG_METHODS else "",
        ...
        **continual_metrics,
        ...
        "scfg_refresh_interval": int(args.scfg_refresh_interval),
        "scfg_param_scope": scope if method in SCFG_METHODS else "",
        "scfg_stats_backend": stats_backend if method in SCFG_METHODS else "",
        "self_state_telemetry_file": telemetry_file,
        "scfg_parameter_count": int(sum(int(p.numel()) for p in scfg_params)) if method in SCFG_METHODS else 0,
        ...
        "operator_build_step_ms": scfg_stats_step_ms,
        "operator_apply_step_ms": scfg_transform_step_ms,
        "operator_build_over_full_ratio": float(scfg_stats_step_ms / max(full_step_ms, 1.0e-12)),
        "operator_apply_over_full_ratio": float(scfg_transform_step_ms / max(full_step_ms, 1.0e-12)),
        "scfg_observe_cumulative_ms": scfg_observe_cumulative_ms,
        "scfg_transform_cumulative_ms": scfg_transform_cumulative_ms,
        "controller_overhead_ratio": float((scfg_stats_step_ms + scfg_transform_step_ms) / max(full_step_ms, 1.0e-12)),
        "peak_memory_mb": peak_memory,
        "extra_state_bytes": int(sum(int(p.numel()) * p.element_size() for p in scfg_params)) if method in SCFG_METHODS else 0,
        ...
        **audit_flags,
        **scfg_diag,
        "operator_PSD_min_eigenvalue": scfg_diag.get("operator_PSD_min", 1.0),
        "source_witness_transfer_LCB": scfg_diag.get("source_witness_transfer_LCB", 0.0),
        "source_witness_gap": scfg_diag.get("source_witness_gap", 0.0),
        **opt_audit,
    }
    out = CHUNK_ROOT / f"v22_58_{safe_fragment(str(args.label))}_summary.csv"
    write_rows(out, [row])
...
def build_clean_audit_bundle(args: argparse.Namespace) -> dict[str, Any]:
    ...
    bundle_dir = pack_root / f"v22_58_audit_{stamp}"
    ...
    with tempfile.TemporaryDirectory(prefix="v22_58_clean_extract_") as tmp:
        tmp_path = Path(tmp)
        ...
        compile_res = run_cmd(
            ...
            task_id="v22_58_clean_tarball_compileall",
        ...
        import_cmds = {
            ...
            "clean_tarball_runner_core_import_pass": "import experiments.run_v22_58_state_coupled_functional_geometry_fu; print('pass')",
            "clean_tarball_wrapper_import_pass": "from dgkan.optim.scfg_optimizer_wrapper import SCFGOptimizerWrapper; print('pass')",
            "clean_tarball_operator_import_pass": "from dgkan.fu.state_coupled_functional_geometry_operator import StateCoupledFunctionalGeometryOperator; print('pass')",
        }
        results = {}
        for key, code in import_cmds.items():
            res = run_cmd([PYTHON, "-c", code], task_id=f"v22_58_{key}", files=str(tar_path.relative_to(ROOT)), cwd=clean_root)
            results[key] = int(res.get("status") == "pass")
    required_core = [
        "dgkan/fu/state_coupled_functional_geometry_operator.py",
        "dgkan/optim/scfg_optimizer_wrapper.py",
        "experiments/run_v22_58_state_coupled_functional_geometry_fu.py",
        "experiments/audit_v22_58_standard_loop.py",
    ]
...
def run_o0(args: argparse.Namespace) -> dict[str, Any]:
    ...
    compile_result = run_cmd(
        [PYTHON, "-m", "compileall", "-q", "dgkan", "experiments", "external/oet_baselines"],
        task_id="v22_58_compileall_repo",
        files="results/v22_58/v22_58_code_truth_gate.csv",
        timeout=int(args.compile_timeout),
    )
    import_cmds = {
        "worktree_full_repo_import_pass": "import dgkan; import dgkan.optim; print('pass')",
        "runner_core_import_pass": "import experiments.run_v22_58_state_coupled_functional_geometry_fu; print('pass')",
        "wrapper_import_pass": "from dgkan.optim.scfg_optimizer_wrapper import SCFGOptimizerWrapper; print('pass')",
        "operator_import_pass": "from dgkan.fu.state_coupled_functional_geometry_operator import StateCoupledFunctionalGeometryOperator; print('pass')",
    ...
    for key, code in import_cmds.items():
        res = run_cmd([PYTHON, "-c", code], task_id=f"v22_58_{key}", files="results/v22_58/v22_58_code_truth_gate.csv")
    ...
    write_rows(OUT_ROOT / "v22_58_standard_loop_static_scan.csv", scan["rows"] or [{"status": "no_forbidden_hits"}])
    runtime_args = argparse.Namespace(**vars(args))
    runtime_args.dataset = "MNIST"
    runtime_args.seed = 0
    runtime_args.method = "scfg-state-transition"
    runtime_args.reference_method = "adamw"
    runtime_args.label = "v22_58_runtime_trace_scfg_smoke"
    ...
    runtime_args.scfg_param_scope = "last_layer"
    try:
        ...
        (LOG_ROOT / "v22_58_runtime_trace_scfg_smoke_exception.log").write_text(runtime_error, encoding="utf-8")
    row = {
    ...
    write_rows(OUT_ROOT / "v22_58_code_truth_gate.csv", [row])
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage o0",
        task_id="v22_58_part_a_truth_gate_summary",
        status="pass" if row["part_a_pass"] else "fail",
        files=(
            "results/v22_58/v22_58_code_truth_gate.csv; "
            "results/v22_58/v22_58_standard_loop_static_scan.csv; "
            "results/v22_58/chunks/v22_58_v22_58_runtime_trace_scfg_smoke_summary.csv"
        ),
...
def recompute_v22_57_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    ensure_out()
    src_root = ROOT / "results/v22_57"
    pairwise = read_rows(src_root / "v22_57_mlp_ccsg_pairwise.csv")
    chunks: list[dict[str, str]] = []
    for path in sorted((src_root / "chunks").glob("v22_57_*_summary.csv")):
        for row in read_rows(path):
            label = str(row.get("run_label", ""))
            if label.startswith("v22_57_reference") or label.startswith("v22_57_ccsg_over"):
                chunks.append(row)
    if not pairwise or not chunks:
        summary = {
            "v57_recompute_pass": 0,
            "blocker": "missing results/v22_57 pairwise or chunk artifacts",
            "pairwise_rows": len(pairwise),
            "chunk_rows": len(chunks),
        }
        write_json(OUT_ROOT / "v22_58_v57_recomputed_summary.json", summary)
        append_exec(
            "read results/v22_57 artifacts",
            task_id="v22_58_part_b_v57_recompute",
            status="blocked",
            files="results/v22_58/v22_58_v57_recomputed_summary.json",
            note=summary["blocker"],
        )
        return summary
    method_summary: list[dict[str, Any]] = []
    for method in sorted({r.get("method", "") for r in chunks}):
        group = [r for r in chunks if r.get("method") == method]
    ...
    candidate_pairwise = [r for r in pairwise if r.get("control_kind") == "candidate"]
    recomputed_gate = candidate_method_gate_rows(candidate_pairwise)
    by_dataset: list[dict[str, Any]] = []
    for dataset in sorted({r.get("dataset", "") for r in candidate_pairwise}):
        group = [r for r in candidate_pairwise if r.get("dataset") == dataset]
        by_dataset.append(
            {
                "dataset": dataset,
                "candidate_rows": len(group),
                "beats_best_control_rows": sum(iflag(r.get("beats_best_control")) for r in group),
                "mean_Delta_NLL_vs_best_control": mean(fval(r.get("Delta_NLL_vs_best_control")) for r in group),
                "best_candidate_delta_vs_control": min([fval(r.get("Delta_NLL_vs_best_control"), float("inf")) or float("inf") for r in group], default=""),
    ...
    for method in sorted({r.get("method", "") for r in candidate_pairwise}):
        ...
        primary = max(means, key=lambda k: means[k] if means[k] is not None else -float("inf"))
        debt_rows.append(
            {
                "method": method,
                "rows": len(group),
                "no_debt_rows": sum(iflag(r.get("no_ECE_Brier_tail_debt")) for r in group),
                "mean_held_ECE_delta": means["ECE"],
                "mean_held_Brier_delta": means["Brier"],
                "mean_held_tail_q95_delta": means["tail_q95"],
                "mean_held_tail_q99_delta": means["tail_q99"],
                "primary_debt_component": primary,
            }
        )
        ...
        overhead_rows.append(
            {
                ...
                "mean_operator_build_ms": mean(fval(r.get("operator_build_ms")) for r in chunks if r.get("method") == method),
                "mean_operator_apply_ms": mean(fval(r.get("operator_apply_ms")) for r in chunks if r.get("method") == method),
                "mean_full_step_ms": mean(fval(r.get("full_step_ms")) for r in chunks if r.get("method") == method),
            }
        )
        ...
        attribution_rows.append(
            {
                ...
                "beats_same_spectrum_rows": sum(iflag(r.get("beats_same_generalized_spectrum_control")) for r in group),
                "beats_reweighting_rows": sum(iflag(r.get("beats_all_reweighting_controls")) for r in group),
                "mean_credit_self_alignment": mean(fval(r.get("credit_self_alignment")) for r in group),
                "credit_only_tracks": int(sum(iflag(r.get("beats_credit_only")) for r in group) < max(1, math.ceil(len(group) * 2 / 3))),
                "self_only_tracks": int(sum(iflag(r.get("beats_self_only")) for r in group) < max(1, math.ceil(len(group) * 2 / 3))),
            }
        )
    audit_rows: list[dict[str, Any]] = []
    tarballs = sorted((ROOT / "code_audit_pack").glob("v22_57_final_audit_*.tar.gz"))
    tarballs.extend(sorted((ROOT / "code_audit_pack").glob("v22_57_audit_*.tar.gz")))
    if tarballs:
        tar_path = tarballs[-1]
        with tempfile.TemporaryDirectory(prefix="v22_58_v57_audit_import_") as tmp:
            tmp_path = Path(tmp)
            try:
                with tarfile.open(tar_path, "r:gz") as tar:
                    tar.extractall(tmp_path)
                clean_root = tmp_path / tar_path.name.replace(".tar.gz", "")
                imports = {
                    "operator_import": "from dgkan.fu.credit_conditioned_self_geometry_operator import CreditConditionedSelfGeometryOperator; print('pass')",
                    "wrapper_import": "from dgkan.optim.ccsg_optimizer_wrapper import CCSGOptimizerWrapper; print('pass')",
                    "runner_import": "import experiments.run_v22_57_credit_conditioned_self_geometry_fu; print('pass')",
                }
                for name, code in imports.items():
                    res = run_cmd([PYTHON, "-c", code], task_id=f"v22_58_part_b_v57_audit_{name}", files="results/v22_58/v22_58_v57_audit_pack_import_check.csv", cwd=clean_root)
                    audit_rows.append({"audit_pack": str(tar_path.relative_to(ROOT)), "check": name, "pass": int(res.get("status") == "pass"), "returncode": res.get("returncode")})
            except Exception as exc:
                audit_rows.append({"audit_pack": str(tar_path.relative_to(ROOT)), "check": "extract_or_import", "pass": 0, "error": repr(exc)})
    else:
        audit_rows.append({"audit_pack": "", "check": "audit_pack_exists", "pass": 0, "error": "no v22_57_final_audit tarball found"})
    write_rows(OUT_ROOT / "v22_58_v57_recomputed_method_summary.csv", method_summary)
    write_rows(OUT_ROOT / "v22_58_v57_recomputed_candidate_gate.csv", recomputed_gate)
    write_rows(OUT_ROOT / "v22_58_v57_control_gap_by_dataset.csv", by_dataset)
    write_rows(OUT_ROOT / "v22_58_v57_debt_decomposition.csv", debt_rows)
    write_rows(OUT_ROOT / "v22_58_v57_overhead_decomposition.csv", overhead_rows)
    write_rows(OUT_ROOT / "v22_58_v57_self_credit_attribution.csv", attribution_rows)
    write_rows(OUT_ROOT / "v22_58_v57_audit_pack_import_check.csv", audit_rows)
    old_route = read_json(src_root / "v22_57_final_route.json")
    full_pack_names = {str(p.relative_to(ROOT)) for p in sorted((ROOT / "code_audit_pack").glob("v22_57_audit_*.tar.gz"))}
    full_pack_rows = [r for r in audit_rows if r.get("audit_pack") in full_pack_names]
    full_pack_import_pass = int(bool(full_pack_rows) and all(iflag(r.get("pass")) for r in full_pack_rows))
    summary = {
        "v57_recompute_pass": int(bool(pairwise) and bool(chunks) and full_pack_import_pass),
        "reference_rows": sum(int(r.get("phase") == "reference" and r.get("run_status") == "completed") for r in chunks),
        "ccsg_rows": sum(int(r.get("phase") == "CCSG" and r.get("run_status") == "completed") for r in chunks),
        "candidate_ccsg_rows": len(candidate_pairwise),
        "candidate_method_gate_rows": len(recomputed_gate),
        "candidate_method_gate_pass_rows": sum(iflag(r.get("method_gate_pass")) for r in recomputed_gate),
        ...
        "beats_same_spectrum_rows": sum(iflag(r.get("beats_same_generalized_spectrum_control")) for r in candidate_pairwise),
        "beats_reweighting_rows": sum(iflag(r.get("beats_all_reweighting_controls")) for r in candidate_pairwise),
        ...
        "old_final_route": old_route.get("final_route", ""),
        "old_mlp_exploration_pass": old_route.get("mlp_exploration_pass", ""),
        "audit_pack_import_pass_rows": sum(iflag(r.get("pass")) for r in audit_rows),
        "audit_pack_import_rows": len(audit_rows),
        "full_audit_pack_import_pass": full_pack_import_pass,
        "inherit_v57_operator_only_route": int(sum(iflag(r.get("method_gate_pass")) for r in recomputed_gate) == 0),
    }
    write_json(OUT_ROOT / "v22_58_v57_recomputed_summary.json", summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage recompute-v57",
        task_id="v22_58_part_b_v57_recompute",
        status="pass" if summary["v57_recompute_pass"] else "fail",
        files=(
            "results/v22_58/v22_58_v57_recomputed_method_summary.csv; "
            "results/v22_58/v22_58_v57_recomputed_candidate_gate.csv; "
            "results/v22_58/v22_58_v57_control_gap_by_dataset.csv; "
            "results/v22_58/v22_58_v57_debt_decomposition.csv; "
            "results/v22_58/v22_58_v57_overhead_decomposition.csv; "
            "results/v22_58/v22_58_v57_self_credit_attribution.csv; "
            "results/v22_58/v22_58_v57_audit_pack_import_check.csv"
        ),
        note=json.dumps(summary, ensure_ascii=False, sort_keys=True),
    )
    append_recap(
        "Part B v22.57 recompute and inherited blocker",
        "从 v22.57 pairwise/chunk artifacts 重新聚合，未复用 final_route 作为计算来源。关键结果：\n\n```json\n"
        + json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True)
        + "\n```\n\nCandidate gate:\n\n"
        + md_table(recomputed_gate, ["method", "rows", "method_gate_pass", "beats_best_control_rows", "beats_credit_only_rows", "beats_self_only_rows", "no_debt_rows", "overhead_le_035_rows"], 12)
        + "\n\nAudit pack import check:\n\n"
        + md_table(audit_rows, ["audit_pack", "check", "pass", "returncode"], 12),
    )
    return summary
...
def reanalyse_v22_56_self_geometry(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not pairwise or not chunks:
        ...
        write_json(OUT_ROOT / "v22_58_v56_self_geometry_reanalysis_summary.json", summary)
        append_exec("read results/v22_56 artifacts", task_id="v22_58_part_b_v56_self_geometry_reanalysis", status="blocked", files="results/v22_58/v22_58_v56_self_geometry_reanalysis_summary.json", note=summary["blocker"])
        return summary
    ...
    write_rows(OUT_ROOT / "v22_58_v56_self_geometry_reanalysis.csv", rows)
    write_rows(OUT_ROOT / "v22_58_v56_credit_self_interaction_matrix.csv", interaction_rows)
    write_rows(OUT_ROOT / "v22_58_v56_failure_mode_by_self_state.csv", failure_rows)
    write_rows(OUT_ROOT / "v22_58_v56_tcwp_success_condition_table.csv", success_rows)
    summary = {
        "reanalysis_pass": 1,
    ...
    write_json(OUT_ROOT / "v22_58_v56_self_geometry_reanalysis_summary.json", summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage reanalyse-v56",
        task_id="v22_58_part_b_v56_self_geometry_reanalysis",
        status="pass",
        files=(
            "results/v22_58/v22_58_v56_self_geometry_reanalysis.csv; "
            "results/v22_58/v22_58_v56_credit_self_interaction_matrix.csv; "
            "results/v22_58/v22_58_v56_failure_mode_by_self_state.csv; "
            "results/v22_58/v22_58_v56_tcwp_success_condition_table.csv; "
            "results/v22_58/v22_58_v56_self_geometry_reanalysis_summary.json"
        ),
...
def synthetic_case_rows(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    def run_operator(case: str, source: Any, witness: Any, raw_grad: Any, weight: Any, cfg: SCFGConfig) -> tuple[dict[str, Any], Any]:
        p = torch.nn.Parameter(weight.clone().to(device))
        op = StateCoupledFunctionalGeometryOperator([p], cfg)
        ...
        row = {
            ...
            "state_transition_prediction_error": diag.get("state_transition_prediction_error", ""),
            "state_transition_cost": diag.get("state_transition_cost", ""),
            "spectrum_drift_predicted_proxy": diag.get("state_transition_cost", ""),
            "spectrum_drift_observed_proxy": diag.get("update_spectrum_drift", ""),
            "activation_drift_predicted_proxy": diag.get("state_transition_cost", ""),
            "activation_drift_observed_proxy": diag.get("source_witness_gap", ""),
            "cotangent_drift_predicted_proxy": diag.get("state_transition_cost", ""),
            "cotangent_drift_observed_proxy": diag.get("source_witness_gap", ""),
            ...
            "operator_PSD_min": diag.get("operator_PSD_min", ""),
            "condition_number": diag.get("operator_condition_number", ""),
            "operator_condition": diag.get("operator_condition_number", ""),
    ...
    row, _ = run_operator("C1_credit_self_consistent", true, true, true, aligned_weight, SCFGConfig(rank=2, alpha=0.25, beta=0.25, random_seed=1))
    ...
    credit_row, credit_tx = run_operator("C2_credit_only_bad_direction", bad, bad, bad, costly_weight, SCFGConfig(control="credit_only_operator", rank=2, alpha=0.35, beta=0.35, random_seed=2))
    scfg_row, scfg_tx = run_operator("C2_SCFG_suppresses_high_self_cost", bad, bad, bad, costly_weight, SCFGConfig(rank=2, alpha=0.35, beta=0.35, random_seed=2, weight_spectrum_weight=2.0))
    bad_projection_credit = float(abs(credit_tx[0, 0].item()))
    bad_projection_scfg = float(abs(scfg_tx[0, 0].item()))
    scfg_row["credit_only_gain"] = credit_row["credit_self_gain"]
    scfg_row["safety_debt_proxy_delta"] = bad_projection_scfg - bad_projection_credit
    scfg_row["pass"] = int(bad_projection_scfg <= bad_projection_credit and (fval(scfg_row["PSD_min"], -1.0) or -1.0) >= -1.0e-6)
    rows.append(scfg_row)

    no_credit_source = true
    no_credit_witness = -true
    self_good_weight = 0.01 * torch.randn(8, 6, device=device, generator=gen)
    row, transformed = run_operator("C3_self_good_no_credit_near_identity", no_credit_source, no_credit_witness, true, self_good_weight, SCFGConfig(rank=2, alpha=0.25, beta=0.25, random_seed=3))
    row["self_only_gain"] = ""
    row["safety_debt_proxy_delta"] = 0.0
    ...
    witness_clean = true
    row, transformed = run_operator("C4_source_only_noise_suppressed_by_witness", source_noise, witness_clean, source_noise, aligned_weight, SCFGConfig(rank=2, alpha=0.20, beta=0.20, random_seed=4))
    source_only_row, source_only_tx = run_operator("C4_source_only_control", source_noise, source_noise, source_noise, aligned_weight, SCFGConfig(control="source_only_crossfit_factor", rank=2, alpha=0.20, beta=0.20, random_seed=4))
    ...
    row, transformed = run_operator("C5_KAN_basis_Gram_controlled_proxy", basis_bad, basis_bad, basis_bad, basis_weight, SCFGConfig(rank=2, alpha=0.30, beta=0.30, random_seed=5, weight_spectrum_weight=2.0))
...
def run_synthetic(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_rows(OUT_ROOT / "v22_58_part_d_state_transition_unit_tests.csv", rows)
    summary = {
        "unit_test_rows": len(rows),
        "unit_test_pass_rows": sum(iflag(r.get("pass")) for r in rows),
        "all_unit_cases_pass": int(rows and sum(iflag(r.get("pass")) for r in rows) == len(rows)),
        "max_generalized_eigen_residual": max([fval(r.get("generalized_eigen_residual"), 0.0) or 0.0 for r in rows], default=0.0),
        "max_state_transition_prediction_error": max([fval(r.get("state_transition_prediction_error"), 0.0) or 0.0 for r in rows], default=0.0),
        "min_operator_PSD": min([fval(r.get("operator_PSD_min"), 1.0) or 1.0 for r in rows], default=1.0),
    }
    summary["part_d_unit_gate_pass"] = int(
        summary["all_unit_cases_pass"]
        and summary["max_generalized_eigen_residual"] <= 1.0e-4
        and summary["max_state_transition_prediction_error"] <= 0.25
        and summary["min_operator_PSD"] >= -1.0e-6
    )
    write_json(OUT_ROOT / "v22_58_part_d_state_transition_unit_summary.json", summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage synthetic",
        task_id="v22_58_part_d_state_transition_unit_tests",
        status="pass" if summary["part_d_unit_gate_pass"] else "fail",
        gpu=str(args.synthetic_device),
        files="results/v22_58/v22_58_part_d_state_transition_unit_tests.csv; results/v22_58/v22_58_part_d_state_transition_unit_summary.json",
    )
    append_recap(
        "Part D SCFG state-transition unit tests",
        md_table(rows, ["unit_case", "pass", "generalized_eigen_residual", "state_transition_prediction_error", "operator_PSD_min", "operator_condition", "operator_apply_ms", "safety_debt_proxy_delta"], 12)
        + "\n```json\n"
        + json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True)
        + "\n```\n",
    )
...
def summarize_telemetry(args: argparse.Namespace) -> dict[str, Any]:
    ensure_out()
    rows: list[dict[str, str]] = []
    for path in sorted(CHUNK_ROOT.glob("v22_58_*_self_state_telemetry.csv")):
        for row in read_rows(path):
            rows.append({**row, "source_file": str(path.relative_to(ROOT))})
    write_rows(OUT_ROOT / "v22_58_self_state_telemetry.csv", rows)
    refresh_rows = len(rows)
    required = [
        "weight_singular_values_topk",
        "weight_spectral_entropy",
        "weight_effective_rank",
        "weight_condition_proxy",
        "activation_cov_trace",
        "activation_cov_condition_proxy",
        "activation_cov_effective_rank",
        "cotangent_cov_trace",
        "cotangent_cov_condition_proxy",
        "cotangent_cov_effective_rank",
        "momentum_alignment",
        "adam_second_moment_condition_proxy",
        "OET_tangent_alignment",
        "radial_component_fraction",
        "tangent_component_fraction",
        "update_spectrum_drift_proxy",
        "jacobian_trace_proxy",
        "ntk_trace_proxy",
        "sharpness_proxy",
        "ECE_proxy",
    ...
    complete_rows = row_complete_count
    train_only = int(rows and all(iflag(r.get("train_only_state")) for r in rows))
    no_future = int(rows and all(iflag(r.get("no_val_test_future_direction")) for r in rows))
    summary = {
        "self_state_complete_rows": complete_rows,
        "operator_refresh_rows": refresh_rows,
        "self_state_complete_ratio": float(complete_rows / max(1, refresh_rows)),
        "missing_core_self_fields": missing,
        "train_only_self_state_pass": train_only,
        "no_val_test_future_direction": no_future,
        "telemetry_gate_pass": int(refresh_rows > 0 and complete_rows / max(1, refresh_rows) >= 0.95 and missing == 0 and train_only == 1 and no_future == 1),
    }
    write_rows(OUT_ROOT / "v22_58_self_state_telemetry_completeness.csv", [summary])
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage telemetry",
        task_id="v22_58_part_c_self_state_telemetry",
        status="pass" if summary["telemetry_gate_pass"] else "fail",
        files="results/v22_58/v22_58_self_state_telemetry.csv; results/v22_58/v22_58_self_state_telemetry_completeness.csv",
        note=json.dumps(summary, ensure_ascii=False, sort_keys=True),
    )
    append_recap(
        "Part C self-state telemetry completeness",
...
def collect_command(spec: dict[str, Any], args: argparse.Namespace, device: str) -> list[str]:
    return [
        ...
        str(args.scfg_eps),
        "--scfg-ema-beta",
        str(args.scfg_ema_beta),
        "--scfg-c-min",
        str(args.scfg_c_min),
        "--scfg-refresh-interval",
        str(args.scfg_refresh_interval),
        "--scfg-param-scope",
        str(args.scfg_param_scope),
        "--scfg-weight-spectrum-weight",
        str(args.scfg_weight_spectrum_weight),
        "--scfg-gradient-cov-weight",
        str(args.scfg_gradient_cov_weight),
        "--scfg-momentum-weight",
        str(args.scfg_momentum_weight),
        "--scfg-state-transition-weight",
        str(args.scfg_state_transition_weight),
        "--scfg-debt-weight",
        str(args.scfg_debt_weight),
        "--scfg-tail-weight",
        str(args.scfg_tail_weight),
        "--debt-tolerance",
...
def run_matrix(args: argparse.Namespace, *, phase: str) -> list[dict[str, Any]]:
    ...
    methods = split_csv(str(args.methods or (DEFAULT_REFERENCE_METHODS if phase == "reference" else DEFAULT_SCFG_METHODS)), str)
    specs = []
    ...
    def launch(item: tuple[int, dict[str, Any]]) -> dict[str, Any]:
        ...
        result = run_cmd(
            ...
            files=f"results/v22_58/chunks/v22_58_{safe_fragment(str(spec['label']))}_summary.csv",
            gpu=str(gpu),
        ...
        return {
            ...
            **result,
            "summary": f"results/v22_58/chunks/v22_58_{safe_fragment(str(spec['label']))}_summary.csv",
        }
    ...
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage {phase}",
        task_id=f"v22_58_{phase}_matrix",
        status="pass" if all(r.get("status") == "pass" for r in dispatch_rows) else "fail",
        files=f"{specs_path}; {dispatch_path}; results/v22_58/chunks/",
...
def collect_chunk_rows(include_prefixes: list[str] | None = None) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    for path in sorted(CHUNK_ROOT.glob("v22_58_*_summary.csv")):
        for row in read_rows(path):
            label = str(row.get("run_label", ""))
...
def classify_route(candidate_pairwise: list[dict[str, Any]], route: dict[str, Any]) -> str:
    ...
    if route["standard_loop_pass_candidate_rows"] != route["candidate_scfg_rows"]:
        return "R0-CodeOrTrainingBoundaryFailed"
    if route.get("mlp_exploration_pass"):
        return "R6-MLPStateCoupledFUOpened"
    if route["beats_strongest_rows"] > 0 and route["beats_POET_or_external_OET_rows"] == 0:
        return "R2-FUWeakOptimizerPatchOnly"
    if route["beats_self_only_rows"] < min(6, max(1, route["candidate_scfg_rows"])):
        return "R4-SelfGeometryOptimizerOnly"
    if route["beats_credit_only_rows"] < min(6, max(1, route["candidate_scfg_rows"])):
        return "R5-DataCreditOnly_NoSelfGeometryGain"
    if route["beats_all_reweighting_controls_rows"] < min(6, max(1, route["candidate_scfg_rows"])):
...
def summarize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scfg_rows = [r for r in rows if r.get("phase") == "SCFG" and r.get("run_status") == "completed"]
    method_summary = []
    ...
    write_rows(OUT_ROOT / "v22_58_method_summary.csv", method_summary)
    ...
    write_rows(OUT_ROOT / "v22_58_mlp_scfg_pairwise.csv", pairwise)
    candidate_pairwise = [r for r in pairwise if r.get("method") in SCFG_CANDIDATES]
    gate_rows = candidate_method_gate_rows(candidate_pairwise)
    write_rows(OUT_ROOT / "v22_58_candidate_method_gate.csv", gate_rows)
    part_a = read_rows(OUT_ROOT / "v22_58_code_truth_gate.csv")
    part_a_pass = iflag(part_a[0].get("part_a_pass")) if part_a else 0
    route = {
        "part_a_pass": part_a_pass,
        "reference_rows": len(reference_rows),
        "scfg_rows": len(scfg_rows),
        "candidate_scfg_rows": len(candidate_pairwise),
    ...
    route["mlp_exploration_pass"] = int(route["candidate_method_gate_pass_rows"] > 0 and route["standard_loop_pass_candidate_rows"] == route["candidate_scfg_rows"])
    route["kan_gate_status"] = "eligible_not_run_yet" if route["mlp_exploration_pass"] else "skipped_MLP_SCFG_gate_not_opened"
    route["final_route"] = classify_route(candidate_pairwise, route)
    write_json(OUT_ROOT / "v22_58_final_route.json", route)
    downstream = [
        {"part": "F_KAN", "status": "skipped" if not route["mlp_exploration_pass"] else "eligible_not_run_yet", "reason": route["kan_gate_status"]},
        {"part": "G_continual_grokking", "status": "skipped" if not route["mlp_exploration_pass"] else "eligible_not_run_yet", "reason": "pressure tests require MLP+SCFG gate opened"},
    ]
    write_rows(OUT_ROOT / "v22_58_downstream_gate_status.csv", downstream)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage summarize",
        task_id="v22_58_summarize",
        status="pass",
        files=(
            "results/v22_58/v22_58_method_summary.csv; results/v22_58/v22_58_mlp_scfg_pairwise.csv; "
            "results/v22_58/v22_58_candidate_method_gate.csv; "
            "results/v22_58/v22_58_final_route.json; results/v22_58/v22_58_downstream_gate_status.csv"
        ),
        note=f"final_route={route['final_route']}",
    )
    append_recap(
        "Part D MLP SCFG summary and route",
        "Method summary:\n\n"
...
def analyze_controls(args: argparse.Namespace) -> dict[str, Any]:
    ensure_out()
    pairwise = read_rows(OUT_ROOT / "v22_58_mlp_scfg_pairwise.csv")
    diagnosis = []
    for row in pairwise:
    ...
    write_rows(OUT_ROOT / "v22_58_machine_readable_diagnosis.csv", diagnosis)
    summary = {
    ...
    write_json(OUT_ROOT / "v22_58_control_audit_summary.json", summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage analyze-controls",
        task_id="v22_58_control_diagnosis",
        status="pass",
        files="results/v22_58/v22_58_machine_readable_diagnosis.csv; results/v22_58/v22_58_control_audit_summary.json",
        note=json.dumps(summary, ensure_ascii=False, sort_keys=True),
    )
    append_recap(
...
def run_figures(args: argparse.Namespace) -> dict[str, Any]:
    ...
    route = read_json(OUT_ROOT / "v22_58_final_route.json")
    write_svg(FIG_ROOT / "v22_58_credit_self_generalized_eigen_spectrum.svg", "Generalized Eigen Residual", chunks, "generalized_eigen_residual", "credit_positive_rank_out")
    write_svg(FIG_ROOT / "v22_58_operator_gain_vs_self_condition.svg", "Gain vs Self Condition", chunks, "self_condition_out", "norm_Pg_over_norm_g")
    write_svg(FIG_ROOT / "v22_58_operator_gain_vs_credit_alignment.svg", "Gain vs Credit Alignment", chunks, "credit_self_alignment", "norm_Pg_over_norm_g")
    write_svg(FIG_ROOT / "v22_58_control_gap_by_dataset.svg", "Control Gap", pairwise, "final_NLL", "best_control_final_NLL")
    write_svg(FIG_ROOT / "v22_58_overhead_vs_gain.svg", "Overhead vs Strongest Gap", pairwise, "controller_overhead_ratio", "Delta_NLL_vs_strongest")
    write_svg(FIG_ROOT / "v22_58_no_debt_decomposition.svg", "Debt Proxy", chunks, "held_ECE_delta", "held_tail_q95_delta")
    write_svg(FIG_ROOT / "v22_58_credit_only_self_only_ablation.svg", "Credit/Self Ablation", pairwise, "Delta_NLL_vs_credit_only", "Delta_NLL_vs_self_only")
    write_svg(FIG_ROOT / "v22_58_layerwise_operator_heatmap.svg", f"KAN/status: {route.get('kan_gate_status', 'unknown')}", chunks, "credit_positive_rank_out", "credit_positive_rank_in")
    files = sorted(str(p.relative_to(ROOT)) for p in FIG_ROOT.glob("v22_58_*.svg"))
    summary = {"figures": files, "figure_count": len(files)}
    write_json(OUT_ROOT / "v22_58_figure_summary.json", summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage figures",
        task_id="v22_58_figures",
        status="pass",
        files="; ".join(files),
    )
...
def repair_report(args: argparse.Namespace) -> dict[str, Any]:
    ensure_out()
    prefixes = split_csv(str(args.summary_include_prefixes or args.label_prefix), str)
    rows = collect_chunk_rows(prefixes)
    ...
    for method in sorted({r.get("method", "") for r in rows}):
        ...
        proxy_sign_matches: list[float] = []
        for row in group:
            full = fval(row.get("full_step_ms"), 0.0) or 0.0
            if full <= 0:
                continue
            build_step = fval(row.get("operator_build_step_ms"))
            apply_step = fval(row.get("operator_apply_step_ms"))
            if build_step is None:
                build_step = (fval(row.get("scfg_observe_cumulative_ms"), 0.0) or 0.0) / max(1.0, fval(row.get("steps"), 1.0) or 1.0)
            if apply_step is None:
                apply_step = (fval(row.get("scfg_transform_cumulative_ms"), 0.0) or 0.0) / max(1.0, fval(row.get("steps"), 1.0) or 1.0)
            build_ratios.append(float(build_step / full))
            apply_ratios.append(float(apply_step / full))
            ...
            proxy_sign_matches.append(float((proxy > 0.0 and held > 0.0) or (proxy <= 0.0 and held <= 0.0)))
        report_rows.append(
            {
                "method": method,
                "rows": len(group),
                "completed_rows": sum(int(r.get("run_status") == "completed") for r in group),
                "standard_loop_pass_rows": sum(iflag(r.get("standard_loop_hard_gate_pass")) for r in group),
                "mean_final_NLL": mean(vals("final_NLL")),
                "mean_accuracy": mean(vals("final_accuracy")),
                "no_debt_rows": sum(iflag(r.get("no_ECE_Brier_tail_debt")) for r in group),
                "overhead_le_035_rows": sum(int((fval(r.get("controller_overhead_ratio"), float("inf")) or float("inf")) <= 0.35) for r in group),
                "mean_controller_overhead_ratio": mean(vals("controller_overhead_ratio")),
                "mean_build_over_full_ratio": mean(build_ratios),
                "mean_apply_over_full_ratio": mean(apply_ratios),
                "mean_full_step_ms": mean(vals("full_step_ms")),
                "mean_optimizer_step_ms": mean(vals("optimizer_step_ms")),
                "mean_state_transition_prediction_error": mean(vals("state_transition_prediction_error")),
                "mean_train_only_debt_proxy": mean(vals("train_only_debt_proxy")),
                "proxy_to_held_sign_match_rate": mean(proxy_sign_matches),
            }
        )
    label = safe_fragment(str(args.label_prefix or "v22_58_repair"))
    csv_path = OUT_ROOT / f"{label}_repair_report.csv"
    json_path = OUT_ROOT / f"{label}_repair_report.json"
    write_rows(csv_path, report_rows)
    candidate_like = [r for r in report_rows if str(r.get("method", "")).startswith("scfg-")]
    summary = {
        "label_prefix": str(args.label_prefix),
        "prefixes": prefixes,
        "rows": len(rows),
        "methods": len(report_rows),
        "candidate_like_rows": sum(int(r.get("rows", 0)) for r in candidate_like),
        "candidate_like_overhead_le_035_rows": sum(int(r.get("overhead_le_035_rows", 0)) for r in candidate_like),
        "candidate_like_no_debt_rows": sum(int(r.get("no_debt_rows", 0)) for r in candidate_like),
        "candidate_like_mean_overhead": mean(fval(r.get("mean_controller_overhead_ratio")) for r in candidate_like),
        "candidate_like_mean_build_over_full": mean(fval(r.get("mean_build_over_full_ratio")) for r in candidate_like),
        "candidate_like_mean_apply_over_full": mean(fval(r.get("mean_apply_over_full_ratio")) for r in candidate_like),
        "candidate_like_proxy_to_held_sign_match_rate": mean(fval(r.get("proxy_to_held_sign_match_rate")) for r in candidate_like),
        "repair_gate_overhead_pass": int(candidate_like and sum(int(r.get("overhead_le_035_rows", 0)) for r in candidate_like) >= max(1, math.ceil(sum(int(r.get("rows", 0)) for r in candidate_like) * 12 / 15))),
        "repair_gate_debt_pass": int(
            candidate_like
            and sum(int(r.get("no_debt_rows", 0)) for r in candidate_like) >= max(1, math.ceil(sum(int(r.get("rows", 0)) for r in candidate_like) * 12 / 15))
            and (mean(fval(r.get("proxy_to_held_sign_match_rate")) for r in candidate_like) or 0.0) >= 0.70
        ),
    }
    write_json(json_path, summary)
    append_exec(
        f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage repair-report --summary-include-prefixes {','.join(prefixes)}",
        task_id=f"{label}_repair_report",
        status="pass",
        files=f"{csv_path.relative_to(ROOT)}; {json_path.relative_to(ROOT)}",
        note=json.dumps(summary, ensure_ascii=False, sort_keys=True),
    )
    append_recap(
        f"Repair report {label}",
        "Repair summary:\n\n```json\n"
        + json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True)
        + "\n```\n\n"
        + md_table(report_rows, ["method", "rows", "no_debt_rows", "overhead_le_035_rows", "mean_controller_overhead_ratio", "mean_build_over_full_ratio", "mean_apply_over_full_ratio", "mean_final_NLL"], 40),
    )
    return summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--stage", default="all", choices=["all", "o0", "recompute-v57", "reanalyse-v56", "telemetry", "synthetic", "collect", "reference", "scfg", "summarize", "analyze-controls", "figures", "repair-report"])
    p.add_argument("--dataset", default="MNIST")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--method", default="adamw")
    p.add_argument("--reference-method", default="adamw")
    p.add_argument("--label", default="v22_58_collect")
    ...
    p.add_argument("--label-prefix", default="v22_58_reference")
    ...
    p.add_argument("--scfg-eta", type=float, default=1.0e-3)
    p.add_argument("--scfg-eps", type=float, default=1.0e-5)
    p.add_argument("--scfg-ema-beta", type=float, default=0.90)
    p.add_argument("--scfg-c-min", type=float, default=0.20)
    ...
    p.add_argument("--scfg-weight-spectrum-weight", type=float, default=0.25)
    p.add_argument("--scfg-gradient-cov-weight", type=float, default=1.0)
    p.add_argument("--scfg-momentum-weight", type=float, default=0.10)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.stage == "o0":
            ...
            return 0
        if args.stage == "recompute-v57":
            print(json.dumps(recompute_v22_57_artifacts(args), ensure_ascii=False, sort_keys=True))
            return 0
        ...
        if args.stage == "telemetry":
            print(json.dumps(summarize_telemetry(args), ensure_ascii=False, sort_keys=True))
            return 0
        ...
        if args.stage == "reference":
            ...
            return 0
        if args.stage == "scfg":
            if args.label_prefix == "v22_58_reference":
                args.label_prefix = f"v22_58_scfg_over_{safe_fragment(args.reference_method)}"
            run_matrix(args, phase="scfg")
            return 0
        ...
        if args.stage == "repair-report":
            print(json.dumps(repair_report(args), ensure_ascii=False, sort_keys=True))
            return 0
        if args.stage == "all":
            gate = run_o0(args)
            if not iflag(gate.get("part_a_pass")):
                ...
                run_figures(args)
                return 2
            reanalysis = recompute_v22_57_artifacts(args)
            if not iflag(reanalysis.get("v57_recompute_pass")):
                summarize(args)
                analyze_controls(args)
                run_figures(args)
                return 3
            telemetry = summarize_telemetry(args)
            if not iflag(telemetry.get("telemetry_gate_pass")):
                summarize(args)
                analyze_controls(args)
                run_figures(args)
                return 3
            synth = run_synthetic(args)
            if not iflag(synth.get("part_d_unit_gate_pass")):
                summarize(args)
                analyze_controls(args)
                run_figures(args)
                return 4
            args.label_prefix = "v22_58_reference"
            run_matrix(args, phase="reference")
            args.label_prefix = f"v22_58_scfg_over_{safe_fragment(args.reference_method)}"
            run_matrix(args, phase="scfg")
            summarize(args)
        ...
        err_path = LOG_ROOT / f"v22_58_stage_{safe_fragment(args.stage)}_exception.log"
        err_path.write_text(traceback.format_exc(), encoding="utf-8")
        append_exec(
            f"{PYTHON} experiments/run_v22_58_state_coupled_functional_geometry_fu.py --stage {args.stage}",
            task_id=f"v22_58_stage_{args.stage}_exception",
            status="exception",
            files=str(err_path),
            exit_code="exception",
        )
