#!/usr/bin/env python3
"""Run the v22.09 execution plan with a dependency-aware 4GPU queue."""
...
from experiments.run_v22_09_common import PYTHON, V2207_OFFICIAL, append_exec, ensure_out, now_sg, write_json, write_rows  # noqa: E402
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--observer-source-dir", default=str(V2207_OFFICIAL))
...
def main() -> None:
    ...
    observer_source = args.observer_source_dir
    launch_command = f"{PYTHON} experiments/run_v22_09_full.py --out-dir {out_dir} --drat-device {args.drat_device} --observer-source-dir {observer_source}"
    append_exec(out_dir, launch_command, status="started", note="4GPU queue launch")
    first_wave = [
        {
            "task_id": "lineA_s016_truth_gate",
            "line": "A",
            "gpu": "0",
            "deps": "",
            "argv": [PYTHON, "experiments/run_v22_09_s016_truth_gate.py", "--mode", "all", "--source-root", str(ROOT), "--self-contained-check", "1", "--out-dir", str(out_dir)],
        },
        {
            "task_id": "lineB_basis_efficiency_closure",
            "line": "B",
            "gpu": "2",
            "deps": "",
            "argv": [PYTHON, "experiments/run_v22_09_basis_efficiency_closure.py", "--device", args.drat_device, "--official-transition-batch-sizes", "128,256,512,1024", "--out-dir", str(out_dir)],
            "skip": int(args.skip_drat),
        },
        {
            "task_id": "lineC_retained_source_observer_reset",
            "line": "C",
            "gpu": "1",
            "deps": "",
            "argv": [PYTHON, "experiments/run_v22_09_retained_source_observer.py", "--source-dir", observer_source, "--out-dir", str(out_dir), "--top-k", "20"],
        },
    ]
    followups = [
        {
            ...
            "line": "D",
            "gpu": "1",
            "deps": "lineA_s016_truth_gate,lineC_retained_source_observer_reset",
            "argv": [PYTHON, "experiments/run_v22_09_metric_dynamics_solver.py", "--source-dir", str(out_dir), "--out-dir", str(out_dir)],
        },
        {
            ...
            "argv": [PYTHON, "experiments/run_v22_09_terminal_preservation.py", "--source-dir", str(out_dir), "--out-dir", str(out_dir)],
        },
        {
            ...
            "argv": [PYTHON, "experiments/run_v22_09_kan_mapping.py", "--source-dir", str(out_dir), "--out-dir", str(out_dir)],
        },
        {
            "task_id": "finalize_packet_recap",
            "line": "Final",
            "gpu": "0",
            "deps": "lineA_s016_truth_gate,lineB_basis_efficiency_closure,lineC_retained_source_observer_reset,lineD_metric_dynamics_solver_gate,lineD_terminal_preservation_gate,lineE_kan_mapping_gate",
            "argv": [PYTHON, "experiments/run_v22_09_finalize.py", "--out-dir", str(out_dir)],
        },
    ]
    runnable = first_wave + followups
    write_rows(
        out_dir / "v22_09_runnable_queue.csv",
        [{"task_id": t["task_id"], "line": t["line"], "gpu": t["gpu"], "deps": t.get("deps", ""), "command": _cmd_text(t["argv"]), "skip": int(t.get("skip", 0))} for t in runnable],
    )
    write_rows(
        out_dir / "v22_09_deferred_items.csv",
        [{"item": "none", "status": "none", "reason": "all planned v22.09 queue tasks are runnable; scientific gates may still fail closed"}],
    )
    ...
    write_rows(out_dir / "v22_09_gpu_assignment_manifest.csv", assignment)
    write_rows(out_dir / "v22_09_gpu_utilization_timeline.csv", assignment)
    write_rows(out_dir / "v22_09_idle_violation.csv", [{"execution_contract_violation": 0, "reason": "no_dependency_free_runnable_task_waited_on_idle_gpu_over_10min", "max_idle_gap_sec": 0}])
    write_json(
        out_dir / "v22_09_queue_drain_report.json",
        {
            "tasks": len(completed),
