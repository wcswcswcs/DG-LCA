#!/usr/bin/env python3
"""DG-KAN v6.18 real-only GT4 S2 restore and task-gap runner."""
...
PLAN_PATH = "docs/DG-KAN_v6.18_GT4_S2Restore_MarginRepair_TaskGap_详细实验计划.md"
SCRIPT_PATH = "experiments/run_gafu_v618_real.py"
...
P0_MATRIX = [
    ("A0-MLP-autograd-reference", "MLP-autograd-reference", "current", True, "reference", "v618", "autograd"),
    ("A1-MLP-manual-linear-reference", "MLP-manual-linear-reference", "current", True, "manual", "v618", "manual"),
        ...
        if not ok:
            summary.append({
                **v616._row_common(stage, args, method=method, variant_id=name),
                ...
                "used_for_gate": 0,
                "reason": "required v6.18 implementation is absent; no measured ratio emitted",
            })
...
def run_p1(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    _write_csv(Path(args.out_dir) / "p1_efficiency_margin_attribution.csv", detail)
    ...
    _write_csv(Path(args.out_dir) / "p1_component_kernel_audit.csv", components)
...
def run_p2(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    _write_csv(Path(args.out_dir) / "p2_step_preserving_memory_repair_detail.csv", detail)
    summary = _summary(args, "P2", detail, P2_MEMORY, "package")
    _augment_repair(summary, {v[0]: v[4] for v in P2_MEMORY})
    _write_csv(Path(args.out_dir) / "p2_step_preserving_memory_repair.csv", summary)
...
def run_p3(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    v616._add_residual_fields(args, detail)
    _write_csv(Path(args.out_dir) / "p3_memory_preserving_step_repair_detail.csv", detail)
    ...
    _write_csv(Path(args.out_dir) / "p3_memory_preserving_step_repair.csv", summary)
    return summary, detail
...
def run_route(args: argparse.Namespace, p0: Sequence[Dict[str, Any]], p4: Sequence[Dict[str, Any]], p6: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    repro_pass = int(any(int(f(r, "v616_reproduction_pass", 0)) == 1 for r in p0 if "GT4" in str(r.get("variant", ""))))
...
def run_failure(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    for fn, stage in [
        ...
        ("p2_step_preserving_memory_repair.csv", "P2"),
        ("p3_memory_preserving_step_repair.csv", "P3"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--packages", default="V6_18_ALL")
    parser.add_argument("--out-dir", type=Path, default=Path("results/real_rerun_20260505/v618_real"))
    ...
    parser.add_argument("--wandb-group", default="v618-real-20260505")
    parser.add_argument("--wandb-name-prefix", default="v618-real")
...
def main() -> int:
    ...
    try:
        ...
        baseline_reproduced = any(int(f(r, "v616_reproduction_pass", 0)) == 1 for r in p0_summary if "GT4" in str(r.get("variant", "")))
        if not baseline_reproduced:
            selected = None
        _run_task_gates(args, selected)
        p6 = _p6_gap_from_task(args)
        run_p7(args)
        route = run_route(args, p0_summary, p4, p6)
        run_failure(args)
