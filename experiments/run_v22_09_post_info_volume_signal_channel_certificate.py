#!/usr/bin/env python3
"""v22.09 post-row information-volume signal-channel certificate.

This attempt follows the plan's signal-channel / information-volume direction.
It measures per-example train-logit displacements from a legal source update:
coherent drift should dominate diffusion, keep nondegenerate effective rank, and
beat random/sign/corrupt controls before a fresh FU run is allowed. Future
horizons are audit labels only.
"""
...
def _vector_update(vector: torch.Tensor, reference: torch.Tensor, kind: str, diagnostics: dict[str, float] | None = None) -> UpdateTensor:
    return UpdateTensor(
        ...
        kind=kind,
        sign_rule="add",
        space="information_volume_signal_channel",
        source="train_only_information_volume_signal_channel_certificate",
        mechanism="IVSC-InformationVolumeSignalChannel",
        role="all",
...
def _function_delta(model: torch.nn.Module, vector: torch.Tensor, reference: torch.Tensor, x: torch.Tensor, lr: float) -> torch.Tensor:
    with torch.no_grad():
        base = model(x).detach().float()
    clone = deepcopy(model)
    apply_update(clone, _vector_update(vector, reference, "ivsc_delta_probe"), lr=lr)
    with torch.no_grad():
        moved = clone(x).detach().float()
    return moved - base


def _channel_stats(delta: torch.Tensor) -> dict[str, float]:
    d = delta.detach().float().reshape(delta.shape[0], -1)
    if d.numel() == 0:
        return {
            "drift_norm": 0.0,
            "diffusion_norm": 0.0,
            ...
            "effective_rank": 0.0,
            "info_volume": 0.0,
            "condition_inverse": 0.0,
        }
    mean = d.mean(dim=0)
    centered = d - mean
    drift = float(torch.linalg.vector_norm(mean).item())
    diffusion = float(torch.sqrt((centered * centered).sum(dim=1).mean().clamp_min(0.0)).item())
    total_energy = float((d * d).sum(dim=1).mean().item())
    if d.shape[0] > 1 and d.shape[1] > 0:
        s = torch.linalg.svdvals(centered)
    else:
        s = torch.zeros(1, device=d.device)
    energy = (s * s).clamp_min(0.0)
    if float(energy.sum().item()) > 0.0:
        p = energy / energy.sum()
        effective_rank = float(torch.exp(-(p * torch.log(p.clamp_min(1.0e-12))).sum()).item())
        nonzero = s[s > 1.0e-12]
        condition_inverse = float((nonzero.min() / nonzero.max()).item()) if int(nonzero.numel()) > 0 else 0.0
        top = s[: min(5, int(s.numel()))]
        info_volume = float(torch.log1p(top).sum().item())
    else:
        effective_rank = 0.0
        condition_inverse = 0.0
        info_volume = 0.0
    return {
        ...
        "DDR": (drift * drift) / (diffusion * diffusion + 1.0e-12),
        "signal_energy_fraction": (drift * drift) / (total_energy + 1.0e-12),
...
def _tiny_effects(
    ...
    clone = deepcopy(model)
    apply_update(clone, _vector_update(vector, reference, "ivsc_tiny_effect"), lr=lr)
    return {
...
def _probe_one(args: argparse.Namespace, row: dict[str, str], job_order: int) -> dict[str, Any]:
    ...
    if int(x_c.shape[0]) == 0:
        x_c, y_c = x_b, y_b
    gen = torch.Generator(device=device).manual_seed(552_113 + train_seed + job_order)
    mechanism = str(row.get("mechanism", ""))
    raw = make_update(model, mechanism, x_a, y_a, seed=train_seed + 113)
    corrupt = make_update(model, mechanism, x_a, (y_a + 1) % max(2, int(args.classes)), seed=train_seed + 114)
    source_vec = _signed_displacement(raw)
    random_vec = _random_like(source_vec, gen)
    sign_vec = -source_vec
    corrupt_vec = _signed_displacement(corrupt)
    source_stats = _channel_stats(_function_delta(model, source_vec, source_vec, x_b, float(args.probe_lr)))
    random_stats = _channel_stats(_function_delta(model, random_vec, source_vec, x_b, float(args.probe_lr)))
    sign_stats = _channel_stats(_function_delta(model, sign_vec, source_vec, x_b, float(args.probe_lr)))
    corrupt_stats = _channel_stats(_function_delta(model, corrupt_vec, source_vec, x_b, float(args.probe_lr)))
    source_eff = _tiny_effects(model, source_vec, source_vec, x_b, y_b, x_c, y_c, float(args.probe_lr))
    random_eff = _tiny_effects(model, random_vec, source_vec, x_b, y_b, x_c, y_c, float(args.probe_lr))
    sign_eff = _tiny_effects(model, sign_vec, source_vec, x_b, y_b, x_c, y_c, float(args.probe_lr))
    corrupt_eff = _tiny_effects(model, corrupt_vec, source_vec, x_b, y_b, x_c, y_c, float(args.probe_lr))
    ...
    control_ddr = max(random_stats["DDR"], sign_stats["DDR"], corrupt_stats["DDR"])
    control_volume = max(random_stats["info_volume"], sign_stats["info_volume"], corrupt_stats["info_volume"])
    control_rank = max(random_stats["effective_rank"], sign_stats["effective_rank"], corrupt_stats["effective_rank"])
    ...
    return {
        ...
        "source_IVSC_B_loss_gain": source_eff["B_loss_gain"],
        "source_IVSC_C_safety_gain": source_eff["C_safety_loss_gain"],
        "B_IVSC_control_gap": source_eff["B_loss_gain"] - control_b,
        "C_IVSC_control_gap": source_eff["C_safety_loss_gain"] - control_c,
        "source_IVSC_B_margin_gain": source_eff["B_margin_gain"],
        "B_margin_control_gap": source_eff["B_margin_gain"] - control_margin,
        "drift_norm": source_stats["drift_norm"],
        "diffusion_norm": source_stats["diffusion_norm"],
        "DDR": source_stats["DDR"],
        "DDR_control_gap": source_stats["DDR"] - control_ddr,
        "signal_energy_fraction": source_stats["signal_energy_fraction"],
        "effective_rank": source_stats["effective_rank"],
        "effective_rank_control_gap": source_stats["effective_rank"] - control_rank,
        "info_volume": source_stats["info_volume"],
        "info_volume_control_gap": source_stats["info_volume"] - control_volume,
        "condition_inverse": source_stats["condition_inverse"],
        "source_control_cosine": cosine(source_vec, random_vec),
...
def _score_probe(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    feature_keys = [
        "source_IVSC_B_loss_gain",
        "source_IVSC_C_safety_gain",
        "B_IVSC_control_gap",
        "C_IVSC_control_gap",
        "source_IVSC_B_margin_gain",
        "DDR",
        "DDR_control_gap",
        "signal_energy_fraction",
        "effective_rank",
        "info_volume_control_gap",
    ]
    z = {key: _zscore([_f(r.get(key)) for r in rows], groups) for key in feature_keys}
    out = []
    for i, row in enumerate(rows):
        score = (
            0.25 * z["source_IVSC_B_loss_gain"][i]
            + 0.20 * z["source_IVSC_C_safety_gain"][i]
            + 0.25 * z["B_IVSC_control_gap"][i]
            + 0.15 * z["C_IVSC_control_gap"][i]
            + 0.15 * z["source_IVSC_B_margin_gain"][i]
            + 0.30 * z["DDR"][i]
            + 0.25 * z["DDR_control_gap"][i]
            + 0.20 * z["signal_energy_fraction"][i]
            + 0.20 * z["effective_rank"][i]
            + 0.20 * z["info_volume_control_gap"][i]
        )
        train_gate = int(
            _f(row.get("source_IVSC_B_loss_gain"), -999.0) > 0.0
            and _f(row.get("B_IVSC_control_gap"), -999.0) >= -1.0e-3
            and _f(row.get("C_IVSC_control_gap"), -999.0) >= -1.0e-3
            and _f(row.get("DDR"), 0.0) > 0.0
            and _f(row.get("effective_rank"), 0.0) >= 1.0
        )
        item = dict(row)
        item["IVSC_score"] = score
        item["IVSC_train_only_certificate_pass"] = train_gate
        item["certificate_family"] = "IVSC_Information_Volume_Signal_Channel_Certificate"
        out.append(item)
    return sorted(out, key=lambda r: _f(r.get("IVSC_score"), -999.0), reverse=True)


def _summary(rows: list[dict[str, Any]], top_k: int) -> dict[str, Any]:
    scores = [_f(r.get("IVSC_score")) for r in rows]
    ...
    return {
        "certificate_family": "IVSC_Information_Volume_Signal_Channel_Certificate",
        "rows": len(rows),
        ...
        "IVSC_train_only_certificate_pass_rows": sum(int_flag(r.get("IVSC_train_only_certificate_pass")) for r in rows),
        "IVSC_official_observer_pass": int(not blockers),
        "blocker": ";".join(dict.fromkeys(blockers)),
    }


def _ivsc_update(model: torch.nn.Module, raw: UpdateTensor, x_probe: torch.Tensor, gen: torch.Generator | None, args: argparse.Namespace) -> UpdateTensor:
    source_vec = _signed_displacement(raw)
    vector = _random_like(source_vec, gen) if gen is not None else source_vec
    stats = _channel_stats(_function_delta(model, vector, source_vec, x_probe, float(args.probe_lr)))
    diagnostics = {
        "DDR": stats["DDR"],
        "signal_energy_fraction": stats["signal_energy_fraction"],
        "effective_rank": stats["effective_rank"],
        "info_volume": stats["info_volume"],
    }
    return _vector_update(vector, raw.tensor.detach().float(), "information_volume_signal_channel", diagnostics)
...
def _train_fresh(args: argparse.Namespace, source_row: dict[str, Any], run_kind: str) -> dict[str, Any]:
    ...
    gen = torch.Generator(device=device).manual_seed(552_907 + train_seed + sum(ord(c) for c in run_kind))
    ...
    for step in range(1, int(args.steps) + 1):
        ...
        if run_kind == "CTRL-AdamW":
            ...
            if step % max(1, int(args.refresh_interval)) == 0:
                ...
                update = _ivsc_update(model, raw, x_b, gen if run_kind == "CTRL-RandomMatchedInfoVolumeFU" else None, args)
                injection_count += 1
                injection_norm_sum += float(torch.linalg.vector_norm(update.tensor.detach().float()).item())
                ddr_sum += float(update.diagnostics.get("DDR", 0.0) if update.diagnostics else 0.0)
                signal_frac_sum += float(update.diagnostics.get("signal_energy_fraction", 0.0) if update.diagnostics else 0.0)
                rank_sum += float(update.diagnostics.get("effective_rank", 0.0) if update.diagnostics else 0.0)
                apply_update(model, update, lr=float(args.fu_lr))
                info_gain_sum += before - _loss_value(model, x_b, y_b)
        if step in horizons:
    ...
    out: dict[str, Any] = {
        ...
        "seed": seed,
        "train_seed": train_seed,
        "IVSC_score": source_row.get("IVSC_score", ""),
        "refresh_interval": int(args.refresh_interval),
        "injection_count": injection_count,
        "injection_norm_sum": injection_norm_sum,
        "DDR_mean": ddr_sum / max(1, injection_count),
        "signal_energy_fraction_mean": signal_frac_sum / max(1, injection_count),
        "effective_rank_mean": rank_sum / max(1, injection_count),
        "info_volume_loss_gain_mean": info_gain_sum / max(1, injection_count),
        "execution_status": "measured",
    }
    for h in HORIZONS:
...
def _fresh_summary(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    candidates = [r for r in enriched if str(r.get("run_kind")) == "IVSC-AdamWPlusInfoVolumeFU"]
    summary = []
        ...
        summary.append(
            {
                ...
                "DDR_mean": row.get("DDR_mean", ""),
                "signal_energy_fraction_mean": row.get("signal_energy_fraction_mean", ""),
                "effective_rank_mean": row.get("effective_rank_mean", ""),
                "info_volume_loss_gain_mean": row.get("info_volume_loss_gain_mean", ""),
                "injection_count": row.get("injection_count", ""),
                "injection_norm_sum": row.get("injection_norm_sum", ""),
                "debt_not_exploded": debt_ok,
                "IVSC_fresh_C3_info_volume_pass": int(source_pass and debt_ok),
                "official_C3_pass": 0,
                "blocker": "" if source_pass and debt_ok else "source_horizon_or_debt_gate_failed",
            }
        )
    return enriched, {"summary": summary, "route": {"fresh_rows": len(summary), "fresh_C3_info_volume_pass_rows": sum(int_flag(r.get("IVSC_fresh_C3_info_volume_pass")) for r in summary), "official_C3_pass_rows": 0}}
...
def main() -> None:
    ...
    eligible = [r for r in scored if int_flag(r.get("IVSC_train_only_certificate_pass"))]
    selected = (eligible if eligible else scored)[: max(0, int(args.fresh_top_k))]
    fresh_rows = []
    for row in selected:
        for run_kind in [
            "IVSC-AdamWPlusInfoVolumeFU",
            "CTRL-AdamW",
            "CTRL-SGD",
            "CTRL-NoOp",
            "CTRL-RandomMatchedInfoVolumeFU",
        ]:
    ...
    blockers = []
    if not int_flag(summary.get("IVSC_official_observer_pass")):
        blockers.append("official_observer_gate_failed")
    if not int(fresh_route["fresh_C3_info_volume_pass_rows"]):
        blockers.append("fresh_C3_source_horizon_failed")
    blockers.append("official_C3_gate_not_claimed")
    route = {
        "route": "PostInfoVolumeFreshC3Opened" if int(fresh_route["fresh_C3_info_volume_pass_rows"]) else "PostInfoVolumeBlocked",
        "previous_route": prev_route.get("route", ""),
        "certificate_family": "IVSC_Information_Volume_Signal_Channel_Certificate",
        "probe_rows": len(scored) + len(blocked),
        "probe_measured_rows": len(scored),
        "IVSC_train_only_certificate_pass_rows": summary.get("IVSC_train_only_certificate_pass_rows", 0),
        "IVSC_official_observer_pass": summary.get("IVSC_official_observer_pass", 0),
        "fresh_C3_info_volume_pass_rows": fresh_route["fresh_C3_info_volume_pass_rows"],
        "official_C3_pass_rows": 0,
        "promotion_allowed": 0,
        "blocker": ";".join(dict.fromkeys(blockers)),
        "next_codex_action": "IVSC failed fresh/official gate; record v22.09 local boundary or change principle again",
    }
    write_rows(out_dir / "v22_09_info_volume_signal_channel_certificate.csv", scored + blocked)
    write_rows(out_dir / "v22_09_info_volume_signal_channel_summary.csv", [summary])
    write_rows(out_dir / "v22_09_info_volume_signal_channel_selected_candidates.csv", selected)
    write_rows(out_dir / "v22_09_info_volume_signal_channel_fresh_c3_matrix.csv", fresh_enriched)
    write_rows(out_dir / "v22_09_info_volume_signal_channel_fresh_c3_summary.csv", fresh_summary)
    write_json(out_dir / "v22_09_info_volume_signal_channel_route.json", route)
    simple_svg(out_dir / "figures/v22_09_info_volume_signal_channel_certificate.svg", "v22.09 IVSC certificate", scored, "IVSC_score")
    simple_svg(out_dir / "figures/v22_09_info_volume_signal_channel_fresh_h3200.svg", "v22.09 IVSC fresh h3200", fresh_summary, "source_vs_best_control_h3200")
    append_exec(
        out_dir,
        f"{PYTHON} experiments/run_v22_09_post_info_volume_signal_channel_certificate.py --source-dir {args.source_dir} --device {args.device} --max-probe-jobs {int(args.max_probe_jobs)} --fresh-top-k {int(args.fresh_top_k)} --steps {int(args.steps)} --refresh-interval {int(args.refresh_interval)} --out-dir {out_dir}",
        status="completed",
        note=f"probe_rows={len(scored)+len(blocked)} measured={len(scored)} train_only_pass={summary.get('IVSC_train_only_certificate_pass_rows', 0)} fresh_pass={fresh_route['fresh_C3_info_volume_pass_rows']} route={route['route']}",
    )
