#!/usr/bin/env python3
"""Shared helpers for v22.07 execution, logs, packets, and summaries."""
...
V2207_ROOT = ROOT / "results/v22_07_metric_dynamics_functional_update_basis_efficiency_4gpu"
V2207_OFFICIAL = V2207_ROOT / "official_v22_07"
V2207_PLAN_DOC = ROOT / "docs/DG-KAN_v22.07_MetricDynamics_FunctionalUpdate_BasisEfficiency_4GPU_完整计划.md"
V2207_EXEC_DOC = ROOT / "docs/DG-KAN_v22.07_MetricDynamics_FunctionalUpdate_BasisEfficiency_4GPU_执行日志.md"
V2207_RECAP_DOC = ROOT / "docs/DG-KAN_v22.07_MetricDynamics_FunctionalUpdate_BasisEfficiency_4GPU_实验结果复盘.md"
...
def ensure_out(out_dir: str | Path | None = None) -> Path:
    out = Path(out_dir) if out_dir else V2207_OFFICIAL
...
def init_docs() -> None:
    V2207_EXEC_DOC.parent.mkdir(parents=True, exist_ok=True)
    if not V2207_EXEC_DOC.exists():
        V2207_EXEC_DOC.write_text(
            "# DG-KAN v22.07 MetricDynamics FunctionalUpdate BasisEfficiency 4GPU 执行日志\n\n"
            f"生成时间：{now_sg()}\n\n"
            "记录原则：只记录真实命令、输入、输出、状态和 blocker；未执行项不写成完成。\n",
            encoding="utf-8",
        )
    if not V2207_RECAP_DOC.exists():
...
def append_exec(out_dir: Path, command: str, *, status: str = "", note: str = "") -> None:
    ...
    journal = out_dir / "v22_07_command_journal.csv"
    ...
    with journal.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow(row)
    append_text(V2207_EXEC_DOC, f"\n## {row['timestamp']}\n\n```bash\n{command}\n```\n\n- status: {status or 'recorded'}\n")
    if note:
        append_text(V2207_EXEC_DOC, f"- note: {note}\n")
...
def build_packet(out_dir: Path) -> tuple[Path, Path]:
    ...
    for rel in sections:
        ...
        if Path(rel).suffix:
            ...
            target.write_text("v22.07 code review packet\n", encoding="utf-8")
        else:
            target.mkdir(parents=True, exist_ok=True)
    write_text(
        packet / "00_README.md",
        "v22.07 packet: self-contained source tree, S0.14 truth gate artifacts, queue records, logs, and result pointers.\n",
    )
    source_tree = packet / "02_SOURCE_TREE"
    if (ROOT / "dgkan").exists():
    ...
    for path in sorted(out_dir.glob("v22_07_*")):
        if not path.is_file() or path.suffix.lower() == ".zip":
            continue
        low = path.name.lower()
        section = "10_RESULTS_POINTERS"
        if "import" in low or "compile" in low or "clean_unzip" in low or "code_truth" in low:
        ...
        elif "metric" in low or "solver" in low or "nds" in low or "target" in low:
            section = "07_FUNCTION_SPACE_SOLVER"
        elif "efficiency" in low or "drat" in low or "drbf" in low or "kernel" in low:
            section = "08_BASIS_KERNELS_AND_PROFILERS"
        elif "gpu" in low or "queue" in low or "idle" in low:
            section = "09_GPU_QUEUE"
    ...
    for doc in [V2207_PLAN_DOC, V2207_EXEC_DOC, V2207_RECAP_DOC]:
    ...
    write_text(
        ...
        f"{PYTHON} experiments/run_v22_07_s014_truth_gate.py --mode all --source-root . --self-contained-import-check 1\n",
    )
    manifest = []
    ...
    zip_path = out_dir / "v22_07_code_review_packet.zip"
    if zip_path.exists():
        zip_path.unlink()
    ...
    bundle = out_dir / "v22_07_results_bundle.zip"
    if bundle.exists():
        bundle.unlink()
    with zipfile.ZipFile(bundle, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for path in sorted(out_dir.rglob("*")):
            if path.is_file() and path.suffix.lower() != ".zip" and "v22_07_code_review_packet" not in str(path):
