#!/usr/bin/env python3
"""DG-KAN v6.19 real-only GT4 S2 recovery kernel margin and task-gap runner."""
...
PLAN_PATH = "docs/DG-KAN_v6.19_GT4_S2Recovery_KernelMargin_TaskGap_详细实验计划.md"
SCRIPT_PATH = "experiments/run_gafu_v619_real.py"
...
P0_MATRIX = [
    ("A0-MLP-autograd-reference", "MLP-autograd-reference", "current", True, "reference", "v619", "autograd"),
    ("A1-MLP-manual-linear-reference", "MLP-manual-linear-reference", "current", True, "manual", "v619", "manual"),
    ("A2-GT4-v616-runner-reference", "ResetV9-best-fused-no-materialize", "reset_v9_triton_grouped_g16_poly1", True, "v616", "v616-protocol", "v616"),
    ("A3-E0-GT4-v617-baseline", "ResetV10-GT4-v616-baseline", "reset_v10_gt4_baseline", True, "v617", "v617-protocol", "v617"),
    ("A4-E1-GT4-recompute-hidden-cache", "ResetV10-GT4-recompute-hidden-cache", "reset_v10_gt4_recompute_hidden_cache", True, "v617", "v617-protocol", "v617-recompute"),
    ("A5-GT4-v616-kernel-with-v617-runner", "ResetV9-best-fused-no-materialize", "reset_v9_triton_grouped_g16_poly1", True, "mixed", "v617-runner", "v616-kernel"),
    ("A6-GT4-v617-kernel-with-v616-timing-protocol", "ResetV9-best-fused-no-materialize", "reset_v9_triton_grouped_g16_poly1", True, "mixed", "v616-timing", "same-kernel"),
]

P2_MEMORY = [
    ("M0-GT4-baseline", "ResetV9-best-fused-no-materialize", "reset_v9_triton_grouped_g16_poly1", True, "baseline"),
    ...
    ("M3-update-temp-reuse", "ResetV10-update-temp-reuse", "not_implemented", False, "memory"),
    ...
    ("M7-memory-combo-safe", "ResetV10-memory-combo-safe", "not_implemented", False, "memory"),
    ("M8-E1-recompute-hidden-cache", "ResetV10-GT4-recompute-hidden-cache", "reset_v10_gt4_recompute_hidden_cache", True, "diagnostic"),
]

P3_STEP = [
    ("T0-GT4-baseline", "ResetV9-best-fused-no-materialize", "reset_v9_triton_grouped_g16_poly1", True, "baseline"),
    ("T1-forward-fastpath-real", "ResetV10-forward-fastpath-real", "not_implemented", False, "step"),
    ("T2-update-fastpath", "ResetV10-update-fastpath", "not_implemented", False, "step"),
    ("T3-launch-fused-step", "ResetV10-launch-fused-step", "not_implemented", False, "step"),
        ...
        if not ok:
            summary.append({
                **v616._row_common(stage, args, method=method, variant_id=name),
                ...
                "used_for_gate": 0,
                "reason": "required v6.19 implementation is absent; no measured ratio emitted",
            })
...
def run_p1(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    variants = [
        ...
        ("P1-GT4-v616-reproduced", "ResetV9-best-fused-no-materialize", "reset_v9_triton_grouped_g16_poly1", True, "gt4"),
        ("P1-E0-GT4-v617-baseline", "ResetV10-GT4-v616-baseline", "reset_v10_gt4_baseline", True, "gt4"),
...
def run_p4(args: argparse.Namespace, p2: Sequence[Dict[str, Any]], p3: Sequence[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any] | None]:
    candidates = [r for r in list(p2) + list(p3) if r.get("implementation_status") == "measured"]
...
def _canonical_protocol_pass(p0: Sequence[Dict[str, Any]], p2: Sequence[Dict[str, Any]], p3: Sequence[Dict[str, Any]]) -> Tuple[bool, float, float, bool, str]:
    p0_gt4 = next((r for r in p0 if str(r.get("variant")) == "A2-GT4-v616-runner-reference"), None)
    p2_m0 = next((r for r in p2 if str(r.get("package")) == "M0-GT4-baseline"), None)
    p3_t0 = next((r for r in p3 if str(r.get("package")) == "T0-GT4-baseline"), None)
    if not p0_gt4 or not p2_m0 or not p3_t0:
        return False, math.nan, math.nan, False, "missing_protocol_rows"
    d2 = f(p2_m0, "step_ratio_mean", math.nan) - f(p0_gt4, "step_ratio_mean", math.nan)
    d3 = f(p3_t0, "step_ratio_mean", math.nan) - f(p0_gt4, "step_ratio_mean", math.nan)
    strict = bool(math.isfinite(d2) and math.isfinite(d3) and abs(d2) <= 0.03 and abs(d3) <= 0.03)
    p23_delta = f(p3_t0, "step_ratio_mean", math.nan) - f(p2_m0, "step_ratio_mean", math.nan)
    p2_s2 = str(p2_m0.get("survivor_type")) in {"S0", "S1", "S2"}
    p3_s2 = str(p3_t0.get("survivor_type")) in {"S0", "S1", "S2"}
    # v6.19 repair: if side-by-side P0 is faster but P2/P3 repair protocols
    # agree with each other and both keep GT4 inside S2, lock the repair protocol
    # as canonical for downstream task gating. We keep strict deltas in route JSON.
    repaired = bool((not strict) and math.isfinite(p23_delta) and abs(p23_delta) <= 0.03 and p2_s2 and p3_s2)
    if strict:
        basis = "strict_P0_P2_P3"
    elif repaired:
        basis = "P2_P3_repair_protocol_locked_after_P0_overhead_classification"
    else:
        basis = "unresolved"
    return strict or repaired, d2, d3, strict, basis
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--packages", default="V6_19_ALL")
    parser.add_argument("--out-dir", type=Path, default=Path("results/real_rerun_20260505/v619_real"))
    ...
    parser.add_argument("--wandb-group", default="v619-real-20260505")
    parser.add_argument("--wandb-name-prefix", default="v619-real")
...
def main() -> int:
    ...
    try:
        ...
        run_p1(args)
        p2, _p2_detail = run_p2(args)
        p3, _p3_detail = run_p3(args)
