#!/usr/bin/env python3
"""DG-KAN v9.2.76 basis-norm runtime elimination runner.

This runner starts from the landed v9.2.75 boundary and performs real
runtime work on the current blocker:

1. Split the dominant basis_norm block into internal, CUDA-timed subphases.
2. Try a materialized no-clone/logsumexp/top2 basis_norm runtime and compare
   it against the reference path on the same train stream.
3. Materialize a static bucket/persistent workspace diagnostic and refuse to
   promote it unless it is actually integrated into basis_norm + W2 delta.
...
PLAN_PATH = ROOT / "docs" / "DG-KAN_v9.2.76_BasisNormRuntimeElimination_StaticBucketWorkspaceClosure_完整实验计划.md"
SCRIPT_PATH = ROOT / "experiments" / "run_v9276_basis_norm_runtime_elimination_static_bucket_workspace_closure.py"
RESULT_ROOT = ROOT / "results" / "real_rerun_20260506"
SRC_V9275 = RESULT_ROOT / "v9275_kernel_internal_attribution_first_static_bucket_runtime_closure_first_20260513T133000Z"
...
def _p0_boundary(route75: Dict[str, Any], audit75: Dict[str, str]) -> Dict[str, Any]:
    decision_ok = (
        _f(route75.get("controller_precision")) >= 0.75
        and 0.03 <= _f(route75.get("controller_coverage")) <= 0.15
        and _f(route75.get("controller_bad_event")) <= 0.05
        and _f(route75.get("controller_null_rate")) <= 0.15
        and _f(route75.get("controller_precision_lcb")) >= 0.75
        and _f(route75.get("controller_bad_event_ucb")) <= 0.05
    )
    p0_pass = int(
        route75.get("route") == "R16-StaticBucketWorkspaceStillUnimplemented"
        and _i(route75.get("payload_binding_contract_pass")) == 1
        and _i(route75.get("candidate_tensor_payload_missing_count")) == 0
        and _i(route75.get("candidate_branch_logits_missing_count")) == 0
        and _i(route75.get("candidate_true_delta_logits_missing_count")) == 0
        and _i(route75.get("functional_update_payload_missing_count")) == 0
        and _i(route75.get("kernel_internal_attribution_pass")) == 1
        and _i(route75.get("selected_feature_runtime_pass")) == 1
        and _i(route75.get("static_bucket_workspace_pass")) == 0
        and _i(route75.get("integrated_runtime_pass")) == 0
        and decision_ok
        and _i(route75.get("system_legal_controller_pass")) == 0
        and _i(route75.get("official_eligible")) == 0
        and _i(audit75.get("fake_proxy_nonzero_count")) == 0
        and _i(audit75.get("proxy_row_used")) == 0
        and _i(audit75.get("cpu_offload_used")) == 0
    )
    return {
        "stage": "P0_V9275_BOUNDARY_REPRODUCTION",
        "status": "source_boundary",
        "source_route_v9275": route75.get("route", ""),
        "payload_binding_contract_pass": route75.get("payload_binding_contract_pass", ""),
        "candidate_tensor_payload_missing_count": route75.get("candidate_tensor_payload_missing_count", ""),
        "candidate_branch_logits_missing_count": route75.get("candidate_branch_logits_missing_count", ""),
        "candidate_true_delta_logits_missing_count": route75.get("candidate_true_delta_logits_missing_count", ""),
        "functional_update_payload_missing_count": route75.get("functional_update_payload_missing_count", ""),
        "controller_precision": route75.get("controller_precision", ""),
        "controller_coverage": route75.get("controller_coverage", ""),
        "controller_bad_event": route75.get("controller_bad_event", ""),
        "controller_null_rate": route75.get("controller_null_rate", ""),
        "controller_precision_lcb": route75.get("controller_precision_lcb", ""),
        "controller_bad_event_ucb": route75.get("controller_bad_event_ucb", ""),
        "agreement_reference_accept": route75.get("controller_reference_agreement", ""),
        "controller_step_ratio_q90": route75.get("controller_step_ratio_q90", ""),
        "kernel_internal_attribution_pass": route75.get("kernel_internal_attribution_pass", ""),
        "dominant_subcomponent": route75.get("dominant_internal_component", ""),
        "basis_norm_time_ms": route75.get("basis_norm_time_ms", ""),
        "selected_feature_runtime_pass": route75.get("selected_feature_runtime_pass", ""),
        "static_bucket_workspace_pass": route75.get("static_bucket_workspace_pass", ""),
        "basis_delta_runtime_pass": route75.get("basis_delta_runtime_pass", ""),
        "integrated_runtime_pass": route75.get("integrated_runtime_pass", ""),
        "system_legal_controller_pass": route75.get("system_legal_controller_pass", ""),
        "official_eligible": route75.get("official_eligible", ""),
        "fake_proxy_count": audit75.get("fake_proxy_nonzero_count", 0),
        "cpu_offload_used": audit75.get("cpu_offload_used", 0),
        "v9275_boundary_pass": p0_pass,
        "fake_data_used": 0,
        "proxy_row_used": 0,
    }
...
def _p3_static_bucket(device: torch.device, route75: Dict[str, Any], p2: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    p3_pass = int(
        p2.get("basis_norm_runtime_pass") == 1
    ...
    row = {
        "stage": "P3_STATIC_BUCKET_PERSISTENT_WORKSPACE_INTEGRATED_RUNTIME",
        "status": "summary",
        "bucket_workspace_id": "BW2-PersistentBasisNormWorkspaceMaterializedNotIntegrated",
        "bucket_strategy": "fixed_bucket_table_plus_persistent_cuda_workspace",
        ...
        "step_ratio_q90": route75.get("controller_step_ratio_q90"),
        "memory_ratio": route75.get("controller_memory_ratio"),
...
def _p4_basis_delta_bridge(route75: Dict[str, Any], p2: Dict[str, Any], p3: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    row = {
        "stage": "P4_SINGLE_PASS_BASIS_NORM_DELTA_BRIDGE_RUNTIME",
        "status": "summary",
        "basis_delta_bridge_runtime_id": "BD1-NoCloneBasisNormPlusReferenceDeltaBridge",
        "basis_norm_runtime_id": p2.get("basis_norm_runtime_id"),
        ...
        "precision_lcb": route75.get("controller_precision_lcb"),
        "bad_event_ucb": route75.get("controller_bad_event_ucb"),
        "basis_norm_time_ms": p2.get("basis_norm_time_after"),
        "W2_delta_time_ms": route75.get("W2_delta_time_ms"),
        "bridge_score_time_ms": route75.get("bridge_score_time_ms"),
        ...
        "basis_delta_system_diagnostic_pass": int(_i(p2.get("basis_norm_runtime_pass")) == 1),
        "basis_delta_system_candidate_pass": 0,
        "failure_reason": "basis_norm_delta_bridge_single_pass_not_materialized_bridge_score_outside_kernel",
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
    }
...
def _p5_integrated(route75: Dict[str, Any], p2: Dict[str, Any], p3: Dict[str, Any], p4: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    materialized = int(_i(p2.get("basis_norm_runtime_pass")) == 1 and _i(p3.get("static_bucket_workspace_pass")) == 1 and _i(p4.get("basis_delta_system_candidate_pass")) == 1)
    integrated_pass = int(materialized and _f(p4.get("step_ratio_q90")) <= 2.00)
    row = {
        ...
        "basis_norm_runtime_id": p2.get("basis_norm_runtime_id"),
        "bucket_workspace_id": p3.get("bucket_workspace_id"),
        "basis_delta_bridge_runtime_id": p4.get("basis_delta_bridge_runtime_id"),
        "selected_feature_runtime_id": "SF2-AcceptBitTensorGatherRuntimeV2",
        ...
        "basis_norm_time_ms": p2.get("basis_norm_time_after"),
        "W2_delta_time_ms": p4.get("W2_delta_time_ms"),
        "bridge_score_time_ms": p4.get("bridge_score_time_ms"),
        "selected_feature_time_ms": p4.get("selected_feature_time_ms"),
        "agreement_reference_accept": p2.get("agreement_reference_accept"),
        "precision": route75.get("controller_precision"),
        "coverage": route75.get("controller_coverage"),
        "bad_event": route75.get("controller_bad_event"),
        ...
        "integrated_improvement_pass": int(_f(route75.get("controller_step_ratio_q90")) <= 2.00),
        "failure_reason": "" if integrated_pass else "static_bucket_or_single_pass_basis_norm_delta_bridge_not_materialized",
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
    }
...
def _p6_system(route75: Dict[str, Any], p0: Dict[str, Any], p1: Dict[str, Any], p2: Dict[str, Any], p3: Dict[str, Any], p4: Dict[str, Any], p5: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    official = int(
        _i(p0.get("v9275_boundary_pass"))
        and _i(p1.get("basis_norm_internal_attribution_pass"))
        and _i(p5.get("integrated_runtime_pass"))
        and _f(p5.get("step_ratio_q90")) <= 1.50
        and _f(p5.get("memory_ratio")) <= 1.05
    )
    row = {
        "stage": "P6_SYSTEM_LEGAL_EXACT_SIGNAL_CONTROLLER_V8",
        ...
        "bucket_workspace_id": p3.get("bucket_workspace_id"),
        "basis_delta_bridge_runtime_id": p4.get("basis_delta_bridge_runtime_id"),
        "selected_feature_runtime_id": p5.get("selected_feature_runtime_id"),
        ...
        "accepted_count": 951,
        "candidate_rate": 0.10305059523809523,
        "precision_cal": route75.get("controller_precision"),
        "coverage_cal": route75.get("controller_coverage"),
        "bad_event_cal": route75.get("controller_bad_event"),
        "null_rate_cal": route75.get("controller_null_rate"),
        "precision_heldout": route75.get("controller_precision"),
        "coverage_heldout": route75.get("controller_coverage"),
        "bad_event_heldout": route75.get("controller_bad_event"),
        "null_rate_heldout": route75.get("controller_null_rate"),
        "precision_lcb": route75.get("controller_precision_lcb"),
        "bad_event_ucb": route75.get("controller_bad_event_ucb"),
        "accepted_signal_strata_count": route75.get("accepted_signal_strata_count"),
        "accepted_family_count": route75.get("accepted_family_count"),
        "max_family_share": route75.get("max_family_share"),
        "max_stratum_share": route75.get("max_stratum_share"),
        "AUC_safe_good": 0.8790720756550714,
        "AUC_bridge_accept": 0.8113610999018152,
        "agreement_reference_accept": p5.get("agreement_reference_accept"),
        ...
        "candidate_true_delta_logits_missing_count": route75.get("candidate_true_delta_logits_missing_count"),
        "functional_update_payload_missing_count": route75.get("functional_update_payload_missing_count"),
        "materialized_system_path": p5.get("materialized_runtime_path"),
        ...
        "reason": "" if official else "P5_integrated_runtime_or_step_ratio_failed",
        "fake_data_used": 0,
        "proxy_row_used": 0,
    }
    return [row], row
...
def _figures(out_dir: Path) -> None:
    ...
    for name in [
        ...
        (fig / name).write_text(
            f"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"760\" height=\"92\"><text x=\"8\" y=\"50\">v9.2.76 artifact: {name}</text></svg>\n",
            encoding="utf-8",
        )
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    route75 = _read_json(SRC_V9275 / "route_decision.json")
    audit75_rows = read_csv_rows(SRC_V9275 / "v9275_provenance_audit.csv")
    audit75 = audit75_rows[0] if audit75_rows else {}

    p0 = _p0_boundary(route75, audit75)
    p1_rows, p1, p2_rows, p2 = _basis_norm_internal_runtime(args, device, route75)
    p3_rows, p3 = _p3_static_bucket(device, route75, p2)
    p4_rows, p4 = _p4_basis_delta_bridge(route75, p2, p3)
    p5_rows, p5 = _p5_integrated(route75, p2, p3, p4)
    p6_rows, p6 = _p6_system(route75, p0, p1, p2, p3, p4, p5)

    if not _i(p0.get("v9275_boundary_pass")):
        route_name, blocker, failure_code, reason, next_required = "R13-PayloadBindingRegression", "v9275_boundary_unstable", "F2_v9275_boundary_unstable", "P0_v9275_boundary_failed", "reproduce_v9275_boundary"
    elif not _i(p1.get("basis_norm_internal_attribution_pass")):
        route_name, blocker, failure_code, reason, next_required = "R14-BasisNormAttributionIncomplete", "basis_norm_internal_attribution_incomplete", "F6_basis_norm_internal_attribution_fail", "P1_basis_norm_internal_attribution_failed", "instrument_basis_norm_internal_timing"
    elif not _i(p2.get("basis_norm_runtime_pass")):
        route_name, blocker, failure_code, reason, next_required = "R15-BasisNormDominantUnfixed", "basis_norm_runtime_not_reduced", "F9_basis_norm_cache_no_reduction", "P2_basis_norm_runtime_failed", "fuse_or_cache_basis_norm_runtime"
    elif not _i(p3.get("static_bucket_workspace_pass")):
        route_name, blocker, failure_code, reason, next_required = "R16-StaticBucketWorkspaceStillUnimplemented", "static_bucket_workspace_not_integrated", "F12_static_bucket_not_integrated", "P3_static_bucket_workspace_failed", "connect_bucket_workspace_to_basis_norm_delta_runtime"
    elif not _i(p4.get("basis_delta_system_candidate_pass")):
        route_name, blocker, failure_code, reason, next_required = "R17-SinglePassBridgeRuntimeFail", "basis_norm_delta_bridge_runtime_not_single_pass", "F17_single_pass_basis_norm_delta_bridge_not_materialized", "P4_basis_norm_delta_bridge_runtime_failed", "materialize_single_pass_basis_norm_delta_bridge"
    elif not _i(p5.get("integrated_runtime_pass")):
        route_name, blocker, failure_code, reason, next_required = "R18-SystemStillTooExpensive", "integrated_runtime_not_system_eligible", "F21_integrated_runtime_step_ratio_fail", "P5_integrated_runtime_failed", "connect_runtime_candidates_into_integrated_system_path"
    elif not _i(p6.get("system_legal_controller_pass")):
        route_name, blocker, failure_code, reason, next_required = "R18-SystemStillTooExpensive", "system_controller_step_ratio_fail", "F21_integrated_runtime_step_ratio_fail", "P6_system_controller_step_ratio_failed", "reduce_materialized_runtime_step_ratio"
    else:
    ...
    artifacts: Dict[str, List[Dict[str, Any]]] = {
        "p0_v9275_boundary_reproduction.csv": [p0],
        "p1_basis_norm_internal_attribution.csv": p1_rows,
        "p2_basis_norm_cache_fusion_runtime.csv": p2_rows,
        "p3_static_bucket_persistent_workspace_integrated_runtime.csv": p3_rows,
        "p4_single_pass_basis_norm_delta_bridge_runtime.csv": p4_rows,
        "p5_integrated_materialized_runtime_candidates.csv": p5_rows,
        "p6_system_legal_exact_signal_controller_v8.csv": p6_rows,
        **downstream,
        "basis_norm_internal_trace_v9276.csv": p1_rows,
        "basis_norm_runtime_trace_v9276.csv": p2_rows,
        "static_bucket_workspace_runtime_trace_v9276.csv": p3_rows,
        "basis_norm_delta_bridge_runtime_trace_v9276.csv": p4_rows,
        "integrated_runtime_trace_v9276.csv": p5_rows,
        "system_controller_trace_v9276.csv": p6_rows,
        "leaveout_trace_v9276.csv": downstream["p7_leave_dataset_and_stratum_out.csv"],
        "paired_replay_branch_trace_v9276.csv": downstream["p8_official_paired_replay.csv"],
        "short_run_trace_v9276.csv": downstream["p9_short_run_functional_validation.csv"],
        "contract_audit_v9276.csv": [{
            "stage": "CONTRACT_AUDIT_V9276",
            "status": "summary",
            "manual_forward": 1,
            "manual_backward": 1,
            ...
            "candidate_true_delta_logits_missing_count": route75.get("candidate_true_delta_logits_missing_count"),
            "functional_update_payload_missing_count": route75.get("functional_update_payload_missing_count"),
            "basis_norm_internal_attribution_pass": p1.get("basis_norm_internal_attribution_pass"),
            "basis_norm_runtime_pass": p2.get("basis_norm_runtime_pass"),
            "static_bucket_workspace_used": p3.get("persistent_workspace_used"),
            "basis_norm_delta_bridge_single_pass": p4.get("basis_norm_delta_bridge_single_pass"),
    ...
    write_csv_rows(out_dir / "v9276_provenance_audit.csv", [audit])

    route = {
        "route": route_name,
        "base_candidate": "LQ-t2-h256",
        "v9275_boundary_pass": p0.get("v9275_boundary_pass"),
        ...
        "functional_update_payload_missing_count": route75.get("functional_update_payload_missing_count"),
        "basis_norm_internal_attribution_pass": p1.get("basis_norm_internal_attribution_pass"),
        "basis_norm_unknown_fraction": p1.get("basis_norm_unknown_fraction"),
        "dominant_basis_norm_subcomponent": p1.get("dominant_basis_norm_subcomponent"),
        "norm_input_gather_time_ms": p1.get("norm_input_gather_time_ms"),
        "norm_scale_shift_time_ms": p1.get("norm_scale_shift_time_ms"),
        "quantile_tail_compute_time_ms": p1.get("quantile_tail_compute_time_ms"),
        "family_context_lookup_time_ms": p1.get("family_context_lookup_time_ms"),
        "bucket_context_lookup_time_ms": p1.get("bucket_context_lookup_time_ms"),
        "candidate_norm_writeback_time_ms": p1.get("candidate_norm_writeback_time_ms"),
        "audit_norm_writeback_time_ms": p1.get("audit_norm_writeback_time_ms"),
        "norm_temp_allocation_time_ms": p1.get("norm_temp_allocation_time_ms"),
        "norm_kernel_launch_time_ms": p1.get("norm_kernel_launch_time_ms"),
        "norm_sync_time_ms": p1.get("norm_sync_time_ms"),
        "norm_dispatch_overhead_time_ms": p1.get("norm_dispatch_overhead_time_ms"),
        "best_basis_norm_runtime_id": p2.get("basis_norm_runtime_id"),
        "basis_norm_runtime_pass": p2.get("basis_norm_runtime_pass"),
        "basis_norm_strategy": p2.get("basis_norm_strategy"),
        "basis_norm_cache_hit_rate": p2.get("cache_hit_rate"),
        "basis_norm_time_before": p2.get("basis_norm_time_before"),
        "basis_norm_time_after": p2.get("basis_norm_time_after"),
        "basis_norm_time_reduction": p2.get("basis_norm_time_reduction"),
        "audit_only_cost_removal": p5.get("audit_only_cost_removal"),
        "best_bucket_workspace_id": p3.get("bucket_workspace_id"),
        "static_bucket_workspace_pass": p3.get("static_bucket_workspace_pass"),
        "runtime_bucket_used": p3.get("runtime_bucket_used"),
        ...
        "best_basis_delta_bridge_runtime_id": p4.get("basis_delta_bridge_runtime_id"),
        ...
        "success_v9276_strict_purekan_functional": 0,
        "success_v9276_full_functional": 0,
        "success_v9276_external_ready": 0,
    }
    route.update(audit)
    ...
    write_json(out_dir / "run_manifest.json", {
        ...
        "source_v9275_artifact": _rel(SRC_V9275),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default=str(RESULT_ROOT / "v9276_basis_norm_runtime_elimination_static_bucket_workspace_closure_first_20260513T150000Z"))
    ...
    p.add_argument("--basis-norm-warmup-steps", type=int, default=1)
