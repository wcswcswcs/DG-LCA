#!/usr/bin/env python3
"""DG-KAN v9.2.59 oracle-decomposed risk/support reset audit."""
...
PLAN_PATH = ROOT / "docs" / "DG-KAN_v9.2.59_OracleDecomposedRiskSupportReset_DeployableExactSignalController_完整实验计划.md"
SCRIPT_PATH = ROOT / "experiments" / "run_v9259_oracle_decomposed_risk_support_reset_deployable_exact_signal_controller.py"
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    _attach_v9259_statistics(fresh_rows)
    reference_bad = _f(p0.get("reference_bad_event"))

    p1_rows, p1 = _p1_factor_labels(fresh_rows)
    p2_rows, p2 = _p2_mode_specific_risk(fresh_rows, reference_bad)
    p3_rows, p3 = _p3_support_reset(fresh_rows, reference_bad)
    p4_rows, p4 = _p4_reference_feasibility_v3(fresh_rows, p2, p3)
    p5_rows, p5 = _p5_compute_v4(fresh_rows, sample, p1_resid, p2_correct)
    p6_rows, p6 = _p6_support_expansion_v9259(fresh_rows, p4)
    p7_rows, p7 = _p7_system_controller_v9259(p4, p5)

    if not _i(p0.get("v9258_boundary_pass")):
        route_name, blocker, failure_code, reason, next_required = "R0-V9258BoundaryUnstable", "v9258_boundary_unstable", "F2_v9258_boundary_unstable", "P0_v9258_boundary_failed", "reproduce_v9258_boundary"
    elif not _i(p1.get("factor_label_autopsy_pass")):
        route_name, blocker, failure_code, reason, next_required = "R1-FactorLabelAutopsyIncomplete", "factor_label_autopsy_incomplete", "F4_factor_label_invalid", "P1_factor_label_failed", "repair_oracle_factor_decomposition"
    elif not (_i(p2.get("mode_specific_risk_pass")) or _i(p2.get("mode_specific_risk_diagnostic_pass"))):
        if _i(p2.get("predictive_submode_count")) >= 3:
            route_name, blocker, failure_code, reason, next_required = "R11-RiskSupportStatsStillTinyCleanCore", "risk_union_gate_tiny_coverage", "F7_risk_union_gate_tiny_coverage", "P2_risk_union_gate_tiny_coverage", "reset_risk_support_target_decomposition"
        else:
            route_name, blocker, failure_code, reason, next_required = "R1-FactorLabelAutopsyPass", "risk_submode_not_predictive", "F6_risk_submode_not_predictive", "P2_mode_specific_risk_failed", "redesign_bad_event_mode_statistics"
    elif not (_i(p3.get("support_stat_pass")) or _i(p3.get("support_diagnostic_pass"))):
        route_name, blocker, failure_code, reason, next_required = "R2-ModeSpecificRiskPass", "support_family_no_oracle_overlap", "F8_support_family_no_oracle_overlap", "P3_support_reset_failed", "redesign_support_family_reset"
    elif not _i(p4.get("exact_reference_feasible")):
        if _f(p4.get("reference_controller_bad_event")) <= 0.05 and _f(p4.get("reference_controller_coverage")) < 0.03:
            route_name, blocker, failure_code, reason, next_required = "R11-RiskSupportStatsStillTinyCleanCore", "risk_support_clean_core_too_tiny", "F7_risk_union_gate_tiny_coverage", "P4_exact_reference_still_infeasible", "reset_risk_support_target_decomposition"
        else:
            route_name, blocker, failure_code, reason, next_required = "R10-ExactReferenceStillInfeasibleAfterTargetReset", "exact_reference_still_infeasible_after_target_reset", "F10_exact_reference_still_infeasible_after_target_reset", "P4_exact_reference_still_infeasible", "reset_risk_support_target_decomposition"
    ...
    downstream = _downstream_v9259(reason)
    artifacts: Dict[str, List[Dict[str, Any]]] = {
        "p0_v9258_boundary_reproduction.csv": [p0],
        "p1_oracle_decomposed_label_construction.csv": p1_rows,
        "p2_mode_specific_risk_statistics.csv": p2_rows,
        "p3_support_sufficient_statistics_reset.csv": p3_rows,
        "p4_exact_reference_feasibility_v3.csv": p4_rows,
        "p5_true_delta_compute_v4_parallel_lane.csv": p5_rows,
        "p6_online_support_stratum_expansion.csv": p6_rows,
        "p7_system_legal_exact_signal_controller.csv": p7_rows,
        **downstream,
        "factor_label_trace_v9259.csv": p1_rows,
        "risk_submode_trace_v9259.csv": p2_rows,
        "support_family_trace_v9259.csv": p3_rows,
        "reference_feasibility_v3_trace.csv": p4_rows,
        "true_delta_compute_v4_trace.csv": p5_rows,
        "accepted_region_geometry_trace_v9259.csv": p4_rows,
        "support_density_trace_v9259.csv": p6_rows,
        "leaveout_trace_v9259.csv": downstream["p8_leave_dataset_and_stratum_out.csv"],
        "paired_replay_branch_trace_v9259.csv": downstream["p9_official_paired_replay.csv"],
        "true_delta_residual_trace_v9259.csv": p1_resid_rows,
        "true_delta_correctness_trace_v9259.csv": p2_correct_rows,
    }
    ...
    write_csv_rows(out_dir / "v9259_provenance_audit.csv", [audit])

    route = {
        "route": route_name,
        "base_candidate": "LQ-t2-h256",
        "v9258_boundary_pass": p0.get("v9258_boundary_pass"),
        "dataset_tuning_detected": 0,
        "factor_label_autopsy_pass": p1.get("factor_label_autopsy_pass"),
        "fp_mode_attribution_fraction": p1.get("fp_mode_attribution_fraction"),
        "fn_mode_attribution_fraction": p1.get("fn_mode_attribution_fraction"),
        "bad_event_mode_attribution_fraction": p1.get("bad_event_mode_attribution_fraction"),
        "best_risk_stat_id": p2.get("best_risk_stat_id"),
        "mode_specific_risk_pass": p2.get("mode_specific_risk_pass"),
        "mode_specific_risk_diagnostic_pass": p2.get("mode_specific_risk_diagnostic_pass"),
        "risk_union_gate_bad_event": p2.get("risk_union_gate_bad_event"),
        "risk_union_gate_coverage": p2.get("risk_union_gate_coverage"),
        ...
        "success_v9259_strict_purekan_functional": 0,
        "success_v9259_full_functional": 0,
        "success_v9259_external_ready": 0,
    ...
    write_csv_rows(out_dir / "contract_audit_v9258.csv", [{
        ...
        "oracle_decomposed_factor_labels": 1,
        "mode_specific_risk_statistics": 1,
        "support_family_reset": 1,
        "exact_reference_feasibility_v3": 1,
    ...
    # Keep the required v9.2.59 audit filename distinct from prior runs.
    contract_rows = read_csv_rows(out_dir / "contract_audit_v9258.csv")
    write_csv_rows(out_dir / "contract_audit_v9259.csv", contract_rows)
    (out_dir / "contract_audit_v9258.csv").unlink(missing_ok=True)
    _figures_v9259(out_dir, route)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default=str(RESULT_ROOT / "v9259_oracle_decomposed_risk_support_reset_deployable_exact_signal_controller_first_20260512T123000Z"))
    ...
    p.add_argument("--microprobe-steps", type=int, default=224)
