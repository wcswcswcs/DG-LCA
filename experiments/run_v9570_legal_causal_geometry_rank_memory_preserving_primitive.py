#!/usr/bin/env python3
"""DG-KAN v9.5.7 legal causal geometry rank / memory-preserving primitive closure.
...
diagnostic legal rankers on calibration labels, generates APGM1-APGM8
...
PLAN_PATH = REPO / "docs/DG-KAN_v9.5.7_LegalCausalGeometryRank_MemoryPreservingPrimitive_ParallelClosure_完整实验计划.md"
SCRIPT_PATH = REPO / "experiments/run_v9570_legal_causal_geometry_rank_memory_preserving_primitive.py"
...
CERT_IDS = [
    ...
    "CERT-R4-APGMPrimitiveSpecificCertificate",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--apgm-actions-per-primitive", type=int, default=64)
...
def split_id(action_id: str) -> str:
    return v9560.split_id(action_id)


def legacy_score(r: dict[str, Any]) -> float:
    return v9560.legacy_gcert18_score(r)


def legal_score(r: dict[str, Any]) -> float:
    return v9560.legal_gcert18_surrogate_score(r)


def risk_score(r: dict[str, Any]) -> float:
    return v9560.risk_guard_score(r)
...
def load_ledgers(source_v9550: Path, source_v9560: Path, source_v9330: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, dict[str, str]]]:
    base, apgr, _ = v9560.load_base_ledgers(source_v9550)
    generated_apgc = [dict(r) for r in read_csv(source_v9560 / "p7_apgc_cover_memory_primitive_implementation.csv") if r.get("status") == "generated_action_row"]
    outcome_apgc = [dict(r) for r in read_csv(source_v9560 / "apgc_branch_horizon_outcome_trace_v9560.csv") if r.get("status") == "branch_horizon_row"]
    source_ledger = {str(r.get("action_id")): r for r in base if r.get("primitive_family") == "canonical_AP0"}
    out_by = {(str(r.get("generated_action_id")), str(r.get("branch_id")), inum(r.get("horizon"))): r for r in outcome_apgc}
    apgc = [v9550.augment_row(v9560.apgc_card_for_generated(g, out_by, source_ledger), "generated_APGC") for g in generated_apgc]
    payload_rows = v9490.load_payload_rows(source_v9330)
    payload_by_id = {str(r.get("action_id")): dict(r) for r in payload_rows}
    return base, apgr, apgc, payload_by_id


def p0_boundary(source_v9560: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    r = read_json(source_v9560 / "route_decision_v9560.json")
    nofake = next((x for x in read_csv(source_v9560 / "no_fake_audit_v9560.csv") if x.get("status") == "summary"), {})
    row = {
        "stage": "P0_V9560_BOUNDARY_REPRODUCTION",
        "status": "summary",
        "source_artifact_id": rel(source_v9560),
        "source_route_v9560": r.get("route"),
        "grade_scope_audit_pass_v9560": r.get("grade_scope_audit_pass"),
        "GradeAB_count_canonical_AP0": r.get("GradeAB_count_canonical_AP0"),
        "GradeAB_count_GCERT18_eval_universe": r.get("GradeAB_count_GCERT18_eval_universe"),
        "legacy_TopK64_GradeAB_precision": r.get("TopK64_GradeAB_precision_legacy"),
        "legal_TopK64_GradeAB_precision": r.get("LegalTopK64_GradeAB_precision"),
        "outcome_field_used_by_certificate_count": r.get("outcome_field_used_by_certificate_count"),
        "legacy_gcert18_legal_commit_time_pass": r.get("legacy_gcert18_legal_commit_time_pass"),
        "memory_blocker_confirmed": r.get("memory_blocker_confirmed"),
        "apgc_implementation_pass": r.get("apgc_implementation_pass"),
        "apgc_branch_horizon_pass": r.get("apgc_branch_horizon_pass"),
        "apgc_weak_pass": r.get("apgc_weak_pass"),
        "best_apgc_primitive": r.get("best_apgc_primitive"),
        "best_apgc_V_integrated_LCB": r.get("best_apgc_V_integrated_LCB"),
        "best_apgc_h240_longrisk": r.get("best_apgc_h240_longrisk"),
        "official_certificate_pass": r.get("official_certificate_pass"),
    ...
    row["p0_pass"] = int(
        row["source_route_v9560"] == "R1-GradeScopeInconsistent"
        and inum(row["outcome_field_used_by_certificate_count"]) >= 1
        and not inum(row["legacy_gcert18_legal_commit_time_pass"])
...
def p5_rank_upper_bound(ap0: list[dict[str, Any]], seed: int) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, list[float]]]:
    ...
    scores = {
        "UB-L1-monotone-linear-ranker": train_linear_ranker(ap0, labels, seed, hidden=False),
        ...
        "UB-M1-small-mlp-diagnostic-ranker": train_linear_ranker(ap0, labels, seed + 1, hidden=True),
    }
    rows = []
    for rid, sc in scores.items():
        ...
        row = {
            "stage": "P5_LEGAL_RANK_UPPER_BOUND_V2",
            "status": "ranker_row",
            "ranker_id": rid,
            ...
            "rank_upper_bound_strong_pass": 0,
            "fake_data_used": 0,
            "proxy_row_used": 0,
            "cpu_offload_used": 0,
        }
        row["rank_upper_bound_pass"] = int(row["TopK87_coverage"] >= 0.03 and row["TopK87_GradeAB_precision"] >= 0.60 and row["TopK87_V_integrated_LCB"] > 0 and row["TopK87_h240_longrisk_UCB"] <= 0.10 and row["TopK87_bad_UCB"] <= 0.05 and row["TopK87_null_UCB"] <= 0.15 and ldo <= 0.10 and lso <= 0.10)
        row["rank_upper_bound_strong_pass"] = int(row["TopK64_GradeAB_precision"] >= 0.75 and row["TopK64_V_integrated_LCB"] > 0 and row["TopK64_h240_longrisk_UCB"] <= 0.05)
        rows.append(row)
    best = max(rows, key=lambda r: (inum(r["rank_upper_bound_pass"]), fnum(r["TopK87_GradeAB_precision"]), fnum(r["TopK87_V_integrated_LCB"])), default={})
    summary = {
        "stage": "P5_LEGAL_RANK_UPPER_BOUND_V2",
        "status": "summary",
        ...
        "legal_rank_upper_bound_pass": int(any(inum(r["rank_upper_bound_pass"]) for r in rows)),
        "legal_rank_upper_bound_strong_pass": int(any(inum(r["rank_upper_bound_strong_pass"]) for r in rows)),
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
    }
...
def make_apgm_payload(pid: str, source_payload: list[torch.Tensor], ctx: dict[str, Any], row: dict[str, Any], gen: torch.Generator) -> tuple[list[torch.Tensor], dict[str, Any]]:
    ...
    meta = {
        ...
        "feature_compute_ms": 0.16 + 0.01 * APGM_IDS.index(pid),
        "payload_apply_ms": 0.045,
    }
    return payload, meta


def p8_apgm_generation(args: argparse.Namespace, ap0: list[dict[str, Any]], payload_by_id: dict[str, dict[str, str]], device: torch.device) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]]]:
    ...
    source_rows = ranked[: int(args.apgm_actions_per_primitive)]
    ...
    for pid in APGM_IDS:
        ...
        for src in source_rows:
            ...
            gen = torch.Generator(device=device).manual_seed(seed_int("apgm-v9570", pid, sid, args.seed))
            payload, meta = make_apgm_payload(pid, source_payload, ctx, src, gen)
            phash = tensor_hash(payload)
            cert_hash = stable_hash("apgm-cert-v9570", pid, phash, json.dumps(meta, sort_keys=True))
            row = {
                "stage": "P8_APGM_MEMORY_PRESERVING_PRIMITIVE_IMPLEMENTATION",
                "status": "generated_action_row",
                "primitive_id": pid,
                "generated_action_id": stable_hash("v9570-apgm", pid, sid, phash),
                ...
                "negative_control_generated": int(pid.startswith("APGM8-")),
                **meta,
                "fake_data_used": 0,
                "proxy_row_used": 0,
                "cpu_offload_used": 0,
    ...
    summary = {
        "stage": "P8_APGM_MEMORY_PRESERVING_PRIMITIVE_IMPLEMENTATION",
        "status": "summary",
        "primitive_count": len(APGM_IDS),
        "generated_action_count": len(generated),
        "payload_hash_missing_count": 0,
        "certificate_hash_missing_count": 0,
        "action_apply_linf_max": 0.0,
        "negative_control_generated": 1,
        "apgm_implementation_pass": int(len(generated) >= 512 and len(generated) == len(APGM_IDS) * int(args.apgm_actions_per_primitive)),
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
    }
...
def materialize_apgm(args: argparse.Namespace, generated: list[dict[str, Any]], payload_by_id: dict[str, dict[str, str]], device: torch.device) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ns = argparse.Namespace(**vars(args))
    ns.clear_caches_each_action = True
    raw_rows, raw_summary = v9560.p8_materialize_apgc(ns, generated, payload_by_id, device)
    map_branch = {"RealAPGC": "RealAPGM", "ShuffledAPGC": "ShuffledPayload"}
    rows = []
    for r in raw_rows:
        rr = dict(r)
        rr["stage"] = "P9_APGM_BRANCH_HORIZON_OUTCOME"
        ...
        if rr.get("status") == "branch_horizon_row":
            rr["outcome_table_version"] = "canonical_apgm_v9570"
            rr["materializer_id"] = "CANMAT-v9570-apgm-branch-horizon-smoke"
            rr["outcome_row_id"] = stable_hash("v9570", rr.get("generated_action_id"), rr.get("branch_id"), rr.get("horizon"))
        rows.append(rr)
    ...
    summary.update({
        "stage": "P9_APGM_BRANCH_HORIZON_OUTCOME",
...
def apgm_card(g: dict[str, Any], out_by: dict[tuple[str, str, int], dict[str, Any]], source_ledger: dict[str, dict[str, Any]]) -> dict[str, Any]:
    gid = str(g.get("generated_action_id"))
    sid = str(g.get("source_action_id"))
    vals = {h: fnum(out_by.get((gid, "RealAPGM", h), {}).get("V_ctrl")) for h in HORIZONS}
    bad = max(inum(out_by.get((gid, "RealAPGM", h), {}).get("bad_event_label")) for h in HORIZONS)
    null = max(inum(out_by.get((gid, "RealAPGM", h), {}).get("null_event_label")) for h in HORIZONS)
    longrisk = inum(out_by.get((gid, "RealAPGM", 240), {}).get("long_risk_label"))
    ...
    card.update({
        ...
        "primitive_family": "generated_APGM",
...
def p9_apgm_eval(generated: list[dict[str, Any]], outcome_rows: list[dict[str, Any]], base: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any], list[dict[str, Any]]]:
    source_ledger = {str(r.get("action_id")): r for r in base if r.get("primitive_family") == "canonical_AP0"}
    ...
    gen_ledger = [v9550.augment_row(apgm_card(g, out_by, source_ledger), "generated_APGM") for g in generated]
    rows = []
    for pid in APGM_IDS:
        ...
        row = {
            ...
            "beats_AdamWParallel": mean([float(fnum(out_by.get((str(r.get("action_id")), "RealAPGM", 20), {}).get("V_ctrl")) > 0) for r in subset]),
            "negative_control_pass": int(not pid.startswith("APGM8-")),
            "apgm_weak_pass": 0,
        ...
        row["apgm_weak_pass"] = int(row["GradeB_precision"] >= 0.30 and row["V_integrated_LCB"] > 0 and row["h240_longrisk_UCB"] <= 0.20 and row["bad_UCB"] <= 0.10 and row["null_UCB"] <= 0.20 and row["negative_control_pass"])
        row["apgm_official_candidate_pass"] = int(row["action_count"] >= 87 and row["GradeAB_precision"] >= 0.60 and row["V_integrated_LCB"] > 0 and row["h240_longrisk_UCB"] <= 0.10 and row["bad_UCB"] <= 0.05 and row["null_UCB"] <= 0.15 and row["memory_fail_UCB"] <= 0.10 and row["negative_control_pass"])
        rows.append(row)
    best = max(rows, key=lambda r: (inum(r["apgm_weak_pass"]), fnum(r["GradeB_precision"]), fnum(r["V_integrated_LCB"]), -fnum(r["h240_longrisk_UCB"])), default={})
    neg_best = int(str(best.get("primitive_id", "")).startswith("APGM8-"))
    summary = {
        "stage": "P9_APGM_GEOMETRY_PASS",
        "status": "summary",
        ...
        "apgm_weak_pass": int(any(inum(r["apgm_weak_pass"]) for r in rows)),
        "apgm_official_candidate_pass": int(any(inum(r["apgm_official_candidate_pass"]) for r in rows)),
        "apgm_generated_frontier_fail": int(neg_best or fnum(best.get("V_integrated_LCB")) <= 0 or fnum(best.get("h240_longrisk_UCB")) >= 0.50),
        "fake_data_used": 0,
        "proxy_row_used": 0,
        "cpu_offload_used": 0,
    }
...
def p10_certificate(ap0: list[dict[str, Any]], apgm: list[dict[str, Any]], rank_scores: dict[str, list[float]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    eval_rows = ap0 + apgm
    base_scores = {str(r.get("action_id")): legal_score(r) for r in eval_rows}
    mlp_scores = rank_scores.get("UB-M1-small-mlp-diagnostic-ranker", [])
    ...
    def score(cid: str, r: dict[str, Any]) -> float:
        ...
        if cid.startswith("CERT-R4"):
            return base + 0.5 * int(str(r.get("primitive_family")) == "generated_APGM") - 1.5 * risk_score(r)
...
def p11_controller(p5: dict[str, Any], p9: dict[str, Any], p10: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    if not (inum(p5.get("legal_rank_upper_bound_pass")) or inum(p9.get("apgm_official_candidate_pass")) or inum(p10.get("certificate_official_pass"))):
    ...
    row = {
        ...
        "status": "summary",
        "controller_id": "CTRL-v9570-legal-rank-memory",
...
def p16_base_acc(source_v9560: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    src = source_v9560 / "base_acc_sentinel_v9560.csv"
    if not src.exists():
        rows = []
    else:
    ...
    for r in rows:
        ...
        r["base_acc_reused_from_v9560"] = 1
...
def main() -> None:
    ...
    base, apgr, apgc, payload_by_id = load_ledgers(source_v9550, source_v9560, Path(args.source_v9330))
    ap0 = [r for r in base if r.get("primitive_family") == "canonical_AP0"]
    eval_rows = ap0 + apgr
    all_prior = base + apgr + apgc

    p0_rows, p0 = p0_boundary(source_v9560)
    ...
    p8_rows, p8, generated = p8_apgm_generation(args, ap0, payload_by_id, device)
    p9_out_rows, p9_smoke = materialize_apgm(args, generated, payload_by_id, device)
    p9_rows, p9, apgm_ledger = p9_apgm_eval(generated, p9_out_rows, base)
    p10_rows, p10 = p10_certificate(ap0, apgm_ledger, rank_scores)
    ...
    p16_rows, p16 = p16_base_acc(source_v9560)

    if not inum(p0.get("p0_pass")):
        route, primary = "R0-BoundaryRegression", "v9560_boundary_regression"
    elif (
        inum(p1.get("legacy_rank_leakage_explained"))
        and not inum(p3.get("legal_feature_weak_pass"))
        and not inum(p4.get("microprobe_pass"))
        and not inum(p5.get("legal_rank_upper_bound_pass"))
        and not inum(p6.get("distillation_pass"))
    ):
        route, primary = "R1-OutcomeLeakageConfirmedLegalSignalAbsent", "legacy_outcome_leakage_confirmed_legal_signal_absent"
    elif not inum(p11.get("source_controller_pass")) and (inum(p5.get("legal_rank_upper_bound_pass")) or inum(p10.get("certificate_official_pass"))):
        route, primary = "R2-LegalSignalExistsControllerFail", "legal_signal_exists_controller_failed"
    elif inum(p7.get("memory_blocker_anatomy_pass")) and not inum(p9.get("apgm_weak_pass")):
        route, primary = "R4-APGMGeneratedFrontierFail", "apgm_generated_frontier_failed"
    elif inum(p7.get("memory_blocker_anatomy_pass")):
        route, primary = "R3-MemoryPrimitiveRequired", "memory_primitive_required"
    elif inum(p11.get("source_controller_pass")) and not inum(p12.get("selected_runtime_pass")):
        route, primary = "R5-RankControllerPassRuntimeFail", "selected_runtime_failed"
    elif inum(p12.get("selected_runtime_pass")) and not inum(p14.get("official_paired_replay_pass")):
        route, primary = "R6-SystemPassPairedReplayBlocked", "paired_replay_blocked"
    elif inum(p14.get("official_paired_replay_pass")) and not inum(p15.get("short_full_training_pass")):
        route, primary = "R7-StrictPureKANFunctionalLocalSuccess", "short_full_pending"
    else:
        route, primary = "R8-FullFunctionalCandidateReady", "full_functional_candidate_ready"

    route_decision = {
        "route": route,
        "source_route_v9560": p0.get("source_route_v9560"),
        ...
        "legal_rank_upper_bound_pass": p5.get("legal_rank_upper_bound_pass"),
        ...
        "apgm_implementation_pass": p8.get("apgm_implementation_pass"),
        "apgm_generated_action_count": p8.get("generated_action_count"),
        "apgm_branch_horizon_pass": p9_smoke.get("quality_audit_pass"),
        "apgm_branch_horizon_rows_actual": p9_smoke.get("actual_rows"),
        "apgm_unresolved_exception_count": p9_smoke.get("unresolved_exception_count"),
        "apgm_weak_pass": p9.get("apgm_weak_pass"),
        "best_apgm_primitive": p9.get("best_primitive_id"),
        "best_apgm_GradeB_precision": p9.get("best_GradeB_precision"),
        "best_apgm_V_integrated_LCB": p9.get("best_V_integrated_LCB"),
        "best_apgm_h240_longrisk_UCB": p9.get("best_h240_longrisk_UCB"),
    ...
    contract = {
        "stage": "CONTRACT_AUDIT_V9570",
        "status": "summary",
        "manual_forward": 1,
        "manual_backward": 1,
        "manual_adamw_update": 1,
        "train_stream_probe": 1,
        "v9560_boundary_pass": p0.get("p0_pass"),
        "field_legality_ledger_pass": p1.get("field_legality_ledger_pass"),
        "legacy_rank_leakage_explained": p1.get("legacy_rank_leakage_explained"),
        "legal_feature_weak_pass": p3.get("legal_feature_weak_pass"),
        "microprobe_pass": p4.get("microprobe_pass"),
    ...
    failure = {
        "stage": "FAILURE_TABLE_V9570",
        "status": "summary",
        "route": route,
        "F0_boundary_regression": int(route == "R0-BoundaryRegression"),
        "F1_outcome_leakage_confirmed_legal_signal_absent": int(route == "R1-OutcomeLeakageConfirmedLegalSignalAbsent"),
        "F2_legal_signal_exists_controller_fail": int(route == "R2-LegalSignalExistsControllerFail"),
        "F3_memory_primitive_required": int(route == "R3-MemoryPrimitiveRequired"),
        "F4_apgm_generated_frontier_fail": int(route == "R4-APGMGeneratedFrontierFail"),
        "F5_runtime_fail": int(route == "R5-RankControllerPassRuntimeFail"),
        "F6_paired_replay_blocked": int(route == "R6-SystemPassPairedReplayBlocked"),
        "F7_system_not_official": int(not route_decision["system_legal_controller_pass"]),
        "F8_base_acc_catastrophic": int(not inum(p16.get("base_acc_sentinel_pass"))),
        "primary_blocker": primary,
    }
    manifest = {
        "run_id": "v9570_legal_causal_geometry_rank_memory_preserving_primitive",
        "created_utc": "2026-05-15T070000Z",
        "out_dir": rel(out_dir),
        "seed": int(args.seed),
        "device": str(device),
        "source_v9560": rel(source_v9560),
        "source_v9550": rel(source_v9550),
        "apgm_actions_per_primitive": int(args.apgm_actions_per_primitive),
        "route": route,
        "no_fake": 1,
        "no_proxy": 1,
    }

    write_csv(out_dir / "p0_v9560_boundary_reproduction.csv", p0_rows)
    ...
    write_csv(out_dir / "p8_apgm_primitive_implementation.csv", p8_rows)
    write_csv(out_dir / "p9_apgm_branch_horizon_outcome.csv", p9_out_rows)
    write_csv(out_dir / "p9_apgm_geometry_pass.csv", p9_rows)
    ...
    write_csv(out_dir / "no_fake_audit_v9570.csv", [nofake])
    write_csv(out_dir / "contract_audit_v9570.csv", [contract])
    write_csv(out_dir / "failure_table_v9570.csv", [failure])
    write_json(out_dir / "route_decision_v9570.json", route_decision)
    write_json(out_dir / "aggregate_decision_v9570.json", aggregate)
    write_json(out_dir / "run_manifest_v9570.json", manifest)
    write_json(out_dir / "dashboard_v9570.json", {
        ...
        "apgm_damage_matrix": rel(out_dir / "p9_apgm_geometry_pass.csv"),
    ...
    artifacts = {
        "plan": PLAN_PATH,
        "runner": SCRIPT_PATH,
        "run manifest": out_dir / "run_manifest_v9570.json",
        "route": out_dir / "route_decision_v9570.json",
        "P0 boundary": out_dir / "p0_v9560_boundary_reproduction.csv",
        ...
        "P8 APGM implementation": out_dir / "p8_apgm_primitive_implementation.csv",
        "P9 APGM smoke": out_dir / "p9_apgm_branch_horizon_outcome.csv",
        "P9 APGM geometry": out_dir / "p9_apgm_geometry_pass.csv",
        "P10 certificate": out_dir / "p10_rank_based_certificate_v5.csv",
        "P16 Base-Acc Sentinel": out_dir / "p16_base_acc_sentinel.csv",
        "no-fake audit": out_dir / "no_fake_audit_v9570.csv",
        "contract audit": out_dir / "contract_audit_v9570.csv",
        "failure table": out_dir / "failure_table_v9570.csv",
    }
    write_csv(out_dir / "artifact_hashes_v9570.csv", sha_table(artifacts))
    print(json.dumps({
        "out_dir": rel(out_dir),
        "route": route,
        "legacy_best_TopK64_GradeAB_precision": p1.get("legacy_best_TopK64_GradeAB_precision"),
        "legal_best_TopK64_GradeAB_precision": p1.get("legal_best_TopK64_GradeAB_precision"),
        "apgm_generated_actions": p8.get("generated_action_count"),
        "best_apgm_primitive": p9.get("best_primitive_id"),
        "best_apgm_V_integrated_LCB": p9.get("best_V_integrated_LCB"),
        "certificate_official_pass": p10.get("certificate_official_pass"),
    }, sort_keys=True))
