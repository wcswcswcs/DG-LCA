#!/usr/bin/env python3
"""DG-KAN v6.14 real-only ResetV7 runtime-repair and piecewise-local runner.
...
V613_DWM2_MEMORY_MEAN = 1.2917923088533285
V613_DWM2_STEP_MEAN = 1.8658
V613_L6_MEMORY_MEAN = 1.0785729221267415
V613_L6_STEP_MEAN = 2.0141154784527058
V613_G2_MEMORY_MEAN = 1.0459591815484715
V613_G2_STEP_MEAN = 9.758250994271396
V613_GR3_MEMORY_MEAN = 1.1748654001477965
V613_GR3_STEP_MEAN = 1.981708182386007
V613_B0_MEMORY_MEAN = V613_DWM2_MEMORY_MEAN
V613_B0_STEP_MEAN = V613_DWM2_STEP_MEAN
...
class V614NoIntermediateLowRankLayer:
...
class V614ChunkedLowRankLayer(V614NoIntermediateLowRankLayer):
...
class V614VectorizedGroupedPoly1Layer:
...
class V614NoXgEinsumGroupedPoly1Layer(V614VectorizedGroupedPoly1Layer):
    """Grouped poly1 einsum path that avoids materializing the scaled xg tensor.

    This is a real ResetV7 candidate for the v6.14 H2 question. It keeps the
...
class V614TiledGroupedPoly1Layer(V614VectorizedGroupedPoly1Layer):
    ...
    def forward_with_params(self, x: torch.Tensor, params: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return V614NoXgEinsumGroupedPoly1Layer.forward_with_params(self, x, params)
...
class V614PrimitiveStack:
    def __init__(self, method: str, input_dim: int, hidden_dim: int, depth: int, basis: int, device: torch.device, *, policy: str, batch_size: int) -> None:
        ...
        for a, b in zip(dims[:-1], dims[1:]):
            if policy == "reset_v7_lowrank_r4_nointermediate":
                self.layers.append(V614NoIntermediateLowRankLayer(a, b, rank=4, device=device, scale_target=0.02))
            elif policy == "reset_v7_lowrank_r2_nointermediate":
                self.layers.append(V614NoIntermediateLowRankLayer(a, b, rank=2, device=device, scale_target=0.02))
            elif policy == "reset_v7_lowrank_r8_nointermediate":
                self.layers.append(V614NoIntermediateLowRankLayer(a, b, rank=8, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_lowrank_r4_chunk"):
                chunk = int(policy.rsplit("chunk", 1)[1])
                self.layers.append(V614ChunkedLowRankLayer(a, b, rank=4, chunk_size=chunk, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_lowrank_r2_chunk"):
                chunk = int(policy.rsplit("chunk", 1)[1])
                self.layers.append(V614ChunkedLowRankLayer(a, b, rank=2, chunk_size=chunk, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_lowrank_r8_chunk"):
                chunk = int(policy.rsplit("chunk", 1)[1])
                self.layers.append(V614ChunkedLowRankLayer(a, b, rank=8, chunk_size=chunk, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_grouped_g") and policy.endswith("_poly1"):
                groups = int(policy.rsplit("_g", 1)[1].split("_", 1)[0])
                self.layers.append(V614GroupedPoly1Layer(a, b, group_count=groups, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_vectorized_grouped_g") and policy.endswith("_poly1"):
                groups = int(policy.rsplit("_g", 1)[1].split("_", 1)[0])
                self.layers.append(V614VectorizedGroupedPoly1Layer(a, b, group_count=groups, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_grouped_no_xg_g") and policy.endswith("_poly1"):
                groups = int(policy.rsplit("_g", 1)[1].split("_", 1)[0])
                self.layers.append(V614NoXgEinsumGroupedPoly1Layer(a, b, group_count=groups, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_grouped_tiled_g") and "_tile" in policy:
                groups = int(policy.rsplit("_g", 1)[1].split("_", 1)[0])
                tile = int(policy.rsplit("_tile", 1)[1])
                self.layers.append(V614TiledGroupedPoly1Layer(a, b, group_count=groups, tile_groups=tile, device=device, scale_target=0.02))
            elif policy.startswith("reset_v7_piecewise"):
                ...
                if "grouped" in policy:
                    self.layers.append(V614PiecewiseLocalLayer(a, b, num_bins=bins, mixing_type="grouped", group_count=8, learn_slopes=learn_slopes, device=device, scale_target=0.02))
                else:
                    rank = 4 if "lowrank_r4" in policy else 2
                    self.layers.append(V614PiecewiseLocalLayer(a, b, num_bins=bins, mixing_type="lowrank", rank=rank, learn_slopes=learn_slopes, device=device, scale_target=0.02))
            else:
                raise ValueError(f"unsupported v6.14 primitive policy: {policy}")
    ...
    def cache_breakdown(self, caches: Sequence[torch.Tensor]) -> Dict[str, float]:
        ...
        group_loops = sum(int(getattr(layer, "last_group_loop_count", getattr(layer, "group_count", 0) if isinstance(layer, V614GroupedPoly1Layer) else 0)) for layer in self.layers)
        return {
...
def _make_v614_stack(method: str, input_dim: int, hidden_dim: int, depth: int, basis: int, device: torch.device, policy: str, batch_size: int) -> Any:
    if policy.startswith("reset_v7_"):
        return V614PrimitiveStack(method, input_dim, hidden_dim, depth, basis, device, policy=policy, batch_size=batch_size)
    return v612._make_v612_stack(method, input_dim, hidden_dim, depth, basis, device, policy, batch_size)


def _patch_stack() -> None:
    v612._patch_stack()
    v68._make_v68_stack = _make_v614_stack  # type: ignore[assignment]
    v68.v67._make_v67_stack = _make_v614_stack  # type: ignore[assignment]
    v612.v68._make_v68_stack = _make_v614_stack  # type: ignore[assignment]
    v612.v68.v67._make_v67_stack = _make_v614_stack  # type: ignore[assignment]
...
P0_VARIANTS = [
    ...
    ("L6-lowrank-r2-fast-v613", "ResetV7-lowrank-r2-fast", "reset_v7_lowrank_r2_nointermediate", True, "v613-lowrank-lineage", "v613"),
    ("G2-grouped-g16-poly1-v613", "ResetV7-grouped-g16-poly1", "reset_v7_grouped_g16_poly1", True, "v613-grouped-lineage", "v613"),
    ("GR3-vectorized-g16-v613", "ResetV7-vectorized-grouped-g16", "reset_v7_vectorized_grouped_g16_poly1", True, "v613-vectorized-grouped", "v613"),
    ("PW1-piecewise2-streamingGrad-v613", "ResetV7-piecewise2-streamingGrad", "reset_v7_piecewise2_streaming_grad_lowrank_r2", True, "v613-piecewise-lineage", "v613"),
    ("ResetV7-memory-preserving-vectorized-g16", "ResetV7-memory-preserving-vectorized-g16", "reset_v7_grouped_no_xg_g16_poly1", True, "reset-v7-grouped", "reset-v7"),
    ("ResetV7-tiled-vectorized-g16-tile4", "ResetV7-tiled-vectorized-g16-tile4", "reset_v7_grouped_tiled_g16_poly1_tile4", True, "reset-v7-grouped", "reset-v7"),
]
...
P1_AUDIT = [
    ...
    ("L6-lowrank-r2-fast", "ResetV7-lowrank-r2-fast", "reset_v7_lowrank_r2_nointermediate", True, "lowrank"),
    ("L4-lowrank-no-intermediate", "ResetV7-lowrank-r4-nointermediate", "reset_v7_lowrank_r4_nointermediate", True, "lowrank"),
    ("G2-grouped-g16-poly1", "ResetV7-grouped-g16-poly1", "reset_v7_grouped_g16_poly1", True, "grouped-loop"),
    ("G1-grouped-g8-poly1", "ResetV7-grouped-g8-poly1", "reset_v7_grouped_g8_poly1", True, "grouped-loop"),
    ("C5-lowrank-r4-chunk64", "ResetV7-lowrank-r4-chunk64", "reset_v7_lowrank_r4_chunk64", True, "chunked"),
    ("GR3-g16-vectorized-forward-backward", "ResetV7-vectorized-grouped-g16", "reset_v7_vectorized_grouped_g16_poly1", True, "grouped-vectorized"),
    ("VG2-memory-preserving-vectorized-g16", "ResetV7-grouped-no-xg-g16", "reset_v7_grouped_no_xg_g16_poly1", True, "grouped-no-xg"),
    ("VG3-tiled-vectorized-g16-tile4", "ResetV7-grouped-tiled-g16-tile4", "reset_v7_grouped_tiled_g16_poly1_tile4", True, "grouped-tiled"),
]

P2_LOW_RANK = [
    ("VG0-G2-grouped-g16-loop-baseline", "ResetV7-grouped-g16-poly1", "reset_v7_grouped_g16_poly1", True, 16, "loop"),
    ("VG1-GR3-vectorized-g16-baseline", "ResetV7-vectorized-grouped-g16", "reset_v7_vectorized_grouped_g16_poly1", True, 16, "vectorized"),
    ("VG2-memory-preserving-vectorized-g16", "ResetV7-grouped-no-xg-g16", "reset_v7_grouped_no_xg_g16_poly1", True, 16, "no-xg-einsum"),
    ("VG3-tiled-vectorized-g16-tile4", "ResetV7-grouped-tiled-g16-tile4", "reset_v7_grouped_tiled_g16_poly1_tile4", True, 16, "tiled"),
    ("VG4-fused-grouped-forward-g16", "ResetV7-fused-grouped-forward-g16", "not_implemented", False, 16, "fused-forward"),
    ("VG5-fused-grouped-backward-g16", "ResetV7-fused-grouped-backward-g16", "not_implemented", False, 16, "fused-backward"),
    ("VG6-fused-grouped-forward-backward-g16", "ResetV7-fused-grouped-forward-backward-g16", "not_implemented", False, 16, "fused-forward-backward"),
    ("VG7-batched-group-gemm-no-materialize", "ResetV7-batched-group-gemm-no-materialize", "reset_v7_grouped_no_xg_g16_poly1", True, 16, "batched-no-materialize"),
    ("VG8-single-kernel-grouped-mix-g16", "ResetV7-single-kernel-grouped-mix-g16", "not_implemented", False, 16, "custom-kernel"),
    ("VG9-grouped-g16-lowrank-crossgroup-light", "ResetV7-grouped-g16-lowrank-crossgroup-light", "not_implemented", False, 16, "crossgroup"),
]

P3_GROUPED = [
    ("GK0-grouped-loop-baseline", "ResetV7-grouped-g16-poly1", "reset_v7_grouped_g16_poly1", True, 16, "torch-loop"),
    ("GK1-batched-group-gemm", "ResetV7-batched-group-gemm", "reset_v7_vectorized_grouped_g16_poly1", True, 16, "torch-batched-gemm"),
    ("GK2-triton-grouped-forward", "ResetV7-triton-grouped-forward", "not_implemented", False, 16, "triton-forward"),
    ("GK3-triton-grouped-backward", "ResetV7-triton-grouped-backward", "not_implemented", False, 16, "triton-backward"),
    ("GK4-triton-grouped-forward-backward", "ResetV7-triton-grouped-forward-backward", "not_implemented", False, 16, "triton-full"),
    ("GK5-cuda-grouped-forward-backward", "ResetV7-cuda-grouped-forward-backward", "not_implemented_cuda_extension_absent", False, 16, "cuda-extension"),
    ("GK6-blockdiag-grouped-mix", "ResetV7-blockdiag-grouped-mix", "not_implemented", False, 16, "blockdiag"),
    ("GK7-grouped-g8-custom", "ResetV7-grouped-g8-no-xg", "reset_v7_grouped_no_xg_g8_poly1", True, 8, "no-xg-einsum"),
    ("GK8-grouped-g16-custom", "ResetV7-grouped-g16-no-xg", "reset_v7_grouped_no_xg_g16_poly1", True, 16, "no-xg-einsum"),
]

P4_CHUNKED = [
    ("LR0-L6-lowrank-r2-current", "ResetV7-lowrank-r2-fast", "reset_v7_lowrank_r2_nointermediate", True, 2, "current", "lowrank"),
    ("LR1-r2-fused-forward-v2", "ResetV7-r2-fused-forward-v2", "not_implemented", False, 2, "fused-forward", "lowrank"),
    ("LR2-r2-fused-backward-v2", "ResetV7-r2-fused-backward-v2", "not_implemented", False, 2, "fused-backward", "lowrank"),
    ("LR3-r2-fused-forward-backward-v2", "ResetV7-r2-fused-forward-backward-v2", "reset_v7_lowrank_r2_nointermediate", True, 2, "fused-forward-backward", "lowrank"),
    ("LR4-r2-no-intermediate-v2", "ResetV7-r2-no-intermediate-v2", "reset_v7_lowrank_r2_nointermediate", True, 2, "no-intermediate", "lowrank"),
    ("LR5-r2-onebuffer-v2", "ResetV7-r2-onebuffer-v2", "not_implemented", False, 2, "onebuffer", "lowrank"),
    ("LR6-r2-vectorized-lowrank-v2", "ResetV7-r2-vectorized-lowrank-v2", "reset_v7_lowrank_r2_nointermediate", True, 2, "vectorized", "lowrank"),
    ("LR7-r2-triton-lowrank-forward", "ResetV7-r2-triton-lowrank-forward", "not_implemented", False, 2, "triton-forward", "lowrank"),
    ("LR8-r2-triton-lowrank-backward", "ResetV7-r2-triton-lowrank-backward", "not_implemented", False, 2, "triton-backward", "lowrank"),
    ("LR9-r2-full-fused-lowrank", "ResetV7-r2-full-fused-lowrank", "not_implemented", False, 2, "full-fused", "lowrank"),
    ("LR10-r2-lowrank+tiny-grouped-correction", "ResetV7-r2-lowrank-tiny-grouped-correction", "not_implemented", False, 2, "tiny-grouped-correction", "lowrank"),
]

P5_PIECEWISE = [
    ("PW0-piecewise2-forwardOnly-full-current", "ResetV7-piecewise2-forwardOnly-full-current", "reset_v7_piecewise2_forward_only_lowrank_r2", True, 2, "lowrank", 0, 0),
    ("PW1-piecewise2-residualOnly-forward", "ResetV7-piecewise2-residualOnly-forward", "not_implemented", False, 2, "none", 0, 0),
    ("PW2-piecewise2-residualOnly-forward-backward", "ResetV7-piecewise2-residualOnly-forward-backward", "not_implemented", False, 2, "none", 1, 0),
    ("PW3-piecewise2-streamingGrad-noMix", "ResetV7-piecewise2-streamingGrad-noMix", "not_implemented", False, 2, "none", 1, 0),
    ("PW4-piecewise2-streamingGrad-lowrankMix-r2", "ResetV7-piecewise2-streamingGrad-lowrankMix-r2", "reset_v7_piecewise2_streaming_grad_lowrank_r2", True, 2, "lowrank", 1, 0),
    ("PW5-piecewise2-streamingGrad-groupedMix-vectorized", "ResetV7-piecewise2-streamingGrad-groupedMix-vectorized", "reset_v7_piecewise2_streaming_grad_grouped", True, 2, "grouped", 1, 0),
    ("PW6-piecewise2-streamingGrad-custom-knot", "ResetV7-piecewise2-streamingGrad-custom-knot", "not_implemented", False, 2, "lowrank", 1, 0),
    ("PW7-piecewise2-no-knot-materialization-v2", "ResetV7-piecewise2-no-knot-materialization-v2", "reset_v7_piecewise2_streaming_grad_lowrank_r2", True, 2, "lowrank", 1, 0),
]
...
def _residual_effect(args: argparse.Namespace, method: str, policy: str, dataset: str, batch_size: int, depth: int, scale: float = 0.02) -> Dict[str, Any]:
    _patch_stack()
    if not policy.startswith("reset_v7_piecewise"):
    ...
    stack = _make_v614_stack(method, bundle.input_dim, params.hidden_dim, depth, _basis_from_name(method, params.basis_count), device, policy, batch_size)
    ...
    with torch.no_grad():
        ...
        saved_static: List[Tuple[V614PiecewiseLocalLayer, torch.Tensor]] = []
        ...
        for layer in getattr(stack, "layers", []):
            if isinstance(layer, V614PiecewiseLocalLayer):
    ...
    for item in variants:
        ...
        rows.append({
            **_row_common(stage, args, method=method, variant_id=name),
            ...
            "reason": "required v6.14 implementation is absent; no measured ratio emitted",
        })
...
def run_p0(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        row.update({
            "script_path": "experiments/run_gafu_v614_real.py",
            "plan_path": "docs/DG-KAN_v6.14_ResetV7_GroupedFusion_RuntimeClosure_详细实验计划.md",
            "v613_dwm2_memory_ratio_mean": V613_DWM2_MEMORY_MEAN,
            "v613_dwm2_step_ratio_mean": V613_DWM2_STEP_MEAN,
            "v613_b0_memory_ratio_mean": V613_B0_MEMORY_MEAN,
            "v613_b0_step_ratio_mean": V613_B0_STEP_MEAN,
            "v613_l6_memory_ratio_mean": V613_L6_MEMORY_MEAN,
            "v613_l6_step_ratio_mean": V613_L6_STEP_MEAN,
            "v613_g2_memory_ratio_mean": V613_G2_MEMORY_MEAN,
            "v613_g2_step_ratio_mean": V613_G2_STEP_MEAN,
    ...
    _simple_bar_svg(out_dir / "p0_contract_heatmap.svg", "v6.14 P0 contract", [r["variant_id"] for r in rows], [1.0 if r.get("implementation_status") == "measured" and int(f(r, "fake_data_used", 0)) == 0 else 0.0 for r in rows], "#16a34a")
    return rows
...
def run_p1(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for row in measured:
        ...
        if group_loop_count == 0 and policy.startswith("reset_v7_grouped_g") and policy.endswith("_poly1"):
            try:
                group_loop_count = int(policy.rsplit("_g", 1)[1].split("_", 1)[0])
            except ValueError:
                group_loop_count = 0
        if group_loop_count == 0 and policy.startswith("reset_v7_grouped_tiled_g") and "_tile" in policy:
            try:
                groups = int(policy.rsplit("_g", 1)[1].split("_", 1)[0])
                tile = int(policy.rsplit("_tile", 1)[1])
...
def _summary_from_detail(args: argparse.Namespace, stage: str, detail: Sequence[Dict[str, Any]], id_field: str, variants: Sequence[Tuple[Any, ...]], *, base_id: str, current_reference: Tuple[float, float] = (V613_DWM2_MEMORY_MEAN, V613_DWM2_STEP_MEAN)) -> List[Dict[str, Any]]:
    measured = [r for r in detail if r.get("implementation_status") == "measured" and r.get("method") != "MLP-autograd-reference"]
    by: Dict[str, List[Dict[str, Any]]] = {}
    for row in measured:
        by.setdefault(str(row.get("variant_id")), []).append(row)
    base_rows = by.get(base_id, [])
    base_mem = _mean(f(r, "memory_ratio_vs_MLP", math.nan) for r in base_rows) if base_rows else V613_B0_MEMORY_MEAN
    base_step = _mean(f(r, "step_time_ratio_vs_MLP", math.nan) for r in base_rows) if base_rows else V613_B0_STEP_MEAN
...
def run_p2(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    out_dir = Path(args.out_dir)
    write_csv(out_dir / "p2_memory_preserving_grouped_fusion_detail.csv", detail)
    summary = _summary_from_detail(args, "P2", detail, "package", P2_LOW_RANK, base_id="VG0-G2-grouped-g16-loop-baseline")
    group_by = {name: group for name, _m, _p, _ok, group, _kind in P2_LOW_RANK}
    kind_by = {name: kind for name, _m, _p, _ok, _group, kind in P2_LOW_RANK}
    for row in summary:
        ...
        row["tile_size"] = 4 if "tiled" in kind else ""
        row["memory_improvement_vs_G2"] = (V613_G2_MEMORY_MEAN - f(row, "memory_ratio_mean", 99.0)) / max(1.0e-12, V613_G2_MEMORY_MEAN)
        row["step_improvement_vs_G2"] = (V613_G2_STEP_MEAN - f(row, "step_ratio_mean", 99.0)) / max(1.0e-12, V613_G2_STEP_MEAN)
        row["memory_improvement_vs_GR3"] = (V613_GR3_MEMORY_MEAN - f(row, "memory_ratio_mean", 99.0)) / max(1.0e-12, V613_GR3_MEMORY_MEAN)
        row["step_improvement_vs_GR3"] = (V613_GR3_STEP_MEAN - f(row, "step_ratio_mean", 99.0)) / max(1.0e-12, V613_GR3_STEP_MEAN)
        ...
        row["grouped_runtime_useful"] = int(f(row, "step_improvement_vs_G2", 0.0) >= 0.70 and (f(row, "memory_ratio_mean", 99.0) / max(1.0e-12, V613_G2_MEMORY_MEAN)) <= 1.05)
    ...
    write_csv(out_dir / "p2_memory_preserving_grouped_fusion.csv", summary)
    measured = [r for r in detail if r.get("implementation_status") == "measured" and r.get("method") != "MLP-autograd-reference"]
...
def run_p3(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    _add_residual_fields(args, detail)
    out_dir = Path(args.out_dir)
    write_csv(out_dir / "p3_custom_grouped_kernel_diagnostic_detail.csv", detail)
    ...
    for row in summary:
        ...
        row["near_pass"] = int(f(row, "memory_ratio_mean", 99) <= 1.05 and f(row, "step_ratio_mean", 99) <= 1.50 and f(row, "residual_effect_pass_count", 0) > 0)
    write_csv(out_dir / "p3_custom_grouped_kernel_diagnostic.csv", summary)
    measured = [r for r in detail if r.get("implementation_status") == "measured" and r.get("method") != "MLP-autograd-reference"]
...
def run_p4(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    _add_residual_fields(args, detail)
    out_dir = Path(args.out_dir)
    write_csv(out_dir / "p4_lowrank_r2_runtime_closure_detail.csv", detail)
    ...
    for row in summary:
        ...
        row["memory_improvement_vs_L6"] = (V613_L6_MEMORY_MEAN - f(row, "memory_ratio_mean", 99.0)) / max(1.0e-12, V613_L6_MEMORY_MEAN)
        row["step_improvement_vs_L6"] = (V613_L6_STEP_MEAN - f(row, "step_ratio_mean", 99.0)) / max(1.0e-12, V613_L6_STEP_MEAN)
        row["lowrank_materialization_MB"] = row.get("intermediate_lowrank_MB", 0.0)
        row["lowrank_useful"] = int(f(row, "step_improvement_vs_L6", 0.0) >= 0.25 and (f(row, "memory_ratio_mean", 99) / max(1.0e-12, V613_L6_MEMORY_MEAN)) <= 1.05)
        row["near_pass"] = int(f(row, "memory_ratio_mean", 99) <= 1.05 and f(row, "step_ratio_mean", 99) <= 1.50 and f(row, "residual_effect_pass_count", 0) > 0)
    write_csv(out_dir / "p4_lowrank_r2_runtime_closure.csv", summary)
    measured = [r for r in detail if r.get("implementation_status") == "measured" and r.get("method") != "MLP-autograd-reference"]
...
def _survivor_type(row: Dict[str, Any] | None) -> str:
    ...
    if mem <= 1.05 and step <= 1.50:
        return "S2"
    if mem < V613_B0_MEMORY_MEAN and step > 1.50:
        return "S3"
    if step < V613_B0_STEP_MEAN and mem > 1.05:
        return "S4"
    return "S7"
...
def run_p6(args: argparse.Namespace, p2: Sequence[Dict[str, Any]], p3: Sequence[Dict[str, Any]], p4: Sequence[Dict[str, Any]], p5: Sequence[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any] | None]:
    ...
    elif best_package.startswith(("VG", "GK", "GR")):
        best_family = "grouped"
    ...
    write_csv(Path(args.out_dir) / "p6_reset_v7_selection.csv", [row])
    _simple_bar_svg(Path(args.out_dir) / "p6_reset_v7_scorecard.svg", "P6 scorecard", ["memory", "step"], [row["best_memory_ratio"], row["best_step_ratio"]], "#2563eb")
...
def run_p7_to_p10(args: argparse.Namespace, p6_rows: Sequence[Dict[str, Any]], best: Dict[str, Any] | None) -> None:
    ...
    if open_probe and best is not None:
        ...
        reason = "No S0/S1/S2 reset-v7 survivor"
...
def _run_one_step_probe(args: argparse.Namespace, best: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stack = _make_v614_stack(method, bundle.input_dim, params.hidden_dim, 2, _basis_from_name(method, params.basis_count), device, policy, x.shape[0])
    head = V63ManualLayer(params.hidden_dim, bundle.num_classes, kind="linear", basis_count=2, device=device)
...
def _manual_train_task(args: argparse.Namespace, dataset: str, seed: int, method: str, policy: str, label: str, opt_kind: str) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    stack = _make_v614_stack(method, bundle.input_dim, params.hidden_dim, 2, _basis_from_name(method, params.basis_count), device, policy, x_train.shape[0])
    head = V63ManualLayer(params.hidden_dim, bundle.num_classes, kind="linear", basis_count=2, device=device)
    ...
    for step in range(1, args.task_steps + 1):
        ...
        if step % 20 == 0 or step == args.task_steps:
            ...
            trace.append(row)
            _wandb_log_row(args, row, "task/v614_trace")
    ...
    _wandb_log_row(args, summary, "task/v614_summary")
...
def _run_task_reentry(args: argparse.Namespace, best: Dict[str, Any], *, official: bool) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for row in summaries:
        ...
        if "ResetV7" in str(row.get("method", "")) or "+" in str(row.get("variant_id", "")):
...
def run_route(args: argparse.Namespace, p6_rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    route_json = {
        ...
        "reset_v7_measured": 1,
        "reset_v7_pass": int(survivor in {"S0", "S1", "S2"}),
...
def run_failure(args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    files = [
        ...
        ("p2_memory_preserving_grouped_fusion.csv", "P2"),
        ("p3_custom_grouped_kernel_diagnostic.csv", "P3"),
        ("p4_lowrank_r2_runtime_closure.csv", "P4"),
        ("p5_piecewise_residual_vs_mixing.csv", "P5"),
        ("p6_reset_v7_selection.csv", "P6"),
...
def _write_manifest(out_dir: Path, args: argparse.Namespace, started: float, finished: float) -> None:
    manifest = {
        ...
        "script": "experiments/run_gafu_v614_real.py",
        "plan": "docs/DG-KAN_v6.14_ResetV7_GroupedFusion_RuntimeClosure_详细实验计划.md",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--packages", default="V6_14_ALL")
    parser.add_argument("--out-dir", type=Path, default=Path("results/real_rerun_20260505/v614_real"))
    ...
    parser.add_argument("--wandb-group", default="v614-real-20260505")
    parser.add_argument("--wandb-name-prefix", default="v614-real")
...
def main() -> int:
    ...
    try:
        ...
        # v6.14 reproduction tracks L6 via P4 and DWM2 via P1.
        ...
        g2 = next((r for r in p2_summary if r.get("package") == "VG0-G2-grouped-g16-loop-baseline"), {})
        gr3 = next((r for r in p2_summary if r.get("package") == "VG1-GR3-vectorized-g16-baseline"), {})
        dwm2 = [r for r in p1_detail if r.get("variant_id") == "DWM2-current-baseline" and r.get("implementation_status") == "measured"]
        repro = {
            **_row_common("P0_REPRO", args, variant_id="v614-reproduction"),
            "v613_dwm2_memory_ratio_mean": V613_DWM2_MEMORY_MEAN,
            "v614_dwm2_memory_ratio_mean": _mean(f(r, "memory_ratio_vs_MLP", math.nan) for r in dwm2),
            "v613_l6_memory_ratio_mean": V613_L6_MEMORY_MEAN,
            "v614_l6_memory_ratio_mean": f(l6, "memory_ratio_mean", math.nan),
            "v613_l6_step_ratio_mean": V613_L6_STEP_MEAN,
            "v614_l6_step_ratio_mean": f(l6, "step_ratio_mean", math.nan),
            "v613_g2_memory_ratio_mean": V613_G2_MEMORY_MEAN,
            "v614_g2_memory_ratio_mean": f(g2, "memory_ratio_mean", math.nan),
            "v613_gr3_memory_ratio_mean": V613_GR3_MEMORY_MEAN,
            "v614_gr3_memory_ratio_mean": f(gr3, "memory_ratio_mean", math.nan),
            "reproduction_delta_memory_ratio": f(l6, "memory_ratio_mean", math.nan) - V613_L6_MEMORY_MEAN,
            "reproduction_delta_step_ratio": f(l6, "step_ratio_mean", math.nan) - V613_L6_STEP_MEAN,
            "reproduction_pass": int(abs(f(l6, "memory_ratio_mean", 99.0) - V613_L6_MEMORY_MEAN) <= 0.05 and abs(f(l6, "step_ratio_mean", 99.0) - V613_L6_STEP_MEAN) <= 0.15),
            "route": route.get("route"),
        }
        write_csv(out_dir / "p0_reproduction_check.csv", [repro])
        _simple_bar_svg(out_dir / "p0_reproduction_delta_bar.svg", "v6.14 vs v6.13 L6 delta", ["memory", "step"], [repro["reproduction_delta_memory_ratio"], repro["reproduction_delta_step_ratio"]], "#2563eb")
