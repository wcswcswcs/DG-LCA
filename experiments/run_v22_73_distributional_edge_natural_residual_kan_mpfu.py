#!/usr/bin/env python3
"""DG-KAN v22.73 distributional edge-natural residual KAN MPFU audit runner."""
...
RUNNER = ROOT / "experiments/run_v22_73_distributional_edge_natural_residual_kan_mpfu.py"
PLAN = ROOT / "docs/DG-KAN_v22.73_DistributionalEdgeNaturalResidualKAN_MPFU_完整计划.md"
EXEC_LOG = ROOT / "docs/DG-KAN_v22.73_DistributionalEdgeNaturalResidualKAN_MPFU_执行日志.md"
RECAP_LOG = ROOT / "docs/DG-KAN_v22.73_DistributionalEdgeNaturalResidualKAN_MPFU_实验结果复盘.md"
OUT_ROOT = ROOT / "results/v22_73"
...
ARCH_SPECS: dict[str, dict[str, Any]] = {
    "DGKAN_DCHE": {"basis_family": "D-CHE4", "basis_name": "chebyshev", "k": 4, "init_variant": "v22_73_dche_dense", "dense": 1},
    "DGKAN_DFOU": {"basis_family": "D-FOU4", "basis_name": "fourier_lowfreq", "k": 4, "init_variant": "v22_73_dfou_dense", "dense": 1},
...
def append_exec(task_id: str, command: str, status: str, *, gpu: str = "", files: str = "", note: str = "") -> None:
    ...
    if not EXEC_LOG.exists():
        EXEC_LOG.write_text(
            "# DG-KAN v22.73 DistributionalEdgeNaturalResidualKAN MPFU 执行日志\n\n"
            f"创建时间：{now_sg()}\n\n"
            f"- 计划文档：`{rel(PLAN)}`\n"
    ...
    with EXEC_LOG.open("a", encoding="utf-8") as f:
        ...
        f.write(f"- note: {note}\n")
    journal = OUT_ROOT / "v22_73_command_journal.csv"
...
def append_recap(title: str, lines: list[str]) -> None:
    ...
    if not RECAP_LOG.exists():
        RECAP_LOG.write_text(
            "# DG-KAN v22.73 DistributionalEdgeNaturalResidualKAN MPFU 实验结果复盘\n\n"
            f"创建时间：{now_sg()}\n\n"
            "## 0. 当前结论\n"
            "- 尚未完成最终 route 判定。\n"
...
def make_probe_kan(arch: str, bundle: dict[str, Any], device: Any, hidden: int, seed: int) -> Any:
    ...
    spec = PrimitiveSpec(
        candidate_id=f"v22.73-{meta['basis_family']}-probe",
        basis_family=str(meta["basis_family"]),
        basis_name=str(meta["basis_name"]),
        k=int(meta["k"]),
        hidden_dim=int(hidden),
        source="v22_73_edge_natural_residual_probe",
...
def w2_readout_edge_design(model: Any, x: Any) -> dict[str, Any]:
    ...
    classes = int(model.output_dim)
    denom = math.sqrt(max(1, int(model.hidden_dim)))
    if tuple(getattr(model, "w2").shape) == (int(model.hidden_dim), classes, int(model.k)):
        scaled_3 = scaled.reshape(n, int(model.hidden_dim), int(model.k))
        raw_3 = raw_feats.reshape(n, int(model.hidden_dim), int(model.k))
        cols = []
        raw_cols = []
        for hid in range(int(model.hidden_dim)):
            for cls in range(classes):
                for kk in range(int(model.k)):
                    block = torch.zeros((n, classes), device=x.device, dtype=torch.float64)
                    block[:, cls] = scaled_3[:, hid, kk] / denom
                    cols.append(block.reshape(n * classes))
                    raw_block = torch.zeros((n, classes), device=x.device, dtype=torch.float64)
                    raw_block[:, cls] = raw_3[:, hid, kk] / denom
                    raw_cols.append(raw_block.reshape(n * classes))
        phi = torch.stack(cols, dim=1) if cols else torch.zeros((n * classes, 0), device=x.device, dtype=torch.float64)
        phi_raw = torch.stack(raw_cols, dim=1) if raw_cols else torch.zeros((n * classes, 0), device=x.device, dtype=torch.float64)
        param_order = "w2.reshape(-1)=hidden,class,basis"
    else:
        blocks = []
        raw_blocks = []
        for cls in range(classes):
        ...
        phi = torch.cat(blocks, dim=1)
        phi_raw = torch.cat(raw_blocks, dim=1)
        param_order = "fallback_class_block"
    raw_col_energy = phi_raw.square().sum(dim=0)
    phi = phi / torch.linalg.norm(phi, dim=0, keepdim=True).clamp_min(1.0e-8)
    return {
        "phi": phi,
...
def initialize_docs(reset_logs: bool = False, *, mode: str = "full", device: str = "") -> None:
    ...
    append_exec(
        ...
        note="v22.73 gate-ordered runner initialized.",
    )
    append_recap(
        "初始化与边界",
        [
            "执行顺序按计划采用 Part A -> Part B -> Part C -> Part D；D 未通过时 E/F/G 写 skipped artifact。",
            "本轮新增 `dgkan/fu/kan_distributional_edge_natural_residual.py`，包含 own-reference residual projector、debt cone projector 与 optimizer-owned gradient transform。",
        ],
    )


def clean_tarball_import_check() -> tuple[int, str]:
    bundle_path = OUT_ROOT / "v22_73_clean_import_bundle.tar.gz"
    with tempfile.TemporaryDirectory(prefix="v22_73_import_pack_") as tmp:
        tmp_path = Path(tmp)
        ...
        code = (
            ...
            "import dgkan.fu.kan_distributional_edge_natural_residual; "
            "import experiments.run_v22_73_distributional_edge_natural_residual_kan_mpfu"
        )
        proc = subprocess.run([PYTHON, "-c", code], cwd=extract_root, text=True, capture_output=True, timeout=60)
        log_path = LOG_ROOT / "v22_73_clean_tarball_import.log"
...
def standard_loop_smoke(device_name: str) -> dict[str, Any]:
    ...
    from dgkan.fu.kan_distributional_edge_natural_residual import DistributionalEdgeNaturalResidualOptimizer, DistributionalEdgeNaturalResidualState, raw_readout_multiplier
    ...
    edge_states = {"w2": DistributionalEdgeNaturalResidualState(raw_multiplier=multiplier)}
    opt = DistributionalEdgeNaturalResidualOptimizer(model.named_parameters(), lr=1.0e-3, weight_decay=0.0, edge_states=edge_states)
...
def run_part_a(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        importlib.import_module("experiments.run_v22_73_distributional_edge_natural_residual_kan_mpfu")
    except Exception as exc:
    ...
    summary = {
        "gate": "v22_73_part_a_code_identity_hard_gate",
    ...
    write_json(OUT_ROOT / "v22_73_part_a_code_identity_hard_gate.json", summary)
    write_rows(OUT_ROOT / "v22_73_part_a_compile_rows.csv", compile_rows)
    write_rows(OUT_ROOT / "v22_73_part_a_static_scan_hits.csv", scan_rows or [{"status": "no_forbidden_hits"}])
    append_exec(
        "A_code_identity_hard_gate",
        command,
        "pass" if summary["part_a_hard_gate_pass"] else "fail",
        gpu=str(args.device),
        files=f"{rel(OUT_ROOT / 'v22_73_part_a_code_identity_hard_gate.json')}; {rel(OUT_ROOT / 'v22_73_part_a_compile_rows.csv')}",
    ...
    append_recap(
        ...
        [
            ...
            f"Forbidden scan hits file: `{rel(OUT_ROOT / 'v22_73_part_a_static_scan_hits.csv')}`。",
        ],
    )
    return summary
...
def run_part_b(args: argparse.Namespace) -> dict[str, Any]:
    ...
    required = {
        "B0_v22_71_part_d_csv": ROOT / "results/v22_71/v22_71_part_d_edge_metric_preflight.csv",
        "B0_v22_71_part_d_summary": ROOT / "results/v22_71/v22_71_part_d_edge_metric_preflight_summary.json",
        "B0_v22_71_part_f_csv": ROOT / "results/v22_71/v22_71_part_f_kan_full_loop_matrix.csv",
        "B0_v22_71_part_f_summary": ROOT / "results/v22_71/v22_71_part_f_kan_full_loop_summary.json",
        "B1_v22_72_recap_log": ROOT / "docs/DG-KAN_v22.72_EdgeNaturalResidualKAN_MPFU_实验结果复盘.md",
        "B2_v22_72_part_f_csv": ROOT / "results/v22_72/v22_72_part_f_edge_native_full_loop_matrix.csv",
        "B2_v22_72_debt_csv": ROOT / "results/v22_72/v22_72_part_f_debt_failure_decomposition.csv",
        "B2_v22_72_part_f_summary": ROOT / "results/v22_72/v22_72_part_f_edge_native_full_loop_summary.json",
        "B2_v22_72_debt_summary": ROOT / "results/v22_72/v22_72_part_f_debt_failure_decomposition_summary.json",
    }
    available = {key: int(path.exists()) for key, path in required.items()}
    d_rows = read_rows(required["B0_v22_71_part_d_csv"])
    f_rows = read_rows(required["B0_v22_71_part_f_csv"])
    d_summary = json.loads(required["B0_v22_71_part_d_summary"].read_text(encoding="utf-8")) if required["B0_v22_71_part_d_summary"].exists() else {}
    f_summary = json.loads(required["B0_v22_71_part_f_summary"].read_text(encoding="utf-8")) if required["B0_v22_71_part_f_summary"].exists() else {}
    v72_rows = read_rows(required["B2_v22_72_part_f_csv"])
    v72_debt = read_rows(required["B2_v22_72_debt_csv"])
    v72_summary = json.loads(required["B2_v22_72_part_f_summary"].read_text(encoding="utf-8")) if required["B2_v22_72_part_f_summary"].exists() else {}
    debt_by_key = {
        (r.get("dataset", ""), r.get("seed", ""), r.get("architecture", ""), r.get("method", "")): r
        for r in v72_debt
    }
    ...
    b1_note = "missing"
    if required["B1_v22_72_recap_log"].exists():
        text = required["B1_v22_72_recap_log"].read_text(encoding="utf-8", errors="replace")
        b1_note = "present_log_only_not_used_for_promotion" if "w2-shape" in text and "not valid final evidence" in text else "present_but_repair_marker_not_found"
    candidate_rows = [r for r in v72_rows if r.get("row_kind") == "candidate" and r.get("run_status") == "completed"]
    for row in candidate_rows:
        key = (row.get("dataset", ""), row.get("seed", ""), row.get("architecture", ""), row.get("method", ""))
        debt = debt_by_key.get(key, {})
        ece_delta = fval(debt.get("ECE_delta_vs_own"), None)
        brier_delta = fval(debt.get("Brier_delta_vs_own"), None)
        tail95_delta = fval(debt.get("tail_q95_delta_vs_own"), None)
        tail99_delta = fval(debt.get("tail_q99_delta_vs_own"), None)
        margin_delta = fval(debt.get("margin_q10_delta_vs_own"), None)
        pred_safe = float(fval(row.get("debt_cone_violation_max"), 1.0) or 0.0) <= 1.0e-8
        mode_debt = {
            "ECE": int(ece_delta is not None and ece_delta > 1.0e-12),
            "Brier": int(brier_delta is not None and brier_delta > 1.0e-12),
            "tail95": int(tail95_delta is not None and tail95_delta > 1.0e-12),
            "tail99": int(tail99_delta is not None and tail99_delta > 1.0e-12),
            "margin": int(margin_delta is not None and margin_delta < -1.0e-12),
        }
        failures = [k for k, v in mode_debt.items() if v]
        rows.append({
            "source_part": "B2_v22_72_valid_after_w2_shape_repair",
            "dataset": row.get("dataset", ""),
            "seed": row.get("seed", ""),
            "candidate_method": row.get("method", ""),
            "architecture": row.get("architecture", ""),
            "Delta_NLL_vs_own": row.get("Delta_NLL_vs_own_ref", ""),
            "Delta_NLL_vs_MLP": row.get("Delta_NLL_vs_MLP_matched", ""),
            "Delta_NLL_vs_same_edge": row.get("Delta_NLL_vs_same_edge_control", ""),
            "Delta_NLL_vs_same_debt": row.get("Delta_NLL_vs_same_debt_cone_control", ""),
            "ECE_delta": "" if ece_delta is None else ece_delta,
            "Brier_delta": "" if brier_delta is None else brier_delta,
            "tail95_delta": "" if tail95_delta is None else tail95_delta,
            "tail99_delta": "" if tail99_delta is None else tail99_delta,
            "margin_delta": "" if margin_delta is None else margin_delta,
            "linear_debt_predicted_safe": int(pred_safe),
            "linear_debt_cone_violation": row.get("debt_cone_violation_max", ""),
            "raw_readout_visible_CVaR25": row.get("raw_readout_visible_energy_CVaR25", ""),
            "own_residual_grad": row.get("own_residual_energy_fraction", ""),
            "own_residual_edge": "",
            "own_residual_func": row.get("own_residual_func_fraction", ""),
            "edge_domain_drift": "",
            "debt_prediction_error": int(pred_safe and bool(failures)),
            "failure_mode": ",".join(failures) if failures else "no_debt_failure_not_observed",
            "derived_fields_finite": int(all(v is not None for v in [ece_delta, brier_delta, tail95_delta, tail99_delta, margin_delta])),
        })
    ...
    mode_names = ["ECE", "Brier", "tail95", "tail99", "margin"]
    b2_rows = [r for r in rows if r.get("source_part") == "B2_v22_72_valid_after_w2_shape_repair"]
    mode_stats: dict[str, Any] = {}
    for mode in mode_names:
        actual_debt = []
        for r in b2_rows:
            if mode == "ECE":
                val = fval(r.get("ECE_delta"), None)
                actual_debt.append(int(val is not None and val > 1.0e-12))
            elif mode == "Brier":
                val = fval(r.get("Brier_delta"), None)
                actual_debt.append(int(val is not None and val > 1.0e-12))
            elif mode == "tail95":
                val = fval(r.get("tail95_delta"), None)
                actual_debt.append(int(val is not None and val > 1.0e-12))
            elif mode == "tail99":
                val = fval(r.get("tail99_delta"), None)
                actual_debt.append(int(val is not None and val > 1.0e-12))
            else:
                val = fval(r.get("margin_delta"), None)
                actual_debt.append(int(val is not None and val < -1.0e-12))
        nmode = max(1, len(actual_debt))
        debt_count = sum(actual_debt)
        # v22.72 linear cone predicted all valid rows safe when violation<=0.
        pred_safe_count = sum(flag(r.get("linear_debt_predicted_safe")) for r in b2_rows)
        mode_stats[f"debt_sign_agreement_{mode}"] = float((nmode - debt_count) / nmode)
        mode_stats[f"false_safe_rate_{mode}"] = float(debt_count / max(1, pred_safe_count))
        mode_stats[f"false_block_rate_{mode}"] = 0.0
    false_safe_macro = sum(mode_stats[f"false_safe_rate_{m}"] for m in mode_names) / len(mode_names)
    summary = {
        "gate": "v22_73_part_b_v22_72_failure_replay_distributional_debt_calibration",
        ...
        "d_rows": len(d_rows),
        "f_rows": len(f_rows),
        "B2_candidate_rows": len(candidate_rows),
        ...
        "linear_debt_cone_violation_rows": sum(1 for r in b2_rows if not flag(r.get("linear_debt_predicted_safe"))),
        "held_no_debt_rows": int(v72_summary.get("no_debt_rows", 0) or 0),
        "mean_delta_ECE_vs_own": sum((fval(r.get("ECE_delta"), 0.0) or 0.0) for r in b2_rows) / max(1, len(b2_rows)),
        "mean_delta_Brier_vs_own": sum((fval(r.get("Brier_delta"), 0.0) or 0.0) for r in b2_rows) / max(1, len(b2_rows)),
        "mean_delta_tail95_vs_own": sum((fval(r.get("tail95_delta"), 0.0) or 0.0) for r in b2_rows) / max(1, len(b2_rows)),
        "mean_delta_tail99_vs_own": sum((fval(r.get("tail99_delta"), 0.0) or 0.0) for r in b2_rows) / max(1, len(b2_rows)),
        "mean_delta_margin_vs_own": sum((fval(r.get("margin_delta"), 0.0) or 0.0) for r in b2_rows) / max(1, len(b2_rows)),
        "linear_cone_false_safe_rate_macro": false_safe_macro,
        **mode_stats,
        "route_flags": route_flags,
        "part_b_reanalysis_complete": 0,
    }
    summary["part_b_reanalysis_complete"] = int(summary["required_artifacts_available"] == 1 and len(b2_rows) >= 45 and false_safe_macro > 0.30)
    write_rows(OUT_ROOT / "v22_73_part_b_failure_replay_debt_calibration.csv", rows)
    write_json(OUT_ROOT / "v22_73_part_b_failure_replay_debt_calibration_summary.json", summary)
    append_exec(
        "B_failure_replay_debt_calibration",
        command,
        "pass" if summary["part_b_reanalysis_complete"] else "fail",
        files=f"{rel(OUT_ROOT / 'v22_73_part_b_failure_replay_debt_calibration.csv')}; {rel(OUT_ROOT / 'v22_73_part_b_failure_replay_debt_calibration_summary.json')}",
        note=json.dumps({k: summary[k] for k in ["part_b_reanalysis_complete", "required_artifacts_available", "B2_candidate_rows", "linear_cone_false_safe_rate_macro"]}, ensure_ascii=False),
    )
    append_recap(
        "Part B v22.72 failure replay + linear debt predictor calibration",
        [
            f"part_b_reanalysis_complete={summary['part_b_reanalysis_complete']}；required_artifacts_available={summary['required_artifacts_available']}；B1_status={b1_note}。",
            ...
            f"v22.72 B2 valid candidate rows={len(candidate_rows)}；linear_debt_cone_violation_rows={summary['linear_debt_cone_violation_rows']}；held_no_debt_rows={summary['held_no_debt_rows']}；false_safe_macro={summary['linear_cone_false_safe_rate_macro']:.4f}。",
            f"false-safe by mode: ECE={summary['false_safe_rate_ECE']:.4f}, Brier={summary['false_safe_rate_Brier']:.4f}, tail95={summary['false_safe_rate_tail95']:.4f}, tail99={summary['false_safe_rate_tail99']:.4f}, margin={summary['false_safe_rate_margin']:.4f}。",
            "结论：linearized train-only cone false-safe 高，按计划进入 Part C distributional debt cone。",
        ],
    )
    return summary
...
def run_part_c(args: argparse.Namespace) -> dict[str, Any]:
    ...
    rows: list[dict[str, Any]] = []
    modes = ["ECE", "Brier", "tail95", "tail99", "margin"]
    for seed in range(6):
        gen = torch.Generator(device="cpu").manual_seed(73000 + seed)
        dim = 16
        grad = torch.randn(dim, generator=gen, dtype=torch.float64)
        grad = grad / torch.linalg.norm(grad).clamp_min(1.0e-12)
        safe = -grad
        unsafe = grad
        neutral = torch.randn(dim, generator=gen, dtype=torch.float64)
        neutral = neutral - grad * torch.dot(neutral, grad)
        neutral = neutral / torch.linalg.norm(neutral).clamp_min(1.0e-12) * 0.05
        for mode in modes:
            c = DistributionalDebtConstraint(direction=grad, mode=mode, cohort=f"synthetic{seed}", curvature=0.02, slack=0.0, budget=0.0)
            pred_unsafe = distributional_debt_predict(unsafe, c)
            pred_safe = distributional_debt_predict(safe, c)
            pred_neutral = distributional_debt_predict(neutral, c)
            projected, diag = distributional_debt_cone_project(unsafe, [c])
            rows.append({
                "part": "C1_C3_synthetic_distributional_cone",
                "case": f"{mode}_seed{seed}",
                "mode": mode,
                "prediction_upward_positive": pred_unsafe,
                "prediction_safe_nonpositive": pred_safe,
                "prediction_neutral_abs": abs(pred_neutral),
                "projection_violation_after": diag["distributional_debt_violation_after"],
                "pass": int(pred_unsafe > 0.0 and pred_safe <= 0.0 and abs(pred_neutral) <= 0.02 and diag["distributional_debt_violation_after"] <= 1.0e-8),
            })
    rows.append({
        "part": "C4_multi_cohort_robustness",
        "case": "one_positive_query_blocks",
        "mode": "Brier",
        "pass": int(
            max(
                distributional_debt_predict(
                    torch.ones(8, dtype=torch.float64),
                    DistributionalDebtConstraint(torch.ones(8, dtype=torch.float64), "Brier", "q0"),
                ),
                distributional_debt_predict(
                    -torch.ones(8, dtype=torch.float64),
                    DistributionalDebtConstraint(torch.ones(8, dtype=torch.float64), "Brier", "q1"),
                ),
            )
            > 0.0
        ),
    ...
    rows.append({
        "part": "C5_no_fake_no_debt",
        "case": "false_safe_rejected",
        "mode": "tail99",
        "pass": int(distributional_debt_predict(torch.ones(6, dtype=torch.float64), DistributionalDebtConstraint(torch.ones(6, dtype=torch.float64), "tail99")) > 0.0),
    })
    rows.append({
        "part": "C6_non_overkill",
        "case": "debt_neutral_not_rejected",
        "mode": "margin",
        "pass": int(distributional_debt_predict(torch.zeros(6, dtype=torch.float64), DistributionalDebtConstraint(torch.ones(6, dtype=torch.float64), "margin")) <= 0.0),
    })

    device = make_device(str(args.device))
    replay_rows: list[dict[str, Any]] = []
    for dataset in parse_csv(args.part_c_datasets):
        for seed_text in parse_csv(args.seeds)[:2]:
            seed = int(seed_text)
            try:
                set_seed(7300 + seed + sum(ord(c) for c in dataset))
                bundle = load_bundle(dataset, int(args.train_size), int(args.held_size), int(args.test_size), seed)
                model = make_probe_kan("DGKAN_DCHE", bundle, device, int(args.hidden), seed + 7300)
                ...
                loss.backward()
                cand_grad = model.w2.grad.detach().reshape(-1).to(dtype=torch.float64)
                model.zero_grad(set_to_none=True)
                constraints, build_diag = build_distributional_constraints(model, design, x_metric, y_metric, cand_grad, args)
                phi_raw = design["phi_raw"].detach().to(device=device, dtype=torch.float64)
                logits = model(x_metric).float().detach().to(dtype=torch.float64)
                n, classes = int(logits.shape[0]), int(logits.shape[1])
                update = (-float(args.lr) * cand_grad.to(device=device, dtype=torch.float64))
                logits_plus = logits + (phi_raw @ update).reshape(n, classes)
                base_hard = hard_debt_values_from_logits(logits, y_metric)
                plus_hard = hard_debt_values_from_logits(logits_plus, y_metric)
                for con in constraints:
                    pred = distributional_debt_predict(cand_grad, con)
                    actual = plus_hard[con.mode] - base_hard[con.mode]
                    sign_eps = float(args.debt_sign_epsilon)
                    pred_pos = pred > sign_eps
                    actual_pos = actual > sign_eps
                    replay_rows.append({
                        "part": "C2_train_query_fd_consistency",
                        "case": f"{dataset}_s{seed}_{con.cohort}_{con.mode}",
                        "dataset": dataset,
                        "seed": seed,
                        "mode": con.mode,
                        "cohort": con.cohort,
                        "prediction_delta": pred,
                        "actual_hard_delta": actual,
                        "sign_deadband_epsilon": sign_eps,
                        "sign_agree": int(pred_pos == actual_pos),
                        "false_safe": int((not pred_pos) and actual_pos),
                        "false_block": int(pred_pos and (not actual_pos)),
                        "distributional_constraints_built": build_diag.get("distributional_constraints_built", 0),
                        "pass": int(pred_pos == actual_pos),
                    })
            except Exception as exc:
                replay_rows.append({"part": "C2_train_query_fd_consistency", "case": f"{dataset}_s{seed}_exception", "run_status": "exception", "error": repr(exc), "traceback_log": rel(write_exception_log("part_c_replay", exc)), "pass": 0})
    rows.extend(replay_rows)
    mode_agree: dict[str, float] = {}
    for mode in modes:
        mr = [r for r in replay_rows if r.get("mode") == mode and r.get("run_status") != "exception"]
        mode_agree[mode] = sum(flag(r.get("sign_agree")) for r in mr) / max(1, len(mr))
    replay_valid = [r for r in replay_rows if r.get("run_status") != "exception"]
    false_safe = sum(flag(r.get("false_safe")) for r in replay_valid) / max(1, len(replay_valid))
    false_block = sum(flag(r.get("false_block")) for r in replay_valid) / max(1, len(replay_valid))
    macro_agree = sum(mode_agree.values()) / max(1, len(mode_agree))
    parts = {p: [r for r in rows if r.get("part") == p] for p in sorted(set(str(r.get("part")) for r in rows))}
    summary = {
        "gate": "v22_73_part_c_distributional_debt_cone_unit_preflight",
        "unit_rows": len(rows),
        "pass_rows": sum(flag(r.get("pass")) for r in rows),
        "train_query_replay_rows": len(replay_valid),
        "debt_sign_agreement_macro": macro_agree,
        "false_safe_rate": false_safe,
        "false_block_rate": false_block,
        "Brier_sign_agreement": mode_agree.get("Brier", 0.0),
        "tail99_sign_agreement": mode_agree.get("tail99", 0.0),
        "ECE_sign_agreement": mode_agree.get("ECE", 0.0),
    ...
    summary["part_c_gate_pass"] = int(
        summary["unit_rows"] >= 24
        and summary["pass_rows"] == summary["unit_rows"]
        and summary["debt_sign_agreement_macro"] >= 0.75
        and summary["false_safe_rate"] <= 0.20
        and summary["false_block_rate"] <= 0.35
        and summary["Brier_sign_agreement"] >= 0.75
        and summary["tail99_sign_agreement"] >= 0.70
    )
    write_rows(OUT_ROOT / "v22_73_part_c_edge_metric_unit_tests.csv", rows)
    write_json(OUT_ROOT / "v22_73_part_c_summary.json", summary)
    append_exec(
        "C_edge_metric_unit_tests",
        command,
        "pass" if summary["part_c_gate_pass"] else "fail",
        gpu=str(args.device),
        files=f"{rel(OUT_ROOT / 'v22_73_part_c_edge_metric_unit_tests.csv')}; {rel(OUT_ROOT / 'v22_73_part_c_summary.json')}",
        note=json.dumps(summary, sort_keys=True),
    )
    append_recap(
        "Part C distributional debt cone unit tests and train-query replay",
        [
            f"part_c_gate_pass={summary['part_c_gate_pass']}；unit_rows={summary['unit_rows']}；pass_rows={summary['pass_rows']}；train_query_replay_rows={summary['train_query_replay_rows']}。",
            f"sign agreement macro={summary['debt_sign_agreement_macro']:.4f}；Brier={summary['Brier_sign_agreement']:.4f}；tail99={summary['tail99_sign_agreement']:.4f}；ECE={summary['ECE_sign_agreement']:.4f}。",
            f"false_safe_rate={summary['false_safe_rate']:.4f}；false_block_rate={summary['false_block_rate']:.4f}。",
            "新增实现：`DistributionalDebtConstraint` + finite-difference/curvature query replay；ECE 使用 soft overconfidence proxy 并记录 hard-ECE delta。",
        ],
    )
    return summary
...
def part_d_one_row(dataset: str, seed: int, arch: str, family: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    from dgkan.fu.kan_distributional_edge_natural_residual import debt_cone_project, distributional_debt_cone_project, functional_residual_fraction, own_reference_residual_project
    ...
    own_func_fraction = float(own_diag["own_residual_energy_fraction"])
    own_func_overlap = float(own_diag["own_overlap_after"])
    try:
        model.zero_grad(set_to_none=True)
        own_loss_w2 = F.cross_entropy(model(x_source).float(), y_source.long())
        own_loss_w2.backward()
        own_grad_w2 = model.w2.grad.detach().reshape(-1).to(dtype=torch.float64)
        model.zero_grad(set_to_none=True)
        dist_loss = F.cross_entropy(model(x_witness).float(), y_witness.long())
        dist_loss.backward()
        cand_grad = model.w2.grad.detach().reshape(-1).to(dtype=torch.float64)
        model.zero_grad(set_to_none=True)
        w2_design = w2_readout_edge_design(model, x_witness)
        dist_constraints, built = build_distributional_constraints(model, w2_design, x_witness, y_witness, cand_grad, args)
        _dist_proj, dist_diag = distributional_debt_cone_project(cand_grad, dist_constraints)
        if int(w2_design["phi"].numel()) and int(cand_grad.numel()) == int(w2_design["phi"].shape[1]):
            cand_func_grad = cand_grad
            own_basis_w2 = own_grad_w2.reshape(-1, 1)
            if "own_residual" in family:
                cand_func_grad, _func_project_diag = own_reference_residual_project(cand_grad, own_basis_w2)
                cand_func_grad = cand_func_grad.reshape(-1)
            cand_disp = w2_design["phi"] @ (-float(args.lr) * cand_func_grad.to(device=w2_design["phi"].device))
            own_disp = w2_design["phi"] @ own_basis_w2.to(device=w2_design["phi"].device)
            own_func_fraction, own_func_overlap = functional_residual_fraction(cand_disp, own_disp)
        dist_diag.update(built)
    except Exception as exc:
        dist_diag = {
            "distributional_debt_violation_before": 1.0,
            "distributional_debt_violation_after": 1.0,
            "distributional_debt_constraints": 0.0,
            "distributional_preflight_error": repr(exc),
        }
    ...
    return {
        ...
        "own_residual_grad_fraction": own_diag["own_residual_energy_fraction"],
        "own_residual_edge_fraction": own_diag["own_residual_energy_fraction"],
        "own_residual_func_fraction": own_func_fraction,
        "own_residual_func_overlap_after": own_func_overlap,
        ...
        "distributional_debt_cone_violation": dist_diag.get("distributional_debt_violation_after", 1.0),
        "distributional_debt_predicted_pass": int(float(dist_diag.get("distributional_debt_violation_after", 1.0)) <= 1.0e-8),
        "distributional_debt_constraints": dist_diag.get("distributional_debt_constraints", 0.0),
...
def _part_d_summary_for_rows(rows: list[dict[str, Any]], gate_family: str) -> dict[str, Any]:
    ...
    n = len(completed)
    summary = {
        "gate": "v22_73_part_d_edge_distributional_preflight",
...
def run_part_d(args: argparse.Namespace) -> dict[str, Any]:
    ...
    rows: list[dict[str, Any]] = []
    kinds = ["no_transport", "affine", "quantile", "monotone_spline"]
    basis_cases = [("chebyshev", 4), ("fourier_lowfreq", 5), ("hat_wavelet", 4), ("compact_rbf", 4)]
    for seed in range(8):
        gen = torch.Generator(device="cpu").manual_seed(730000 + seed)
        old = torch.sort(torch.randn(512, generator=gen, dtype=torch.float64) * (0.6 + 0.03 * seed) + 0.1 * seed).values
        new = torch.sort(1.15 * old + 0.08 * old.pow(3) + 0.15 * torch.sin(old) + 0.1).values
        for basis_name, k in basis_cases:
            old_g, _ = domain_gram(old, basis_name, k)
            new_g, _ = domain_gram(new, basis_name, k)
            base_extrap = float(((new < old.min()) | (new > old.max())).to(dtype=torch.float64).mean().item())
            for kind in kinds:
                try:
                    if kind == "no_transport":
                        inv = new
                        forward = old
                        diag = {"transport_wasserstein1_before": float(torch.mean(torch.abs(old - new)).item()), "transport_wasserstein1_after": float(torch.mean(torch.abs(old - new)).item()), "transport_ks_distance": 1.0, "transport_monotonicity_violation": 0.0}
                        inverse_error = float(torch.max(torch.abs(inv - old)).item())
                    else:
                        transport = fit_transport(old, new, kind)
                        inv = transport.inverse(new)
                        forward = transport.forward(old)
                        diag = transport_diagnostics(old, new, kind)
                        inverse_error = transport.inverse_error(old[:256])
                    phi_old = basis_eval(old, basis_name, k)
                    phi_inv = basis_eval(inv, basis_name, k)
                    value_err = float((phi_old - phi_inv).abs().max().detach().cpu().item())
                    deriv_old = torch.gradient(phi_old[:, 0], spacing=(old,))[0]
                    deriv_inv = torch.gradient(phi_inv[:, 0], spacing=(old,))[0]
                    deriv_err = float((deriv_old - deriv_inv).abs().median().detach().cpu().item())
                    trans_g, _ = domain_gram(inv, basis_name, k)
                    drift = gram_relative_drift(old_g, trans_g)
                    no_transport_drift = gram_relative_drift(old_g, new_g)
                    extrap = float(((forward < new.min()) | (forward > new.max())).to(dtype=torch.float64).mean().item())
                    debt_delta = extrap - base_extrap
                    value_tol = 5.0e-2 if kind in {"quantile", "monotone_spline"} else 1.0e-4
                    pass_row = int(
                        kind == "no_transport"
                        or (
                            inverse_error <= 0.05
                            and drift <= 0.10
                            and value_err <= value_tol
                            and debt_delta <= 0.05
                            and float(diag.get("transport_monotonicity_violation", 0.0)) == 0.0
                        )
                    )
                    rows.append({
                        "run_status": "completed",
                        "dataset": "synthetic_activation_domain",
                        "seed": seed,
                        "basis_name": basis_name,
                        "basis_k": k,
                        "transport_type": kind,
                        "edge_domain_W1_drift": diag.get("transport_wasserstein1_before", ""),
                        "edge_domain_KS_distance": diag.get("transport_ks_distance", ""),
                        "domain_transport_error": inverse_error,
                        "basis_Gram_transport_drift": drift,
                        "basis_Gram_no_transport_drift": no_transport_drift,
                        "edge_function_value_preservation_error_on_train_points": value_err,
                        "edge_function_derivative_preservation_error": deriv_err,
                        "edge_extrapolation_rate": extrap,
                        "extrapolation_rate_delta": debt_delta,
                        "held_debt_delta_under_transport_only": debt_delta,
                        "domain_transport_unit_pass": pass_row,
                    })
                except Exception as exc:
                    rows.append({"run_status": "exception", "seed": seed, "basis_name": basis_name, "transport_type": kind, "error": repr(exc), "traceback_log": rel(write_exception_log("part_d_transport", exc)), "domain_transport_unit_pass": 0})
    official = [r for r in rows if r.get("transport_type") in {"quantile", "monotone_spline"} and r.get("run_status") == "completed"]
    affine_rows = [r for r in rows if r.get("transport_type") == "affine" and r.get("run_status") == "completed"]
    total = len(official)
    transport_errors = [fval(r.get("domain_transport_error"), 1.0) or 1.0 for r in official]
    gram_drifts = [fval(r.get("basis_Gram_transport_drift"), 1.0) or 1.0 for r in official]
    summary = {
        "gate": "v22_73_part_d_edge_domain_transport_internal_metric",
        "total_rows": len(rows),
        "official_transport_rows": total,
        "selected_transport_types": "quantile,monotone_spline",
        "affine_unit_pass_rows": sum(flag(r.get("domain_transport_unit_pass")) for r in affine_rows),
        "affine_rows": len(affine_rows),
        "domain_transport_unit_pass_rows": sum(flag(r.get("domain_transport_unit_pass")) for r in official),
        "transport_error_median": percentile(transport_errors, 0.5),
        "basis_Gram_transport_drift_median": percentile(gram_drifts, 0.5),
        "extrapolation_rate_nonworse_rows": sum(1 for r in official if (fval(r.get("extrapolation_rate_delta"), 1.0) or 1.0) <= 0.05),
        "transport_only_no_debt_rows": sum(1 for r in official if (fval(r.get("held_debt_delta_under_transport_only"), 1.0) or 1.0) <= 0.05),
        "part_d_transport_gate_pass": 0,
    }
    summary["part_d_transport_gate_pass"] = int(
        total > 0
        and summary["domain_transport_unit_pass_rows"] == total
        and summary["transport_error_median"] <= 0.05
        and summary["basis_Gram_transport_drift_median"] <= 0.10
        and summary["extrapolation_rate_nonworse_rows"] >= math.ceil(0.80 * total)
        and summary["transport_only_no_debt_rows"] >= math.ceil(0.80 * total)
    )
    summary["part_d_preflight_gate_pass"] = summary["part_d_transport_gate_pass"]
    summary["blockers"] = []
    if not summary["part_d_transport_gate_pass"]:
        if summary["domain_transport_unit_pass_rows"] != total:
            summary["blockers"].append("selected_transport_unit_rows_failed")
        if summary["transport_error_median"] > 0.05:
            summary["blockers"].append("transport_error_high")
        if summary["basis_Gram_transport_drift_median"] > 0.10:
            summary["blockers"].append("basis_Gram_transport_drift_high")
        if summary["extrapolation_rate_nonworse_rows"] < math.ceil(0.80 * total):
            summary["blockers"].append("extrapolation_transport_worse")
    write_rows(OUT_ROOT / "v22_73_part_d_edge_domain_transport.csv", rows)
    write_json(OUT_ROOT / "v22_73_part_d_edge_domain_transport_summary.json", summary)
    append_exec(
        "D_edge_domain_transport",
        command,
        "pass" if summary["part_d_transport_gate_pass"] else "fail",
        gpu=str(args.device),
        files=f"{rel(OUT_ROOT / 'v22_73_part_d_edge_domain_transport.csv')}; {rel(OUT_ROOT / 'v22_73_part_d_edge_domain_transport_summary.json')}",
        note=json.dumps({k: summary[k] for k in ["part_d_transport_gate_pass", "official_transport_rows", "blockers"]}, ensure_ascii=False),
    )
    append_recap(
        "Part D edge-domain transport and internal metric",
        [
            f"part_d_transport_gate_pass={summary['part_d_transport_gate_pass']}；official_transport_rows={summary['official_transport_rows']}；unit_pass={summary['domain_transport_unit_pass_rows']}/{summary['official_transport_rows']}。",
            f"selected_transport_types={summary['selected_transport_types']}；affine_unit_pass={summary['affine_unit_pass_rows']}/{summary['affine_rows']}（affine 失败保留为证据，不进入 full-loop gate）。",
            f"transport_error_median={summary['transport_error_median']}；basis_Gram_transport_drift_median={summary['basis_Gram_transport_drift_median']}。",
            f"extrapolation_nonworse={summary['extrapolation_rate_nonworse_rows']}/{summary['official_transport_rows']}；transport_only_no_debt={summary['transport_only_no_debt_rows']}/{summary['official_transport_rows']}；blockers={summary['blockers']}。",
        ],
    )
    return summary
...
def run_part_e(args: argparse.Namespace, part_d: dict[str, Any]) -> dict[str, Any]:
    path = OUT_ROOT / "v22_73_part_e_edge_natural_distributional_preflight.csv"
    if not int(part_d.get("part_d_transport_gate_pass", part_d.get("part_d_preflight_gate_pass", 0))):
        reason = "Part D edge-domain transport gate failed; plan forbids Part E/F escalation."
        write_skipped_artifact("E", path, reason)
        summary = {"gate": "v22_73_part_e_edge_natural_distributional_preflight", "run_status": "skipped", "reason": reason, "part_e_gate_pass": 0}
        write_json(OUT_ROOT / "v22_73_part_e_edge_natural_distributional_preflight_summary.json", summary)
        append_exec("E_edge_natural_distributional_preflight", command_text([PYTHON, rel(RUNNER), "--mode", "part-e"]), "skipped", files=rel(path), note=reason)
        return summary
    datasets = parse_csv(args.part_d_datasets)
    seeds = [int(s) for s in parse_csv(args.part_e_seeds)]
    arches = parse_csv(args.part_e_architectures)
    gate_family = "enr_full_edge_natural_quantile_rawreadout_own_residual_debtcone_controlcontrastive"
    rows: list[dict[str, Any]] = []
    combos = [(d, s, a) for d in datasets for s in seeds for a in arches]
    for dataset, seed, arch in combos[: int(args.part_e_row_limit)]:
        try:
            row = part_d_one_row(dataset, seed, arch, gate_family, args)
            row["candidate_family"] = gate_family
            row["distributional_debt_predicted_pass_rate"] = row.get("distributional_debt_predicted_pass", 0)
            row["debt_false_safe_estimate"] = ""
            row["beats_same_domain_random_preflight"] = flag(row.get("edge_extrapolation_nonworse"))
            row["same_control_gap_mean"] = (
                (fval(row.get("projection_energy_task"), 0.0) or 0.0)
                - max(
                    fval(row.get("same_edge_metric_spectrum_random_capacity"), 0.0) or 0.0,
                    fval(row.get("same_downstream_sensitivity_random_capacity"), 0.0) or 0.0,
                )
            )
            rows.append(row)
        except Exception as exc:
            rows.append({"run_status": "exception", "dataset": dataset, "seed": seed, "architecture": arch, "candidate_family": gate_family, "error": repr(exc), "traceback_log": rel(write_exception_log("part_e_preflight", exc))})
    completed = [r for r in rows if r.get("run_status") == "completed"]
    n = len(completed)
    summary = {
        "gate": "v22_73_part_e_edge_natural_distributional_preflight",
        "run_status": "completed",
        "gate_family": gate_family,
        "completed_rows": n,
        "projection_energy_task_ge_035_rows": sum(1 for r in completed if (fval(r.get("projection_energy_task"), 0.0) or 0.0) >= 0.35),
        "raw_readout_visible_energy_CVaR25_ge_015_rows": sum(1 for r in completed if (fval(r.get("raw_readout_visible_energy_CVaR25"), 0.0) or 0.0) >= 0.15),
        "own_residual_func_fraction_ge_025_rows": sum(1 for r in completed if (fval(r.get("own_residual_func_fraction"), 0.0) or 0.0) >= 0.25),
        "distributional_debt_predicted_pass_rows": sum(flag(r.get("distributional_debt_predicted_pass")) for r in completed),
        "control_contrastive_margin_positive_rows": sum(1 for r in completed if (fval(r.get("control_margin_p10"), 0.0) or 0.0) > 0.0),
        "beats_same_edge_random_preflight_rows": sum(flag(r.get("beats_same_edge_metric_spectrum_random")) for r in completed),
        "beats_same_debt_random_preflight_rows": sum(flag(r.get("beats_same_debt_cone_random")) for r in completed),
        "beats_same_domain_random_preflight_rows": sum(flag(r.get("beats_same_domain_random_preflight")) for r in completed),
    ...
    summary["part_e_gate_pass"] = int(
        ...
        and summary["raw_readout_visible_energy_CVaR25_ge_015_rows"] >= 36
        and summary["own_residual_func_fraction_ge_025_rows"] >= 30
        and summary["distributional_debt_predicted_pass_rows"] >= 36
        and summary["control_contrastive_margin_positive_rows"] >= 30
        and summary["beats_same_edge_random_preflight_rows"] >= 30
        and summary["beats_same_debt_random_preflight_rows"] >= 30
        and summary["beats_same_domain_random_preflight_rows"] >= 30
        and summary["basis_Gram_condition_pass_rows"] >= 36
        and summary["edge_extrapolation_rate_nonworse_rows"] >= 36
    )
    blockers = []
    if summary["projection_energy_task_ge_035_rows"] < 36:
        blockers.append("projection_low")
    if summary["raw_readout_visible_energy_CVaR25_ge_015_rows"] < 36:
        blockers.append("raw_readout_low")
    if summary["own_residual_func_fraction_ge_025_rows"] < 30:
        blockers.append("own_residual_func_low")
    if summary["distributional_debt_predicted_pass_rows"] < 36:
        blockers.append("debt_predicted_pass_low")
    if summary["control_contrastive_margin_positive_rows"] < 30:
        blockers.append("control_margin_negative")
    if summary["beats_same_edge_random_preflight_rows"] < 30:
        blockers.append("same_edge_control_not_beaten")
    if summary["beats_same_debt_random_preflight_rows"] < 30:
        blockers.append("same_debt_control_not_beaten")
    summary["blockers"] = blockers
    summary["repair_attempts"] = [
        "raw readout: active edge support uses direct raw w2 readout energy in parameter flatten order.",
        "own residual: preflight records source/witness functional residual through phi @ update.",
        "debt: preflight builds train-only query distributional constraints with ECE/Brier/tail95/tail99/margin modes.",
        "controls: same edge/raw/own/debt/domain random controls are recorded before full-loop.",
    ]
    write_rows(path, rows)
    write_json(OUT_ROOT / "v22_73_part_e_edge_natural_distributional_preflight_summary.json", summary)
    append_exec("E_edge_natural_distributional_preflight", command_text([PYTHON, rel(RUNNER), "--mode", "part-e"]), "pass" if summary["part_e_gate_pass"] else "fail", files=f"{rel(path)}; {rel(OUT_ROOT / 'v22_73_part_e_edge_natural_distributional_preflight_summary.json')}", note=json.dumps({"part_e_gate_pass": summary["part_e_gate_pass"], "completed_rows": n, "blockers": blockers}, ensure_ascii=False, sort_keys=True))
    append_recap(
        "Part E edge-natural residual preflight with matched controls",
        [
            f"part_e_gate_pass={summary['part_e_gate_pass']}；completed_rows={summary['completed_rows']}；blockers={blockers}。",
            f"projection>=0.35 {summary['projection_energy_task_ge_035_rows']}/{n}；raw>=0.15 {summary['raw_readout_visible_energy_CVaR25_ge_015_rows']}/{n}；own_func>=0.25 {summary['own_residual_func_fraction_ge_025_rows']}/{n}；dist_debt_pass {summary['distributional_debt_predicted_pass_rows']}/{n}。",
            f"controls: margin>0 {summary['control_contrastive_margin_positive_rows']}/{n}；same_edge {summary['beats_same_edge_random_preflight_rows']}/{n}；same_debt {summary['beats_same_debt_random_preflight_rows']}/{n}；same_domain {summary['beats_same_domain_random_preflight_rows']}/{n}。",
            f"Artifacts: `{rel(path)}`, `{rel(OUT_ROOT / 'v22_73_part_e_edge_natural_distributional_preflight_summary.json')}`。",
        ],
    )
    return summary
...
def differentiable_debt_value(logits: Any, y: Any, mode: str) -> Any:
    import torch
    import torch.nn.functional as F

    losses = F.cross_entropy(logits.float(), y.long(), reduction="none")
    probs = torch.softmax(logits.float(), dim=1)
    one_hot = F.one_hot(y.long(), num_classes=int(logits.shape[1])).float()
    if mode == "Brier":
        return (probs - one_hot).square().sum(dim=1).mean()
    if mode == "tail95":
        take = max(1, int(math.ceil(0.05 * int(losses.numel()))))
        return torch.topk(losses, take).values.mean()
    if mode == "tail99":
        take = max(1, int(math.ceil(0.01 * int(losses.numel()))))
        return torch.topk(losses, take).values.mean()
    if mode == "margin":
        true = logits.gather(1, y.long().reshape(-1, 1)).reshape(-1)
        masked = logits.clone()
        masked.scatter_(1, y.long().reshape(-1, 1), -1.0e9)
        margin = true - masked.max(dim=1).values
        return -torch.quantile(margin.float(), 0.10)
    conf, pred = probs.max(dim=1)
    correct = (pred == y.long()).float()
    # Differentiable overconfidence / soft-ECE proxy used only for train-query prediction.
    return torch.relu(conf - correct).mean() + 0.25 * torch.abs(conf.mean() - correct.mean())


def hard_debt_values_from_logits(logits: Any, y: Any) -> dict[str, float]:
    import torch
    import torch.nn.functional as F
    from dgkan.metrics.calibration import ece_from_confidence
    ...
    margin = true - masked.max(dim=1).values
    return {
        "ECE": float(ece_from_confidence(conf.detach().cpu().tolist(), correct.detach().cpu().tolist(), bins=15)),
        "Brier": float((probs - one_hot).square().sum(dim=1).mean().detach().cpu().item()),
        "tail95": float(torch.quantile(losses.float(), 0.95).detach().cpu().item()),
        "tail99": float(torch.quantile(losses.float(), 0.99).detach().cpu().item()),
        "margin": float((-torch.quantile(margin.float(), 0.10)).detach().cpu().item()),
    }


def build_distributional_constraints(
    model: Any,
    design: dict[str, Any],
    x_metric: Any,
    y_metric: Any,
    ...
    from dgkan.fu.kan_distributional_edge_natural_residual import DistributionalDebtConstraint, distributional_debt_cone_project

    phi_raw = design["phi_raw"].detach().to(dtype=torch.float64)
    with torch.no_grad():
        base_logits = model(x_metric).float().detach()
    n, classes = int(base_logits.shape[0]), int(base_logits.shape[1])
    if int(phi_raw.shape[0]) != n * classes:
        return [], {"distributional_constraint_build_error": "phi_raw/logit_shape_mismatch"}
    ref = reference_grad.detach().reshape(-1).to(device=phi_raw.device, dtype=torch.float64)
    if int(ref.numel()) != int(phi_raw.shape[1]):
        return [], {"distributional_constraint_build_error": "reference_grad_dim_mismatch"}
    ref_norm = torch.linalg.norm(ref).clamp_min(1.0e-12)
    unit_param_update = -float(args.lr) * ref / ref_norm
    eps = float(args.debt_fd_epsilon)
    constraints = []
    pred_rows = []
    splits = []
    half = max(8, n // 2)
    splits.append(("q0", torch.arange(0, half, device=phi_raw.device)))
    splits.append(("q1", torch.arange(half, n, device=phi_raw.device) if half < n else torch.arange(0, half, device=phi_raw.device)))
    for cohort_name, idx in splits:
        if int(idx.numel()) < 2:
            continue
        ...
        logits_q = base_logits[idx].to(device=phi_raw.device, dtype=torch.float64)
        y_q = y_metric[idx].long()
        for mode in ["ECE", "Brier", "tail95", "tail99", "margin"]:
            delta = torch.zeros(int(phi_q.shape[1]), device=phi_raw.device, dtype=torch.float64, requires_grad=True)
            logits_delta = logits_q + (phi_q @ delta).reshape(int(idx.numel()), classes)
            value = differentiable_debt_value(logits_delta, y_q, mode)
            grad = torch.autograd.grad(value, delta, retain_graph=False, create_graph=False)[0].detach().to(dtype=torch.float64)
            plus_logits = logits_q + (phi_q @ (eps * unit_param_update)).reshape(int(idx.numel()), classes)
            minus_logits = logits_q - (phi_q @ (eps * unit_param_update)).reshape(int(idx.numel()), classes)
            base_hard = hard_debt_values_from_logits(logits_q, y_q)[mode]
            plus_hard = hard_debt_values_from_logits(plus_logits, y_q)[mode]
            minus_hard = hard_debt_values_from_logits(minus_logits, y_q)[mode]
            fd = (plus_hard - minus_hard) / max(2.0 * eps, 1.0e-12)
            curvature = (plus_hard - 2.0 * base_hard + minus_hard) / max(eps * eps, 1.0e-12)
            # Optimizer receives gradient g, while the parameter update is -lr*g.
            direction = -float(args.lr) * grad
            curvature_used = max(float(curvature), 0.0)
            if mode == "Brier":
                curvature_used = max(curvature_used, float(args.debt_brier_curvature_floor))
            elif mode in {"tail95", "tail99"}:
                curvature_used = max(curvature_used, float(args.debt_tail_curvature_floor))
            constraints.append(
                DistributionalDebtConstraint(
                    direction=direction.detach().cpu(),
                    mode=mode,
                    cohort=cohort_name,
                    curvature=curvature_used * float(args.debt_curvature_scale),
                    slack=float(args.debt_slack),
                    budget=0.0,
                    alpha=1.0,
                )
            )
            pred_rows.append({
                ...
                "mode": mode,
                "hard_base": base_hard,
                "fd_along_reference": fd,
                "curvature_along_reference": curvature,
                "curvature_used": curvature_used * float(args.debt_curvature_scale),
                "direction_norm": float(torch.linalg.norm(direction).detach().cpu().item()),
            })
    if constraints:
        _proj, diag = distributional_debt_cone_project(ref, constraints)
    else:
        diag = {}
    return constraints, {
        "distributional_constraints_built": len(constraints),
        "distributional_predictor_param_order": design.get("param_order", ""),
        "distributional_predictor_modes": "ECE,Brier,tail95,tail99,margin",
        "distributional_replay_rows": len(pred_rows),
        **diag,
    }
...
def build_edge_optimizer(model: Any, x_metric: Any, y_metric: Any, method: str, args: argparse.Namespace, *, control_seed: int = 0) -> tuple[Any, dict[str, Any]]:
    ...
    use_raw = method.startswith(("edge_nat", "enr_")) or any(token in method for token in ["rawreadout", "raw_readout", "transport", "domain", "bankbudget", "poetpion"])
    use_own = any(token in method for token in ["ownresidual", "own_residual", "funcres", "functional_own", "poetpion_residual"])
    use_debt = any(token in method for token in ["debt", "distdebt", "tailcvar"])
    ...
    if use_debt and int(own_grad.numel()) == int(raw_mult.numel()):
        dist_constraints, debt_diag = build_distributional_constraints(model, design, x_metric, y_metric, cand_grad, args)
    if "same_edge_random_control" in method:
        ...
        own_basis = None
        dist_constraints = []
    if "same_debt_cone_control" in method and dist_constraints:
        gen = torch.Generator(device="cpu").manual_seed(int(control_seed) + 991)
        dist_constraints = [
            DistributionalDebtConstraint(
        ...
        _proj, debt_diag = distributional_debt_cone_project(cand_grad, dist_constraints)
    if int(phi.numel()) and int(cand_grad.numel()) == int(phi.shape[1]):
        cand_func_grad = cand_grad.detach().reshape(-1).to(dtype=torch.float64)
        if own_basis is not None:
            cand_func_grad, _func_project_diag = own_reference_residual_project(cand_func_grad, own_basis)
            cand_func_grad = cand_func_grad.reshape(-1)
        cand_disp = phi @ (-float(args.lr) * cand_func_grad.to(device=phi.device, dtype=torch.float64))
        own_disp = None
        if own_basis is not None:
            own_disp = phi @ own_basis.to(device=phi.device, dtype=torch.float64)
        own_func_frac, own_func_overlap = functional_residual_fraction(cand_disp, own_disp)
    else:
        own_func_frac, own_func_overlap = 1.0, 0.0
    state = DistributionalEdgeNaturalResidualState(
        ...
        distributional_constraints=dist_constraints if use_debt or "same_debt_cone_control" in method else [],
        transform_scale=float(args.edge_transform_scale),
    )
    opt = DistributionalEdgeNaturalResidualOptimizer(model.named_parameters(), lr=float(args.lr), weight_decay=float(args.weight_decay), edge_states={"w2": state})
    diag = {
        **raw_diag,
        **own_diag,
        **debt_diag,
        ...
        "own_residual_func_fraction": own_func_frac,
        "own_residual_func_overlap_after": own_func_overlap,
        ...
        "distributional_debt_constraints": debt_diag.get("distributional_debt_constraints", 0.0),
    }
    model.zero_grad(set_to_none=True)
    return opt, diag
...
def train_task_model(model: Any, opt: Any, bundle: dict[str, Any], args: argparse.Namespace, device: Any, *, method: str, seed: int) -> dict[str, Any]:
    ...
    losses: list[float] = []
    nan_inf_count = 0
    refresh_context = getattr(args, "_edge_refresh_context", None)
    ...
    for _step in range(int(args.steps)):
        if refresh_context is not None and refresh_interval > 0 and _step > 0 and _step % refresh_interval == 0 and hasattr(opt, "edge_states"):
            try:
                refresh_opt, refresh_diag = build_edge_optimizer(
                    model,
                    refresh_context["x_metric"],
                    refresh_context["y_metric"],
                    str(refresh_context["method"]),
                    args,
                    control_seed=int(refresh_context["control_seed"]),
                )
...
def part_f_train_group(dataset: str, seed: int, arch: str, args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    def run_kan(method: str, row_kind: str) -> dict[str, Any]:
        shared_kan_seed = int(seed) + sum(ord(c) for c in arch) + 73073
        set_seed(shared_kan_seed)
        model = make_probe_kan(arch, bundle, device, int(args.hidden), shared_kan_seed)
        if row_kind == "reference":
            opt = torch.optim.AdamW(model.parameters(), lr=float(args.lr), weight_decay=float(args.weight_decay))
            edge_diag: dict[str, Any] = {}
            refresh_context = None
        else:
            control_seed = int(seed) + sum(ord(c) for c in method)
            opt, edge_diag = build_edge_optimizer(model, x_metric, y_metric, method, args, control_seed=control_seed)
            refresh_context = {
                "x_metric": x_metric,
                "y_metric": y_metric,
                "method": method,
        ...
        old_refresh_context = getattr(args, "_edge_refresh_context", None)
        args._edge_refresh_context = refresh_context
        try:
            metrics = train_task_model(model, opt, bundle, args, device, method=method, seed=shared_kan_seed)
        finally:
            args._edge_refresh_context = old_refresh_context
        return {
            "run_status": "completed",
            "row_kind": row_kind,
            "dataset": dataset,
...
def run_part_f(args: argparse.Namespace, part_d: dict[str, Any], part_e: dict[str, Any]) -> dict[str, Any]:
    path = OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_matrix.csv"
    if not int(part_e.get("part_e_gate_pass", 0)):
        reason = "Part E edge-natural distributional preflight failed; plan says do not enter true target-free full-loop."
        write_skipped_artifact("F", path, reason)
        summary = {"gate": "v22_73_part_f_edge_distributional_full_loop", "run_status": "skipped", "reason": reason, "part_f_exploration_gate_pass": 0, "official_candidate_gate_pass": 0}
        write_json(OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_summary.json", summary)
        append_exec("F_edge_distributional_full_loop", command_text([PYTHON, rel(RUNNER), "--mode", "part-f"]), "skipped", files=rel(path), note=reason)
        return summary
    command = command_text([
        ...
        args.hidden,
        "--edge-active-cols",
        args.edge_active_cols,
        "--control-contrastive-cols",
        args.control_contrastive_cols,
        "--debt-fd-epsilon",
        args.debt_fd_epsilon,
        "--debt-curvature-scale",
        args.debt_curvature_scale,
        "--debt-slack",
        args.debt_slack,
        "--debt-brier-curvature-floor",
        args.debt_brier_curvature_floor,
        "--debt-tail-curvature-floor",
        args.debt_tail_curvature_floor,
        ...
        "--part-f-architectures",
        args.part_f_architectures,
        "--part-f-group-limit",
    ...
    finite_candidates = [
        r
        for r in candidates
        if int(fval(r.get("NaN_or_inf_count"), 1.0) or 0) == 0
        and fval(r.get("final_NLL"), None) is not None
        and fval(r.get("Delta_NLL_vs_own_ref"), None) is not None
        and fval(r.get("Delta_NLL_vs_MLP_matched"), None) is not None
        and fval(r.get("Delta_NLL_vs_same_edge_control"), None) is not None
        and fval(r.get("Delta_NLL_vs_same_debt_cone_control"), None) is not None
    ]
    n_finite = len(finite_candidates)

    def finite_mean(col: str) -> float:
        vals = [fval(r.get(col), None) for r in finite_candidates]
        vals = [float(v) for v in vals if v is not None]
        return sum(vals) / max(1, len(vals))

    summary = {
        "gate": "v22_73_part_f_edge_distributional_full_loop",
        "run_status": "completed_true_edge_optimizer",
        "part_f_runtime_source": "v22_73_DistributionalEdgeNaturalResidualOptimizer",
        "completed_rows": n,
        "finite_candidate_rows": n_finite,
        "nan_or_inf_candidate_rows": n - n_finite,
        ...
        "own_residual_func_fraction_ge_025_rows": sum(1 for r in candidates if (fval(r.get("own_residual_func_fraction"), 0.0) or 0.0) >= 0.25),
        "distributional_debt_cone_violation_le_0_rows": sum(1 for r in candidates if float(fval(r.get("distributional_debt_cone_violation"), 1.0) if fval(r.get("distributional_debt_cone_violation"), 1.0) is not None else 1.0) <= 1.0e-8),
        "loss_total_is_task_loss_only_rows": sum(flag(r.get("loss_total_is_task_loss_only")) for r in candidates),
        "MLP_target_used_in_official_runtime": 0,
        "edge_refresh_interval": int(getattr(args, "edge_refresh_interval", 0) or 0),
        "edge_refresh_rows": sum(1 for r in candidates if (fval(r.get("edge_refresh_count"), 0.0) or 0.0) > 0.0),
        "edge_refresh_total_count": sum(int(fval(r.get("edge_refresh_count"), 0.0) or 0) for r in candidates),
        "edge_refresh_failure_rows": sum(1 for r in candidates if (fval(r.get("edge_refresh_failures"), 0.0) or 0.0) > 0.0),
        "edge_transform_scale": float(getattr(args, "edge_transform_scale", 1.0) or 1.0),
        ...
        "finite_mean_Delta_NLL_vs_own": finite_mean("Delta_NLL_vs_own_ref"),
        "finite_mean_Delta_NLL_vs_MLP": finite_mean("Delta_NLL_vs_MLP_matched"),
        "finite_mean_Delta_NLL_vs_same_edge": finite_mean("Delta_NLL_vs_same_edge_control"),
        "finite_mean_Delta_NLL_vs_same_debt": finite_mean("Delta_NLL_vs_same_debt_cone_control"),
        "part_f_exploration_gate_pass": 0,
        "official_candidate_gate_pass": 0,
    }
    gate_components = {
        "completed": summary["completed_rows"] >= 45,
        "finite_candidates": summary["finite_candidate_rows"] >= 45,
        ...
        "own_residual_func": summary["own_residual_func_fraction_ge_025_rows"] >= 30,
        "distributional_debt_cone": summary["distributional_debt_cone_violation_le_0_rows"] >= 36,
    }
    ...
    write_json(OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_summary.json", summary)
    append_exec(
        "F_edge_distributional_full_loop",
        command,
        "pass" if summary["part_f_exploration_gate_pass"] else "fail",
        gpu=str(args.device),
        files=f"{rel(path)}; {rel(OUT_ROOT / 'v22_73_part_f_edge_distributional_full_loop_summary.json')}",
    ...
    append_recap(
        ...
        [
            ...
            f"finite_candidate_rows={summary['finite_candidate_rows']}/{n}；nan_or_inf_candidate_rows={summary['nan_or_inf_candidate_rows']}/{n}。",
            ...
            f"edge counts: raw>=0.15 {summary['raw_readout_visible_ge_015_rows']}/{n}；own_func>=0.25 {summary['own_residual_func_fraction_ge_025_rows']}/{n}；dist_debt<=0 {summary['distributional_debt_cone_violation_le_0_rows']}/{n}。",
            f"edge refresh: interval={summary['edge_refresh_interval']}；rows={summary['edge_refresh_rows']}/{n}；total_count={summary['edge_refresh_total_count']}；failure_rows={summary['edge_refresh_failure_rows']}。",
            f"edge transform scale={summary['edge_transform_scale']}。",
            ...
            f"finite mean deltas: vs_own={summary['finite_mean_Delta_NLL_vs_own']}；vs_MLP={summary['finite_mean_Delta_NLL_vs_MLP']}；vs_same_edge={summary['finite_mean_Delta_NLL_vs_same_edge']}；vs_same_debt={summary['finite_mean_Delta_NLL_vs_same_debt']}。",
...
def run_part_g(args: argparse.Namespace, part_f: dict[str, Any]) -> dict[str, Any]:
    path = OUT_ROOT / "v22_73_part_g_failure_decomposition.csv"
    f_path = OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_matrix.csv"
    if not f_path.exists():
        reason = "Part F matrix is missing; failure decomposition cannot be built."
        write_skipped_artifact("G", path, reason)
        summary = {"gate": "v22_73_part_g_failure_decomposition", "run_status": "skipped", "reason": reason, "part_g_gate_pass": 0}
        write_json(OUT_ROOT / "v22_73_part_g_failure_decomposition_summary.json", summary)
        append_exec("G_failure_decomposition", command_text([PYTHON, rel(RUNNER), "--mode", "part-g"]), "skipped", files=rel(path), note=reason)
        return summary

    f_rows = read_rows(f_path)
    refs = {
        (r.get("dataset", ""), r.get("seed", ""), r.get("architecture", "")): r
        for r in f_rows
    ...
    mode_specs = [
        ("ECE", "ECE", "positive"),
        ("Brier", "Brier", "positive"),
        ("tail95", "tail_loss_q95", "positive"),
        ("tail99", "tail_loss_q99", "positive"),
        ("margin", "margin_q10", "negative"),
    ]
    rows: list[dict[str, Any]] = []
    for row in candidates:
        key = (row.get("dataset", ""), row.get("seed", ""), row.get("architecture", ""))
        ref = refs.get(key, {})
        mode_flags: dict[str, int] = {}
        mode_deltas: dict[str, float | str] = {}
        finite_modes = 1
        for mode, col, direction in mode_specs:
            cand_val = fval(row.get(col), None)
            ref_val = fval(ref.get(col), None)
            if cand_val is None or ref_val is None:
                mode_deltas[mode] = ""
                mode_flags[mode] = 0
                finite_modes = 0
                continue
            delta = float(cand_val) - float(ref_val)
            mode_deltas[mode] = delta
            if direction == "negative":
                mode_flags[mode] = int(delta < -1.0e-12)
            else:
                mode_flags[mode] = int(delta > 1.0e-12)
        failures = [m for m, v in mode_flags.items() if v]
        dist_violation = fval(row.get("distributional_debt_cone_violation"), None)
        pred_safe = int(dist_violation is not None and dist_violation <= 1.0e-8)
        finite_candidate = int(
            int(fval(row.get("NaN_or_inf_count"), 1.0) or 0) == 0
            and fval(row.get("final_NLL"), None) is not None
            and finite_modes
        )
        rows.append({
            "dataset": row.get("dataset", ""),
            ...
            "NaN_or_inf_count": row.get("NaN_or_inf_count", ""),
            "Delta_NLL_vs_own": row.get("Delta_NLL_vs_own_ref", ""),
            "Delta_NLL_vs_MLP": row.get("Delta_NLL_vs_MLP_matched", ""),
            "Delta_NLL_vs_same_edge": row.get("Delta_NLL_vs_same_edge_control", ""),
            "Delta_NLL_vs_same_debt": row.get("Delta_NLL_vs_same_debt_cone_control", ""),
            "ECE_delta_vs_own": mode_deltas["ECE"],
            "Brier_delta_vs_own": mode_deltas["Brier"],
            "tail95_delta_vs_own": mode_deltas["tail95"],
            "tail99_delta_vs_own": mode_deltas["tail99"],
            "margin_q10_delta_vs_own": mode_deltas["margin"],
            "ECE_debt": mode_flags["ECE"],
            "Brier_debt": mode_flags["Brier"],
            "tail95_debt": mode_flags["tail95"],
            "tail99_debt": mode_flags["tail99"],
            "margin_debt": mode_flags["margin"],
            "actual_any_debt": int(bool(failures)),
            "distributional_debt_predicted_safe": pred_safe,
            "distributional_debt_cone_violation": row.get("distributional_debt_cone_violation", ""),
            "distributional_false_safe": int(pred_safe and bool(failures)),
            "raw_readout_visible_energy_CVaR25": row.get("raw_readout_visible_energy_CVaR25", ""),
            "own_residual_func_fraction": row.get("own_residual_func_fraction", ""),
            "failure_mode": ",".join(failures) if failures else "no_debt_failure_not_observed",
        })

    candidate_n = len(rows)
    pred_safe_n = sum(flag(r.get("distributional_debt_predicted_safe")) for r in rows)
    finite_n = sum(flag(r.get("finite_candidate")) for r in rows)
    false_safe_any = sum(flag(r.get("distributional_false_safe")) for r in rows)
    mode_counts = {f"{mode}_debt_rows": sum(flag(r.get(f"{mode}_debt")) for r in rows) for mode, _col, _direction in mode_specs}
    mode_false_safe = {
        f"{mode}_false_safe_rows": sum(flag(r.get("distributional_debt_predicted_safe")) and flag(r.get(f"{mode}_debt")) for r in rows)
        for mode, _col, _direction in mode_specs
    }
    dominant_debt_mode = ""
    if mode_counts:
        dominant_debt_mode = max(mode_counts.items(), key=lambda kv: kv[1])[0].replace("_debt_rows", "")
    recommendation = "no_debt_gate_passed_or_no_dominant_mode"
    if dominant_debt_mode == "Brier":
        recommendation = "Brier dominates: increase probability-space/Brier curvature guard or calibrated positive slack, then rerun Part F while preserving task/control margins."
    elif dominant_debt_mode in {"tail95", "tail99"}:
        recommendation = "Tail debt dominates: adjust CVaR cohort size and tail quantile smoothing before another full-loop."
    elif dominant_debt_mode == "ECE":
        recommendation = "ECE dominates: refine soft-ECE/overconfidence margin proxy before another full-loop."
    elif dominant_debt_mode == "margin":
        recommendation = "Margin debt dominates: add margin-query cohorts or a margin-specific budget before another full-loop."
    summary = {
        "gate": "v22_73_part_g_failure_decomposition",
        "run_status": "completed_failure_decomposition",
        "candidate_rows": candidate_n,
        "finite_candidate_rows": finite_n,
        "nan_or_inf_candidate_rows": candidate_n - finite_n,
        "no_debt_rows": int(part_f.get("no_debt_rows", 0) or 0),
        "distributional_predicted_safe_rows": pred_safe_n,
        "false_safe_any_debt_rows": false_safe_any,
        "false_safe_rate_among_predicted_safe": false_safe_any / max(1, pred_safe_n),
        "dominant_debt_mode": dominant_debt_mode,
    ...
    write_json(OUT_ROOT / "v22_73_part_g_failure_decomposition_summary.json", summary)
    append_exec(
        "G_failure_decomposition",
        command_text([PYTHON, rel(RUNNER), "--mode", "part-g"]),
        "pass",
        files=f"{rel(path)}; {rel(OUT_ROOT / 'v22_73_part_g_failure_decomposition_summary.json')}",
        note=json.dumps({k: summary[k] for k in ["candidate_rows", "false_safe_any_debt_rows", "dominant_debt_mode"]}, ensure_ascii=False),
    )
    append_recap(
        "Part G failure decomposition",
        [
            f"candidate_rows={candidate_n}；finite_candidate_rows={finite_n}；nan_or_inf_candidate_rows={candidate_n - finite_n}。",
            f"distributional_predicted_safe_rows={pred_safe_n}；false_safe_any_debt_rows={false_safe_any}；false_safe_rate={summary['false_safe_rate_among_predicted_safe']}。",
            f"mode debt rows: ECE={mode_counts['ECE_debt_rows']}；Brier={mode_counts['Brier_debt_rows']}；tail95={mode_counts['tail95_debt_rows']}；tail99={mode_counts['tail99_debt_rows']}；margin={mode_counts['margin_debt_rows']}。",
            f"dominant_debt_mode={dominant_debt_mode}；repair_recommendation={recommendation}。",
            f"artifact=`{rel(path)}`。",
        ],
    )
    return summary
...
def final_route(part_a: dict[str, Any], part_b: dict[str, Any], part_c: dict[str, Any], part_d: dict[str, Any], part_e: dict[str, Any], part_f: dict[str, Any], part_g: dict[str, Any]) -> dict[str, Any]:
    if not int(part_a.get("part_a_hard_gate_pass", 0)):
        ...
        route = "R1-DebtPredictorNotCalibrated"
        reason = "Part C distributional debt predictor gate failed."
    elif not int(part_b.get("part_b_reanalysis_complete", 0)):
        route = "R0-CodeOrTrainingBoundaryFailed"
        reason = "Part B required reanalysis artifacts were incomplete."
    elif not int(part_d.get("part_d_transport_gate_pass", part_d.get("part_d_preflight_gate_pass", 0))):
        route = "R2-EdgePreflightNotOpened"
        reason = "Part D edge-domain transport/internal metric gate failed."
    elif not int(part_e.get("part_e_gate_pass", 0)):
        failures = set(part_e.get("blockers", []) or [])
        if "same_edge_control_not_beaten" in failures or "same_debt_control_not_beaten" in failures:
            route = "R3-EdgeSupportOrDebtConeExplained_NoFU"
        else:
            route = "R2-EdgePreflightNotOpened"
        reason = "Part E edge-natural distributional preflight failed."
    elif str(part_f.get("run_status")) in {"not_executed_after_part_d_pass", "skipped_pending_implementation", "skipped"}:
        route = "R2-EdgePreflightNotOpened"
        reason = "Part F was not executed after preflight."
        ...
        if "no_debt" in failures:
            route = "R4-EdgeNaturalRuntimeOpenedButDebtBlocked"
        elif "KAN_improves_own" in failures:
            route = "R5-EdgeNaturalRuntimeOpenedButOwnReferenceBlocked"
        elif "KAN_beats_MLP_matched" in failures:
            route = "R8-KANBasisFamilyNotSuperiorUnderCurrentEdgeMetric"
        else:
            route = "R3-EdgeSupportOrDebtConeExplained_NoFU"
        reason = "Part F exploration failed."
    else:
        route = "R6-EdgeNaturalKANExplorationOpened"
    ...
    obj = {
        ...
        "part_d_preflight_gate_pass": int(part_d.get("part_d_transport_gate_pass", part_d.get("part_d_preflight_gate_pass", 0))),
        ...
        "artifacts": {
            "part_a": rel(OUT_ROOT / "v22_73_part_a_code_identity_hard_gate.json"),
            "part_b": rel(OUT_ROOT / "v22_73_part_b_failure_replay_debt_calibration.csv"),
            "part_c": rel(OUT_ROOT / "v22_73_part_c_edge_metric_unit_tests.csv"),
            "part_d": rel(OUT_ROOT / "v22_73_part_d_edge_domain_transport.csv"),
            "part_e": rel(OUT_ROOT / "v22_73_part_e_edge_natural_distributional_preflight.csv"),
            "part_f": rel(OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_matrix.csv"),
            "part_g": rel(OUT_ROOT / "v22_73_part_g_failure_decomposition.csv"),
        },
    }
    write_json(OUT_ROOT / "v22_73_final_route.json", obj)
    append_recap(
        "最终 route 判定",
        [
            f"final_route={route}；reason={reason}",
            f"Part gates: A={obj['part_a_hard_gate_pass']} B={obj['part_b_reanalysis_complete']} C={obj['part_c_gate_pass']} D={obj['part_d_preflight_gate_pass']} E={obj['part_e_gate_pass']} F_exploration={obj['part_f_exploration_gate_pass']} official={obj['official_candidate_gate_pass']}。",
            f"Part D blockers={obj['part_d_blockers']}。",
            f"关键 artifact：`{rel(OUT_ROOT / 'v22_73_final_route.json')}`。",
        ],
    )
    append_exec("final_route", command_text([PYTHON, rel(RUNNER), "--mode", "full"]), "done", files=rel(OUT_ROOT / "v22_73_final_route.json"), note=json.dumps({"final_route": route, "reason": reason}, ensure_ascii=False))
    return obj
...
def run_full(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not int(part_a.get("part_a_hard_gate_pass", 0)):
        for part, path in [
            ("B", OUT_ROOT / "v22_73_part_b_failure_replay_debt_calibration.csv"),
            ("C", OUT_ROOT / "v22_73_part_c_edge_metric_unit_tests.csv"),
            ("D", OUT_ROOT / "v22_73_part_d_edge_domain_transport.csv"),
            ("E", OUT_ROOT / "v22_73_part_e_edge_natural_distributional_preflight.csv"),
            ("F", OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_matrix.csv"),
            ("G", OUT_ROOT / "v22_73_part_g_mlp_matched_audit.csv"),
        ]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--edge-active-cols", type=int, default=128)
    ...
    p.add_argument("--part-e-architectures", default="DGKAN_DFOU")
    p.add_argument("--part-e-seeds", default="0,1,2,3,4,5,6,7,8")
    ...
    p.add_argument("--part-f-architectures", default="DGKAN_DFOU")
    p.add_argument("--part-f-group-limit", type=int, default=15)
    p.add_argument("--part-f-methods", default="enr_distdebt_w2_rank4,enr_domain_quantile_funcres_rank4,enr_full_edge_natural_tailcvar_rank4")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.mode == "part-a":
            ...
            dpath = OUT_ROOT / "v22_73_part_d_edge_domain_transport_summary.json"
            part_d = json.loads(dpath.read_text(encoding="utf-8")) if dpath.exists() else {"part_d_transport_gate_pass": 0}
            ...
            dpath = OUT_ROOT / "v22_73_part_d_edge_domain_transport_summary.json"
            epath = OUT_ROOT / "v22_73_part_e_edge_natural_distributional_preflight_summary.json"
            part_d = json.loads(dpath.read_text(encoding="utf-8")) if dpath.exists() else {"part_d_transport_gate_pass": 0}
            ...
            fpath = OUT_ROOT / "v22_73_part_f_edge_distributional_full_loop_summary.json"
