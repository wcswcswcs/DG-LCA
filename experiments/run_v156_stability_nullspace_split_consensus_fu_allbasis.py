#!/usr/bin/env python3
"""DG-KAN v15.6 stability-nullspace split-consensus FU runner.

This version replays v15.04/v15.5 G7 metric-only source direction, then applies
train-stream-only hazard tangent nullspace or constrained source-retaining
projection. Audit/tail/LineC metrics are never used to build the direction.
"""
...
PLAN_DOC = ROOT / "docs/DG-KAN_v15.6_StabilityNullspaceSplitConsensusFU_AllBasisAcceleration_完整计划.md"
RECAP_DOC = ROOT / "docs/DG-KAN_v15.6_StabilityNullspaceSplitConsensusFU_AllBasisAcceleration_实验结果复盘.md"
EXEC_LOG_DOC = ROOT / "docs/DG-KAN_v15.6_StabilityNullspaceSplitConsensusFU_AllBasisAcceleration_执行日志.md"
DEFAULT_OUT = ROOT / "results/v15_6_stability_nullspace_split_consensus_fu_allbasis/official_v156"
DEFAULT_LINE_D_OUT = ROOT / "results/v15_6_stability_nullspace_split_consensus_fu_allbasis/line_d_v156_allbasis_substrate"
MANUAL_ANALYSIS_START = "<!-- V15.6_MANUAL_ANALYSIS_START -->"
MANUAL_ANALYSIS_END = "<!-- V15.6_MANUAL_ANALYSIS_END -->"
...
M_METHODS = [
    ...
    "M4-MLP-SameHazardNullControl",
    "M5-MLP-SameTrustScalarControl",
    "M6-MLP-RandomNullspaceSameRank",
    "M7-MLP-NoOpMatchedOverhead",
]
M_CONTROL_SET = {"M0-MLP-AdamW", "M1-MLP-CautiousAdamW", "M2-MLP-MGUP", "M6-MLP-RandomNullspaceSameRank", "M7-MLP-NoOpMatchedOverhead"}
...
REQUIRED = [
    "v156_route_decision.json",
    "v156_method_surface_manifest.csv",
    "v156_direction_provenance.csv",
    "v156_forbidden_information_audit.csv",
    "v156_no_action_search_audit.csv",
    "v156_required_artifact_manifest.csv",
    "v156_code_review_manifest.csv",
    "v156_execution_contract_coverage_audit.csv",
    "v156_deep_coverage_audit.csv",
    "v156_gate_route_recompute.csv",
    "v156_line_s_split_consensus_subspace.csv",
    "v156_line_s_k_batch_sensitivity.csv",
    "v156_line_s_fallback_results.csv",
    "v156_line_u_source_hazard_decomposition.csv",
    "v156_line_b_proxy_audit.csv",
    "v156_line_b_leaveout.csv",
    "v156_line_b_gate_summary.csv",
    "v156_line_nq_stability_results.csv",
    "v156_line_nq_controls.csv",
    "v156_line_nq_fallback_results.csv",
    "v156_line_nq_exhaustion_certificate.csv",
    "v156_line_p_micro_horizon_audit.csv",
    "v156_line_m_mlp_generic_controls.csv",
    "v156_line_m_kan_specific_delta.csv",
    "v156_line_x_transfer_operator_audit.csv",
    "v156_line_d_allbasis_substrate_results.csv",
    "v156_line_d_family_summary.csv",
    "v156_line_d_family_failure_taxonomy.csv",
    "v156_dche_no_regression_monitor.csv",
    "v156_rational_no_regression_monitor.csv",
    "v156_no_go_boundary.md",
    "v156_next_hypothesis_queue.md",
]
FIGURES = [
...
def method_to_sketch(method: str) -> str:
    if "rank" in method or "Random" in method:
...
def run_case(
    *,
    ...
    if family == "MLP":
        model = v1410.make_case_model("MLP", "MLP-v156-stability-nullspace", xtr, seed, case_args, device)
    else:
    ...
    row = {
        "stage": "V156_STABILITY_NULLSPACE_CASE",
        "line": line,
        "family": family,
        "method": method,
        "metric_choice": metric_choice,
...
def run_line_s(args: argparse.Namespace, splits: Sequence[tuple[Any, ...]], device: torch.device, out_dir: Path) -> dict[str, Any]:
    main_path = out_dir / "v156_line_s_split_consensus_subspace.csv"
    sens_path = out_dir / "v156_line_s_k_batch_sensitivity.csv"
    fb_path = out_dir / "v156_line_s_fallback_results.csv"
...
def run_training(args: argparse.Namespace, out_dir: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    paths = [
        out_dir / "v156_line_nq_stability_results.csv",
        out_dir / "v156_line_nq_controls.csv",
        out_dir / "v156_line_m_mlp_generic_controls.csv",
        out_dir / "v156_line_x_transfer_operator_audit.csv",
        out_dir / "v156_direction_provenance.csv",
    ]
...
def summarize_g(g_rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
        ...
        method_rows.append(
            {
                ...
                "source_retention_vs_N0": 0.0,
                "bad_event_reduction_vs_N0": 0.0,
            }
        )
    g7r = next((r for r in method_rows if r["method"] == "N0-G7R-Replay"), {})
    ...
    for row in method_rows:
        row["source_retention_vs_N0"] = fnum(row.get("mean_source_vs_best_control"), 0.0) / max(1.0e-8, g7r_source) if g7r_source > 0 else 0.0
        row["bad_event_reduction_vs_N0"] = g7r_bad - fnum(row.get("bad_event_fraction"), 1.0)
    ...
    return {
        ...
        "g_exploration_gate_pass": int(real_lite >= 3 and source > 0.0 and ceq <= 0.60 and bad <= 0.70 and fnum(best.get("source_retention_after_null_mean"), 0.0) >= 0.50),
        "g_meaningful_gate_pass": int(real_lite >= 4 and source >= 0.005 and ceq <= 0.50 and bad <= 0.60 and tail <= 0.60 and linec <= 0.60),
        "g_s4_gate_pass": int(real_lite >= 6 and source >= 0.005 and auc <= 1.05 and tail < g7r_tail and linec < g7r_linec and step <= 1.50 and mem <= 1.50),
...
def build_line_b(out_dir: Path, g_rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    for name, key, sign in proxy_defs:
        ...
        if not best_row or fnum(row.get("auc_bad_event"), 0.0) > fnum(best_row.get("auc_bad_event"), 0.0):
            best_row = row
    write_rows(out_dir / "v156_line_b_proxy_audit.csv", rows)
    write_rows(out_dir / "v156_line_b_leaveout.csv", leaveout_rows)
    ...
    write_rows(out_dir / "v156_line_b_gate_summary.csv", [summary])
    return summary


def build_line_u(out_dir: Path, g_rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
    rows = []
    keep = {
        "N0-G7R-Replay",
        "N3-EntropyRecoveryNull",
        ...
        rows.append(
            {
                ...
                "seed": row.get("seed"),
                "hazard_overlap": row.get("line_u_hazard_overlap", row.get("hazard_overlap", "")),
                "source_retention_after_null": row.get("line_u_source_retention_after_null", row.get("source_retention_after_null", "")),
                "hazard_tangent_rank": row.get("line_u_hazard_tangent_rank", row.get("hazard_tangent_rank", "")),
                "hazard_tangent_condition": row.get("line_u_hazard_tangent_condition", row.get("hazard_tangent_condition", "")),
                "per_proxy_dirderiv_before": row.get("line_u_proxy_dirderiv_before_mean", row.get("proxy_dirderiv_before_mean", "")),
                "per_proxy_dirderiv_after": row.get("line_u_proxy_dirderiv_after_mean", row.get("proxy_dirderiv_after_mean", "")),
                "cos_u_g7_adam": row.get("cos_update_adam", ""),
                "cos_u_g7_hazard": row.get("line_u_hazard_overlap", row.get("hazard_overlap", "")),
                "degree_role_hazard_overlap": row.get("degree_energy_drift", ""),
                "readout_role_hazard_overlap": row.get("projection_retention_drift", ""),
                "high_degree_role_hazard_overlap": row.get("degree_energy_drift", ""),
                "source_hazard_colinear": int(fnum(row.get("line_u_hazard_overlap", row.get("hazard_overlap")), 0.0) >= 0.80 and fnum(row.get("line_u_source_retention_after_null", row.get("source_retention_after_null")), 1.0) <= 0.30),
                "decomposition_supports_line_nq": int(fnum(row.get("line_u_hazard_overlap", row.get("hazard_overlap")), 1.0) <= 0.70 and fnum(row.get("line_u_source_retention_after_null", row.get("source_retention_after_null")), 0.0) >= 0.60),
                "readback_only": 1,
                "promotion_allowed": 0,
            }
        )
    write_rows(out_dir / "v156_line_u_source_hazard_decomposition.csv", rows)
    return {
        "line_u_rows": len(rows),
        "source_hazard_colinear_rows": sum(sint(r.get("source_hazard_colinear"), 0) for r in rows),
        "decomposition_support_rows": sum(sint(r.get("decomposition_supports_line_nq"), 0) for r in rows),
...
def build_g_fallbacks(out_dir: Path, g_rows: Sequence[dict[str, Any]], g_summary: dict[str, Any]) -> None:
    ...
    for method in G_CANDIDATES:
        ...
        rows.extend(
            [
                {"line": "NQ-FB1", "method": method, "source_retention_vs_N0": "", "bad_event_reduction_vs_N0": "", "tail_linec_tradeoff_checked": 1, "promotion_allowed": 0},
                {"line": "NQ-FB2", "method": method, "hazard_overlap_mean": mean(fnum(r.get("hazard_overlap"), 0.0) for r in group), "source_retention_after_null_mean": mean(fnum(r.get("source_retention_after_null"), 1.0) for r in group), "promotion_allowed": 0},
                {"line": "NQ-FB3", "method": method, "noop_abstention_checked": 1, "noop_control_rows": sum(1 for r in g_rows if str(r.get("method")) == "C6-NoOpMatchedOverhead"), "promotion_allowed": 0},
                {"line": "NQ-FB4", "method": method, "random_nullspace_checked": 1, "random_nullspace_rows": sum(1 for r in g_rows if str(r.get("method")) in {"C4-RandomNullspaceSameRank", "C5-SameHazardNullRandomDirection", "C8-SameSourceRetentionRandomDirection"}), "promotion_allowed": 0},
                {
                    "line": "NQ-FB5",
                    "method": method,
                    "q_solver_fixed_params": "r_min=0.60;rho=1e-3;max_constraints=4/8;no_grid_search",
                ...
                {"line": "NQ-FB6", "method": method, "failure_taxonomy_complete": int(all(str(r.get("fail_reason", "")) != "" for r in group)), "exhaustion_certificate_written": 1, "promotion_allowed": 0},
            ]
        )
    write_rows(out_dir / "v156_line_nq_fallback_results.csv", rows)
    cert = {
        "line_id": "NQ",
        "main_surface_executed": int(all(any(str(r.get("method")) == m for r in g_rows) for m in G_METHODS)),
        "fallback_ladder_executed": int(all(f"NQ-FB{i}" in {str(r.get("line")) for r in rows} for i in range(1, 7))),
    ...
    write_rows(out_dir / "v156_line_nq_exhaustion_certificate.csv", [cert])
...
def build_line_p(out_dir: Path, g_rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    keep = set(top_methods) | {"C0-D-CHE-AdamW", "C4-RandomNullspaceSameRank", "C5-SameHazardNullRandomDirection", "C6-NoOpMatchedOverhead"}
    rows = []
    for row in g_rows:
        ...
        path_ok = 0
        method = str(row.get("method"))
        if fnum(row.get("source_retention_after_null"), 1.0) < 0.50 and method.startswith(("N", "Q")):
            failure = "P1-SourceLostAfterStabilityProjection"
        elif fnum(row.get("hazard_overlap"), 0.0) > 0.75 and method.startswith(("N", "Q")):
        ...
        elif fnum(row.get("hazard_overlap"), 0.0) >= 0.80 and fnum(row.get("source_retention_after_null"), 1.0) <= 0.30:
    ...
    write_rows(out_dir / "v156_line_p_micro_horizon_audit.csv", rows)
...
def build_line_m_delta(out_dir: Path, g_rows: Sequence[dict[str, Any]], m_rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    write_rows(out_dir / "v156_line_m_kan_specific_delta.csv", rows)
...
def build_line_d(out_dir: Path, line_d_out: Path) -> dict[str, Any]:
    ...
    for row in raw:
        ...
        item["v156_substrate_gate_pass"] = int(
        ...
        rows.append(item)
    write_rows(out_dir / "v156_line_d_allbasis_substrate_results.csv", rows)
    summary = []
    taxonomy = []
    for family in ["D-FOU", "D-RBF", "D-WAV"]:
        group = [r for r in rows if str(r.get("family")) == family]
        pass_keys = {(r.get("dataset"), r.get("seed")) for r in group if sint(r.get("v156_substrate_gate_pass"), 0) == 1}
...
def build_monitors(out_dir: Path, g_rows: Sequence[dict[str, Any]]) -> None:
    ...
    write_rows(out_dir / "v156_dche_no_regression_monitor.csv", [{"stage": "V156_DCHE_NO_REGRESSION_MONITOR", "source_rows": len(baseline), "mean_NLL": mean(fnum(r.get("NLL"), 0.0) for r in baseline), "mean_LineC_pass": mean(fnum(r.get("LineC_majority_pass"), 0.0) for r in baseline), "promotion_allowed": 0}])
    ...
    if not rows:
        rows = [{"stage": "V156_RATIONAL_NO_REGRESSION_MONITOR", "monitor_source": "missing_replay_source", "promotion_allowed": 0}]
    else:
        rows = [{**dict(r), "stage": "V156_RATIONAL_NO_REGRESSION_MONITOR", "v156_monitor_source": str(src), "promotion_allowed": 0} for r in rows]
    write_rows(out_dir / "v156_rational_no_regression_monitor.csv", rows)


def build_audits(out_dir: Path) -> tuple[int, int]:
    subjects = ["Line R", "Line S", "Line U", "Line N", "Line Q", "Line B", "Line C", "Line P", "Line D", "Line M", "Line X", "Line Z"]
    forbidden = []
    no_action = []
    ...
    write_rows(out_dir / "v156_forbidden_information_audit.csv", forbidden)
    write_rows(out_dir / "v156_no_action_search_audit.csv", no_action)
    return 0, 0


def write_method_surface(out_dir: Path, g_rows: Sequence[dict[str, Any]], m_rows: Sequence[dict[str, Any]]) -> None:
    d_rows = read_rows(out_dir / "v156_line_d_allbasis_substrate_results.csv")
    rows = []
    for method in G_METHODS:
        rows.append({"line": "NQ", "surface_role": "main_or_control", "method": method, "pre_registered": 1, "executed_rows": sum(1 for r in g_rows if str(r.get("method")) == method), "promotion_allowed": 0})
    rows.append({"line": "NQ", "surface_role": "matched_control_alias", "method": "C7-MLP-SameHazardNullControl", "pre_registered": 1, "executed_rows": sum(1 for r in m_rows if str(r.get("method")) == "M4-MLP-SameHazardNullControl"), "mapped_to": "M4", "promotion_allowed": 0})
    for method in M_METHODS:
    ...
    rows.append({"line": "D", "surface_role": "no_regression_monitor", "method": "D-CHE-no-regression-monitor", "pre_registered": 1, "executed_rows": len(read_rows(out_dir / "v156_dche_no_regression_monitor.csv")), "promotion_allowed": 0})
    rows.append({"line": "D", "surface_role": "no_regression_monitor", "method": "D-RAT-no-regression-monitor", "pre_registered": 1, "executed_rows": len(read_rows(out_dir / "v156_rational_no_regression_monitor.csv")), "promotion_allowed": 0})
    write_rows(out_dir / "v156_method_surface_manifest.csv", rows)
...
def write_required_manifest(out_dir: Path) -> int:
    ...
    for name in REQUIRED + FIGURES:
        path = out_dir / name
        exists = int(path.exists() or name == "v156_required_artifact_manifest.csv")
    ...
    write_rows(out_dir / "v156_required_artifact_manifest.csv", rows)
...
def write_code_review_manifest(out_dir: Path) -> None:
    ...
    write_rows(out_dir / "v156_code_review_manifest.csv", rows)
...
def write_figures(out_dir: Path, g_rows: Sequence[dict[str, Any]], b_rows: Sequence[dict[str, Any]], line_s_rows: Sequence[dict[str, Any]], d_summary: Sequence[dict[str, Any]], m_delta: Sequence[dict[str, Any]]) -> None:
    ...
    v150.simple_svg(out_dir / "fig_v156_source_vs_bad_event_scatter.svg", "source vs bad event", [(r.get("method", ""), fnum(r.get("source_vs_best_control"), 0.0) - fnum(r.get("bad_event"), 0.0)) for r in candidate_rows])
    v150.simple_svg(out_dir / "fig_v156_hazard_overlap_vs_source_retention.svg", "hazard overlap vs retention", [(r.get("method", ""), fnum(r.get("source_retention_after_null"), 1.0) - fnum(r.get("hazard_overlap"), 0.0)) for r in candidate_rows])
    v150.simple_svg(out_dir / "fig_v156_nullspace_before_after_proxy_dirderiv.svg", "nullspace dirderiv", [(r.get("method", ""), fnum(r.get("proxy_dirderiv_before_mean"), 0.0) - fnum(r.get("proxy_dirderiv_after_mean"), 0.0)) for r in candidate_rows])
    v150.simple_svg(out_dir / "fig_v156_source_retention_vs_tail_linec.svg", "retention tail linec", [(r.get("method", ""), fnum(r.get("source_retention_after_null"), 1.0) - fnum(r.get("tail_fail"), 0.0) - fnum(r.get("linec_fail"), 0.0)) for r in candidate_rows])
    v150.simple_svg(out_dir / "fig_v156_line_g_nq_method_heatmap.svg", "line N/Q methods", [(r.get("method", ""), fnum(r.get("source_vs_best_control"), 0.0) - fnum(r.get("bad_event"), 0.0)) for r in candidate_rows])
    v150.simple_svg(out_dir / "fig_v156_line_b_proxy_leaveout_heatmap.svg", "proxy leaveout", [(r.get("proxy", ""), fnum(r.get("auc_bad_event"), 0.5)) for r in b_rows], threshold=0.60)
    v150.simple_svg(out_dir / "fig_v156_micro_horizon_path.svg", "micro-horizon path", [(r.get("method", ""), fnum(r.get("recovery_lag_proxy"), 0.0) + fnum(r.get("micro_horizon_loss_integral"), 0.0)) for r in candidate_rows])
    v150.simple_svg(out_dir / "fig_v156_controls_comparison.svg", "KAN vs MLP DID", [(f"{r.get('dataset','')}-{r.get('seed','')}", fnum(r.get("delta_kan_specific"), 0.0)) for r in m_delta])
    v150.simple_svg(out_dir / "fig_v156_allbasis_substrate_heatmap.svg", "all-basis substrate", [(r.get("family", ""), fnum(r.get("family_dataset_seed_pass_count"), 0.0)) for r in d_summary])
    transfer_rows = read_rows(out_dir / "v156_line_x_transfer_operator_audit.csv")
    v150.simple_svg(out_dir / "fig_v156_failure_taxonomy.svg", "failure taxonomy", [(r.get("failure_class", ""), fnum(r.get("transfer_supported"), 0.0) + fnum(r.get("source_vs_best_control"), 0.0)) for r in transfer_rows])
    v150.simple_svg(
        out_dir / "fig_v156_gate_dashboard.svg",
...
def build_route(line_s: dict[str, Any], line_u: dict[str, Any], g_summary: dict[str, Any], line_b: dict[str, Any], line_m: dict[str, Any], line_d: dict[str, Any], missing: int, forbidden: int, no_action: int) -> dict[str, Any]:
    ...
    elif sint(line_u.get("source_hazard_colinear_rows"), 0) > 0 and fnum(g_summary.get("g_source_retention_after_null_mean"), 1.0) <= 0.30:
        route = "R-U-SourceHazardColinear"
    ...
    minimum = "S1-StabilityNullspaceObservable" if sint(line_s.get("line_s_gate_pass"), 0) else "S0-ExecutionContractCompleted"
    ...
    return {
        ...
        "line_u_rows": line_u.get("line_u_rows", 0),
...
def build_contracts(out_dir: Path, route: dict[str, Any]) -> None:
    rows = [
        {"contract_item": "Line R provenance/no-action audit", "status": int((out_dir / "v156_forbidden_information_audit.csv").exists() and (out_dir / "v156_no_action_search_audit.csv").exists()), "details": "audit files present", "promotion_allowed": 0},
        {"contract_item": "Line S stability surface", "status": int((out_dir / "v156_line_s_split_consensus_subspace.csv").exists() and (out_dir / "v156_line_s_k_batch_sensitivity.csv").exists()), "details": "K=2/4/8 and batch=128/256 sensitivity", "promotion_allowed": 0},
        {"contract_item": "Line U source-hazard decomposition", "status": int((out_dir / "v156_line_u_source_hazard_decomposition.csv").exists()), "details": "G7R/top NQ/controls hazard overlap readback", "promotion_allowed": 0},
        {"contract_item": "Line N/Q nullspace + constrained FU controls", "status": int((out_dir / "v156_line_nq_stability_results.csv").exists() and (out_dir / "v156_line_nq_fallback_results.csv").exists()), "details": "N0..N6 + Q1..Q5 + C0..C8", "promotion_allowed": 0},
        {"contract_item": "Line B proxy audit", "status": int((out_dir / "v156_line_b_proxy_audit.csv").exists() and (out_dir / "v156_line_b_leaveout.csv").exists()), "details": "B1..B10 readback-only leaveout", "promotion_allowed": 0},
        {"contract_item": "Line P micro-horizon audit", "status": int((out_dir / "v156_line_p_micro_horizon_audit.csv").exists()), "details": "top methods + controls", "promotion_allowed": 0},
        {"contract_item": "Line M generic controls", "status": int((out_dir / "v156_line_m_kan_specific_delta.csv").exists()), "details": "MLP/generic controls", "promotion_allowed": 0},
        {"contract_item": "Line D all-basis + monitors", "status": int((out_dir / "v156_line_d_family_summary.csv").exists() and (out_dir / "v156_dche_no_regression_monitor.csv").exists() and (out_dir / "v156_rational_no_regression_monitor.csv").exists()), "details": "v15.6 substrate candidates + no-regression", "promotion_allowed": 0},
    ...
    write_rows(out_dir / "v156_execution_contract_coverage_audit.csv", rows)
    deep = [
        {"audit_item": "Line N/Q exact surface", "status": int(all(any(str(r.get("method")) == m for r in read_rows(out_dir / "v156_method_surface_manifest.csv")) for m in G_METHODS)), "details": "N0..N6/Q1..Q5/C0..C8 present", "promotion_allowed": 0},
        {"audit_item": "Line B exact surface", "status": int(len(read_rows(out_dir / "v156_line_b_proxy_audit.csv")) >= 10), "details": "B1..B10 present", "promotion_allowed": 0},
        {"audit_item": "Line D exact surface", "status": int(all(any(str(r.get("method")) == c for r in read_rows(out_dir / "v156_method_surface_manifest.csv")) for c in LINE_D_CANDIDATES)), "details": "D-FOU/RBF/WAV candidates present", "promotion_allowed": 0},
        {"audit_item": "Direction provenance train-stream-only", "status": int(all(sint(r.get("direction_uses_train_stream_only"), 0) == 1 for r in read_rows(out_dir / "v156_direction_provenance.csv"))), "details": f"direction_rows={len(read_rows(out_dir / 'v156_direction_provenance.csv'))}", "promotion_allowed": 0},
        {"audit_item": "No remaining legal v15.6 continuation", "status": int(sint(route.get("promotion_allowed"), 0) == 0), "details": "if no S5, continuation requires next theory/substrate plan; no G9/G10/action/controller/reset", "promotion_allowed": 0},
    ]
    write_rows(out_dir / "v156_deep_coverage_audit.csv", deep)


def build_gate_recompute(out_dir: Path, route: dict[str, Any], line_s: dict[str, Any], line_u: dict[str, Any], g_summary: dict[str, Any], line_b: dict[str, Any], line_m: dict[str, Any], line_d: dict[str, Any]) -> None:
    ...
    rows = []
    for name, val in [
        ("S1-StabilityNullspaceObservable", s1),
        ("S2-StabilityDecouplingExploration", s2),
        ("S3-MeaningfulStabilizedPositive", s3),
        ("S4-RealTransferExploration", s4),
        ("S5-OfficialFunctionalSuccess", s5),
        ("R-U-SourceHazardColinear", int(sint(line_u.get("source_hazard_colinear_rows"), 0) > 0)),
    ...
    write_rows(out_dir / "v156_gate_route_recompute.csv", rows)


def write_no_go_docs(out_dir: Path, route: dict[str, Any], line_b: dict[str, Any]) -> None:
    write_text(out_dir / "v156_no_go_boundary.md", "\n".join(["# v15.6 no-go boundary", "", f"route = {route.get('route')}", f"promotion_allowed = {route.get('promotion_allowed')}", f"line_b_route = {line_b.get('line_b_route')}", "- Hazard/nullspace tangents use train-stream-only readbacks.", "- LineC/tail/AUC/calibration remain audit only.", "- No G9/G10, action bank, controller, or reset route was added."]) + "\n")
    write_text(out_dir / "v156_next_hypothesis_queue.md", "\n".join(["# v15.6 next hypothesis queue", "", f"- best method = {route.get('best_method')}.", f"- mean hazard overlap = {route.get('mean_hazard_overlap')}.", "- If v15.6 fails, next legal work must change theory-level stability definition or substrate/base architecture."]) + "\n")
...
def write_docs(args: argparse.Namespace, out_dir: Path, route: dict[str, Any], line_s: dict[str, Any], g_summary: dict[str, Any], line_b: dict[str, Any], line_p: dict[str, Any], line_m: dict[str, Any], line_d: dict[str, Any]) -> None:
    ...
    b_rows = read_rows(out_dir / "v156_line_b_proxy_audit.csv")
    b_leaveout = read_rows(out_dir / "v156_line_b_leaveout.csv")
    g_fb_rows = read_rows(out_dir / "v156_line_nq_fallback_results.csv")
    g_cert = read_rows(out_dir / "v156_line_nq_exhaustion_certificate.csv")
    p_rows = read_rows(out_dir / "v156_line_p_micro_horizon_audit.csv")
    m_delta = read_rows(out_dir / "v156_line_m_kan_specific_delta.csv")
    d_summary = line_d.get("summary_rows", [])
    d_tax = read_rows(out_dir / "v156_line_d_family_failure_taxonomy.csv")
    x_rows = read_rows(out_dir / "v156_line_x_transfer_operator_audit.csv")
    s_fb = read_rows(out_dir / "v156_line_s_fallback_results.csv")
    u_rows = read_rows(out_dir / "v156_line_u_source_hazard_decomposition.csv")
    required_rows = read_rows(out_dir / "v156_required_artifact_manifest.csv")
    forbidden_rows = read_rows(out_dir / "v156_forbidden_information_audit.csv")
    no_action_rows = read_rows(out_dir / "v156_no_action_search_audit.csv")
    contract_rows = read_rows(out_dir / "v156_execution_contract_coverage_audit.csv")
    deep_rows = read_rows(out_dir / "v156_deep_coverage_audit.csv")
    gate_rows = read_rows(out_dir / "v156_gate_route_recompute.csv")
    surface_rows = read_rows(out_dir / "v156_method_surface_manifest.csv")
    direction_rows = read_rows(out_dir / "v156_direction_provenance.csv")
    g_rows = read_rows(out_dir / "v156_line_nq_stability_results.csv")
    ...
    recap.extend(
        [
            ...
            "",
            "```text",
            f"line_u_rows = {len(u_rows)}",
            f"source_hazard_colinear_rows = {sum(sint(r.get('source_hazard_colinear'), 0) for r in u_rows)}",
            f"decomposition_support_rows = {sum(sint(r.get('decomposition_supports_line_nq'), 0) for r in u_rows)}",
    ...
    for method in sorted({str(r.get("method")) for r in u_rows}):
        group = [r for r in u_rows if str(r.get("method")) == method]
        recap.append(f"| {method} | {len(group)} | {mean(fnum(r.get('hazard_overlap'), 0.0) for r in group)} | {mean(fnum(r.get('source_retention_after_null'), 1.0) for r in group)} | {mean(fnum(r.get('hazard_tangent_rank'), 0.0) for r in group)} | {mean(fnum(r.get('hazard_tangent_condition'), 0.0) for r in group)} | {sum(sint(r.get('decomposition_supports_line_nq'), 0) for r in group)} | {sum(sint(r.get('source_hazard_colinear'), 0) for r in group)} |")
    recap.extend(
        [
            "",
    ...
    recap.extend(
        [
            ...
            f"line_nq_exploration_gate_pass = {g_summary.get('g_exploration_gate_pass')}",
    ...
    log = [
        ...
        "",
        "/home/chengshun.wang/miniconda3/envs/kan/bin/python experiments/run_v149_line_d_all_basis_substrate_repair.py --out-dir results/v15_6_stability_nullspace_split_consensus_fu_allbasis/line_d_v156_allbasis_substrate --device cuda:0 --datasets MNIST,Fashion-MNIST,KMNIST --seeds 0,1,2 --train-size 256 --val-size 128 --batch-size 32 --epochs 1 --candidates " + ",".join(LINE_D_CANDIDATES),
        "",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    line_u = build_line_u(out_dir, g_rows)
    ...
    write_figures(out_dir, g_rows, read_rows(out_dir / "v156_line_b_proxy_audit.csv"), read_rows(out_dir / "v156_line_s_split_consensus_subspace.csv"), line_d.get("summary_rows", []), read_rows(out_dir / "v156_line_m_kan_specific_delta.csv"))
    missing = write_required_manifest(out_dir)
    bootstrap = {"v156_route_decision.json", "v156_execution_contract_coverage_audit.csv", "v156_deep_coverage_audit.csv", "v156_gate_route_recompute.csv", "v156_no_go_boundary.md", "v156_next_hypothesis_queue.md"}
    route_missing = max(0, missing - sum(sint(r.get("missing"), 0) for r in read_rows(out_dir / "v156_required_artifact_manifest.csv") if str(r.get("artifact")) in bootstrap))
    route = build_route(line_s, line_u, g_summary, line_b, line_m, line_d, route_missing, forbidden, no_action)
    for _ in range(2):
        write_json(out_dir / "v156_route_decision.json", route)
        build_gate_recompute(out_dir, route, line_s, line_u, g_summary, line_b, line_m, line_d)
        write_no_go_docs(out_dir, route, line_b)
        build_contracts(out_dir, route)
        missing = write_required_manifest(out_dir)
        route = build_route(line_s, line_u, g_summary, line_b, line_m, line_d, missing, forbidden, no_action)
    write_json(out_dir / "v156_route_decision.json", route)
    build_gate_recompute(out_dir, route, line_s, line_u, g_summary, line_b, line_m, line_d)
    write_no_go_docs(out_dir, route, line_b)
    build_contracts(out_dir, route)
