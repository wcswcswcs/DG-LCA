#!/usr/bin/env python3
"""DG-KAN v22.52 Conflict Residual Pareto Credit FU runner.

This runner implements a conservative, auditable v22.52 slice:

* Part A code/import/forbidden-feature gates.
* Part B v22.51R conflict-diagnosis reanalysis when fields exist.
* Part C conflict-residual quadratic solver unit tests.
* Part D/E/H/J MLP full-loop CR-PCF, fixed-reweighting controls, strong optimizer
  baselines, and efficiency accounting.

The CR-PCF training implementation uses the strongest available optimizer
reference and solves only the conflict-residual correction in the cohort
disagreement span. It uses all cohorts as constraints and never performs
cohort/action/layer top-k or candidate argmax runtime selection.
KAN/layerwise/continual artifacts are written as skipped unless the MLP
exploration gate is opened by real rows.
"""
...
OUT_ROOT = ROOT / "results/v22_52"
CHUNK_ROOT = OUT_ROOT / "chunks"
LOG_ROOT = OUT_ROOT / "logs"
DOCS = ROOT / "docs"
EXEC_DOC = DOCS / "DG-KAN_v22.52_ConflictResidualParetoCreditFU_执行日志.md"
RECAP_DOC = DOCS / "DG-KAN_v22.52_ConflictResidualParetoCreditFU_实验结果复盘.md"
PLAN_DOC = DOCS / "DG-KAN_v22.52_ConflictResidualParetoCreditFU_完整计划.md"

CORE_REQUIRED_ARTIFACTS = [
    "v22_52_code_truth_gate.csv",
    "v22_52_full_import_closure.csv",
    "v22_52_runtime_forbidden_feature_audit.csv",
    "v22_52_gate_route.json",
    "v22_52_v51R_conflict_diagnosis_matrix.csv",
    "v22_52_v51R_cpfcf_vs_population_cosine.csv",
    "v22_52_v51R_strong_optimizer_gap_matrix.csv",
    "v22_52_v51R_fixed_control_gap_matrix.csv",
    "v22_52_v51R_dual_reweighting_audit.csv",
    "v22_52_credit_solver_unit_matrix.csv",
    "v22_52_pareto_credit_mlp_matrix.csv",
    "v22_52_fixed_reweighting_controls_matrix.csv",
    "v22_52_dual_degeneracy_audit.csv",
    "v22_52_safety_gradient_exactness_matrix.csv",
    "v22_52_layerwise_realization_matrix.csv",
    "v22_52_kan_carrier_realization_matrix.csv",
    "v22_52_kan_vs_mlp_credit_matrix.csv",
    "v22_52_strong_optimizer_matrix.csv",
    "v22_52_continual_memory_credit_matrix.csv",
    "v22_52_grokking_credit_matrix.csv",
    "v22_52_efficiency_breakdown.csv",
    "v22_52_repair_attempts_matrix.csv",
    "v22_52_final_route.json",
    "v22_52_command_journal.csv",
]
...
GEOMETRY_CONTROL_METHODS = {
    ...
    "cr_shuffled_dual_cautious",
    "cr_frozen_dual_cautious",
    "cr_random_conflict_cautious",
    "cr_signflip_conflict_cautious",
    "cr_optref_shuffled_dual_cautious_direct",
    "cr_optref_frozen_dual_cautious_direct",
    "cr_optref_random_conflict_cautious_direct",
    "cr_optref_signflip_conflict_cautious_direct",
}
...
CR_PCF_OPTREF_CANDIDATE_METHODS = {"cr_pcf_optref_cautious_direct"}

CR_PCF_CANDIDATE_METHODS = {"cr_pcf_adamw", "cr_pcf_cautious"} | CR_PCF_OPTREF_CANDIDATE_METHODS

CR_PCF_CONTROL_METHODS = {
    "cr_shuffled_dual_cautious",
    "cr_frozen_dual_cautious",
    "cr_random_conflict_cautious",
    "cr_signflip_conflict_cautious",
...
CR_PCF_METHODS = CR_PCF_CANDIDATE_METHODS | CR_PCF_CONTROL_METHODS

FUNCTIONAL_FLOW_METHODS = FUNCTIONAL_FLOW_METHODS | CR_PCF_METHODS

CR_PCF_OPTREF_METHODS = CR_PCF_OPTREF_CANDIDATE_METHODS | {
    "cr_optref_shuffled_dual_cautious_direct",
    "cr_optref_frozen_dual_cautious_direct",
    "cr_optref_random_conflict_cautious_direct",
    "cr_optref_signflip_conflict_cautious_direct",
...
CR_PCF_RANDOM_CONFLICT_CONTROLS = {
    "cr_random_conflict_cautious",
    "cr_optref_random_conflict_cautious_direct",
}

CR_PCF_SIGNFLIP_CONTROLS = {
    "cr_signflip_conflict_cautious",
    "cr_optref_signflip_conflict_cautious_direct",
}

DIRECT_UPDATE_METHODS = {"cpfcf_direct"} | CR_PCF_OPTREF_METHODS

PRIMARY_FU_METHOD = "cr_pcf_cautious"
...
def ensure_out() -> None:
    ...
    if not EXEC_DOC.exists():
        EXEC_DOC.write_text(
            "# DG-KAN v22.52 ConflictResidualParetoCreditFU 执行日志\n\n"
            f"创建时间：{now_sg()}\n\n"
    ...
    if not RECAP_DOC.exists():
        RECAP_DOC.write_text(
            "# DG-KAN v22.52 ConflictResidualParetoCreditFU 实验结果复盘\n\n"
            f"创建时间：{now_sg()}\n\n"
...
def append_exec(
    ...
    *,
    ...
    journal = read_rows(OUT_ROOT / "v22_52_command_journal.csv")
    journal.append({key: str(value) for key, value in row.items()})
    write_rows(
        OUT_ROOT / "v22_52_command_journal.csv",
        journal,
...
def optimizer_reference_direction(
    named: list[tuple[str, Any]],
    mean_grad: Any,
    ref_state: dict[str, dict[str, Any]],
    *,
    cautious: bool,
    betas: tuple[float, float] = (0.9, 0.999),
    eps: float = 1.0e-8,
) -> Any:
    """Return AdamW/CautiousAdamW update direction in gradient units.

    The caller applies decoupled weight decay separately via apply_flat_update,
    matching the local baseline optimizer convention used in this runner.
    """
    import torch
    ...
    for name, p in named:
        n = int(p.numel())
        grad = mean_grad[offset : offset + n].view_as(p).to(device=p.device, dtype=p.dtype)
        offset += n
        st = ref_state.setdefault(
            name,
            {
                "step": 0,
        ...
        update = (st["m"] / bias1) / ((st["v"] / bias2).sqrt() + float(eps))
        if cautious:
            ...
            if active > 1.0e-8:
                mask = mask / active
            update = update * mask
        pieces.append(update.detach().reshape(-1))
    return torch.cat(pieces) if pieces else mean_grad.detach().clone()
...
def safety_component_gradients(
    model: Any,
    x: Any,
    y: Any,
    idx: Any,
    ...
    k = max(1, int(math.ceil(tail_frac * int(losses.numel()))))
    tail = torch.sort(losses.float(), descending=True).values[:k].mean()
    components = {"ECE_proxy": soft_ece, "Brier": brier, "tail": tail}
    grads: dict[str, Any] = {}
    values: dict[str, float] = {}
    for name, value in components.items():
        grad_tensors = torch.autograd.grad(value, params, retain_graph=True, allow_unused=True)
        grads[name] = flatten_grads(grad_tensors, params)
        values[name] = float(value.detach().item())
    return grads, values


def subset_eval_metrics(model: Any, x: Any, y: Any, idx: Any, num_classes: int) -> dict[str, float]:
    import torch
    import torch.nn.functional as F
    ...
    with torch.no_grad():
        ...
        return {
            ...
            "Brier": float(((probs - target) ** 2).sum(dim=-1).mean().detach().item()),
            "tail_q95": float(torch.quantile(losses.float(), 0.95).detach().item()),
            "tail_q99": float(torch.quantile(losses.float(), 0.99).detach().item()),
        }


def flat_param_vector(named: list[tuple[str, Any]]) -> Any:
    import torch

    return torch.cat([p.detach().reshape(-1) for _name, p in named]) if named else torch.empty(0)
...
def solve_cr_pcf(
    ...
    *,
    ...
    reference_source: str = "mean_gradient",
    safety_grads: list[Any] | None = None,
    safety_bounds: list[float] | None = None,
) -> dict[str, Any]:
    import numpy as np
    ...
    if not grads:
        raise ValueError("solve_cr_pcf requires at least one cohort gradient")
    device = grads[0].device
    dtype = grads[0].dtype
    stacked = torch.stack(grads, dim=0)
    k = int(stacked.shape[0])
    population = stacked.mean(dim=0)
    if reference is None:
        reference = population
        reference_source = "mean_gradient"
    else:
        reference = reference.detach().to(device=device, dtype=dtype).reshape(-1)
    conflict = stacked - population.view(1, -1)
    gram_c = (conflict @ conflict.T).detach().double().cpu().numpy()
    cohort_conflict_dot = (stacked @ conflict.T).detach().double().cpu().numpy()
    ref_gain = (stacked @ reference).detach().double().cpu().numpy()
    ref_norm = float(reference.norm().detach().double().cpu().item())
    ref_norm = max(ref_norm, 1.0e-12)
    ref_gain_scale = max(0.0, float(np.mean(ref_gain)))
    gamma_value = float(gamma) if float(gamma) < 0.0 else max(float(gamma), float(gamma_frac) * ref_gain_scale)
    base_deficit = np.maximum(gamma_value - ref_gain, 0.0)
    safety_grads = safety_grads or []
    safety_bounds = safety_bounds or [0.0 for _ in safety_grads]
    if safety_grads:
        safety_stack = torch.stack(safety_grads, dim=0)
        safety_ref = (safety_stack @ reference).detach().double().cpu().numpy()
        safety_conflict = (safety_stack @ conflict.T).detach().double().cpu().numpy()
    else:
        safety_ref = np.zeros((0,), dtype=np.float64)
        safety_conflict = np.zeros((0, k), dtype=np.float64)
    h_bounds = np.asarray(safety_bounds, dtype=np.float64)
    j_safety = int(len(safety_grads))
    safety_slack_rho = 10.0
    xi0 = base_deficit.copy()
    eta0 = np.maximum(safety_ref - h_bounds, 0.0) if j_safety else np.zeros((0,), dtype=np.float64)
    x0 = np.concatenate([np.zeros((k,), dtype=np.float64), xi0, eta0])
    xi_start = k
    eta_start = k + k

    def objective(z: np.ndarray) -> float:
        a = z[:k]
        xi = z[xi_start:eta_start]
        eta = z[eta_start:]
        norm2 = float(a @ gram_c @ a)
        return 0.5 * (1.0 + float(cost)) * norm2 + 0.5 * float(rho) * float(xi @ xi) + 0.5 * safety_slack_rho * float(eta @ eta)

    def grad_obj(z: np.ndarray) -> np.ndarray:
        a = z[:k]
        xi = z[xi_start:eta_start]
        eta = z[eta_start:]
        return np.concatenate([(1.0 + float(cost)) * (gram_c @ a), float(rho) * xi, safety_slack_rho * eta])

    def pad_jac(a_part: np.ndarray, xi_part: np.ndarray | None = None, eta_part: np.ndarray | None = None) -> np.ndarray:
        if xi_part is None:
            xi_part = np.zeros((k,), dtype=np.float64)
        if eta_part is None:
            eta_part = np.zeros((j_safety,), dtype=np.float64)
        return np.concatenate([a_part, xi_part, eta_part])

    constraints: list[dict[str, Any]] = [
        {
            "type": "eq",
            "fun": lambda z: float(np.sum(z[:k])),
            "jac": lambda z: pad_jac(np.ones((k,), dtype=np.float64)),
        }
    ]
    for i in range(k):
        xi_i = np.zeros((k,), dtype=np.float64)
        xi_i[i] = 1.0
        constraints.append(
            {
                "type": "ineq",
                "fun": lambda z, i=i: float(ref_gain[i] + cohort_conflict_dot[i] @ z[:k] + z[xi_start + i] - gamma_value),
                "jac": lambda z, i=i, xi_i=xi_i: pad_jac(cohort_conflict_dot[i], xi_i),
            }
        )
    for j in range(j_safety):
        eta_j = np.zeros((j_safety,), dtype=np.float64)
        ...
        constraints.append(
            {
                "type": "ineq",
                "fun": lambda z, j=j: float(h_bounds[j] + z[eta_start + j] - safety_ref[j] - safety_conflict[j] @ z[:k]),
                "jac": lambda z, j=j, eta_j=eta_j: pad_jac(-safety_conflict[j], None, eta_j),
            }
        )
    start = time.perf_counter()
    res = minimize(
        ...
        method="SLSQP",
        jac=grad_obj,
        bounds=[(None, None) for _ in range(k)] + [(0.0, None) for _ in range(k)] + [(0.0, None) for _ in range(j_safety)],
        constraints=constraints,
        options={"maxiter": 80, "ftol": 1.0e-10, "disp": False},
    )
    solver_ms = (time.perf_counter() - start) * 1000.0
    z = res.x if getattr(res, "x", None) is not None else x0
    a = z[:k]
    xi = np.maximum(z[xi_start:eta_start], 0.0)
    eta = np.maximum(z[eta_start:], 0.0) if j_safety else np.zeros((0,), dtype=np.float64)
    delta_raw = torch.tensor(a, device=device, dtype=dtype).view(1, -1) @ conflict
    delta_raw = delta_raw.reshape(-1)
    delta_norm = float(delta_raw.norm().detach().double().cpu().item())
    alpha_trust = min(1.0, float(trust_ratio) * ref_norm / max(delta_norm, 1.0e-12)) if delta_norm > 0.0 else 1.0
    delta = delta_raw * float(alpha_trust)
    raw_out = reference + delta_raw
    out = reference + delta
    gain_before = ref_gain
    gain_after_raw = (stacked @ raw_out).detach().double().cpu().numpy()
    gain_after = (stacked @ out).detach().double().cpu().numpy()
    safety_after_raw = safety_ref + safety_conflict @ a if j_safety else np.zeros((0,), dtype=np.float64)
    safety_after = safety_ref + safety_conflict @ a * float(alpha_trust) if j_safety else np.zeros((0,), dtype=np.float64)
    primal_gain_violation = np.maximum(gamma_value - gain_after_raw - xi, 0.0)
    primal_safety_violation = np.maximum(safety_after_raw - h_bounds - eta, 0.0) if j_safety else np.zeros((0,), dtype=np.float64)
    active = [1.0 if float(v) > 1.0e-8 else 0.0 for v in (gamma_value - gain_after_raw)]
    if sum(active) <= 0.0 and int(np.sum(base_deficit > 0.0)) > 0:
        active = [1.0 if float(v) > 1.0e-8 else 0.0 for v in base_deficit]
    if sum(active) <= 0.0:
        active = [0.0 for _ in range(k)]
    conflict_energy_fraction = float(delta_norm / max(delta_norm, 1.0e-12)) if delta_norm > 0.0 else 0.0
    correction_ratio = float((delta.norm().detach().double().cpu().item()) / ref_norm)
    return {
        "direction": out,
        "reference": reference,
        "correction": delta,
        ...
        "solver_iters": int(getattr(res, "nit", -1)),
        "solver_ms": float(solver_ms),
        "KKT_residual": float(np.linalg.norm(primal_gain_violation) + np.linalg.norm(primal_safety_violation)),
        "primal_feasibility_violation": float(max(np.max(primal_gain_violation) if len(primal_gain_violation) else 0.0, np.max(primal_safety_violation) if len(primal_safety_violation) else 0.0)),
        "dual_feasibility_violation": 0.0,
        "slack_norm": float(math.sqrt(float(xi @ xi) + float(eta @ eta))),
        "slack_vector": [float(v) for v in xi],
        "safety_slack_norm": float(np.linalg.norm(eta)),
        ...
        "active_dual_entropy": entropy([float(v) for v in active]),
        "max_dual_fraction": float(max(active) / max(1.0e-20, float(sum(active)))) if sum(active) > 0 else 0.0,
        "cohort_gain_values": [float(v) for v in gain_after],
        "cohort_gain_before_values": [float(v) for v in gain_before],
        "cohort_gain_after_values": [float(v) for v in gain_after],
        "cohort_gain_min_before": float(np.min(gain_before)) if len(gain_before) else 0.0,
        "cohort_gain_min_after": float(np.min(gain_after)) if len(gain_after) else 0.0,
        "cohort_gain_CVaR25_before": float(np.mean(np.sort(gain_before)[: max(1, int(math.ceil(0.25 * len(gain_before))))])) if len(gain_before) else 0.0,
        "cohort_gain_CVaR25_after": float(np.mean(np.sort(gain_after)[: max(1, int(math.ceil(0.25 * len(gain_after))))])) if len(gain_after) else 0.0,
        "base_harm_count": int(np.sum(gain_before < gamma_value)),
        "base_harm_rate": float(np.mean(gain_before < gamma_value)) if len(gain_before) else 0.0,
        "constraint_inactive_rate": float(np.mean(base_deficit <= 1.0e-12)) if len(base_deficit) else 1.0,
        "correction_norm_over_reference_norm": correction_ratio,
        "conflict_subspace_energy_fraction": conflict_energy_fraction,
        "v_out_vs_reference_cosine": cosine(out, reference),
        "v_out_vs_population_cosine": cosine(out, population),
        "v_star_norm": vector_norm(out),
        "v_ref_norm": vector_norm(reference),
        "v_ref_vs_population_cosine": cosine(reference, population),
        "v_star_vs_population_cosine": cosine(out, population),
        "reference_source": str(reference_source),
        "safety_debt_predicted_delta": float(safety_after[0]) if len(safety_after) else 0.0,
        "trust_alpha": float(alpha_trust),
        "gamma": gamma_value,
...
def run_solver_unit_tests(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for repeat in range(repeats):
        ...
        for name, grads in cases.items():
            gamma = 0.0
            if name == "random_noisy":
                gamma = -1.0e9
            if name == "infeasible":
                gamma = float(base.norm().item() ** 2)
            sol = solve_cr_pcf(
                grads,
                gamma=gamma,
                gamma_frac=0.0,
                rho=float(args.cr_pcf_rho),
                cost=float(args.cr_pcf_cost),
                trust_ratio=float(args.cr_pcf_trust_ratio),
            )
            rows.append(
                {
                    "case": name,
                    ...
                    "safety_slack_norm": sol["safety_slack_norm"],
                    "correction_norm_over_reference_norm": sol["correction_norm_over_reference_norm"],
                    "conflict_subspace_energy_fraction": sol["conflict_subspace_energy_fraction"],
                    "v_out_vs_reference_cosine": sol["v_out_vs_reference_cosine"],
                    "cohort_gain_min_before": sol["cohort_gain_min_before"],
                    "cohort_gain_min_after": sol["cohort_gain_min_after"],
                    "cohort_gain_CVaR25_before": sol["cohort_gain_CVaR25_before"],
                    "cohort_gain_CVaR25_after": sol["cohort_gain_CVaR25_after"],
                    "base_harm_count": sol["base_harm_count"],
                    "base_harm_rate": sol["base_harm_rate"],
                    "constraint_inactive_rate": sol["constraint_inactive_rate"],
                    "active_constraint_count": sol["active_constraint_count"],
                    "max_dual_fraction": sol["max_dual_fraction"],
                    "bounded_correction": int(sol["correction_norm_over_reference_norm"] <= float(args.cr_pcf_trust_ratio) + 1.0e-6),
                    ...
                    "no_nan_inf": int(all(math.isfinite(float(v)) for v in [
                        ...
                        sol["slack_norm"],
                        sol["correction_norm_over_reference_norm"],
                    ])),
                }
            )
        safety_grads = [-base]
    ...
    write_rows(OUT_ROOT / "v22_52_credit_solver_unit_matrix.csv", rows)
    append_exec(
        f"{shlex.quote(PYTHON)} experiments/run_v22_52_conflict_residual_pareto_credit_fu.py --stage solver --unit-repeats {args.unit_repeats}",
        task_id="v22_52_solver_units",
        status="pass" if solver_gate(rows)["pass"] else "fail",
        gpu="cpu",
        files="results/v22_52/v22_52_credit_solver_unit_matrix.csv",
...
def solver_gate(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    conflict = [r for r in rows if r.get("case") == "one_conflict"]
    noisy = [r for r in rows if r.get("case") == "random_noisy"]
    infeasible = [r for r in rows if r.get("case") == "infeasible"]
    safety = [r for r in rows if r.get("case") == "safety_conflict"]
    checks = {
        ...
        "max_aligned_correction_ratio": max(float(r.get("correction_norm_over_reference_norm") or 0.0) for r in aligned) if aligned else math.inf,
        "conflict_gain_improved_rows": sum(1 for r in conflict if (fval(r.get("cohort_gain_min_after"), -math.inf) or -math.inf) > (fval(r.get("cohort_gain_min_before"), math.inf) or math.inf)),
        "conflict_rows": len(conflict),
        "max_noisy_dual_fraction": max(float(r.get("max_dual_fraction") or 0.0) for r in noisy) if noisy else 0.0,
        "infeasible_bounded_rows": sum(iflag(r.get("bounded_correction")) for r in infeasible),
        "infeasible_rows": len(infeasible),
        "max_safety_predicted_delta": max(float(r.get("safety_debt_predicted_delta") or 0.0) for r in safety) if safety else 0.0,
    }
    checks["pass"] = int(
        checks["no_nan_inf_rows"] == checks["total_rows"]
        and checks["max_KKT_residual"] <= 1.0e-4
        and checks["max_feasible_primal_violation"] <= 1.0e-5
        and checks["max_aligned_correction_ratio"] <= 0.01
        and checks["conflict_gain_improved_rows"] == checks["conflict_rows"]
        and checks["max_noisy_dual_fraction"] <= 0.80
        and checks["infeasible_bounded_rows"] == checks["infeasible_rows"]
        and checks["max_safety_predicted_delta"] <= 0.0
    )
    return checks
...
def cpfcf_direction_for_batch(
    ...
    reference_state: dict[str, dict[str, Any]] | None,
    step: int,
    ...
    reference_for_controls = grad_stack.mean(dim=0)
    conflict_for_controls = grad_stack - reference_for_controls.view(1, -1)
    reference_override = None
    reference_source = "mean_gradient"
    if method in CR_PCF_OPTREF_METHODS:
        if reference_state is None:
            raise ValueError("CR-PCF optimizer-reference methods require a mutable reference_state")
        reference_override = optimizer_reference_direction(
            named,
            reference_for_controls,
            reference_state,
            cautious=True,
        )
        reference_source = "cautious_adamw_reference_direct"
    ...
    if float(args.safety_weight) > 0.0:
        ...
        debt_value = math.nan
    if method in CR_PCF_METHODS:
        sol = solve_cr_pcf(
            grads,
            gamma=float(args.cr_pcf_gamma),
            gamma_frac=float(args.cr_pcf_gamma_frac),
            rho=float(args.cr_pcf_rho),
            cost=float(args.cr_pcf_cost),
            trust_ratio=float(args.cr_pcf_trust_ratio),
    ...
    elif method in MINIMAX_METHODS:
    ...
    control_reference = sol.get("reference", reference_for_controls)
    if method in CR_PCF_DUAL_REPLAY_CONTROLS:
        ...
        weights = torch.tensor(alpha, device=direction.device, dtype=direction.dtype)
        if float(weights.abs().sum().detach().item()) <= 1.0e-12:
            weights = torch.ones_like(weights)
        weights = weights - weights.mean()
        ctrl = weights @ conflict_for_controls
        target_norm = sol.get("correction", direction - reference_for_controls).norm().detach().clamp_min(1.0e-12)
        ctrl = ctrl * (target_norm / ctrl.norm().detach().clamp_min(1.0e-12))
        direction = control_reference + ctrl
    elif method in CR_PCF_RANDOM_CONFLICT_CONTROLS:
        coeff = torch.randn((len(grads),), generator=generator, device=idx.device)
        coeff = coeff - coeff.mean()
        ctrl = coeff @ conflict_for_controls
        target_norm = sol.get("correction", direction - reference_for_controls).norm().detach().clamp_min(1.0e-12)
        ctrl = ctrl * (target_norm / ctrl.norm().detach().clamp_min(1.0e-12))
        direction = control_reference + ctrl
    elif method in CR_PCF_SIGNFLIP_CONTROLS:
        direction = control_reference - sol.get("correction", direction - control_reference)
    elif method in DUAL_REPLAY_CONTROL_METHODS:
        alpha = list(sol["active_dual_vector"])
    ...
    diag = {
        **stats,
        ...
        "cohort_gain_min_before": sol.get("cohort_gain_min_before", ""),
        "cohort_gain_min_after": sol.get("cohort_gain_min_after", ""),
        "cohort_gain_CVaR25_before": sol.get("cohort_gain_CVaR25_before", ""),
        "cohort_gain_CVaR25_after": sol.get("cohort_gain_CVaR25_after", ""),
        "base_harm_count": sol.get("base_harm_count", ""),
        "base_harm_rate": sol.get("base_harm_rate", ""),
        "constraint_inactive_rate": sol.get("constraint_inactive_rate", ""),
        "correction_norm_over_reference_norm": sol.get("correction_norm_over_reference_norm", ""),
        "conflict_subspace_energy_fraction": sol.get("conflict_subspace_energy_fraction", ""),
        "v_out_vs_reference_cosine": sol.get("v_out_vs_reference_cosine", ""),
        "v_out_vs_population_cosine": sol.get("v_out_vs_population_cosine", ""),
        "v_ref_vs_population_cosine": sol.get("v_ref_vs_population_cosine", ""),
        "reference_source": sol.get("reference_source", ""),
        "trust_alpha": sol.get("trust_alpha", ""),
        "active_constraint_count": sol["active_constraint_count"],
...
def run_collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if tensors["used_fake_data"]:
        raise RuntimeError("fake data is not allowed for v22.52")
    ...
    opt_method = (
        "cautious_adamw_reference_direct"
        if str(args.method) in CR_PCF_OPTREF_METHODS
        else ("cautious_adamw" if uses_cautious_optimizer(str(args.method)) else "adamw")
    )
    ...
    reference_state: dict[str, dict[str, Any]] = {}
    solver_time = 0.0
    ...
    for step in range(int(args.steps)):
        ...
        if args.method in FUNCTIONAL_FLOW_METHODS:
            direction, diag, frozen_dual = cpfcf_direction_for_batch(
                model,
                x_train,
                y_train,
                idx,
    ...
    row = {
        ...
        "cr_pcf_gamma": float(args.cr_pcf_gamma),
        "cr_pcf_gamma_frac": float(args.cr_pcf_gamma_frac),
        "cr_pcf_rho": float(args.cr_pcf_rho),
        "cr_pcf_cost": float(args.cr_pcf_cost),
        "cr_pcf_trust_ratio": float(args.cr_pcf_trust_ratio),
        ...
        "cohort_gain_min_before": mean(fval(r.get("cohort_gain_min_before")) for r in step_diags),
        "cohort_gain_min_after": mean(fval(r.get("cohort_gain_min_after")) for r in step_diags),
        "cohort_gain_CVaR25_before": mean(fval(r.get("cohort_gain_CVaR25_before")) for r in step_diags),
        "cohort_gain_CVaR25_after": mean(fval(r.get("cohort_gain_CVaR25_after")) for r in step_diags),
        "base_harm_count": mean(fval(r.get("base_harm_count")) for r in step_diags),
        "base_harm_rate": mean(fval(r.get("base_harm_rate")) for r in step_diags),
        "constraint_inactive_rate": mean(fval(r.get("constraint_inactive_rate")) for r in step_diags),
        "correction_norm_over_reference_norm": mean(fval(r.get("correction_norm_over_reference_norm")) for r in step_diags),
        "conflict_subspace_energy_fraction": mean(fval(r.get("conflict_subspace_energy_fraction")) for r in step_diags),
        "v_out_vs_reference_cosine": mean(fval(r.get("v_out_vs_reference_cosine")) for r in step_diags),
        "v_out_vs_population_cosine": mean(fval(r.get("v_out_vs_population_cosine")) for r in step_diags),
        "v_ref_vs_population_cosine": mean(fval(r.get("v_ref_vs_population_cosine")) for r in step_diags),
        "reference_source": next((r.get("reference_source") for r in step_diags if r.get("reference_source")), ""),
        "trust_alpha": mean(fval(r.get("trust_alpha")) for r in step_diags),
        "leave_cohort_gain": "",
        ...
        "claim_limit": "v22.52 MLP full-parameter first-order CR-PCF/continuous-FU exploration row; no KAN architecture claim",
    }
...
def run_gates() -> dict[str, Any]:
    ...
    for rel in [
        ...
        if proc.returncode != 0:
            compileall_pass = 0
    runner_path = ROOT / "experiments/run_v22_52_conflict_residual_pareto_credit_fu.py"
    code_rows.append({"target": str(runner_path.relative_to(ROOT)), "compileall": "source_sha256", "sha256": source_sha256(runner_path)})
    write_rows(OUT_ROOT / "v22_52_code_truth_gate.csv", code_rows)
    ...
    modules = [
        ...
        "experiments.run_v22_52_conflict_residual_pareto_credit_fu",
    ]
    ...
    write_rows(OUT_ROOT / "v22_52_full_import_closure.csv", import_rows)
    forbidden_rows = runtime_forbidden_audit()
    write_rows(OUT_ROOT / "v22_52_runtime_forbidden_feature_audit.csv", forbidden_rows)
    forbidden_sum = sum(iflag(r.get("forbidden_present")) for r in forbidden_rows)
    route = {
        "python": PYTHON,
        "compileall_pass": int(compileall_pass),
        "runner_core_clean_import_pass": int(
            all(r["module"] != "experiments.run_v22_52_conflict_residual_pareto_credit_fu" or r["import"] == "pass" for r in import_rows)
        ),
        "full_package_import_pass": int(not missing),
        "full_package_clean_import_pass": int(not missing),
    ...
    write_json(OUT_ROOT / "v22_52_gate_route.json", route)
    append_exec(
        f"KAN_PYTHON={shlex.quote(PYTHON)} {shlex.quote(PYTHON)} experiments/run_v22_52_conflict_residual_pareto_credit_fu.py --stage gates",
        task_id="v22_52_gates",
        status="pass" if route["hard_gate_pass"] else "fail",
        gpu="cpu",
        files="results/v22_52/v22_52_code_truth_gate.csv, results/v22_52/v22_52_full_import_closure.csv, results/v22_52/v22_52_runtime_forbidden_feature_audit.csv, results/v22_52/v22_52_gate_route.json",
...
def runtime_forbidden_audit() -> list[dict[str, Any]]:
    ...
    files = [ROOT / "experiments/run_v22_52_conflict_residual_pareto_credit_fu.py"]
    rows = []
...
def reanalyze_v22_51R() -> list[dict[str, Any]]:
    ensure_out()
    source = ROOT / "results/v22_51R/v22_51R_pareto_credit_mlp_matrix.csv"
    rows = read_rows(source)
    cp_rows = [r for r in rows if str(r.get("method")) == "cpfcf"]
    baselines = [r for r in rows if str(r.get("method")) in STRONG_BASELINES]
    controls = [r for r in rows if str(r.get("method")) in CONTROL_METHODS]
    route = read_json(ROOT / "results/v22_51R/v22_51R_final_route.json")

    def key(row: dict[str, Any]) -> tuple[str, str]:
        return (str(row.get("dataset", "")), str(row.get("seed", "")))

    def best_for(pool: list[dict[str, Any]], item_key: tuple[str, str], metric: str = "final_NLL") -> dict[str, Any] | None:
        subset = [r for r in pool if key(r) == item_key]
        if not subset:
            return None
        return min(subset, key=lambda r: fval(r.get(metric), math.inf) or math.inf)

    def delta(row: dict[str, Any], other: dict[str, Any] | None, metric: str) -> float | str:
        if other is None:
            return ""
        a = fval(row.get(metric))
        b = fval(other.get(metric))
        if a is None or b is None:
            return ""
        return a - b

    no_debt_rate = sum(iflag(r.get("no_ECE_Brier_tail_debt")) for r in cp_rows) / max(1, len(cp_rows))
    conflict_rows: list[dict[str, Any]] = []
    cosine_rows: list[dict[str, Any]] = []
    strong_gap_rows: list[dict[str, Any]] = []
    fixed_gap_rows: list[dict[str, Any]] = []
    dual_rows: list[dict[str, Any]] = []
    for row in cp_rows:
        item_key = key(row)
        strongest = best_for(baselines, item_key)
        best_control = best_for(controls, item_key)
        nll_delta = delta(row, strongest, "final_NLL")
        auc_delta = delta(row, strongest, "AUC_loss_time")
        control_delta = delta(row, best_control, "final_NLL")
        nll_value = fval(nll_delta)
        control_value = fval(control_delta)
        cosine = fval(row.get("v_star_vs_population_cosine"))
        correction = fval(row.get("correction_norm_over_reference_norm"))
        population_like = int(cosine is not None and cosine >= 0.995)
        correction_small = "" if correction is None else int(correction <= 0.02)
        beats_strongest = int(nll_value is not None and nll_value < 0.0)
        beats_control = int(control_value is not None and control_value < 0.0)
        if population_like and correction is not None and correction <= 0.02 and not beats_strongest:
            diagnosis = "CPFCFCollapsedToPopulationGradient"
        elif population_like and correction is None and not beats_strongest:
            diagnosis = "PopulationLike_StrongOptimizerEatsFU_CorrectionFieldMissing"
        elif not beats_control:
            diagnosis = "FixedControlsCatchCPFCF"
        elif not beats_strongest:
            diagnosis = "StrongOptimizerEatsFU"
        else:
            diagnosis = "NoCollapseByNLL"
        common = {
            "source_artifact": str(source.relative_to(ROOT)),
            "v22_51R_route": route.get("route", ""),
            "dataset": row.get("dataset"),
            "seed": row.get("seed"),
            "method": row.get("method"),
            "final_NLL": row.get("final_NLL"),
            "AUC_loss_time": row.get("AUC_loss_time"),
            "v_star_vs_population_cosine": row.get("v_star_vs_population_cosine"),
            "correction_norm_over_reference_norm": row.get("correction_norm_over_reference_norm", ""),
            "constraint_inactive_rate": row.get("constraint_inactive_rate", ""),
            "active_constraint_count": row.get("active_constraint_count"),
            "max_dual_fraction": row.get("max_dual_fraction"),
            "fixed_control_best_delta": control_delta,
            "strongest_optimizer_delta": nll_delta,
            "no_debt_rate": no_debt_rate,
            "AUC_delta_vs_strongest": auc_delta,
            "NLL_delta_vs_strongest": nll_delta,
            "beats_strongest_NLL": beats_strongest,
            "beats_best_fixed_control_NLL": beats_control,
            "population_like_cosine_ge_0_995": population_like,
            "correction_small_le_0_02": correction_small,
            "collapse_rule_evaluable": int(correction is not None),
            "diagnosis_route": diagnosis,
            "claim_limit": "v22.51R did not record correction_norm_over_reference_norm/constraint_inactive_rate; missing fields are left blank, not inferred",
        }
        conflict_rows.append(common)
        cosine_rows.append(
            {
                "dataset": row.get("dataset"),
                "seed": row.get("seed"),
                ...
                "population_like_cosine_ge_0_995": population_like,
                "NLL_delta_vs_strongest": nll_delta,
                "claim_limit": common["claim_limit"],
            }
        )
        strong_gap_rows.append(
            {
                "dataset": row.get("dataset"),
                "seed": row.get("seed"),
                "cpfcf_final_NLL": row.get("final_NLL"),
                "strongest_method": strongest.get("method") if strongest else "",
                "strongest_final_NLL": strongest.get("final_NLL") if strongest else "",
                "NLL_delta_vs_strongest": nll_delta,
                "cpfcf_AUC_loss_time": row.get("AUC_loss_time"),
                "strongest_AUC_loss_time": strongest.get("AUC_loss_time") if strongest else "",
                "AUC_delta_vs_strongest": auc_delta,
                "beats_strongest_NLL": beats_strongest,
            }
        )
        fixed_gap_rows.append(
            {
                "dataset": row.get("dataset"),
                "seed": row.get("seed"),
                "cpfcf_final_NLL": row.get("final_NLL"),
                "best_fixed_control_method": best_control.get("method") if best_control else "",
                "best_fixed_control_NLL": best_control.get("final_NLL") if best_control else "",
                "fixed_control_best_delta": control_delta,
                "beats_best_fixed_control_NLL": beats_control,
                "fixed_control_gap": control_delta,
            }
        )
        dual_rows.append(
            {
                ...
                "CreditDegeneratedToReweighting": int(
                    (fval(row.get("max_dual_fraction"), 0.0) or 0.0) > 0.80
                    or abs(fval(row.get("dual_class_count_correlation"), 0.0) or 0.0) > 0.80
                    or abs(fval(row.get("dual_loss_correlation"), 0.0) or 0.0) > 0.80
                ),
                "claim_limit": "v22.51R dual audit from recorded dual summary fields only",
            }
        )
    write_rows(OUT_ROOT / "v22_52_v51R_conflict_diagnosis_matrix.csv", conflict_rows)
    write_rows(OUT_ROOT / "v22_52_v51R_cpfcf_vs_population_cosine.csv", cosine_rows)
    write_rows(OUT_ROOT / "v22_52_v51R_strong_optimizer_gap_matrix.csv", strong_gap_rows)
    write_rows(OUT_ROOT / "v22_52_v51R_fixed_control_gap_matrix.csv", fixed_gap_rows)
    write_rows(OUT_ROOT / "v22_52_v51R_dual_reweighting_audit.csv", dual_rows)
    append_exec(
        f"{shlex.quote(PYTHON)} experiments/run_v22_52_conflict_residual_pareto_credit_fu.py --stage reanalysis",
        task_id="v22_52_v51R_reanalysis",
        status="pass",
        gpu="cpu",
        files=(
            "results/v22_52/v22_52_v51R_conflict_diagnosis_matrix.csv, "
            "results/v22_52/v22_52_v51R_cpfcf_vs_population_cosine.csv, "
            "results/v22_52/v22_52_v51R_strong_optimizer_gap_matrix.csv, "
            "results/v22_52/v22_52_v51R_fixed_control_gap_matrix.csv, "
            "results/v22_52/v22_52_v51R_dual_reweighting_audit.csv"
        ),
        note=f"rows={len(conflict_rows)}; missing correction/inactive fields left blank",
        exit_code=0,
    )
...
def command_for_collect(spec: dict[str, Any], args: argparse.Namespace, gpu: str) -> list[str]:
    return [
        PYTHON,
        "experiments/run_v22_52_conflict_residual_pareto_credit_fu.py",
        "--stage",
        "collect",
        "--dataset",
        str(spec["dataset"]),
        ...
        "--cr-pcf-gamma",
        str(args.cr_pcf_gamma),
        "--cr-pcf-gamma-frac",
        str(args.cr_pcf_gamma_frac),
        "--cr-pcf-rho",
        str(args.cr_pcf_rho),
        "--cr-pcf-cost",
        str(args.cr_pcf_cost),
        "--cr-pcf-trust-ratio",
        str(args.cr_pcf_trust_ratio),
        "--safety-weight",
...
def run_matrix(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    def launch(i_spec: tuple[int, dict[str, Any]]) -> dict[str, Any]:
        ...
        append_exec(
            ...
            status=row["status"],
            gpu=str(gpu),
            files=f"{row['stdout']}, {row['stderr']}, results/v22_52/chunks/{safe_fragment(spec['label'])}_summary.csv",
...
def collect_chunk_rows() -> list[dict[str, str]]:
    ...
    for path in sorted(CHUNK_ROOT.glob("v22_52_mlp_*_summary.csv")):
...
def summarize(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_rows(OUT_ROOT / "v22_52_pareto_credit_mlp_matrix.csv", rows)
    controls = [r for r in rows if str(r.get("method")) in CONTROL_METHODS]
    write_rows(OUT_ROOT / "v22_52_fixed_reweighting_controls_matrix.csv", controls)
    strong = [r for r in rows if str(r.get("method")) in STRONG_BASELINES or str(r.get("method")) in CR_PCF_CANDIDATE_METHODS]
    write_rows(OUT_ROOT / "v22_52_strong_optimizer_matrix.csv", strong)
    efficiency_rows = [
        {
    ...
    write_rows(OUT_ROOT / "v22_52_efficiency_breakdown.csv", efficiency_rows)
    if not (OUT_ROOT / "v22_52_v51R_conflict_diagnosis_matrix.csv").exists():
        reanalyze_v22_51R()
    degeneracy_rows: list[dict[str, Any]] = []
    for r in rows:
        if str(r.get("method")) not in CR_PCF_CANDIDATE_METHODS:
            continue
        ...
        degeneracy_rows.append(
            {
                "source_artifact": "results/v22_52/v22_52_pareto_credit_mlp_matrix.csv",
                "method": r.get("method"),
                "architecture": "MLP",
                "dataset": r.get("dataset"),
                "seed": r.get("seed"),
                ...
                "claim_limit": "v22.52 CR-PCF row; degeneracy rule is max_dual>0.80 or abs(class/loss corr)>0.80",
            }
        )
    write_rows(OUT_ROOT / "v22_52_dual_degeneracy_audit.csv", degeneracy_rows)
    route = decide_route(rows, degeneracy_rows)
    write_json(OUT_ROOT / "v22_52_final_route.json", route)
    write_skipped_downstream(route)
    write_repair_attempts_matrix(rows)
    write_visualizations(rows, efficiency_rows)
    write_recap()
    append_exec(
        f"{shlex.quote(PYTHON)} experiments/run_v22_52_conflict_residual_pareto_credit_fu.py --stage summarize",
        task_id="v22_52_summarize",
        status="pass",
        gpu="cpu",
        files=", ".join(f"results/v22_52/{name}" for name in CORE_REQUIRED_ARTIFACTS),
...
def run_safety_exactness(args: argparse.Namespace) -> list[dict[str, Any]]:
    import torch

    ensure_out()
    datasets = split_csv(str(args.datasets), str)
    seeds = split_csv(str(args.seeds), int)
    methods = split_csv(str(args.methods), str) if args.methods else [PRIMARY_FU_METHOD]
    gpus = split_csv(str(args.gpus), str) or ["cpu"]
    specs = [
        {"dataset": dataset, "seed": seed, "method": method, "gpu": gpus[i % len(gpus)]}
        for i, (dataset, seed, method) in enumerate((d, s, m) for d in datasets for s in seeds for m in methods)
    ]

    def one(spec: dict[str, Any]) -> dict[str, Any]:
        local_args = argparse.Namespace(**vars(args))
        local_args.dataset = spec["dataset"]
        local_args.seed = int(spec["seed"])
        local_args.method = str(spec["method"])
        gpu = str(spec["gpu"])
        local_args.device = f"cuda:{gpu}" if gpu != "cpu" else "cpu"
        device = torch_device(str(local_args.device))
        torch.manual_seed(int(local_args.seed) + 52000)
        tensors = load_bundle_tensors(str(local_args.dataset), int(args.train_size), int(args.held_size), int(args.test_size), int(local_args.seed))
        if tensors["used_fake_data"]:
            raise RuntimeError("fake data is not allowed for v22.52 safety exactness")
        local_args.num_classes_runtime = int(tensors["num_classes"])
        model = make_model(int(tensors["input_dim"]), int(tensors["num_classes"]), int(args.hidden), int(local_args.seed) + 51000, device)
        named = named_params(model)
        params = [p for _n, p in named]
        x_train = tensors["x_train"].to(device)
        y_train = tensors["y_train"].to(device)
        ...
        y_held = tensors["y_held"].to(device)
        generator = torch.Generator(device=device)
        generator.manual_seed(int(local_args.seed) + int(hashlib.sha256(str(local_args.method).encode("utf-8")).hexdigest()[:8], 16))
        idx = batch_indices(int(x_train.shape[0]), int(args.batch_size), generator, device)
        pre_train = subset_eval_metrics(model, x_train, y_train, idx, int(tensors["num_classes"]))
        pre_held = evaluate_tensors(model, x_held, y_held, device, int(tensors["num_classes"]), int(args.eval_batch_size))
        component_grads, component_values = safety_component_gradients(
            model,
            x_train,
            y_train,
            idx,
        ...
        direction, diag, _ = cpfcf_direction_for_batch(model, x_train, y_train, idx, params, local_args, generator, None, 0)
        direction, raw_norm = clipped(direction, float(args.grad_clip))
        param_vec = flat_param_vector(named).to(direction.device, dtype=direction.dtype)
        update_vec = -float(args.lr) * direction
        if float(args.weight_decay) != 0.0:
            update_vec = update_vec - float(args.lr) * float(args.weight_decay) * param_vec
        predicted_ece_delta = float((component_grads["ECE_proxy"].to(update_vec.device, dtype=update_vec.dtype) @ update_vec).detach().cpu().item())
        predicted_brier_delta = float((component_grads["Brier"].to(update_vec.device, dtype=update_vec.dtype) @ update_vec).detach().cpu().item())
        predicted_tail_delta = float((component_grads["tail"].to(update_vec.device, dtype=update_vec.dtype) @ update_vec).detach().cpu().item())
        old_params = [p.detach().clone() for _n, p in named]
        apply_flat_update(named, direction, float(args.lr), float(args.weight_decay))
        post_train = subset_eval_metrics(model, x_train, y_train, idx, int(tensors["num_classes"]))
        post_held = evaluate_tensors(model, x_held, y_held, device, int(tensors["num_classes"]), int(args.eval_batch_size))
        with torch.no_grad():
            for (_name, p), old in zip(named, old_params):
                p.copy_(old)
        exact_train_ece_delta = post_train["ECE"] - pre_train["ECE"]
        exact_train_brier_delta = post_train["Brier"] - pre_train["Brier"]
        exact_train_tail_delta = post_train["tail_q95"] - pre_train["tail_q95"]
        exact_held_ece_delta = post_held["ECE"] - pre_held["ECE"]
        exact_held_brier_delta = post_held["Brier"] - pre_held["Brier"]
        exact_held_tail_delta = post_held["tail_q95"] - pre_held["tail_q95"]
        return {
            "dataset": local_args.dataset,
            "seed": local_args.seed,
            "method": local_args.method,
            "device": local_args.device,
            "batch_size": int(args.batch_size),
            "predicted_ECE_delta": predicted_ece_delta,
            "predicted_Brier_delta": predicted_brier_delta,
            "predicted_tail_delta": predicted_tail_delta,
            "predicted_combined_safety_debt_delta": diag.get("safety_debt_predicted_delta", ""),
            "exact_train_ECE_delta": exact_train_ece_delta,
            "exact_train_Brier_delta": exact_train_brier_delta,
            "exact_train_tail_q95_delta": exact_train_tail_delta,
            "exact_held_ECE_delta": exact_held_ece_delta,
            "exact_held_Brier_delta": exact_held_brier_delta,
            "exact_held_tail_q95_delta": exact_held_tail_delta,
            "ECE_sign_agrees_train": int(predicted_ece_delta == 0.0 or exact_train_ece_delta == 0.0 or (predicted_ece_delta > 0.0) == (exact_train_ece_delta > 0.0)),
            "Brier_sign_agrees_train": int(predicted_brier_delta == 0.0 or exact_train_brier_delta == 0.0 or (predicted_brier_delta > 0.0) == (exact_train_brier_delta > 0.0)),
            "tail_sign_agrees_train": int(predicted_tail_delta == 0.0 or exact_train_tail_delta == 0.0 or (predicted_tail_delta > 0.0) == (exact_train_tail_delta > 0.0)),
            "raw_direction_norm": raw_norm,
            "correction_norm_over_reference_norm": diag.get("correction_norm_over_reference_norm", ""),
            "base_harm_rate": diag.get("base_harm_rate", ""),
            "constraint_inactive_rate": diag.get("constraint_inactive_rate", ""),
            "component_ECE_proxy_value": component_values.get("ECE_proxy", ""),
            "component_Brier_value": component_values.get("Brier", ""),
            "component_tail_value": component_values.get("tail", ""),
            "claim_limit": "predicted_ECE uses differentiable soft-ECE proxy; exact ECE uses hard 10-bin ECE. Exact train is the same sampled batch; exact held is diagnostic only.",
        }

    with concurrent.futures.ThreadPoolExecutor(max_workers=min(int(args.workers), max(1, len(specs)))) as ex:
        rows = list(ex.map(one, specs))
    write_rows(OUT_ROOT / "v22_52_safety_gradient_exactness_matrix.csv", rows)
    append_exec(
        f"{shlex.quote(PYTHON)} experiments/run_v22_52_conflict_residual_pareto_credit_fu.py --stage safety_exactness --datasets {shlex.quote(str(args.datasets))} --seeds {shlex.quote(str(args.seeds))} --methods {shlex.quote(str(args.methods or PRIMARY_FU_METHOD))} --gpus {shlex.quote(str(args.gpus))} --workers {int(args.workers)}",
        task_id="v22_52_safety_exactness",
        status="pass",
        gpu=str(args.gpus),
        files="results/v22_52/v22_52_safety_gradient_exactness_matrix.csv",
        note=f"rows={len(rows)}; exact temporary application audit",
        exit_code=0,
    )
    return rows
...
def decide_route(rows: list[dict[str, Any]], degeneracy_rows: list[dict[str, Any]]) -> dict[str, Any]:
    gate_route = read_json(OUT_ROOT / "v22_52_gate_route.json")
    solver_rows = read_rows(OUT_ROOT / "v22_52_credit_solver_unit_matrix.csv")
    ...
    if not gate_route.get("hard_gate_pass"):
        ...
        route = "R1-SolverOnlyNoTrainingValue"
    else:
        route = "R1-SolverOnlyNoTrainingValue"
    ...
    cpfcf_rows = [r for r in rows if str(r.get("method")) == PRIMARY_FU_METHOD]
    nll_improve = 0
    auc_improve = 0
    beats_controls = 0
    no_debt = 0
    ...
    for r in cpfcf_rows:
        ...
        if (fval(r.get("max_dual_fraction"), math.inf) or math.inf) <= 0.80:
            dual_ok += 1
        corr = fval(r.get("correction_norm_over_reference_norm"), math.inf)
        if corr is not None and 0.02 <= corr <= 0.35:
            correction_ok += 1
        class_corr = abs(fval(r.get("dual_class_count_correlation"), math.inf) or math.inf)
        if class_corr <= 0.70:
            class_corr_ok += 1
    for r in degeneracy_rows:
        if str(r.get("method")) in CR_PCF_CANDIDATE_METHODS and iflag(r.get("CreditDegeneratedToReweighting")):
            degeneracy_hits += 1
    ...
    if exploration_gate:
        route = "R5-ConflictResidualCreditOpened_MLPExploration"
    elif solver.get("pass") and total > 0 and (degeneracy_hits > total / 2 or beats_controls < 6):
        route = "R3-ReweightingExplained_NoFU"
    elif solver.get("pass") and total > 0 and correction_ok < 6:
        route = "R2-ConstraintInactive_NoCreditFU"
    elif solver.get("pass") and total >= 9 and beats_adamw >= 5 and beats_strongest < 5:
        route = "R4-AdamWOnlyWeakPatch"
    ...
    return {
        ...
        "correction_norm_over_reference_norm in [0.02,0.35]": f"{correction_ok}/{total}",
        "max_dual_fraction <= 0.80": f"{dual_ok}/{total}",
        "abs(dual_class_count_correlation) <= 0.70": f"{class_corr_ok}/{total}",
...
def write_repair_attempts_matrix(main_rows: list[dict[str, Any]]) -> None:
    ...
    for path in sorted(glob.glob(str(CHUNK_ROOT / "v22_52_repair_*_summary.csv"))):
    ...
    write_rows(OUT_ROOT / "v22_52_repair_attempts_matrix.csv", repair_rows)


def write_skipped_downstream(route: dict[str, Any]) -> None:
    mlp_gate = iflag(route.get("mlp_exploration_gate_pass"))
    downstream = [
        ("v22_52_layerwise_realization_matrix.csv", "Part F layerwise realization", "requires follow-up implementation after CR-PCF MLP gate"),
        ("v22_52_kan_carrier_realization_matrix.csv", "Part G KAN carrier realization", "requires MLP exploration gate before KAN full matrix"),
        ("v22_52_kan_vs_mlp_credit_matrix.csv", "Part G KAN vs MLP matched support", "requires MLP exploration gate before architecture claim"),
        ("v22_52_continual_memory_credit_matrix.csv", "Part I continual memory credit", "not executed in this v22.52 slice; no delayed-generalization claim"),
        ("v22_52_grokking_credit_matrix.csv", "Part I grokking credit", "not executed in this v22.52 slice; no grokking-delay claim"),
    ]
...
def write_visualizations(rows: list[dict[str, Any]], efficiency_rows: list[dict[str, Any]]) -> None:
    ...
    cp = [r for r in rows if str(r.get("method")) == PRIMARY_FU_METHOD]
    labels = [f"{r.get('dataset')}-s{r.get('seed')}" for r in cp]
    write_rows(OUT_ROOT / "v22_52_visualization_source.csv", rows)
    svg_bar(OUT_ROOT / "v22_52_dual_distribution.svg", "CR-PCF max dual fraction by row", labels, [fval(r.get("max_dual_fraction"), 0.0) or 0.0 for r in cp])
    svg_bar(OUT_ROOT / "v22_52_cohort_gain_pareto_front.svg", "CR-PCF cohort gain min by row", labels, [fval(r.get("cohort_gain_min"), 0.0) or 0.0 for r in cp])
    svg_bar(OUT_ROOT / "v22_52_mlp_general_value_heatmap.svg", "CR-PCF held NLL gain by row", labels, [fval(r.get("held_NLL_gain"), 0.0) or 0.0 for r in cp])
    svg_bar(OUT_ROOT / "v22_52_safety_debt_tradeoff.svg", "CR-PCF ECE delta by row", labels, [fval(r.get("ECE_delta"), 0.0) or 0.0 for r in cp])
    svg_bar(OUT_ROOT / "v22_52_efficiency_breakdown.svg", "Controller overhead ratio by row", labels, [fval(r.get("controller_overhead_ratio"), 0.0) or 0.0 for r in cp])
    (OUT_ROOT / "v22_52_kan_vs_mlp_credit_carrier_heatmap.svg").write_text(
...
def write_recap() -> None:
    ensure_out()
    route = read_json(OUT_ROOT / "v22_52_final_route.json")
    mlp_rows = read_rows(OUT_ROOT / "v22_52_pareto_credit_mlp_matrix.csv")
    solver_rows = read_rows(OUT_ROOT / "v22_52_credit_solver_unit_matrix.csv")
    degeneracy_rows = read_rows(OUT_ROOT / "v22_52_dual_degeneracy_audit.csv")
    v51r_rows = read_rows(OUT_ROOT / "v22_52_v51R_conflict_diagnosis_matrix.csv")
    safety_exactness_rows = read_rows(OUT_ROOT / "v22_52_safety_gradient_exactness_matrix.csv")
    repair_rows = read_rows(OUT_ROOT / "v22_52_repair_attempts_matrix.csv")
    primary_rows = [r for r in mlp_rows if str(r.get("method")) == PRIMARY_FU_METHOD]
            ...
            out.append(
                {
                    ...
                    "mean_correction_norm_over_reference_norm": mean(fval(r.get("correction_norm_over_reference_norm")) for r in group),
                    "mean_base_harm_rate": mean(fval(r.get("base_harm_rate")) for r in group),
                    "mean_constraint_inactive_rate": mean(fval(r.get("constraint_inactive_rate")) for r in group),
                    "mean_controller_overhead_ratio": mean(fval(r.get("controller_overhead_ratio")) for r in group),
                }
            )
        return out
    ...
    primary_cols = [
        ...
        "base_harm_rate",
        "constraint_inactive_rate",
        "cohort_gain_min_before",
        "cohort_gain_min_after",
        "correction_norm_over_reference_norm",
        "v_out_vs_reference_cosine",
    ...
    analysis_lines = [
        "- v22.52 当前主方法是 `cr_pcf_cautious`；旧 CPFCF 仅作为 v22.51R 历史诊断对象，不作为本轮 official FU。",
        f"- 主矩阵共 `{len(mlp_rows)}` 行；primary CR-PCF 行 `{len(primary_rows)}` 行；dispatch 已在 `v22_52_mlp_dispatch_status.csv` 中记录。",
        f"- Part B v22.51R reanalysis 产出 `{len(v51r_rows)}` 行，缺失的 correction/inactive 字段保持空值，不做反推。",
        "- 若 route 未打开 MLP gate，KAN/layerwise/continual 文件只记录 skipped/pending，不构成 KAN 结论。",
    ]
    if route.get("fixed_control_blocker"):
        analysis_lines.append("- fixed/geometry controls 未被充分击败，按计划不能 promotion；需从 residual correction 是否被同几何扰动解释继续查。")
    no_debt_text = str(route.get("no ECE/Brier/tail debt", ""))
    if no_debt_text and not no_debt_text.startswith("7/") and not no_debt_text.startswith("8/") and not no_debt_text.startswith("9/"):
        analysis_lines.append("- no-debt gate 未达标；按计划后续优先做 safety-gradient exactness，而不是直接调参宣称修复。")
    correction_text = str(route.get("correction_norm_over_reference_norm in [0.02,0.35]", ""))
    if correction_text:
        analysis_lines.append(f"- correction activity gate: `{correction_text}`；同时查看 base_harm_rate/constraint_inactive_rate 判断 correction 是否长期 inactive。")
    if repair_rows:
        analysis_lines.append(f"- repair rows 已生成 `{len(repair_rows)}` 行，见 `v22_52_repair_attempts_matrix.csv`；复盘表只从这些真实 rows 汇总。")
        optref_candidate = [r for r in repair_rows if str(r.get("method")) == "cr_pcf_optref_cautious_direct"]
        if optref_candidate:
            analysis_lines.append(
                "- optimizer-reference direct repair 已验证：candidate rows "
                f"`{len(optref_candidate)}`；beats strongest "
                f"`{sum(iflag(r.get('beats_reference_strongest_NLL')) for r in optref_candidate)}/{len(optref_candidate)}`，"
                f"beats matched controls `{sum(iflag(r.get('beats_reference_controls_NLL')) for r in optref_candidate)}/{len(optref_candidate)}`；"
                "该分支未打开 gate，说明 strong-reference direct 本身仍被同 residual-geometry controls 解释。"
            )
    else:
        analysis_lines.append("- 当前尚未生成 v22.52 repair rows；这不是成功结论，后续若 gate 失败仍需按 blocker 分支继续。")
    if safety_exactness_rows:
        ece_agree = sum(iflag(r.get("ECE_sign_agrees_train")) for r in safety_exactness_rows)
        brier_agree = sum(iflag(r.get("Brier_sign_agrees_train")) for r in safety_exactness_rows)
        tail_agree = sum(iflag(r.get("tail_sign_agrees_train")) for r in safety_exactness_rows)
        analysis_lines.append(
            f"- safety exactness rows `{len(safety_exactness_rows)}`：train sign agreement ECE `{ece_agree}/{len(safety_exactness_rows)}`，Brier `{brier_agree}/{len(safety_exactness_rows)}`，tail `{tail_agree}/{len(safety_exactness_rows)}`。"
        )
    else:
        analysis_lines.append("- safety exactness artifact 尚未生成；若 no-debt gate 未过，应先生成它再继续 full repair matrix。")
    ...
    text = [
        ...
        f"- MLP rows: `{len(primary_rows)}` primary CR-PCF rows / `{len(mlp_rows)}` total MLP rows。",
        ...
        "## 方法均值",
        "",
        md_table(method_summary(mlp_rows), ["method", "rows", "mean_final_NLL", "mean_AUC_loss_time", "no_debt_rows", "mean_correction_norm_over_reference_norm", "mean_base_harm_rate", "mean_constraint_inactive_rate", "mean_controller_overhead_ratio"], 40),
        "",
        "## 关键 CR-PCF 行",
        "",
        md_table(primary_rows, primary_cols, 20),
        "",
        "## Part B v22.51R 诊断",
        "",
        md_table(
            v51r_rows,
            ["dataset", "seed", "v_star_vs_population_cosine", "correction_norm_over_reference_norm", "constraint_inactive_rate", "fixed_control_best_delta", "NLL_delta_vs_strongest", "AUC_delta_vs_strongest", "diagnosis_route", "claim_limit"],
            20,
        ),
        "",
        "## Solver 证据链",
        "",
        md_table(solver_rows[:20], ["case", "repeat", "solver_status", "KKT_residual", "primal_feasibility_violation", "correction_norm_over_reference_norm", "cohort_gain_min_before", "cohort_gain_min_after", "safety_debt_predicted_delta", "solver_ms"], 20),
        "",
        "## Dual / Reweighting 退化审计",
        "",
        md_table(
            degeneracy_rows[:20],
        ...
        "",
        "## Safety Gradient Exactness",
        "",
        md_table(
            safety_exactness_rows,
            ["dataset", "seed", "method", "predicted_ECE_delta", "exact_train_ECE_delta", "exact_held_ECE_delta", "predicted_Brier_delta", "exact_train_Brier_delta", "exact_held_Brier_delta", "predicted_tail_delta", "exact_train_tail_q95_delta", "exact_held_tail_q95_delta", "claim_limit"],
            20,
        ),
        "",
        "## 修复尝试证据链",
        ...
        *analysis_lines,
        ...
        "## 审计修改记录",
        "",
        "- 新增/修复 `solve_cr_pcf`：默认以 mean gradient 为 reference，也支持外部 optimizer reference；仅在 conflict residual span 中求 correction，并记录 before/after gain、base_harm_rate、constraint_inactive_rate、trust_alpha 等诊断。",
        "- 新增 CR-PCF runtime 方法：`cr_pcf_adamw`、`cr_pcf_cautious`，以及 `cr_shuffled_dual_cautious`、`cr_frozen_dual_cautious`、`cr_random_conflict_cautious`、`cr_signflip_conflict_cautious` controls。",
        "- 新增 optimizer-reference direct CR-PCF repair 分支：`cr_pcf_optref_cautious_direct` 先构造 CautiousAdamW reference update，再只在 conflict residual subspace 上加 correction，并配套 optref shuffled/frozen/random/signflip controls。",
        "- 修复 optref direct 更新裁剪：optimizer-reference direction 已是 preconditioned update，不再套用 gradient clip，避免把 CautiousAdamW reference 压到近似不学习。",
        "- 修复 gate import closure：从旧模块名 `experiments.run_v22_52_continuous_pareto_credit_flow` 改为当前 `experiments.run_v22_52_conflict_residual_pareto_credit_fu`，并增加 `runner_core_clean_import_pass` / `full_package_clean_import_pass`。",
        "- 新增 Part B v22.51R 五个 reanalysis artifact；缺失字段以空值和 `claim_limit` 说明，不补造 correction 数据。",
        "- 更新 summarize/visualization/recap 以 primary CR-PCF 方法为准，移除未执行 repair 扫描的旧叙述。",
        "",
        "## 复现实验命令",
        "",
        "```bash",
        f"KAN_PYTHON={shlex.quote(PYTHON)} {shlex.quote(PYTHON)} experiments/run_v22_52_conflict_residual_pareto_credit_fu.py --stage all --gpus 0,1,2,3 --workers 4",
        "```",
        "",
        "更细命令与逐行输出路径见 `docs/DG-KAN_v22.52_ConflictResidualParetoCreditFU_执行日志.md` 与 `results/v22_52/v22_52_command_journal.csv`。",
        "",
    ]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--stage", default="all", choices=["all", "gates", "reanalysis", "solver", "matrix", "collect", "summarize", "recap", "safety_exactness"])
    ...
    parser.add_argument("--label-prefix", default="v22_52_mlp")
    ...
    parser.add_argument("--cr-pcf-gamma", type=float, default=0.0)
    parser.add_argument("--cr-pcf-gamma-frac", type=float, default=0.80)
    parser.add_argument("--cr-pcf-rho", type=float, default=25.0)
    parser.add_argument("--cr-pcf-cost", type=float, default=1.0e-4)
    parser.add_argument("--cr-pcf-trust-ratio", type=float, default=0.25)
    ...
    if not args.label:
        args.label = f"v22_52_{args.stage}_{args.method}_{args.dataset}_s{args.seed}"
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        if args.stage == "all":
            ...
            summarize(args)
        elif args.stage == "safety_exactness":
            run_safety_exactness(args)
        ...
        append_exec(
            " ".join(shlex.quote(part) for part in [PYTHON, "experiments/run_v22_52_conflict_residual_pareto_credit_fu.py", "--stage", str(args.stage)]),
