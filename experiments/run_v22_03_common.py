#!/usr/bin/env python3
"""Shared helpers for v22.03 execution, logs, and packets."""
...
V2203_ROOT = ROOT / "results/v22_03_terminal_retention_diffeomorphic_source_channel_basis_efficiency_4gpu"
V2203_OFFICIAL = V2203_ROOT / "official_v22_03"
V2203_PLAN_DOC = ROOT / "docs/DG-KAN_v22.03_TerminalRetention_DiffeomorphicSourceChannel_BasisEfficiency_4GPU_完整计划.md"
V2203_EXEC_DOC = ROOT / "docs/DG-KAN_v22.03_TerminalRetention_DiffeomorphicSourceChannel_BasisEfficiency_4GPU_执行日志.md"
V2203_RECAP_DOC = ROOT / "docs/DG-KAN_v22.03_TerminalRetention_DiffeomorphicSourceChannel_BasisEfficiency_4GPU_实验结果复盘.md"
...
def ensure_out(out_dir: str | Path | None = None) -> Path:
    out = Path(out_dir) if out_dir else V2203_OFFICIAL
...
def init_docs() -> None:
    if not V2203_EXEC_DOC.exists():
        V2203_EXEC_DOC.write_text(
    ...
    if not V2203_RECAP_DOC.exists():
        V2203_RECAP_DOC.write_text(
            "# DG-KAN v22.03 TerminalRetention DiffeomorphicSourceChannel BasisEfficiency 4GPU 实验结果复盘\n\n"
            f"生成时间：{now_sg()}\n\n"
            "本文件由 v22.03 runner/finalizer 从落盘 artifacts 汇总；禁止编造数据。\n",
            encoding="utf-8",
        )
...
def append_exec(out_dir: Path, command: str, *, status: str = "", note: str = "") -> None:
    ...
    journal = out_dir / "v22_03_command_journal.csv"
    ...
    with journal.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow(row)
    append_text(V2203_EXEC_DOC, f"\n## {row['timestamp']}\n\n```bash\n{command}\n```\n\n- status: {status or 'recorded'}\n")
    if note:
        append_text(V2203_EXEC_DOC, f"- note: {note}\n")
...
def build_packet(out_dir: Path) -> tuple[Path, Path]:
    packet = out_dir / "v22_03_code_review_packet"
    if packet.exists():
    ...
    for rel in [
        ...
        if Path(rel).suffix:
            ...
            target.write_text("v22.03 code review packet\n", encoding="utf-8")
        else:
            target.mkdir(parents=True, exist_ok=True)
    write_text(packet / "00_README.md", "v22.03 packet: clean source tree, truth-gate artifacts, logs, and reproduction commands.\n")
    ...
    for doc in [V2203_PLAN_DOC, V2203_EXEC_DOC, V2203_RECAP_DOC]:
    ...
    write_text(
        packet / "11_REPRO_COMMANDS" / "clean_unzip_self_test.sh",
        "python -m compileall -q .\npython experiments/run_v22_03_code_truth_gate.py --self-contained-import-check 1 --source-root .\n",
    )
    manifest = []
    ...
    zip_path = out_dir / "v22_03_code_review_packet.zip"
    if zip_path.exists():
        zip_path.unlink()
    ...
    bundle = out_dir / "v22_03_results_bundle.zip"
    if bundle.exists():
        bundle.unlink()
    with zipfile.ZipFile(bundle, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for path in sorted(out_dir.rglob("*")):
            if path.is_file() and path.suffix.lower() != ".zip" and "v22_03_code_review_packet" not in str(path):
