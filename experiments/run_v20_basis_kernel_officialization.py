#!/usr/bin/env python3
"""v20 basis-kernel official-ish profiling runner."""
...
from dgkan.profiling.efficiency_v20 import efficiency_waterfall_rows, profile_isolated, v20_officialish_gate  # noqa: E402
...
def jobs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for family in families:
        for variant in V20_EFFICIENCY_VARIANTS.get(family, ["R0-current"]):
...
def run_shard(args: argparse.Namespace) -> None:
    ...
    append_exec(out_dir, f"{PYTHON} experiments/run_v20_basis_kernel_officialization.py --shard-index {args.shard_index} --shard-count {args.shard_count} --device {args.device}", status="started", note=f"jobs={len(selected)}")
    rows = []
    waterfall = []
    grad_rows = []
    for job in selected:
        ...
        try:
            ...
            x_train, y_train, x_val, y_val = load_dataset("MNIST", Path(args.data_root), load_n, int(args.val_size), 20_200 + int(job["job_index"]), device, int(args.input_size))
            ...
            row.update(v20_officialish_gate(row))
            rows.append(row)
            ...
            rows.append({**job, "execution_status": f"blocked:{type(exc).__name__}", "blocker": str(exc)[:500], "runtime_sec": time.perf_counter() - start, "v20_officialish_gate": 0, "v20_officialish_blocker": f"Exception:{type(exc).__name__}", "promotion_allowed": 0})
    suffix = f"_b{int(args.shard_index)}"
    write_rows(out_dir / f"v20_efficiency_truth_table{suffix}.csv", rows)
    write_rows(out_dir / f"v20_efficiency_waterfall{suffix}.csv", waterfall)
    write_rows(out_dir / f"v20_kernel_gradcheck{suffix}.csv", grad_rows)
    append_exec(out_dir, f"{PYTHON} experiments/run_v20_basis_kernel_officialization.py --shard-index {args.shard_index}", status="completed", note=f"rows={len(rows)}")


def merge(args: argparse.Namespace) -> None:
    out_dir = ensure_out(args.out_dir)
    rows = []
    for row in merge_csvs(out_dir, "v20_efficiency_truth_table_b*.csv", "v20_efficiency_truth_table.csv"):
        row.update(v20_officialish_gate(row))
        rows.append(row)
    write_rows(out_dir / "v20_efficiency_truth_table.csv", rows)
    waterfall = merge_csvs(out_dir, "v20_efficiency_waterfall_b*.csv", "v20_efficiency_waterfall.csv")
    grad = merge_csvs(out_dir, "v20_kernel_gradcheck_b*.csv", "v20_kernel_gradcheck.csv")
    summary = []
        ...
        summary.append(
            {
                "carrier": family,
                "rows": len(fam_rows),
                "officialish_pass_rows": sum(int_flag(r.get("v20_officialish_gate")) for r in fam_rows),
                "best_forward_ratio": min([finite_float(r.get("forward_ratio_vs_mlp")) for r in fam_rows if finite_float(r.get("forward_ratio_vs_mlp")) == finite_float(r.get("forward_ratio_vs_mlp"))] or [999.0]),
                "best_step_ratio": min([finite_float(r.get("step_ratio_vs_mlp")) for r in fam_rows if finite_float(r.get("step_ratio_vs_mlp")) == finite_float(r.get("step_ratio_vs_mlp"))] or [999.0]),
            }
        )
    write_rows(out_dir / "v20_efficiency_officialization_summary.csv", summary)
    append_exec(out_dir, f"{PYTHON} experiments/run_v20_basis_kernel_officialization.py --merge-only", status="completed", note=f"rows={len(rows)} waterfall={len(waterfall)} grad={len(grad)}")
