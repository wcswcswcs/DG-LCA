#!/usr/bin/env python3
"""Shared artifact and logging helpers for v22.13 execution."""
...
V2213_ROOT = ROOT / "results/v22_13_true_loss_interface_operator_fu"
V2213_OFFICIAL = V2213_ROOT / "official_v22_13"
V2213_PLAN_DOC = ROOT / "docs/DG-KAN_v22.13_TrueLossInterfaceOperatorFU_完整计划.md"
V2213_EXEC_DOC = ROOT / "docs/DG-KAN_v22.13_TrueLossInterfaceOperatorFU_执行日志.md"
V2213_RECAP_DOC = ROOT / "docs/DG-KAN_v22.13_TrueLossInterfaceOperatorFU_实验结果复盘.md"
...
def ensure_out(out_dir: str | Path | None = None) -> Path:
    out = Path(out_dir) if out_dir else V2213_OFFICIAL
...
def init_docs() -> None:
    V2213_EXEC_DOC.parent.mkdir(parents=True, exist_ok=True)
    if not V2213_EXEC_DOC.exists():
        V2213_EXEC_DOC.write_text(
            "# DG-KAN v22.13 TrueLossInterfaceOperatorFU 执行日志\n\n"
            f"生成时间：{now_sg()}\n\n"
            "记录原则：只记录真实命令、文件、输入、输出、状态、blocker 与修复尝试；未执行项不写成完成。\n",
            encoding="utf-8",
        )
    if not V2213_RECAP_DOC.exists():
...
def append_exec(out_dir: Path, command: str, *, status: str = "", note: str = "") -> None:
    ...
    journal = out_dir / "v22_13_command_journal.csv"
    ...
    with journal.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow(row)
    with V2213_EXEC_DOC.open("a", encoding="utf-8") as f:
...
def packet_source_paths() -> list[Path]:
    ...
    for path in sorted((ROOT / "dgkan").glob("**/*.py")):
        if path not in seen:
            seen.add(path)
            paths.append(path)
    for pattern in ["experiments/run_v22_13*.py", "experiments/run_v22_12*.py", "experiments/run_v22_11*.py"]:
        for path in sorted(ROOT.glob(pattern)):
            if path.exists() and path not in seen:
                seen.add(path)
                paths.append(path)
    for path in [V2213_PLAN_DOC, V2213_EXEC_DOC, V2213_RECAP_DOC]:
        if path.exists() and path not in seen:
            seen.add(path)
            paths.append(path)
...
def build_code_review_packet(out_dir: Path) -> Path:
    packet = out_dir / "v22_13_code_review_packet.zip"
    if packet.exists():
        packet.unlink()
    ...
    with zipfile.ZipFile(packet, "w", compression=zipfile.ZIP_DEFLATED) as z:
        ...
        for path in sorted(out_dir.rglob("v22_13_*")):
            if path.is_file() and path.suffix.lower() != ".zip" and "_code_packet_unzip" not in path.parts:
                ...
                z.write(path, arc)
                included.append({"packet_path": str(arc), "source_path": str(path.resolve()), "size_bytes": path.stat().st_size, "sha256": sha256_file(path)})
    write_rows(out_dir / "v22_13_code_review_packet_manifest.csv", included)
    return packet
...
def build_results_bundle(out_dir: Path) -> Path:
    bundle = out_dir / "v22_13_results_bundle.zip"
    if bundle.exists():
        bundle.unlink()
