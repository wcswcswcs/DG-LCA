# DG-KAN v7.3：表达力-优化器-几何 Pareto 与 Kernel-Native PureKAN-NG 完整实验计划

> 本计划基于 v7.0、v7.1、v7.2 的真实 real-only 结果重新制定，并采纳用户的最新纠正：**classwise-safe 不应作为主目标**。本项目不是刷榜，也不是追求每个类别都比 MLP 好看；核心目标是判断 KAN 作为函数空间结构是否能在表达力、优化可达性、几何性质和效率上系统性强于 MLP。  
> 因此，本计划把 classwise 指标降级为“局部失败诊断”，不再作为主优化目标或硬成功标准。v7.3 的核心问题改为：**KAN 基函数/结构表达力是否真的强于 MLP？如果强，为什么高效/几何约束后优势会消失？这是表达力被约束压坏，还是优化器没有把可表达函数训出来？**

---

## 0. 当前结果的重新判断

### 0.1 不能再把问题说成“KAN 表达力不行”

v7.0 的 dense manual KAN 已经给出非常关键的证据。`D3-dense-poly2-gate-d3-warmup-smooth-240` 在 3 个 primary datasets 上打开了 basic Beyond-MLP task gate：
...
$$
\boxed{
\text{KAN basis / dense poly2\_gate 方向存在超过 MLP 的表达力信号。}
}
$$
...
### 0.2 也不能把问题说成“只是 optimizer 没调好”

v7.0 对 grouped/T3/G2 路线已经尝试过 optimizer、schedule、lr、weight decay、basis smoothing、head、input normalization 等 targeted probes，但 grouped/T3 的 KMNIST gap 没闭合；dense manual KAN stack 才首次打开 task gate。v7.2 又做了 NoSync、SGD optimizer-state、hidden shrink、low-rank primitive 等自修复，结果显示：

```text
NoSync:
  step 明显下降，但仍不进 S2，task 没解决。

SGD:
  update ratio 降到约 0.56，memory 小幅下降，
  但所有 SGD candidate 都没有 basic pass。

hidden dim 32:
  step 有下降，但 task signal 明显损失，memory 仍约 1.29。

low-rank:
  task signal 没保住，memory 也没有接近 S2，gradient gate 还不完整。
```

这说明普通 optimizer 小调不是主因。更合理的判断是：

$$
\boxed{
\text{optimizer 可能不完美，但已有证据不支持“只靠 optimizer 小调就能解决”。}
}
$$
...
当前真正的 blocker 不是 classwise，也不是简单 optimizer，而是：
...
$$
\boxed{
\text{找出 dense KAN 的优势来自哪里，判断这些优势是否能在高效约束下被保留。}
}
$$
...
v7.3 的目标不是继续扩大 classwise gate，也不是继续调 AdamW / smoothing / sampler。v7.3 的目标是建立一条清晰的科学证据链：
...
\text{几何约束是否伤害表达力}
\rightarrow
\text{能否 kernel-native 高效实现}
}
$$

具体要回答四个问题。

### Q1：dense poly2_gate 的优势到底来自哪里？
...
poly2 basis？
gate？
dense cross-channel interaction？
strict KAN head？
depth？
training recipe？
```

### Q2：grouped / low-rank / hidden shrink 为什么掉？

是因为：

```text
模型函数类变小，表达力真的不足；
还是参数化病态，optimizer 没训出来；
还是几何约束 / smoothness / basis sharing 伤害了任务函数？
```

### Q3：几何性质应该是硬约束还是 Pareto 维度？

如果几何约束让 accuracy、AUC、NLL 明显下降，它就不应该作为硬目标。应该测量：

$$
L = L_{\text{task}}+\lambda L_{\text{geo}}
$$

下的 Pareto 曲线，而不是强行保几何。

### Q4：高效实现时应先压缩，还是先 kernelize dense 表达力？

已有 low-rank / grouped / hidden shrink 结果提示 aggressive compression 会丢 task signal。因此 v7.3 优先考虑：

```text
先保 dense 表达力，再做 materialization-free kernelization；
不要一开始就用低秩/分组把表达力压坏。
```
...
### 2.1 不再使用 classwise-safe 作为硬成功标准

classwise metrics 只作为诊断：

```text
local failure diagnostic
mode tradeoff detector
是否牺牲某些局部模式换取 macro gain 的证据
```

但它不再是主目标。除非出现极端 collapse，例如某个 class 完全失效，否则不应把 classwise gate 当成路线判断的核心。

### 2.2 Macro task advantage gate

KAN 至少需要在 macro validation accuracy 上超过 MLP：
...
### 2.3 Calibration / NLL gate

KAN 不应通过更差的 calibration 换取 accuracy：

$$
ECE_{\text{KAN}}\leq ECE_{\text{MLP}}+0.005.
$$

$$
NLL_{\text{KAN}}\leq NLL_{\text{MLP}}+0.01.
$$

Strong gate 要求：

$$
ECE_{\text{KAN}}<ECE_{\text{MLP}},
$$

$$
NLL_{\text{KAN}}<NLL_{\text{MLP}}.
$$

### 2.4 Optimization reachability gate

如果一个压缩/分组 KAN 经过 stronger training 或 teacher distillation 后仍无法接近 dense KAN，则认为是表达力/结构约束问题。
...
则说明 student 结构可能够，主要是 optimizer / training objective 问题。

如果：

$$
Acc_{\text{student-distill}}\ll Acc_{\text{D3}},
$$

则说明 student 有效表达力不足。

### 2.5 Geometry Pareto gate

几何不是硬目标，而是 Pareto 维度。对于几何正则强度 $\lambda$：
...
$$
\lambda \rightarrow Acc,
$$

$$
\lambda \rightarrow ECE,
$$

$$
\lambda \rightarrow NLL,
$$

$$
\lambda \rightarrow ValLossAUC,
$$

$$
\lambda \rightarrow G_{\text{geo}},
$$

其中 $G_{\text{geo}}$ 可以是曲率、basis smoothness、Jacobian norm、local Lipschitz、path length 或其他几何指标。

如果：

$$
\frac{d Acc}{d\lambda}<0
$$

且 task 下降明显，而几何改善很小，则几何约束应降级为 diagnostic / weak regularizer。
...
`C3` cached-backward probe 更强：

```text
C3 mean val gap = +0.0214 in 10-seed confirmation
CI95 low = +0.0158
Holm p = 2.19e-07
test gap = +0.0208
```

因此 macro task 目标基本已经接近或局部达到。问题不是“完全没有 advantage”，而是这个 advantage 还没有和 efficiency 结合。
...
这说明 efficiency 不是小修，还需要真正的 materialization-free kernelization 或新 primitive。

### 3.3 Optimizer gap

NoSync 证明 optimizer sync overhead 不是零，但不是根因；SGD 降 update 但破坏 task；lr/smoothing/weight decay/sampler 也没有解决。下一步如果还看 optimizer，必须升级到 KAN-specific optimizer，而不是继续 AdamW 小参数 sweep。

### 3.4 Geometry gap

当前还没有系统测几何 Pareto。之前把几何或 classwise 当硬约束过早了。v7.3 必须直接回答：

```text
几何约束是否真的带来泛化 / calibration / AUC 优势？
还是主要在伤害表达力？
```
...
第一，不允许使用 fake data、proxy rows、固定占位 ratio 或手填结论。所有未实现项必须写成：
...
第二，不允许继续把 classwise-safe 当主目标。classwise 只作为局部失败诊断，不作为路线成功的核心 gate。
...
第四，不允许把 dense oracle 当 final success。dense D3/C3/H3 证明表达力，但不等于 kernel-native efficient success。

第五，不允许把 grouped/low-rank 低 task signal 直接解释为 KAN 表达力弱。必须先做 distillation / optimization reachability 区分。

第六，不允许用 test accuracy 调参。test 只用于最终报告。

第七，没有 GradPass 的 candidate 不能进入 route best。

第八，没有 S2/S1 的 candidate 不能做 official efficiency success claim。

第九，不允许把几何性质作为未验证的硬约束。必须测 Pareto。

第十，不允许在没有 attribution 的情况下盲目做 kernelization。必须先定位 materialized live set、kernel count、top source。

---

## 5. 核心假设

### H1：dense poly2_gate 的优势来自函数结构，而不是单纯训练 recipe

v7.0 dense D3 证明了 task advantage。H1 假设：这个 advantage 主要来自 dense cross-channel poly2_gate，而不是 optimizer、depth 或 smoothing。
...
如果 H1 不成立，dense D3 的优势不来自 poly2_gate，应重新定位 architecture source。

---

### H2：压缩 / 分组 / 低秩导致 effective expressivity 损失

已有结果表明 grouped/T3/G2 efficiency 强但 task gap 大；low-rank 与 hidden shrink 不能保住 task signal。H2 假设：当前压缩约束导致有效表达力下降。
...
且 distillation 也不能恢复，则压缩结构表达力不足。

---

### H3：optimizer 可达性需要通过 distillation / stronger optimizer 判断

普通 optimizer 小调无法区分“结构不够”还是“训练不到”。H3 假设：如果 efficient student 结构有足够表达力，D3/C3 teacher distillation 能显著提升它。
...
说明 optimizer/objective 是主 blocker。

若 distillation 后仍远低于 dense teacher：
...
### H4：几何约束会形成 task-geometry Pareto，不能作为硬约束

H4 假设：几何约束对泛化/校准可能有帮助，但过强会伤害表达力。
...
edge curvature
Jacobian norm
local Lipschitz penalty
basis coefficient TV
...
### H5：efficiency fail 来自 materialization，而不是 KAN 数学本身

H5 假设：H3/C3 的 step/memory fail 来自 dense basis、gate temp、head temp、grad temp、kernel fragmentation，而不是 poly2_gate 数学不可高效。
...
若成立，应做 materialization-free fused kernel。

---

### H6：若 dense-preserving kernelization 仍失败，应设计新 primitive，而不是继续小修
...
distillation
geometry Pareto
materialization-free kernel
factorized/overlap-grouped bridge
```

仍无 S2 + task candidate，则说明当前 poly2_gate dense family 不适合当前 efficiency target，应设计新 function-space primitive。
...
### 6.3 Optimization reachability candidates
...
### 6.4 Geometry Pareto candidates
...
## 8. P0：v7.2 reproduction and provenance lock

### 8.1 目的

确认 v7.3 与 v7.2 可比，锁定 no-fake/no-proxy、strict/manual、gradient correctness、MLP denominator。
...
$$
|\Delta Acc_{\text{C3,v73}}-\Delta Acc_{\text{C3,v72}}|\leq0.005.
$$
...
## 11. P3：optimizer reachability and distillation

### 11.1 目的

区分“表达力不够”与“optimizer 训不出来”。这是 v7.3 最关键的实验之一。
...
AdanLite
basis-wise AdamW
edge-wise update normalization
...
如果 C3 kernelization始终太重，直接进 new low-live-set primitive。
...
则说明 student 的表达力可能够，优化器 / objective 是主问题。下一轮应该研究 KAN-specific optimizer，而不是换 primitive。
...
$$
\boxed{
\text{不要再以 classwise-safe 为主线；主线应是表达力归因、优化可达性、几何 Pareto 与 dense-preserving kernelization。}
}
$$
